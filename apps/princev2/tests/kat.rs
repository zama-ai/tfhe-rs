...
use tfhe_princev2::{decrypt, encrypt, BLOCK_NB_U2};
...
type Block = [Ciphertext; BLOCK_NB_U2];

fn check_kat(
    client_key: &ClientKey,
    server_key: &ServerKey,
    kat: &Kat,
    cipher: fn(&ServerKey, &Block, &Block, &Block) -> Box<Block>,
    input: u64,
    expected: u64,
) {
    ...
    let pt_out = decrypt_u2l_as_u64(client_key, &ct_out);
    assert_eq!(
        pt_out, expected,
