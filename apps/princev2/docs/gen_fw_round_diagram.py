#!/usr/bin/env python3
"""
Generate an SVG wiring diagram of ONE forward round of PRINCEv2.
...
folds the round constants into the S-box LUTs (`build_lut_xor_sbox_xor`) and fuses ShiftRows
into the M-layer gather. See `gen_fhe_fw_round_diagram.py` for the dataflow actually evaluated.
...
A forward round is:  s ^= k ; s ^= RC ; s = SubCells(s) ; s = M-hat(s) ; s = ShiftRows(s)
"""

# ShiftRows nibble permutation (gather form: out[i] = in[P_PERM[i]]), from tables::P_PERM
P_PERM = [0x0, 0x5, 0xa, 0xf, 0x4, 0x9, 0xe, 0x3, 0x8, 0xd, 0x2, 0x7, 0xc, 0x1, 0x6, 0xb]
...
# --- faithfulness check: M-hat + ShiftRows above == what `fw_round` evaluates ----------------
# The implementation never holds a bit-plane in spec order: `pack_bit_lanes_to_u4` packs bit-plane
# `m` of the four nibbles of a chunk into one u4 (position = nibble index in the chunk), `m_layer`
# drops one bit of it per output (`u4_exor`) through the FHE_MP_PERM_FW gather, and the pair-fold
# reassembles two e-xor results into one output nibble. Replaying that on bit indices must
# reproduce the edges drawn above, ShiftRows included.
...
FHE_MP_PERM_FW = [FHE_M_PERM[4 * P_PERM[_b >> 2] + (_b & 3)] for _b in range(64)]

def packed_u4_bit(u4_idx, pos):
    """Bit index sitting at `pos` (0 = MSB) of the u4 that `pack_bit_lanes_to_u4` builds."""
    return 4 * (4 * (u4_idx // 4) + pos) + u4_idx % 4

replayed = set()
out_bits_hit = set()
for out_u2 in range(32):
    for half in range(2):
        src = FHE_MP_PERM_FW[2 * out_u2 + half]
        u4_idx, excluded_u4_bit = src >> 2, src & 3
        # LUT_EXOR_FW alternates TO_3 / TO_2 by u4 parity: an even u4 lands on the MSB of its half
        out_bit = 4 * (out_u2 >> 1) + 2 * (out_u2 & 1) + u4_idx % 2
        out_bits_hit.add(out_bit)
        for pos in range(4):
            if pos != excluded_u4_bit:
                replayed.add((packed_u4_bit(u4_idx, pos), out_bit))
assert out_bits_hit == set(range(64)), "the pair-fold does not cover every output bit"
# ShiftRows moves nibble n to position P_PERM.index(n), so shift the drawn edges' output side.
assert replayed == {(in_bit, 4 * P_PERM.index(out_bit >> 2) + (out_bit & 3))
...
# ----------------------------------------------------------------------------- y-bands
Y_TITLE   = 34
Y_IN_LBL  = 70          # input bit labels
Y_IN      = 88          # top of input wires
Y_KEY     = 132         # AddKey / AddRC XOR row
Y_SBOX_T  = 176         # S-box boxes top
Y_SBOX_B  = 224         # S-box boxes bottom
Y_M_TAP   = 250         # where M-hat input taps leave the S-box outputs
Y_M_NODE  = 316         # base y for M-hat XOR nodes (staggered per bit-plane)
Y_SR_T    = 424         # ShiftRows bundles top
Y_SR_B    = 512         # ShiftRows bundles bottom
Y_OUT     = 512
Y_OUT_LBL = 532
HEIGHT    = 580
...
emit(f'<text x="{LEFT}" y="{Y_TITLE}" font-size="15" font-weight="bold" fill="#222">'
    f'PRINCEv2 — one forward round (64 bits wired individually; nibbles bundled)</text>')
emit(f'<text x="{LEFT}" y="{Y_TITLE+16}" font-size="10" fill="#666">'
    f'&#8853; k &#8594; &#8853; RC &#8594; SubCells (S) &#8594; M&#770; (block-diagonal, XOR-of-3) '
    f'&#8594; ShiftRows.  Thin wire = 1 bit; thick line = whole nibble (4 bits).</text>')
...
stage_label((Y_SBOX_T+Y_SBOX_B)//2+3, "S-box")
stage_label(Y_M_NODE+3, "M&#770;")
stage_label((Y_SR_T+Y_SR_B)//2, "Shift")
stage_label((Y_SR_T+Y_SR_B)//2+12, "Rows")

# ------- nibble group brackets + labels at the top (thick = nibble bundle) -------
for nibble in range(16):
    ...
    for bit_in_nibble in range(4):
        ...
        emit(f'<text x="{bit_wire_x(bit_idx)}" y="{Y_IN_LBL+2}" text-anchor="middle" '
            f'font-size="7.5" fill="#99a">{bit_idx}</text>')

# chunk separators / labels
for chunk in range(4):
    xa = bit_wire_x(16 * chunk) - 12
    xb = bit_wire_x(16 * chunk + 15) + 12
    mtype = "M0&#770;" if chunk in (0, 3) else "M1&#770;"
    emit(f'<text x="{(xa+xb)/2:.1f}" y="{Y_M_NODE+58}" text-anchor="middle" '
...
# AddKey / AddRC : one XOR node per wire
for bit_idx in range(64):
    x = bit_wire_x(bit_idx)
    ...
    # wire down into the S-box band
    emit(f'<line x1="{x}" y1="{Y_KEY+3.4}" x2="{x}" y2="{Y_SBOX_T}" stroke="#c7ccd6" stroke-width="1"/>')
...
for nibble in range(16):
    ...
    # outputs of the S-box drop to the M-hat tap line
    for bit_in_nibble in range(4):
        x = bit_wire_x(4 * nibble + bit_in_nibble)
        emit(f'<line x1="{x}" y1="{Y_SBOX_B}" x2="{x}" y2="{Y_M_TAP}" stroke="#c7ccd6" stroke-width="1"/>')

# ---------------------------------------------------------------- M-hat layer
edges = m_layer_edges()
# group incoming edges per output wire to place a node and fan-in
from collections import defaultdict
ins = defaultdict(list)
for in_bit, out_bit, bit_in_nibble in edges:
    ins[out_bit].append((in_bit, bit_in_nibble))

# stagger node height by bit-plane so the four planes don't overlap
def exor_node_y(bit_in_nibble):
    return Y_M_NODE + bit_in_nibble * 13

# draw the fan-in wires first (so nodes sit on top): straight diagonals converging on each node
...
# draw XOR nodes + wire down to ShiftRows
for out_bit in range(64):
    ...
    emit(f'<line x1="{xo}" y1="{ny+3.6}" x2="{xo}" y2="{Y_SR_T}" stroke="#c7ccd6" stroke-width="1"/>')

# bit-plane legend (in the reserved right gutter, clear of the wires)
...
# ---------------------------------------------------------------- ShiftRows (nibble bundles)
# out nibble i = in nibble P_PERM[i]; draw each moving nibble as ONE thick bundle line.
def nibble_center_x(nibble):
    return (bit_wire_x(4 * nibble) + bit_wire_x(4 * nibble + 3)) / 2

for i in range(16):
    src = P_PERM[i]
    x_src = nibble_center_x(src)
    x_dst = nibble_center_x(i)
    # colour bundles by source chunk for readability
    hue = (src // 4) * 90
    col = f'hsl({hue},45%,45%)'
    emit(f'<path d="M {x_src:.1f} {Y_SR_T} C {x_src:.1f} {(Y_SR_T+Y_SR_B)/2:.1f} '
        f'{x_dst:.1f} {(Y_SR_T+Y_SR_B)/2:.1f} {x_dst:.1f} {Y_SR_B:.1f}" '
...
for nibble in range(16):
    ...
    emit(f'<rect x="{x0}" y="{Y_OUT-6}" width="{x1-x0}" height="12" rx="3" '
        f'fill="none" stroke="#8894aa" stroke-width="2.2"/>')
    emit(f'<text x="{(x0+x1)/2:.1f}" y="{Y_OUT_LBL+8}" text-anchor="middle" '
        f'font-size="9" font-weight="bold" fill="#556">N{nibble}</text>')
    emit(f'<text x="{(x0+x1)/2:.1f}" y="{Y_OUT_LBL+20}" text-anchor="middle" '
        f'font-size="7.5" fill="#99a">&#8592; N{P_PERM[nibble]}</text>')

emit('</svg>')

out = __file__.rsplit("/", 1)[0] + "/princev2_fw_round.svg"
with open(out, "w") as f:
    f.write("\n".join(svg))
print("wrote", out, "  size", WIDTH, "x", HEIGHT)
print("Mhat edges:", len(edges), "(should be 64*3 = 192)")
assert sorted(P_PERM) == list(range(16)), "P_PERM not a permutation!"
print("P_PERM OK (permutation over 0..15)")
