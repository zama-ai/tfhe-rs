...
use crate::{tables, BLOCK_NB_BITS, BLOCK_NB_U2, BLOCK_NB_U4};
...
fn xor_to_u4(
    ...
    let ct_key_sum: [Ciphertext; BLOCK_NB_U2] = sum_with_key(server_key, in_u2h, ct_k);
    // [Parallel:32] Apply xor LUT to high or low bit
    let ct_xor_halves: [Ciphertext; BLOCK_NB_U2] = par_array_from_fn(|out_u2| {
...
fn fw_round(
    ...
    let ct_sbox_bits: [Ciphertext; BLOCK_NB_BITS] = sbox_to_bits(server_key, in_u4, lut_sbox);
    let ct_mlayer_in_u4: [Ciphertext; BLOCK_NB_U4] =
        pack_bit_lanes_to_u4(server_key, &ct_sbox_bits);
    let ct_mlayer_out_bits: [Ciphertext; BLOCK_NB_BITS] = m_layer(
        server_key,
        &ct_mlayer_in_u4,
        &tables::LUT_EXOR_FW,
...
fn xor_then_fw_round(
    server_key: &ServerKey,
    ct_u2h: &mut [Ciphertext; BLOCK_NB_U2], // in/out: 2-bits (high)
    ct_k: &[Ciphertext; BLOCK_NB_U2],
    lut_sbox: &[[u8; 16]; BLOCK_NB_U4],
) {
    let ct_xor_u4: [Ciphertext; BLOCK_NB_U4] = xor_to_u4(server_key, ct_u2h, ct_k);
...
fn mid_round(
    server_key: &ServerKey,
    in_u4: &[Ciphertext; BLOCK_NB_U4],
    ct_k_after_sbox: &[Ciphertext; BLOCK_NB_U2],
    ct_k_after_m_layer: &[Ciphertext; BLOCK_NB_U2],
    lut_sbox: &[[u8; 16]; BLOCK_NB_U4],
    lut_inv_sbox: &[[u8; 16]; BLOCK_NB_U4],
) -> [Ciphertext; BLOCK_NB_U2] {
    // S-Boxes /!\ output for xor
    let ct_sbox_u2h: [Ciphertext; BLOCK_NB_U2] = sbox_to_u2h(server_key, in_u4, lut_sbox);

    let ct_xor_bits: [Ciphertext; BLOCK_NB_BITS] =
        xor_to_bits(server_key, &ct_sbox_u2h, ct_k_after_sbox);

    let ct_mlayer_in_u4: [Ciphertext; BLOCK_NB_U4] = pack_bit_lanes_to_u4(server_key, &ct_xor_bits);
    let ct_mlayer_out_bits: [Ciphertext; BLOCK_NB_BITS] = m_layer(
        server_key,
        &ct_mlayer_in_u4,
        &tables::LUT_EXOR_FW,
    ...
    /* Bridging M-Layer --> Xor: combine pairs */
    let ct_mlayer_out_u2h: [Ciphertext; BLOCK_NB_U2] =
        sum_adjacent_pairs(server_key, &ct_mlayer_out_bits);

    let ct_xor_u4: [Ciphertext; BLOCK_NB_U4] =
        xor_to_u4(server_key, &ct_mlayer_out_u2h, ct_k_after_m_layer);
...
fn bw_round(
    ...
    // iPerm + M-Layer
    let ct_mlayer_in_u4: [Ciphertext; BLOCK_NB_U4] =
        pack_bit_lanes_to_u4_inv_p(server_key, in_bits);
    let ct_mlayer_out_bits: [Ciphertext; BLOCK_NB_BITS] = m_layer(
        server_key,
        &ct_mlayer_in_u4,
        &tables::LUT_EXOR_BW,
    ...
    let ct_bit_pairs: [Ciphertext; BLOCK_NB_U2] =
        sum_adjacent_pairs(server_key, &ct_mlayer_out_bits);
    let ct_sbox_in_u4: [Ciphertext; BLOCK_NB_U4] = sum_adjacent_pairs(server_key, &ct_bit_pairs);
...
fn xor_then_bw_round(
    ...
    let ct_xor_bits: [Ciphertext; BLOCK_NB_BITS] = xor_to_bits(server_key, ct_u2h, ct_k);
...
pub fn encrypt(
    server_key: &ServerKey,
    ct_m: &[Ciphertext; BLOCK_NB_U2],
    ct_k0: &[Ciphertext; BLOCK_NB_U2],
    ct_k1: &[Ciphertext; BLOCK_NB_U2],
) -> [Ciphertext; BLOCK_NB_U2] {
    ...
    let mut ct_u2h: [Ciphertext; BLOCK_NB_U2] =
