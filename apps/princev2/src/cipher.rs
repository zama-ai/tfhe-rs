...
fn xor_to_u2l(
    ...
    par_boxed_array_from_fn(|u2_idx| server_key.apply_lookup_table(&ct_key_sum[u2_idx], &lut_xor))
}
...
fn fw_round(
    ...
    let ct_sbox_bits = sbox_to_bits(server_key, in_u4, lut_sbox);
    let ct_mlayer_in_u4 = pack_bit_lanes_to_u4(server_key, &ct_sbox_bits);
    let ct_mlayer_out_bits = m_layer(
        server_key,
        &ct_mlayer_in_u4,
        &tables::LUT_EXOR_FW,
    ...
    sum_adjacent_pairs(server_key, ct_mlayer_out_bits.as_slice())
}

/// Xor layer with `ct_k`, then a forward round.
fn xor_then_fw_round(
    server_key: &ServerKey,
    ct_u2h: &mut Box<[Ciphertext; BLOCK_NB_U2]>, // in/out: 2-bits (high)
    ct_k: &[Ciphertext; BLOCK_NB_U2],
    lut_sbox: &[[u8; 16]; BLOCK_NB_U4],
) {
    let ct_xor_u4 = xor_to_u4(server_key, ct_u2h, ct_k);
...
fn mid_round(
    ...
    let ct_sbox_u2h = sbox_to_u2h(server_key, in_u4, lut_sbox);

    let ct_xor_bits = xor_to_bits(server_key, &ct_sbox_u2h, ct_k_after_sbox);

    let ct_mlayer_in_u4 = pack_bit_lanes_to_u4(server_key, &ct_xor_bits);
    let ct_mlayer_out_bits = m_layer(
        server_key,
        &ct_mlayer_in_u4,
        &tables::LUT_EXOR_FW,
    ...
    /* Bridging M-Layer --> Xor: combine pairs */
    let ct_mlayer_out_u2h = sum_adjacent_pairs(server_key, ct_mlayer_out_bits.as_slice());

    let ct_xor_u4 = xor_to_u4(server_key, &ct_mlayer_out_u2h, ct_k_after_m_layer);
...
fn bw_round(
    ...
    // iPerm + M-Layer
    let ct_mlayer_in_u4 = pack_bit_lanes_to_u4_inv_p(server_key, in_bits);
    let ct_mlayer_out_bits = m_layer(
        server_key,
        &ct_mlayer_in_u4,
        &tables::LUT_EXOR_BW,
    ...
    let ct_bit_pairs: Box<[Ciphertext; BLOCK_NB_U2]> =
        sum_adjacent_pairs(server_key, ct_mlayer_out_bits.as_slice());
    let ct_sbox_in_u4 = sum_adjacent_pairs(server_key, ct_bit_pairs.as_slice());
...
fn xor_then_bw_round(
    ...
    let ct_xor_bits = xor_to_bits(server_key, ct_u2h, ct_k);
...
pub fn encrypt(
    ...
    let mut ct_u2h = boxed_array_from_fn(|u2_idx| server_key.unchecked_scalar_mul(&ct_m[u2_idx], 4));
...
pub fn decrypt(
    ...
    let mut ct_u2h = boxed_array_from_fn(|u2_idx| server_key.unchecked_scalar_mul(&ct_c[u2_idx], 4));
