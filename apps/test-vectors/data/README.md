...
### Encodings
#### Native encoding 
Standard ciphertexts use 32 bit or 64 bit ciphertext modulus as this matches the native word size of the cpu.
...
_Note:_ in the CBOR files, the structure contains a list of integers called `data`, representing the coefficients of the ciphertext(s) or the secret key bits.  This is followed by additional parameter information, such as the `ciphertext_modulus`. `scalar_bits` is the size in bits of the integer type used to store the coefficients (u32 or u64). `modulus` is the ciphertext modulus.
When the ciphertext modulus is equal to the native range of the integer type defined in `scalar_bits`, it is represented as 0 in the CBOR file. For example, when using 64-bit integers, a `modulus` value of 0 indicates a ciphertext modulus of  $2^{64}$.
<pre>```
{
    "data": [
