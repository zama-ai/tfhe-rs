...
/// Execution mode for the worker pool.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum PoolMode {
    /// Async mode: workers are spawned directly from the main thread.
    /// Results are delivered via async/await.
    Async,
    /// Sync mode: spawns a SyncExecutor worker which manages compute workers.
    /// Enables blocking `collect_vec_sync()` calls.
    /// Requires a coordinator (service worker) to be registered.
    #[cfg(feature = "sync-api")]
    Sync { coordinator_url: String },
}
...
/// For sync mode, the user must deploy a Service Worker that calls `setupCoordinator()`
/// (exported from `coordinator.js`). The `coordinator_url` points to that SW file.
///
/// # Arguments
/// * `mode` - `Async` for async/await results, `Sync` for blocking `collect_vec_sync()`.
...
/// # Example
/// ```ignore
/// use wasm_par_mq::PoolMode;
///
/// // Async mode - no additional JS files needed
/// init_pool(PoolMode::Async, None, "/pkg/app_bg.wasm", "/pkg/app.js").await?;
///
/// // Sync mode - requires to copy coordinator.js
/// init_pool(PoolMode::Sync { coordinator_url: "/sw.js".to_string() }, Some(4), "/pkg/app_bg.wasm", "/pkg/app.js").await?;
/// ```
pub async fn init_pool(
    ...
    match mode {
        ...
        #[cfg(feature = "sync-api")]
        PoolMode::Sync { coordinator_url } => {
            init_pool_sync(num_workers, &wasm_url, &bindgen_url, &coordinator_url).await
        }
    }
}
...
///
/// Used internally by `init_pool` (async) and `SyncExecutor` (sync).
...
///
/// Registers the coordinator service worker and spawns the SyncExecutor.
/// The compute workers pool will be created by the SyncExecutor.
#[cfg(feature = "sync-api")]
async fn init_pool_sync(
    num_workers: Option<u32>,
    wasm_url: &str,
    bindgen_url: &str,
    coordinator_url: &str,
) -> Result<(), String> {
    use crate::coordinator::register_coordinator;
    use crate::sync_executor::register_sync_executor;

    // Register the coordinator service worker
    register_coordinator(coordinator_url).await?;
...
#[cfg(feature = "sync-api")]
pub async fn init_pool_sync_executor_only(
    num_workers: Option<u32>,
    wasm_url: &str,
    bindgen_url: &str,
) -> Result<(), String> {
    ...
    F: RegisteredFn<Input = I, Output = O>,
{
    ...
    let chunks: Vec<Vec<I>> = data.chunks(chunk_size).map(|c| c.to_vec()).collect();
...
where
    ...
    F: RegisteredFn<Input = I, Output = O>,
{
    use crate::coordinator::{cancel_task, wait_task};
