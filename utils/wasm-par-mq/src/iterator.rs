    ...
    F: RegisteredFn<Input = I, Output = O>,
{
    ...
    pub async fn collect_vec(self) -> Vec<O> {
        execute_par_map(self.f, self.base.slice)
            .await
    ...
    pub fn collect_vec_sync(self) -> Vec<O> {
        execute_par_map_sync(self.f, self.base.slice).expect("parallel map failed")
    }
    ...
    F: RegisteredFn<Input = I, Output = O>,
{
    ...
    /// Panics if the worker pool has not been initialized.
    pub async fn collect_vec(self) -> Vec<O> {
        execute_par_map(self.f, &self.base.data)
            .await
            .expect("parallel map failed")
    }
    ...
    #[cfg(feature = "sync-api")]
    pub fn collect_vec_sync(self) -> Vec<O> {
        execute_par_map_sync(self.f, &self.base.data).expect("parallel map failed")
    }
...
impl<'data, T: Clone + Serialize> IntoParallelIterator for &'data [T] {
    type Item = T;
