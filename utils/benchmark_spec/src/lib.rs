...
use layer::OpsLayer;
pub use layer::{HlapiOp, TfheLayer};
use serde::Serialize;
...
///
/// Format: `{crate}::{layer}[::{backend}]::{ops}::{op}[::throughput]::{param}[::scalar][::{type}]`
///
...
impl<'a> BenchmarkSpec<'a> {
    ...
    pub fn hlapi(
        ...
        Self {
            bench_crate: BenchCrate::Tfhe(TfheLayer::Hlapi(OpsLayer::Ops(hlapi_op))),
...
impl fmt::Display for BenchmarkSpec<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.bench_crate.fmt_with_backend(f, &self.backend)?;
...
mod tests {
    ...
    fn hlapi_cpu_latency() {
        let spec = BenchmarkSpec::new(
            BenchCrate::Tfhe(TfheLayer::Hlapi(OpsLayer::Ops(HlapiOp::Add))),
            Backend::Cpu,
    ...
    fn hlapi_cuda_latency() {
        let spec = BenchmarkSpec::new(
            BenchCrate::Tfhe(TfheLayer::Hlapi(OpsLayer::Ops(HlapiOp::Mul))),
            Backend::Cuda,
            "PARAM_MESSAGE_2_CARRY_2",
            &OperandType::CipherText,
            Some("FheUint128"),
        ...
        assert_eq!(
            spec.to_string(),
            "tfhe::hlapi::cuda::ops::mul::PARAM_MESSAGE_2_CARRY_2::FheUint128"
        );
    }
    ...
    fn hlapi_hpu_throughput() {
        ...
        assert_eq!(
            spec.to_string(),
            "tfhe::hlapi::hpu::ops::add::throughput::PARAM_MESSAGE_2_CARRY_2::FheUint64"
        );
    }
    ...
    fn hlapi_scalar() {
        let spec = BenchmarkSpec::new(
            BenchCrate::Tfhe(TfheLayer::Hlapi(OpsLayer::Ops(HlapiOp::LeftShift))),
            Backend::Cpu,
            "PARAM_MESSAGE_2_CARRY_2",
    ...
    fn hlapi_no_type_name() {
        let spec = BenchmarkSpec::new(
            BenchCrate::Tfhe(TfheLayer::Hlapi(OpsLayer::Ops(HlapiOp::Neg))),
            Backend::Cpu,
            "PARAM_MESSAGE_2_CARRY_2",
            &OperandType::CipherText,
            None,
