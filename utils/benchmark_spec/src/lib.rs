...
impl TypeName for CudaKeyswitchConfig {
    fn type_name(&self) -> String {
        match (self.uses_gemm, self.trivial_indices) {
            (Some(uses_gemm), Some(trivial_indices)) => {
                let gemm = if uses_gemm { "gemm" } else { "classical" };
                let idx = if trivial_indices {
                    "trivial"
                } else {
                    "complex"
                };
                format!("{}b::{}::{}_indices", self.bits, gemm, idx)
            }
            _ => format!("{}b", self.bits),
        }
    }
}
