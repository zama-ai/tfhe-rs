...
pub(crate) trait OptionalSegment: Sized {
...
impl OptionalSegment for Backend {
    ...
    fn from_segment(segment: &str) -> Option<Self> {
        segment.parse().ok().filter(|b| b != &Self::Cpu)
    }
}
...
impl OptionalSegment for OperandType {
    fn segment(&self) -> Option<&'static str> {
        match self {
            OperandType::CipherText => None,
            _ => Some(self.into()),
        }
    }

    fn from_segment(segment: &str) -> Option<Self> {
        segment.parse().ok().filter(|m| m != &Self::CipherText)
    }
}
...
impl OptionalSegment for BenchmarkMetric {
    ...
    fn from_segment(segment: &str) -> Option<Self> {
        segment.parse().ok().filter(|m| m != &Self::Latency)
    }
}
