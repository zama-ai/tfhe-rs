...
fn normalize_const_param_item(param: &syn::ItemConst, param_name_prefix: &str) -> syn::ItemConst {
    let mut normalized_param = param.clone();
    let current_param_ident_string = normalized_param.ident.to_string();
    let current_param_normalized_ident_str = current_param_ident_string
        .strip_prefix(param_name_prefix)
        .unwrap();
...
fn main() {
    ...
    shortint_parameters_per_version
        .iter()
        .find_map(|(version, _dir)| {
            let version_as_str = format!("v{}_{}", version.major, version.minor);
            if version_as_str == args.to_deduplicate {
                Some(version_as_str)
            } else {
                None
            }
    ...
    println!("{shortint_parameters_per_version:?}");
    ...
    for (version, shortint_param_dir) in shortint_parameters_per_version.iter() {
        ...
        for param_sub_dir in ["classic", "multi_bit"] {
                ...
                for item in syn_file.items {
                    if let syn::Item::Const(param) = item {
                        ...
                        let normalized_param =
                            normalize_const_param_item(&param, &param_ident_prefix);

                        match param_version_and_associated_file_parameters
                            .entry((version, shortint_param_dir))
                        {
                            Entry::Occupied(occupied_entry) => {
                                let associated_files_and_parameters = occupied_entry.into_mut();
                                // Recreated to avoid issues with the HashMap logic and moves
                                let param_file_relative_path = maybe_param_file
                                    .strip_prefix(shortint_param_dir)
                                    .unwrap()
                                    .to_path_buf();
                                let hash_set = match associated_files_and_parameters
                                    .entry(param_file_relative_path)
                                {
                            ...
                            Entry::Vacant(_) => {
                                panic!(
                                    "Uninitialized Entry for {version}, {}",
                                    shortint_param_dir.display()
                                )
                            }
                        }
                    }
    ...
    for dir_entry in deduped_dir_entries {
        ...
        let relative_file_path = file_to_process
            .strip_prefix(deduped_dir)
            .unwrap()
            .to_path_buf();
        ...
        for (old_version, old_dir) in shortint_parameters_per_version.iter().rev() {
            ...
            if let Some(old_file_and_params) =
                param_version_and_associated_file_parameters.get(&(old_version, old_dir))
            {
                // Get the existing parameters from that relative file for the old version
                if let Some(existing_normalized_parameters) =
                    old_file_and_params.get(&relative_file_path)
                {
                    // Now check the items in the current file
                    for item in syn_file.items.iter_mut() {
                        if let syn::Item::Const(param) = item {
                            param_types.insert(param.ty.clone());
                            let current_normalized_param =
                            ...
                            let current_normalized_param_as_item =
                                syn::Item::Const(current_normalized_param);

                            // Does it exist and is it the same as the one in the version we are
                            // checking.unwrap()
                            if existing_normalized_parameters
                                .get(&current_normalized_param_as_item)
                                .is_some()
                            {
