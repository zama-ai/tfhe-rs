...
//! The global `generate` crate will also be updated with this version.
...
fn parse_version(input: &str) -> Result<CrateVersion, String> {
    const ERROR: &str = "Invalid version, expected <major.minor.patch> (e.g, '1.4.0')";

    let [major_str, minor_str, patch_str] = input
        .split('.')
        .collect::<Vec<_>>()
        .try_into()
        .map_err(|_| ERROR)?;

    let major = major_str.parse().map_err(|_| ERROR)?;
    let minor = minor_str.parse().map_err(|_| ERROR)?;
    let patch = patch_str.parse().map_err(|_| ERROR)?;
    ...
    fs::read_dir(crates_dir)?
        ...
        .flat_map(|version_str| {
            parse_version(&version_str)
                .map(|version| check_existing_version(crates_dir, version).map(|_| version))
        })
        .collect()
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let tfhe_version_str = args.tfhe_version.exact();
    ...
    let vars = TemplateVars {
        tfhe_version_exact: tfhe_version_str.to_string(),
        tfhe_version: args.tfhe_version.short(),
        tfhe_version_underscored: tfhe_version_underscored.clone(),
    };

    println!(
        "Instantiating data generation crate '{}' for TFHE-rs version {}\n",
        crate_name, tfhe_version_str
    );
