...
struct Args {
    ...
    tfhe_version: Version,
}

fn parse_version(input: &str) -> Result<Version, String> {
    let parsed = Version::parse(input).map_err(|e| e.to_string())?;
    Ok(parsed)
}
...
fn get_existing_versions(crates_dir: &Path) -> Result<Vec<Version>, Box<dyn Error>> {
...
fn check_existing_version(crates_dir: &Path, version: Version) -> Result<(), Box<dyn Error>> {
