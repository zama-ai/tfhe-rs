...
#[derive(Default)]
pub struct VersionizeDispatchEnumInner {
...
impl VersionizeDispatchEnumInner {
...
pub struct VersionizeDispatchEnum(pub Arc<VersionizeDispatchEnumInner>);
...
declare_lint! {
    ...
    /// }
    /// ```
    pub VERSIONIZE_DISPATCH_ENUM,
    Warn,
    "Detects if variants in a DispatchEnum are correctly ordered"
}

impl_lint_pass!(VersionizeDispatchEnum => [VERSIONIZE_DISPATCH_ENUM]);

impl<'tcx> LateLintPass<'tcx> for VersionizeDispatchEnum {
            ...
            if let Some(type_def_id) = get_def_id_from_ty(ty) {
                ...
                if found_impl {
                    ...
                    if !enum_name.to_string().ends_with(TARGET_SUFFIX) {
                        cx.span_lint(VERSIONIZE_DISPATCH_ENUM, enu_id.span, |diag| {
                    ...
                    for (id, variant) in enu_variants_iterator {
                        if let Some((name, span)) = dispatch_variant_inner_type_name(cx, variant) {
                            ...
                            if id == enu_variants_length - 1 {
                                if name.as_str() != suggested_type_name {
                                    cx.span_lint(VERSIONIZE_DISPATCH_ENUM, span, |diag| {
                        ...
                        if variant.ident.as_str() != format!("V{}", id) {
                            cx.span_lint(VERSIONIZE_DISPATCH_ENUM, variant.ident.span, |diag| {
                                diag.primary_message(format!(
...
#[test]
fn ui() {
    dylint_testing::ui_test_example(env!("CARGO_PKG_NAME"), "versionize_dispatch_enum");
}
