                    ...
                    for (id, variant) in enu_variants_iterator {
                        if let Some((name, span)) = dispatch_variant_inner_type_name(cx, variant) {
                            ...
                            if id == enu_variants_length - 1 {
                                ...
                                if name.as_str() != format!("{}V{id}", suggested_type_name) {
                                    cx.span_lint(INVALID_VERSIONIZE_DISPATCH, span, |diag| {
                                        ...
                                        diag.note(format!("The inner type of the firsts variants should be named like the enum without the 'Versions' suffix followed by a version suffix, i.e. {}V{id}", suggested_type_name));
                                    });
                                }
                            }
                        } else {
                            cx.span_lint(INVALID_VERSIONIZE_DISPATCH, variant.span, |diag| {
                                ...
                                diag.note(
                                    "Do not inline fields; variants must wrap a struct type.",
                                );
                            });
                        }
