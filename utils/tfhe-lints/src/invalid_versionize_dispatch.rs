...
const DEPRECATED_TYPE_PATH: &str = "tfhe_versionable::deprecation::Deprecated";
const IGNORE_LINT_MESSAGE: &str = "If you want to ignore this rule, you can add `#[cfg_attr(dylint_lib = \"tfhe_lints\", allow(invalid_dispatch_enum))]` to the enum definition";
        ...
        if let ItemKind::Enum(enu_id, _, enu) = item.kind {
            ...
            if let Some(type_def_id) = get_def_id_from_ty(ty) {
                ...
                for (id, variant) in enu_variants_iterator {
                    ...
                    {
                        if cx.tcx.def_path_str(variant_def_id) == DEPRECATED_TYPE_PATH {
                            continue;
