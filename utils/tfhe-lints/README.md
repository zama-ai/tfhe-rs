...
```
make backward_snapshot_check BASE_SNAPSHOT_DIR=<path> HEAD_SNAPSHOT_DIR=<path>
```
...
```
make backward_snapshot_report BASE_SNAPSHOT_DIR=<path> HEAD_SNAPSHOT_DIR=<path> OUTPUT_FILE=<path>
```
...
**Error** (blocks merge):
- Variants were **removed** from a versions dispatch enum

**Warning** (requires review):
- A versions dispatch enum was **removed**
- A variant's `type_hash` **changed** (the inner type's fields were modified)
- An upgrade's `body_hash` **changed** (the upgrade implementation was modified)
- An upgrade was **removed**

**Neutral** (expected):
- New enums, variants, or upgrades were added
