...
#[derive(Serialize, Clone)]
pub struct LintEnumSnapshot {
    pub enum_name: String,
    pub variants: Vec<LintVariantMeta>,
    pub upgrades: Vec<LintUpgradeMeta>,
}
...
pub struct VersionsDispatchSnapshotInner {
    ...
    // Collected metadata
    pub collected_enums: Mutex<Vec<LintEnumSnapshot>>,
    pub collected_upgrades: Mutex<Vec<LintUpgradeMeta>>,
}
...
/// Compute a SHA-256 hash of the fields of a type (struct or enum).
/// For structs: hashes `"field_name:field_type;"` for each field.
/// For enums: hashes `"variant_name:field_types;"` for each variant.
fn compute_struct_hash<'tcx>(
    tcx: ty::TyCtxt<'tcx>,
    ...
    if adt_def.is_enum() {
        ...
        for field in adt_def.all_fields() {
            ...
            hasher.update(format!("{}:{};", field_name, field_ty));
        }
    }
...
fn hash_upgrade_body(
    ...
    for item_ref in impl_block.items {
        ...
        if impl_item.ident.as_str() != "upgrade" {
            continue;
        }
...
impl<'tcx> LateLintPass<'tcx> for VersionsDispatchSnapshot {
    ...
    fn check_crate_post(&mut self, cx: &LateContext<'tcx>) {
        ...
        let mut final_enums: Vec<LintEnumSnapshot> = enums.clone();
        ...
        let file_suffix = std::env::var("TFHE_BACKWARD_COMPAT_FILE_SUFFIX")
            .unwrap_or_else(|_| "default".to_string());
        let path = std::path::Path::new(&data_dir).join(format!(
            "lint_enum_snapshots_{}_{}.json",
            crate_name, file_suffix
        ));
...
impl VersionsDispatchSnapshot {
        ...
        for (index, variant) in enu.variants.iter().enumerate() {
            ...
            let (inner_type_def_path, inner_type_display, struct_hash) = match field_ty.kind() {
                TyKind::Adt(adt_def, args) => {
                    ...
                    let hash = compute_struct_hash(cx.tcx, *adt_def, args);
    ...
    fn handle_impl<'tcx>(&self, cx: &LateContext<'tcx>, item: &'tcx Item<'_>) {
        // Check if this impl is for the Upgrade trait
        ...
        let body_hash = hash_upgrade_body(cx, item, source_simple, target_simple);

        self.0
            .collected_upgrades
            .lock()
            .unwrap()
            .push(LintUpgradeMeta {
                source_def_path,
                target_def_path,
                body_hash,
            });
