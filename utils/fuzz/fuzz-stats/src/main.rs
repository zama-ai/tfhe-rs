...
/// not sum to `input_samples_count`.
...
impl Histogram {
    fn merge(mut self, other: Self) -> Self {
        ...
        self.input_samples_count += input_samples_count;
...
fn print_table(hist: &Histogram) {
    let pct = |n: usize| {
        if hist.input_samples_count == 0 {
            0.0
        } else {
            100.0 * n as f64 / hist.input_samples_count as f64
        }
    };

    println!("corpus: {} unique inputs", hist.input_samples_count);
