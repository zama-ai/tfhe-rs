...
fn run_parse(args: &Cli) -> anyhow::Result<ParseOutcome> {
    if args.object_sizes {
        ...
        return parse_object_sizes(&args.results, args.backend);
    }

    if args.key_gen {
        println!("Parsing key generation time results... ");
        return parse_key_gen_time(&args.results, args.backend);
    }

    println!("Parsing benchmark results... ");
    if args.bench_type == BenchType::Throughput {
