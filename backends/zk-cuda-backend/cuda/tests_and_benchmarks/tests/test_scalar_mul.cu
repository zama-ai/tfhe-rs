...
TEST_F(ScalarMulTest, G1ScalarMulOne) {
  ...
  G_mont.infinity = false; // CRITICAL: ensure infinity flag is set correctly!

  // DEBUG: Check what we're sending
  std::cout << "G_mont.infinity before MSM: " << G_mont.infinity << std::endl;
  std::cout << "G_mont.x[0]: 0x" << std::hex << G_mont.x.limb[0] << std::dec
            << std::endl;
  ...
  bool all_zero = true;
  for (int i = 0; i < 7; i++) {
    if (h_result.X.limb[i] != 0 || h_result.Y.limb[i] != 0 ||
        h_result.Z.limb[i] != 0) {
      all_zero = false;
      break;
    }
  }
  std::cout << "Result is all zeros: " << (all_zero ? "yes" : "no")
            << std::endl;

  // Debug: Check Z coordinate
  bool z_is_zero = true;
  for (int i = 0; i < 7; i++) {
    if (h_result.Z.limb[i] != 0) {
      z_is_zero = false;
      break;
    }
  }
  std::cout << "Z is zero (infinity): " << (z_is_zero ? "yes" : "no")
            << std::endl;
  ...
  // Debug: Print first limbs of coordinates
  std::cout << "Result X[0] (normal): " << std::hex << result_normal.x.limb[0]
            << std::dec << std::endl;
  std::cout << "Expected X[0] (normal): " << std::hex << G.x.limb[0] << std::dec
            << std::endl;
  std::cout << "Result Y[0] (normal): " << std::hex << result_normal.y.limb[0]
            << std::dec << std::endl;
  std::cout << "Expected Y[0] (normal): " << std::hex << G.y.limb[0] << std::dec
            << std::endl;
