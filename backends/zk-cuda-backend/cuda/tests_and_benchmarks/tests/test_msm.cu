...
TEST_F(MSMTest, G1MSMWithGenerator) {
  ...
  printf("generator: \n");
  // Generator is stored in normal form, but print function expects Montgomery
  // form Convert to Montgomery for printing
  G1Affine G_for_print = g1_generator();
  point_to_montgomery_inplace(G_for_print);
  print_g1_affine(G_for_print);
