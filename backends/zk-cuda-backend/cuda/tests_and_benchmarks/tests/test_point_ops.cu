...
TEST_F(PointOpsTest, G1PointDouble) {
  ...
  print_point("Input G (Montgomery)", G_mont);
  print_point("Output 2*G (Montgomery)", result);
  ...
  // Print for debugging
  print_point("2*G (normal form)", result_normal);
...
TEST_F(PointOpsTest, G1PointAdd) {
  ...
  // Convert to normal form for comparison
  point_from_montgomery<G1Affine>(stream, gpu_index, d_3G, d_3G);
  G1Affine result_normal;
  cuda_memcpy_async_to_cpu(&result_normal, d_3G, sizeof(G1Affine), stream,
                           gpu_index);
  cuda_synchronize_stream(stream, gpu_index);

  // Print for debugging
  print_point("3*G (normal form)", result_normal);
...
TEST_F(PointOpsTest, G1ScalarMul55) {
  ...
  // Convert to normal form for debugging
  point_from_montgomery<G1Affine>(stream, gpu_index, d_result, d_result);
  cuda_synchronize_stream(stream, gpu_index);
  G1Affine result_normal;
  cuda_memcpy_async_to_cpu(&result_normal, d_result, sizeof(G1Affine), stream,
  ...
  // Print for debugging
  print_point("55*G (normal form)", result_normal);
