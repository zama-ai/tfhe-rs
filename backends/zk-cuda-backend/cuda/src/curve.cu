...
// ============================================================================
// Naive MSM Implementation (simple loop-based kernel)
// ============================================================================

// Naive MSM kernel: Each thread computes scalar[i] * point[i], then reduce
template <typename AffineType, typename ProjectiveType>
__global__ void
kernel_naive_msm(ProjectiveType *block_results, // One result per block
                 const AffineType *points, const Scalar *scalars, uint32_t n) {
  using ProjectivePoint = Projective<ProjectiveType>;

  // Each thread handles one point-scalar pair
  ...
  if (idx < n) {
    ...
    ProjectivePoint::affine_to_projective(proj_point, points[idx]);

    // Compute scalar * point
    projective_scalar_mul(local_result, proj_point, scalars[idx]);
  }
...
// Reduction kernel to combine block results into final result
template <typename PointType>
__global__ void kernel_reduce_block_results(PointType *result,
                                            const PointType *block_results,
                                            uint32_t num_blocks) {
  using ProjectivePoint = Projective<PointType>;

  if (threadIdx.x == 0 && blockIdx.x == 0) {
    PointType total;
    ProjectivePoint::point_at_infinity(total);

    for (int i = 0; i < num_blocks; i++) {
      if (!ProjectivePoint::is_infinity(block_results[i])) {
        if (ProjectivePoint::is_infinity(total)) {
          total = block_results[i];
        } else {
          PointType temp;
          ProjectivePoint::projective_add(temp, total, block_results[i]);
          total = temp;
        }
      }
    }
...
point_scalar_mul(PointType &result, const PointType &point,
  ...
  for (int limb = 0; limb <= msb_limb; limb++) {
    int end_bit = (limb == msb_limb) ? msb_bit : 63;
...
// implemented in src/msm.cu, which dispatches to either naive or Pippenger
// implementations based on the USE_NAIVE_MSM compile flag.
  ...
  if (num_warps > 1 && n > 1) {
    ...
    for (uint32_t bucket = lane_id; bucket < MSM_G1_BUCKET_COUNT;
      ...
      for (int t = 0; t < blockDim.x; t++) {
        uint32_t t_warp_id = t / WARP_SIZE;
    ...
    for (uint32_t i = lane_id; i < MSM_G1_BUCKET_COUNT; i += WARP_SIZE) {
...
__global__ void kernel_accumulate_buckets_bigint5_projective_g2(
  ...
  if (num_warps > 1 && n > 1) {
    ...
    // Phase 2: Reduce warp buckets to shared buckets
    // Match template exactly
    for (uint32_t i = lane_id; i < MSM_G2_BUCKET_COUNT; i += WARP_SIZE) {
