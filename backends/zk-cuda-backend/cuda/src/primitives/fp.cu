...
__host__ __device__ void fp_mont_reduce(Fp &c, const UNSIGNED_LIMB *a) {
  ...
  for (int i = 0; i < FP_LIMBS; i++) {
    ...
    for (int j = 0; j < FP_LIMBS; j++) {
      uint64_t acc =
          static_cast<uint64_t>(t[i + j]) +
          static_cast<uint64_t>(u) * static_cast<uint64_t>(p.limb[j]) + carry;
      t[i + j] = static_cast<UNSIGNED_LIMB>(acc);
      carry = acc >> LIMB_BITS;
    }
    // Propagate remaining carry (carry ≤ 2^32-1 at this point)
    int idx = i + FP_LIMBS;
    while (carry != 0 && idx <= 2 * FP_LIMBS) {
      uint64_t acc = static_cast<uint64_t>(t[idx]) + carry;
      t[idx] = static_cast<UNSIGNED_LIMB>(acc);
      carry = acc >> LIMB_BITS;
      idx++;
    }
#else
  ...
  for (int i = 0; i < FP_LIMBS; i++) {
    ...
    for (int j = 0; j < FP_LIMBS; j++) {
      uint64_t acc =
          ...
          static_cast<uint64_t>(a.limb[j]) * static_cast<uint64_t>(b.limb[i]) +
          carry64;
      t[j] = static_cast<UNSIGNED_LIMB>(acc);
      carry64 = acc >> LIMB_BITS;
    }
    uint64_t sum64 = static_cast<uint64_t>(t[FP_LIMBS]) + carry64;
    UNSIGNED_LIMB overflow = static_cast<UNSIGNED_LIMB>(sum64 >> LIMB_BITS);
    t[FP_LIMBS] = static_cast<UNSIGNED_LIMB>(sum64);
#else
    UNSIGNED_LIMB carry = 0;
    for (int j = 0; j < FP_LIMBS; j++) {
      UNSIGNED_LIMB hi, lo;
    ...
    for (int j = 0; j < FP_LIMBS; j++) {
      uint64_t acc =
          static_cast<uint64_t>(t[j]) +
          static_cast<uint64_t>(m) * static_cast<uint64_t>(p.limb[j]) + carry64;
      t[j] = static_cast<UNSIGNED_LIMB>(acc);
      carry64 = acc >> LIMB_BITS;
    }
    // Merge carry from reduction with the overflow from step 1.
    // sum64 ≤ (2^32-1) + (2^32-1) + 1 = 2^33-1, so the new overflow is 0 or 1.
    uint64_t s64 = static_cast<uint64_t>(t[FP_LIMBS]) + carry64 +
                   static_cast<uint64_t>(overflow);
    t[FP_LIMBS] = static_cast<UNSIGNED_LIMB>(s64);
    overflow = static_cast<UNSIGNED_LIMB>(s64 >> LIMB_BITS);
#else
    carry = 0;
    for (int j = 0; j < FP_LIMBS; j++) {
      UNSIGNED_LIMB hi, lo;
