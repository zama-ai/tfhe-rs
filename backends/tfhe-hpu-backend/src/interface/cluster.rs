...
impl HpuCluster {
    pub fn new(config: &HpuConfig, force_reload: bool) -> Result<Self, HpuInstError> {
        ...
        let memload = Arc::new(core::array::from_fn(|_| atomic::AtomicUsize::new(0)));
...
impl HpuCluster {
    /// Compute targeted node for allocation
    /// Nodes are sorted by memload/workload and rhs position
    pub(crate) fn alloc_node(&self) -> PhysId {
        let hpu_id = self.keys().copied().collect::<Vec<_>>();

        // Compute node weight
        let mut hpu_weight = std::iter::zip(self.workload.iter(), self.memload.iter())
            .enumerate()
            .map(|(i, (w, m))| {
                let work_weight = w.load(atomic::Ordering::SeqCst);
                let mem_weight = m.load(atomic::Ordering::SeqCst);
                (i as u8, work_weight + mem_weight)
            })
            .collect::<Vec<_>>();
        hpu_weight.sort_by_key(|&(_i, weight)| weight);

        // Kept only the available ones
        PhysId(
            hpu_weight
                .into_iter()
                .filter(|&(i, _w)| hpu_id.contains(&i))
                .map(|(i, _w)| i)
                .next()
    ...
    pub(crate) fn work_nodes(&self, rhs_ct: &[HpuVarWrapped]) -> Vec<PhysId> {
        let hpu_id = self.keys().copied().collect::<Vec<_>>();
    ...
    pub(crate) fn compute_cmd_map(
        ...
        let dst_pos = dst.iter().map(|v| v.hpu_id).collect::<HashSet<_>>();
