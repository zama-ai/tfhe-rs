...
impl HpuCluster {
    pub fn new(config: &HpuConfig, force_reload: bool) -> Result<Self, HpuInstError> {
        ...
        // Check for HpuNode state
        // Hw is reloaded if needed
        // Warn: This involved kerne load/unload and must be sequential -> No par_iter
        let has_reload = config
            .fpga
            .node_id
            .iter()
            .map(|id| HpuHw::lazy_load(*id, &config.fpga.ffi, force_reload))
            .collect::<Vec<_>>();

        // In case of reload dma interface must be recreate
        if has_reload.contains(&true) {
            for id in config.fpga.node_id.iter() {
                HpuHw::cfg_dma(*id, &config.fpga.ffi)
            }
        }
