...
impl HpuVarWrapped {
    ...
    pub(crate) fn dispatch_and_sync(&self, hid: PhysId, sync: bool) -> Result<(PhysId, SlotId), HpuInternalError> {
    ...
    pub(crate) fn dispatch_on(&self, hid: PhysId) -> (PhysId, SlotId) {
        self.dispatch_and_sync(hid, false).expect("Unexpected sync error")
    }
