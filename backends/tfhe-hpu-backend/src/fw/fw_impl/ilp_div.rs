...
pub fn iop_add_hillissteel_v(
    ...
    let pbs_solve_prop_group_final_v = vec![pbs_solve_prop_group_final0,
                                            pbs_solve_prop_group_final1,
                                            pbs_solve_prop_group_final2];
    let pbs_extract_prop_group_v = vec![pbs_extract_prop_group_0,
    ...
    long_src.iter().zip(short_src.iter()).for_each(
            |(a,b)| {
                let s = a + b;
                sum_a.push(s);
            });
    long_src[add_size..].iter().for_each(
            |x| {
            sum_a.push(x.clone());
        }
    );
    ...
    sum_a.chunks(4).enumerate().for_each( |(c_idx,chk)| {
        chk.iter().enumerate().for_each(|(idx, v)| {
            if c_idx == 0 { // Proceed differently for the 1rst group
    ...
    prop_group_a.chunks_mut(4).enumerate().for_each(|(c_idx,chk)| {
    ...
    let group_nb = group_prop_a.len().div_ceil(4) as usize;
    ...
    (0..step_nb).for_each(|step| {
        ...
        for idx in 0..p_a.len() {
            group_prop_a[(stride + idx)*4+3] = p_a.pop_front().unwrap(); // Fill with new values for next step
        }
    });
...
pub fn iop_is_not_null_vector_v(
    ...
    while stride_size < grp_nb {
        for chk in g_a.chunks_mut(op_nb_bool * stride_size * op_nb) {
            chk.chunks_mut(stride_size * op_nb).rev().fold(
                None, |acc, sub_chk| {
                    if let Some(x) = acc {
                        let tmp = &x + &sub_chk[0];
    ...
    // Third step
    // Apply
    g_a.chunks_mut(op_nb).rev().fold(
        None, |acc, chk| {
            if let Some(x) = acc {
                for v in chk.iter_mut().skip(1) { // [0] is already complete.
                    *v = &*v + x;
...
pub fn iop_opposite_nopropv(
    ...
    // Add the sign
    // !! Workaround
    let mut ct = &src[0] - &src[0];
    ct = &ct + &cst_msg_m1;
    m_src_a.push(ct);
    //m_src_a.push(cst_msg_m1);
...
/// * div_x1_is_not_null_a : list of booleans indicating if the divider is null
///             from a certain block position.
/// * div_x2_is_not_null_a : list of booleans indicating if the divider x2 is null
///             from a certain block position.
/// * div_x3_is_not_null_a : list of booleans indicating if the divider x3 is null
///             from a ce;rtain block position.
/// * mdiv_x1_a : opposite value of divider
/// * mdiv_x2_a : opposite value of divider x2
/// * mdiv_x3_a : opposite value of divider x3
pub fn iop_div_initv(
    prog: &mut Program,
    div_x1_a: &[metavar::MetaVarCell],
) -> (metavar::MetaVarCell,Vec<metavar::MetaVarCell>,Vec<metavar::MetaVarCell>,Vec<metavar::MetaVarCell>,Vec<metavar::MetaVarCell>,Vec<metavar::MetaVarCell>,Vec<metavar::MetaVarCell>,Vec<metavar::MetaVarCell>,Vec<metavar::MetaVarCell>){
    //let props = prog.params();
    //let tfhe_params: asm::DigitParameters = props.clone().into();
    ...
    let div_x1_is_not_null_v = iop_is_not_null_vector_v(prog,div_x1_a);
    let div_x2_is_not_null_v = iop_is_not_null_vector_v(prog,&div_x2_a);
    let div_x3_is_not_null_v = iop_is_not_null_vector_v(prog,&div_x3_a);

    // If the divider is null set quotient to 0
    let keep_div = div_x1_is_not_null_v[0].clone();
    ...
    (keep_div,
        div_x2_a, div_x3_a,
        div_x1_is_not_null_v,div_x2_is_not_null_v,div_x3_is_not_null_v,
        mdiv_x1_a,mdiv_x2_a,mdiv_x3_a)
}
...
pub fn iop_if_then_else_0v(
    ...
    src.iter().map(|ct| {
        ...
        match select_type.unwrap_or(IfThenElse0Select::SelPos0Msg) {
           ...
           IfThenElse0Select::SelPos1MsgCarry1 => x.pbs(&pbs_if_pos1_false_zeroed_msg_carry1,false),
        }
    }).collect::<Vec<_>>()
...
pub fn iop_divx(
    ...
    let (keep_div,
        ...
        mdiv_x1_a,
        mdiv_x2_a,
        mdiv_x3_a) = iop_div_initv(prog,&div_x1_a);

    let div_x_v = vec![div_x1_a, div_x2_a, div_x3_a];
    let div_x_is_not_null_v = vec![div_x1_is_not_null_a,div_x2_is_not_null_a,div_x3_is_not_null_a];
    let mdiv_x_v = vec![mdiv_x1_a,mdiv_x2_a,mdiv_x3_a];
    ...
    for loop_idx in 0..num_a.len(){
        ...
        for xi in 0..3 { // for x1, x2, x3
            // Step 1
            // Sign extension
            let mut ext_mdiv_x_a : Vec<metavar::MetaVarCell> = Vec::new();
            for (_k,ct) in (0..block_nb).into_iter().zip(mdiv_x_v[xi].iter()){
                ext_mdiv_x_a.push(ct.clone());
            };
            for _k in (mdiv_x_v[xi].len()..block_nb).into_iter(){
            ...
            let diff_x_a = iop_add_hillissteel_v(prog,&remain_a,&ext_mdiv_x_a, Some(AddHSPostProcess::NoPostProcess));
