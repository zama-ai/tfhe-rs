...
// 3 bit parities (the kap_ buffers) that let the linear layers absorb the
// key as a levelled sum rather than a bootstrap. 5 PBS layers, paid here so
// the circuit itself never bootstraps key material. Carries no lane
// dimension, whatever the batch size: host_integer_prince tiles it.
//
...
// The middle round's second key xor: the one key xor no parity layer can
// absorb, so it costs a PBS layer of its own. The parity layer upstream
// already emitted the state bits at weights 8 and 4, so buf_u2q holds 4 * m
// and adding the key levelled is enough to build the 4 * m + k that
// lut_flat32 decodes; host_integer_prince builds the same packing with a
// bivariate apply at the input, where the state arrives unscaled.
// buf_u2q -> buf_u4.
template <typename Torus>
...
// Fuses the linear layer with the key xor. Sums the 3 source bits plus the
// key bit levelled, then one PBS takes the parity and scales it by the place
// value that bit holds in its nibble, 8, 4, 2 or 1. State bits sit one per
// block here, so map_comb4_id can then merge 4 blocks into the single nibble
// block the S-box wants, by plain addition. buf_b -> buf_b.
template <typename Torus>
__host__ void
...
// the PBS is the bit extraction the next linear layer needs anyway. One
// nibble block in, its 4 bits out. buf_u4 -> buf_b.
template <typename Torus>
...
// Backward round with its key xor, buf_b -> buf_b: 2 PBS layers. The last
// round instead lands in buf_u2q, since the circuit has to be back in the u2
// shape for the output key xor.
template <typename Torus>
