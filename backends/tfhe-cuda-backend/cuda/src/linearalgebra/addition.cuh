...
__host__ void host_pack_for_overflowing_ops(cudaStream_t stream,
  ...
  // Create a 1-dimensional grid of threads
  int num_blocks = 0, num_threads = 0;
  int num_entries = input_lwe_dimension;
  getNumBlocksAndThreads(num_entries, 512, num_blocks, num_threads);
  dim3 grid(num_blocks, 1, 1);
  dim3 thds(num_threads, 1, 1);

  pack_for_overflowing_ops<T>
      <<<grid, thds, 0, stream>>>(&output[input_lwe_ciphertext_count - 1],
                                  &input_1[input_lwe_ciphertext_count - 1],
                                  &input_2[input_lwe_ciphertext_count - 1],
                                  input_lwe_dimension, message_modulus);
  check_cuda_error(cudaGetLastError());
}
