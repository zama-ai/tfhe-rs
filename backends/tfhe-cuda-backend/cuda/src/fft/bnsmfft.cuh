...
template <class params> __device__ void NSMFFT_direct2(double2 *A, double2 *sh_negtwiddles) {
...
#pragma unroll
    for (Index i = 0; i < BUTTERFLY_DEPTH; i++) {
      ...
      u[i] = (u_stays_in_register) ? u[i] : w;
      v[i] = (u_stays_in_register) ? w : v[i];
      w = sh_negtwiddles[tid / lane_mask + twiddle_shift];
 ...
 * negacyclic inverse fft
 */
template <class params> __device__ void NSMFFT_inverse2(double2 *A, double2 *sh_negtwiddles) {
...
#pragma unroll
    for (Index i = 0; i < BUTTERFLY_DEPTH; ++i) {
      w = (u[i] - v[i]);
      u[i] += v[i];
      v[i] = w * conjugate(sh_negtwiddles[tid / lane_mask + twiddle_shift]);
