...
/**
 * Encrypted AES, bitsliced.
 *
 *   block = bit * num_aes_inputs + input
 *
 *   bit   0 | in_0 in_1 ... in_N |
 *   bit   1 | in_0 in_1 ... in_N |
 *   ...
 *   bit 127 | in_0 in_1 ... in_N |
 *
 * Every AES step acts on the same bit index of every input, so this order
 * puts those blocks on one contiguous run and a single launch or bootstrap
 * batch covers all the inputs. Bit 0 is the top bit of state byte 0.
 *
 * XOR is the sum of two LWE blocks, so it needs no bootstrap, but noise and
 * degree add up. Bootstraps are only needed for the non linear part, the
 * S-box AND gates, and for the flushes that reset noise and degree once the
 * budget is exhausted. Hence bootstrap batches as few and as wide as
 * possible.
 *
 * A round:
 *
 *   SubBytes     the only non linear step, Boyar-Peralta, 32 ANDs in 6
 *                layers plus 9 flushes, so 15 bootstrap layers deep
 *   ShiftRows    fixed GF(2) map, index table, one launch, no bootstrap
 *   xtime        same, writes the doubled operands into the upper half of
 *                the MixColumns workspace
 *   MixColumns   same, reading [shifted | xtime of shifted] in one go
 *   AddRoundKey  a levelled addition
 *   flush        one bootstrap batch over the whole state
 *
 * The index tables live in int_aes_linear_tables
 *
 * The counter is added first. A bit pair encoded as the sum a + b already
 * reads as kill, propagate or generate, and a Sklansky scan turns those
 * into carries.
 *
 * The five things that keep the bootstrap count down, and where they live:
 *
 *   flushes only where the budget forces them   vectorized_sbox_n_bytes
 *   one kernel per layer, not one per wire      int_aes_linear_tables
 *   the four columns in one pass                vectorized_mix_columns
 *   carries scanned in log depth                the counter adder
 *   AND on a sum, entering the PBS at noise 2   batch_vec_and_inplace
 */

/**
 * AddRoundKey is the one levelled step that accumulates into the state
 * instead of gathering a fresh one, so it keeps a kernel of its own where
 * every other layer goes through host_radix_gather_sum.
 */
static constexpr uint32_t AES_ADD_ROUND_KEY_THREADS = 256;
...
__host__ void host_aes_add_round_key(cudaStream_t stream, uint32_t gpu_index,
  ...
  PANIC_IF_FALSE(state->ptr != nullptr && round_key->ptr != nullptr,
                 "Cuda error: AddRoundKey cannot run on unallocated (dry-run) "
                 "ciphertexts");
  PANIC_IF_FALSE(state->lwe_dimension == round_key->lwe_dimension,
                 "Cuda error: AddRoundKey operands must have the same lwe "
                 "dimension");

  cuda_set_device(gpu_index);
  uint32_t lwe_size = state->lwe_dimension + 1;
  uint32_t chunks = CEIL_DIV(lwe_size, AES_ADD_ROUND_KEY_THREADS);
  uint32_t num_out_blocks = num_bits * num_aes_inputs;

  device_aes_add_broadcast_bits<Torus>
      <<<num_out_blocks * chunks, AES_ADD_ROUND_KEY_THREADS, 0, stream>>>(
 ...
 * enters the PBS at noise 2. A bivariate LUT would read the same pair
 * packed as 4a + b, which lands at noise 5 and leaves the chain feeding
 * the gate no room at all. Array-reference parameters make the three
...
__host__ void vectorized_sbox_n_bytes(CudaStreams streams,
  ...
  PANIC_IF_FALSE(num_bytes_parallel * AES_BITS_PER_BYTE ==
                 ...
                 "built for");
  PANIC_IF_FALSE(num_aes_inputs == tables->sbox_gather.num_lanes,
                 "S-box called with a batch size the index tables were not "
                 "built for");
  ...
  host_radix_gather_sum<Torus>(streams, mem->params, reordered_input_buffer,
                               sbox_io_bytes, tables->sbox_gather);
...
#define ADD_ONE(target)                                                        \
  ...
  host_radix_gather_sum<Torus>(streams, mem->params, sbox_io_bytes,
                               &sbox_output_region, tables->sbox_scatter);
...
__host__ void vectorized_shift_rows(CudaStreams streams,
  ...
  auto *tables = mem->linear_tables;
  host_radix_gather_sum<Torus>(streams, mem->params, dest, state,
                               tables->shift_rows);
}
 ...
 * That flush covers the 128 bits of the four columns together, so no
 * column waits on the one before it.
 */
template <typename Torus, typename KSTorus>
__host__ void vectorized_mix_columns(CudaStreams streams,
  ...
  host_radix_gather_sum<Torus>(streams, mem->params, &xtime, shifted,
                               tables->xtime);

  aes_flush_inplace<Torus>(streams, &xtime, mem, bsks, ksks);

  host_radix_gather_sum<Torus>(streams, mem->params, state, workspace,
                               tables->mix_columns);
}
...
__host__ void vectorized_aes_rounds_inplace(
  ...
  for (uint32_t round = 1; round <= num_rounds; ++round) {
    ...
    vectorized_shift_rows<Torus>(streams, &shifted, all_states_bitsliced, mem);
...
/**
 * Stages the plaintext counter bits in the bitsliced, MSB-first order the
 * state uses, and hands them to the device.
 */
template <typename Torus>
__host__ void aes_ctr_stage_counter_bits(
    CudaStreams streams, const Torus *counter_bits_le_all_blocks,
    uint32_t num_aes_inputs, int_aes_encrypt_buffer<Torus> *mem) {
  ...
  cuda_memcpy_async_to_gpu(mem->counter_workspaces->d_counter_bits_buffer,
                           h_bits,
                           safe_mul_sizeof<Torus>(NUM_BITS * num_aes_inputs),
                           streams.stream(0), streams.gpu_index(0));
}

/**
 * Adds the counter to the encrypted IV in place. A bit pair encoded as
 * a + b reads as kill, propagate or generate for 0, 1 and 2, and a
 * counter makes that sum levelled, so only the carry scan and the final sum
 * bits cost a bootstrap. Sklansky walks the 128 positions in seven levels
 * and the sum bits close it, eight bootstrap layers where a ripple carry
 * spends 128.
 */
template <typename Torus, typename KSTorus>
__host__ void vectorized_aes_add_counter_inplace(
  ...
  aes_ctr_stage_counter_bits<Torus>(streams, counter_bits_le_all_blocks,
                                    num_aes_inputs, mem);
  Torus *h_bits = mem->counter_workspaces->h_counter_bits_buffer;

  CudaRadixCiphertextFFI symbols_0, scan, carries;
  as_radix_ciphertext_slice<Torus>(
      &symbols_0, mem->main_workspaces->ctr_adder_workspace, 0, num_blocks);
  as_radix_ciphertext_slice<Torus>(
      &scan, mem->main_workspaces->sbox_input_buffer, 0, num_blocks);
  as_radix_ciphertext_slice<Torus>(&carries,
                                   mem->main_workspaces->sbox_input_buffer,
                                   num_aes_inputs, num_blocks + num_aes_inputs);
  ...
  set_trivial_radix_ciphertext_async<Torus>(
      streams.stream(0), streams.gpu_index(0), &symbols_0,
      mem->counter_workspaces->d_counter_bits_buffer, h_bits, num_blocks,
      mem->params.message_modulus, mem->params.carry_modulus);
  host_addition<Torus>(streams.stream(0), streams.gpu_index(0), &symbols_0,
                       transposed_states, &symbols_0, num_blocks,
                       mem->params.message_modulus, mem->params.carry_modulus);
  set_zero_radix_ciphertext_slice_async<Torus>(
      streams.stream(0), streams.gpu_index(0),
      mem->main_workspaces->sbox_input_buffer, num_blocks,
      num_blocks + num_aes_inputs);

  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0),
                                     &scan, &symbols_0);
  lut->set_lut_indexes_and_broadcast_constant(lut_streams, AES_LUT_CTR_SELECT);

  CudaRadixCiphertextFFI sk_targets, sk_sources;
  const uint32_t half = (NUM_BITS / 2) * num_aes_inputs;
  as_radix_ciphertext_slice<Torus>(
      &sk_targets, mem->main_workspaces->sbox_internal_workspace, 0, half);
  as_radix_ciphertext_slice<Torus>(
      &sk_sources, mem->main_workspaces->sbox_internal_workspace, half,
      2 * half);

  host_prefix_scan_sklansky_inplace<Torus>(streams, &scan, &sk_targets,
                                           &sk_sources, lut, bsks, ksks,
                                           NUM_BITS, num_aes_inputs, false);

  lut->set_lut_indexes_and_broadcast_constant(lut_streams, AES_LUT_CTR_SUM);
  integer_radix_apply_bivariate_lookup_table<Torus>(
      streams, transposed_states, &symbols_0, &carries, bsks, ksks, lut,
