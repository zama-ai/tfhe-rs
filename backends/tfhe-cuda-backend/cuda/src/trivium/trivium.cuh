...
trivium_add(CudaStreams streams, int_trivium_buffer<Torus> *mem,
...
trivium_compute_64_steps(CudaStreams streams, int_trivium_buffer<Torus> *mem,
  ...
  trivium_add(streams, mem, s->temp_t1, &a65_slice, &a92_slice);
  trivium_add(streams, mem, s->temp_t2, &b68_slice, &b83_slice);
  trivium_add(streams, mem, s->temp_t3, &c65_slice, &c110_slice);
  ...
  trivium_add(streams, mem, s->new_a, s->temp_t3, &a68_slice);
  trivium_add(streams, mem, s->new_a, s->new_a, &and_res_a);

  trivium_add(streams, mem, s->new_b, s->temp_t1, &b77_slice);
  trivium_add(streams, mem, s->new_b, s->new_b, &and_res_b);

  trivium_add(streams, mem, s->new_c, s->temp_t2, &c86_slice);
  trivium_add(streams, mem, s->new_c, s->new_c, &and_res_c);

  if (output_dest != nullptr) {
    trivium_add(streams, mem, output_dest, s->temp_t1, s->temp_t2);
    trivium_add(streams, mem, output_dest, output_dest, s->temp_t3);
  }
