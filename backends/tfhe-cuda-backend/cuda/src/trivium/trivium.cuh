...
trivium_compute_64_steps(CudaStreams streams, int_trivium_buffer<Torus> *mem,
  ...
  integer_radix_apply_univariate_lookup_table<Torus>(
      streams, s->temp_t3, s->temp_t3, bsks, ksks, mem->luts->flush_lut,
      s->temp_t3->num_radix_blocks);
  ...
  shift_and_insert_batch(streams, mem, s->a_reg, &flushed_a,
                         TRIVIUM_REGISTER_A_BITS, N);
  shift_and_insert_batch(streams, mem, s->b_reg, &flushed_b,
                         TRIVIUM_REGISTER_B_BITS, N);
  shift_and_insert_batch(streams, mem, s->c_reg, &flushed_c,
...
__host__ void trivium_init(CudaStreams streams, int_trivium_buffer<Torus> *mem,
  ...
  slice_reg_batch<Torus>(&dest_a_slice, s->a_reg, 0, TRIVIUM_KEY_BITS, N);

  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0),
                                     &dest_a_slice, &src_key_slice);

  reverse_bitsliced_radix_inplace<Torus>(streams, mem, s->a_reg,
                                         TRIVIUM_KEY_BITS);
  ...
  slice_reg_batch<Torus>(&dest_b_slice, s->b_reg, 0, TRIVIUM_IV_BITS, N);

  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0),
                                     &dest_b_slice, &src_iv_slice);

  reverse_bitsliced_radix_inplace<Torus>(streams, mem, s->b_reg,
                                         TRIVIUM_IV_BITS);
...
template <typename Torus>
void reverse_bitsliced_radix_inplace_stateful(
...
template <typename Torus>
__host__ void shift_and_insert_batch_stateful(
    CudaStreams streams, CudaRadixCiphertextFFI *shift_workspace,
...
template <typename Torus>
__host__ void trivium_compute_64_steps_stateful(
    CudaStreams streams, int_trivium_stateful_buffer<Torus> *buffer,
  ...
  shift_and_insert_batch_stateful<Torus>(streams, ws->shift_workspace, a_reg,
                                         &flushed_a, TRIVIUM_REGISTER_A_BITS,
                                         N);
  shift_and_insert_batch_stateful<Torus>(streams, ws->shift_workspace, b_reg,
                                         &flushed_b, TRIVIUM_REGISTER_B_BITS,
                                         N);
  shift_and_insert_batch_stateful<Torus>(streams, ws->shift_workspace, c_reg,
                                         &flushed_c, TRIVIUM_REGISTER_C_BITS,
                                         N);
  ...
  if (output_dest != nullptr) {
    ...
    reverse_bitsliced_radix_inplace_stateful<Torus>(
        streams, ws->shift_workspace, output_dest, 64, N);
  }
}

template <typename Torus>
__host__ void host_trivium_init_stateful(
    CudaStreams streams, int_trivium_stateful_buffer<Torus> *buffer,
    CudaRadixCiphertextFFI *a_reg, CudaRadixCiphertextFFI *b_reg,
  ...
  reverse_bitsliced_radix_inplace_stateful<Torus>(streams, ws->shift_workspace,
                                                  a_reg, TRIVIUM_KEY_BITS, N);
  ...
  reverse_bitsliced_radix_inplace_stateful<Torus>(streams, ws->shift_workspace,
                                                  b_reg, TRIVIUM_IV_BITS, N);

  CudaRadixCiphertextFFI dest_c_ones;
  slice_reg_batch<Torus>(&dest_c_ones, c_reg, 108, 3, N);
  ...
  for (int i = 0; i < 18; i++) {
    trivium_compute_64_steps_stateful(streams, buffer, a_reg, b_reg, c_reg,
                                      nullptr, bsks, ksks);
  }
}

template <typename Torus>
__host__ void host_trivium_step_stateful(
    CudaStreams streams, CudaRadixCiphertextFFI *keystream_output,
    CudaRadixCiphertextFFI *a_reg, CudaRadixCiphertextFFI *b_reg,
    CudaRadixCiphertextFFI *c_reg, uint32_t num_steps,
    int_trivium_stateful_buffer<Torus> *buffer, void *const *bsks,
    uint64_t *const *ksks) {
