...
void cuda_trivium_init_async(CudaStreamsFFI streams,
  ...
  host_trivium_init_async<uint64_t>(CudaStreams(streams), buffer, a_reg, b_reg, c_reg,
...
void cuda_trivium_step_async(
  ...
  auto buffer = (int_trivium_buffer<uint64_t> *)mem_ptr;
  host_trivium_step_async<uint64_t>(CudaStreams(streams), keystream_output, a_reg,
