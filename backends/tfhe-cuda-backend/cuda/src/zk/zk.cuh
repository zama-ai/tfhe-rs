...
__host__ void host_expand_without_verification(
  ...
  cuda_lwe_expand_64(streams.stream(0), streams.gpu_index(0), expanded_lwes,
  ...
  if (casting_key_type == SMALL_TO_BIG) {
    ...
    execute_keyswitch_async<Torus>(
        streams.streams(), streams.gpu_indexes(), 1,
        ksed_small_to_big_expanded_lwes, lwe_trivial_indexes_vec[0],
        expanded_lwes, lwe_trivial_indexes_vec[0], casting_keys,
        casting_input_dimension, casting_output_dimension, casting_ks_base_log,
        casting_ks_level, num_lwes);
  ...
  integer_radix_apply_univariate_lookup_table_kb<Torus>(
      streams.streams(), streams.gpu_indexes(), streams.count(), output, input,
      bsks, ksks, ms_noise_reduction_key, message_and_carry_extract_luts,
      2 * num_lwes);
}
