...
//
// Written with the _rn intrinsics, like two_sum in f128.cuh, so nvcc cannot
// contract the TwoSum chain into FMAs and break the error term.
...
relaxed_fp128_accumulate_term(double &dst_hi, double &dst_lo, double xh,
  ...
  const double p = __dmul_rn(signed_xh, yh);
  const double e = __fma_rn(signed_xh, yh, -p);
  const double s = __dadd_rn(dst_hi, p);
  const double bb = __dsub_rn(s, dst_hi);
  const double err =
      __dadd_rn(__dsub_rn(dst_hi, __dsub_rn(s, bb)), __dsub_rn(p, bb));
  dst_hi = s;
  dst_lo = __dadd_rn(dst_lo, __dadd_rn(e, err));
...
// exactly ONE pair, the one at 2*threadIdx.x, which is what opt == 4 gives each
// thread.
...
polynomial_product_accumulate_in_fourier_domain_128_pairs_relaxed_regs(
  ...
  static_assert(
      params::opt == 4,
      "the paired mapping expects exactly 2 complex slots per thread");
  // Planes viewed as double2: thread t owns pair t, planes are degree/4 pairs
  // apart.
  constexpr int pairs_per_plane = params::degree / 4;
  const double2 *second_pairs = reinterpret_cast<const double2 *>(second);
  const int pair = threadIdx.x;

  const double2 b_re_hi = __ldg(&second_pairs[pair + 0 * pairs_per_plane]);
  const double2 b_re_lo = __ldg(&second_pairs[pair + 1 * pairs_per_plane]);
  const double2 b_im_hi = __ldg(&second_pairs[pair + 2 * pairs_per_plane]);
  const double2 b_im_lo = __ldg(&second_pairs[pair + 3 * pairs_per_plane]);
...
// Same product with the left operand read from shared memory. Only the
// bootstrapping key in `second` goes through __ldg.
template <class params, bool opening>
__device__ void
polynomial_product_accumulate_in_fourier_domain_128_pairs_relaxed(
  ...
  constexpr int pairs_per_plane = params::degree / 4;
  const double2 *first_pairs = reinterpret_cast<const double2 *>(first);
  const int pair = threadIdx.x;

  const double2 a_re_hi = first_pairs[pair + 0 * pairs_per_plane];
  const double2 a_re_lo = first_pairs[pair + 1 * pairs_per_plane];
  const double2 a_im_hi = first_pairs[pair + 2 * pairs_per_plane];
  const double2 a_im_lo = first_pairs[pair + 3 * pairs_per_plane];
