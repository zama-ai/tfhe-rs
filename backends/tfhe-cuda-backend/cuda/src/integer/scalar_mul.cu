...
uint64_t scratch_cuda_integer_scalar_mul_64_ks32(
  ...
  return scratch_cuda_scalar_mul<uint64_t, uint32_t>(
      CudaStreams(streams),
      (int_scalar_mul_buffer<uint64_t, uint32_t> **)mem_ptr, num_blocks, params,
      num_scalar_bits, allocate_gpu_memory);
}
...
void cuda_scalar_multiplication_ciphertext_64_ks32_inplace(
  ...
  host_integer_scalar_mul_radix<uint64_t, uint32_t>(
      CudaStreams(streams), lwe_array, decomposed_scalar, has_at_least_one_set,
      reinterpret_cast<int_scalar_mul_buffer<uint64_t, uint32_t> *>(mem), bsks,
