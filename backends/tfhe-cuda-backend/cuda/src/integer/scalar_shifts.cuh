 ...
 * @param lwe_array Ciphertext shifted in place; overwritten with the sign.
 * @param mem Scratch buffer holding the padding-block LUT and tmp_rotated.
 * @param num_blocks Number of radix blocks in lwe_array.
...
__host__ void host_arithmetic_scalar_shift_overshift(
    ...
    KSTorus *const *ksks, uint32_t num_blocks, size_t num_bits_in_block) {
  ...
  as_radix_ciphertext_slice<Torus>(&last_block, lwe_array, num_blocks - 1,
                                   num_blocks);

  if (num_bits_in_block == 1) {
    // A 1-bit-message block already is its own sign bit, so no PBS is needed.
    copy_radix_ciphertext_slice_async<Torus>(
        streams.stream(0), streams.gpu_index(0), &sign_block, 0, 1, lwe_array,
  ...
  for (uint i = 0; i < num_blocks; i++) {
    copy_radix_ciphertext_slice_async<Torus>(streams.stream(0),
                                             streams.gpu_index(0), lwe_array, i,
                                             i + 1, &sign_block, 0, 1);
  }
}
...
__host__ void host_arithmetic_scalar_shift_inplace(
  ...
  if (shift >= total_num_bits) {
    host_arithmetic_scalar_shift_overshift<Torus, KSTorus>(
        streams, lwe_array, mem, bsks, ksks, num_blocks, num_bits_in_block);
    return;
  }
