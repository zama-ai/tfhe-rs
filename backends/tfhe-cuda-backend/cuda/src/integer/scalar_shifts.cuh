...
//      {
        integer_radix_apply_univariate_lookup_table_kb(
            streams, gpu_indexes, gpu_count, padding_block,
        ...
        for (uint i = 0; i < rotations; i++) {
          cuda_memcpy_async_gpu_to_gpu(
              ...
              padding_block, big_lwe_size_bytes, streams[0],
              gpu_indexes[0]);
        }
//      }
//#pragma omp section
//      {
        if (shift_within_block != 0 && rotations != num_blocks) {
          integer_radix_apply_univariate_lookup_table_kb(
              streams, gpu_indexes, gpu_count, last_block,
