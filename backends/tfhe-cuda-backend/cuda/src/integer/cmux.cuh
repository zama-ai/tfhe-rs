...
zero_out_if(CudaStreams streams, CudaRadixCiphertextFFI *lwe_array_out,
  ...
  host_pack_bivariate_blocks_with_single_block<Torus>(streams, tmp_lwe_array_input,
      predicate->lwe_indexes_in, lwe_array_input, lwe_condition,
      predicate->lwe_indexes_in, params.message_modulus, num_radix_blocks);

  integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, lwe_array_out, tmp_lwe_array_input, bsks,
...
__host__ void host_integer_radix_cmux_kb(
  ...
  copy_radix_ciphertext_slice_async<Torus>(
      streams.stream(0), streams.gpu_index(0), mem_ptr->buffer_in, 0, num_radix_blocks,
      lwe_array_true, 0, num_radix_blocks);
  copy_radix_ciphertext_slice_async<Torus>(
      streams.stream(0), streams.gpu_index(0), mem_ptr->buffer_in, num_radix_blocks,
  ...
  integer_radix_apply_bivariate_lookup_table_kb<Torus>(streams, mem_ptr->buffer_out, mem_ptr->buffer_in,
  ...
  host_addition<Torus>(streams.stream(0), streams.gpu_index(0), &mem_true, &mem_true,
                       &mem_false, num_radix_blocks, params.message_modulus,
                       params.carry_modulus);

  integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, lwe_array_out, &mem_true, bsks, ksks,
