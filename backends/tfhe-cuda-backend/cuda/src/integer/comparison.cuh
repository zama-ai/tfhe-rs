...
__host__ void are_all_comparisons_block_true(
  ...
  copy_radix_ciphertext_slice_async<Torus>(streams.stream(0), streams.gpu_index(0), tmp_out,
  ...
  while (remaining_blocks > 0) {
    ...
    for (int i = 0; i < num_chunks; i++) {
      ...
      accumulate_all_blocks<Torus>(streams.stream(0), streams.gpu_index(0), accumulator_ptr,
    ...
    if (are_all_block_true_buffer->op == COMPARISON_TYPE::NE) {
      ...
      if (chunk_lengths[num_chunks - 1] != max_value) {
        ...
        generate_device_accumulator<Torus>(
            streams.stream(0), streams.gpu_index(0), is_max_value_lut->get_lut(0, 1),
        ...
        is_max_value_lut->broadcast_lut(streams);
      }
    ...
    if (remaining_blocks == 1) {
      ...
      integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, lwe_array_out, accumulator, bsks,
      ...
      integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, tmp_out, accumulator, bsks, ksks,
  ...
  while (remaining_blocks > 0) {
    ...
    for (int i = 0; i < num_chunks; i++) {
      uint32_t chunk_length =
          std::min(max_value, begin_remaining_blocks - i * max_value);
      chunk_lengths[i] = chunk_length;
      accumulate_all_blocks<Torus>(streams.stream(0), streams.gpu_index(0), accumulator,
                                   input_blocks, big_lwe_dimension,
                                   chunk_length);
    ...
    if (remaining_blocks == 1) {
      // In the last iteration we copy the output to the final address
      integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, lwe_array_out,
          buffer->tmp_block_accumulated, bsks, ksks, ms_noise_reduction_key,
          lut, 1);
      return;
    } else {
      integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, mem_ptr->tmp_lwe_array_out,
          buffer->tmp_block_accumulated, bsks, ksks, ms_noise_reduction_key,
          lut, num_chunks);
    }
  }
...
__host__ void host_compare_blocks_with_zero(
  ...
  if (num_radix_blocks == 1) {
    // Just copy
    copy_radix_ciphertext_slice_async<Torus>(streams.stream(0), streams.gpu_index(0), sum, 0,
                                             1, lwe_array_in, 0, 1);
    num_sum_blocks = 1;
  } else {
    ...
    while (remainder_blocks > 1) {
      ...
      accumulate_all_blocks<Torus>(streams.stream(0), streams.gpu_index(0), sum_i, chunk,
  ...
  integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, lwe_array_out, sum, bsks, ksks,
...
__host__ void host_integer_radix_equality_check_kb(
  ...
  integer_radix_apply_bivariate_lookup_table_kb<Torus>(streams, comparisons, lwe_array_1, lwe_array_2,
  ...
  are_all_comparisons_block_true<Torus>(streams, lwe_array_out, comparisons, mem_ptr,
...
__host__ void compare_radix_blocks_kb(
  ...
  host_integer_radix_add_scalar_one_inplace<Torus>(streams, lwe_array_out, message_modulus,
      carry_modulus);
}
...
__host__ void tree_sign_reduction(
  ...
  while (partial_block_count > 2) {
    pack_blocks<Torus>(streams.stream(0), streams.gpu_index(0), y, x, partial_block_count,
                       message_modulus);

    integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, x, y, bsks, ksks,
  ...
  last_lut->broadcast_lut(streams);

  // Last leaf
  integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, lwe_array_out, y, bsks, ksks,
...
__host__ void host_integer_radix_difference_check_kb(
  ...
  if (carry_modulus >= message_modulus) {
    ...
    pack_blocks<Torus>(streams.stream(0), streams.gpu_index(0), &lhs, lwe_array_left,
                       packed_num_radix_blocks, message_modulus);
    pack_blocks<Torus>(streams.stream(0), streams.gpu_index(0), &rhs, lwe_array_right,
                       packed_num_radix_blocks, message_modulus);
    ...
    integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, diff_buffer->tmp_packed,
  ...
  if (!mem_ptr->is_signed) {
    ...
    // inputs
    compare_radix_blocks_kb<Torus>(streams, comparisons, &lhs, &rhs, mem_ptr, bsks,
    ...
    if (carry_modulus >= message_modulus) {
      ...
      as_radix_ciphertext_slice<Torus>(&shifted_lwe_array_left, lwe_array_left,
                                       num_radix_blocks - 2,
                                       num_radix_blocks - 1);
      integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, &last_left_block_before_sign_block,
      ...
      as_radix_ciphertext_slice<Torus>(&shifted_lwe_array_right,
                                       ...
                                       num_radix_blocks - 1);
      integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, &last_right_block_before_sign_block,
      ...
      compare_radix_blocks_kb<Torus>(streams, &shifted_comparisons,
      ...
      as_radix_ciphertext_slice<Torus>(&last_right_block, lwe_array_right,
                                       num_radix_blocks - 1, num_radix_blocks);
      integer_radix_apply_bivariate_lookup_table_kb<Torus>(streams, &shifted_comparisons,
  ...
  // final sign
  tree_sign_reduction<Torus>(streams, lwe_array_out,
...
__host__ uint64_t scratch_cuda_integer_radix_comparison_check_kb(
  ...
  *mem_ptr = new int_comparison_buffer<Torus>(streams, op, params, num_radix_blocks, is_signed,
...
__host__ void host_integer_radix_maxmin_kb(
  ...
  // Compute the sign
  host_integer_radix_difference_check_kb<Torus>(streams, mem_ptr->tmp_lwe_array_out,
...
__host__ void host_integer_are_all_comparisons_block_true_kb(
  ...
  // It returns a block encrypting 1 if all input blocks are 1
  // otherwise the block encrypts 0
  are_all_comparisons_block_true<Torus>(streams, lwe_array_out, lwe_array_in, mem_ptr,
      bsks, ksks, ms_noise_reduction_key, num_radix_blocks);
}
...
__host__ void host_integer_is_at_least_one_comparisons_block_true_kb(
  ...
  // It returns a block encrypting 1 if all input blocks are 1
  // otherwise the block encrypts 0
  is_at_least_one_comparisons_block_true<Torus>(streams, lwe_array_out, lwe_array_in, mem_ptr,
      bsks, ksks, ms_noise_reduction_key, num_radix_blocks);
}
#endif
