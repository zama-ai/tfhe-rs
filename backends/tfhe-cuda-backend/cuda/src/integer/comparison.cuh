  ...
  while (remaining_blocks > 0) {
    ...
    if (are_all_block_true_buffer->op == COMPARISON_TYPE::NE) {
      ...
      if (chunk_lengths[num_chunks - 1] != max_value) {
        ...
        generate_device_accumulator_with_cache<Torus>(
            ...
            is_equal_to_num_blocks_lut_f, true, are_all_block_true_buffer->cached_h_lut);
...
__host__ void tree_sign_reduction(
  ...
  generate_device_accumulator_with_cache<Torus>(
      ...
      polynomial_size, message_modulus, carry_modulus, f, true, tree_buffer->cached_h_lut);
