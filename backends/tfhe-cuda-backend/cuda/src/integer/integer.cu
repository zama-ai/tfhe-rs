...
uint64_t scratch_cuda_add_and_propagate_single_carry_64_ks32_inplace(
  ...
  return scratch_cuda_propagate_single_carry_inplace<uint64_t, uint32_t>(
      CudaStreams(streams), (int_sc_prop_memory<uint64_t, uint32_t> **)mem_ptr,
      num_blocks, params, requested_flag, allocate_gpu_memory);
}
...
void cuda_integer_overflowing_sub_64_inplace(
  ...
  host_integer_overflowing_sub<uint64_t, uint64_t>(
