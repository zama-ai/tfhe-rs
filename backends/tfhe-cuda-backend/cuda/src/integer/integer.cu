...
void cuda_full_propagation_64_inplace_async(
  ...
  host_full_propagate_inplace_async<uint64_t>(CudaStreams(streams), input_blocks,
...
void cuda_integer_reverse_blocks_64_inplace_async(
  ...
  host_radix_blocks_reverse_inplace_async<uint64_t>(CudaStreams(streams), lwe_array);
}
