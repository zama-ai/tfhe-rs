...
__host__ void host_integer_scalar_mul_radix(
  ...
  for (size_t shift_amount = 0; shift_amount < msg_bits; shift_amount++) {
    ...
    if (has_at_least_one_set[shift_amount] == 1) {
      copy_radix_ciphertext_slice_async<T>(streams.stream(0), streams.gpu_index(0),
                                           &shift_input, 0, num_radix_blocks,
                                           lwe_array, 0, num_radix_blocks);
      host_integer_radix_logical_scalar_shift_kb_inplace<T>(streams, &shift_input, shift_amount,
          mem->logical_scalar_shift_buffer, bsks, ksks, ms_noise_reduction_key,
          num_radix_blocks);
    } else {
      // create trivial assign for value = 0
      set_zero_radix_ciphertext_slice_async<T>(
          streams.stream(0), streams.gpu_index(0), &shift_input, 0, num_radix_blocks);
    }
  }
  size_t j = 0;
  for (size_t i = 0; i < std::min(num_scalars, num_ciphertext_bits); i++) {
    if (decomposed_scalar[i] == 1) {
      ...
      // create trivial assign for value = 0
      set_zero_radix_ciphertext_slice_async<T>(
          streams.stream(0), streams.gpu_index(0), &block_shift_buffer, 0, i / msg_bits);
      j++;
    }
  }
  ...
  if (j == 0) {
    // lwe array = 0
    set_zero_radix_ciphertext_slice_async<T>(streams.stream(0), streams.gpu_index(0),
                                             lwe_array, 0, num_radix_blocks);
  } else {
    host_integer_partial_sum_ciphertexts_vec_kb<T>(streams, lwe_array, all_shifted_buffer, bsks,
    ...
    host_propagate_single_carry<T>(streams, lwe_array,
...
__host__ void host_integer_small_scalar_mul_radix(
  ...
  device_small_scalar_radix_multiplication<<<grid, thds, 0, streams.stream(0)>>>(
...
__host__ void host_integer_radix_scalar_mul_high_kb(
  ...
  if (scalar_divisor_ffi->is_chosen_multiplier_zero) {
    set_zero_radix_ciphertext_slice_async<Torus>(streams.stream(0), streams.gpu_index(0), ct,
  ...
  if (scalar_divisor_ffi->active_bits != (uint32_t)0 &&
    ...
    if (scalar_divisor_ffi->is_chosen_multiplier_pow2) {
      host_integer_radix_logical_scalar_shift_kb_inplace<Torus>(streams, tmp_ffi,
      ...
      host_integer_scalar_mul_radix<Torus>(streams, tmp_ffi,
...
__host__ void host_integer_radix_signed_scalar_mul_high_kb(
  ...
  host_extend_radix_with_sign_msb<Torus>(streams, tmp_ffi, ct, mem_ptr->extend_radix_mem,
