...
__host__ uint64_t scratch_cuda_integer_radix_signed_scalar_mul_high_kb(
    cudaStream_t const *streams, uint32_t const *gpu_indexes,
    uint32_t gpu_count, int_signed_scalar_mul_high<Torus> **mem_ptr,
    uint32_t num_radix_blocks, uint32_t num_additional_blocks,
    int_radix_params params, bool anticipated_buffer_drop,
    bool allocate_gpu_memory) {

  uint64_t size_tracker = 0;

  *mem_ptr = new int_signed_scalar_mul_high<Torus>(
      streams, gpu_indexes, gpu_count, params, num_radix_blocks,
      num_additional_blocks, allocate_gpu_memory, LEFT_SHIFT,
...
__host__ void host_integer_radix_signed_scalar_mul_high_kb(
    cudaStream_t const *streams, uint32_t const *gpu_indexes,
    uint32_t gpu_count, CudaRadixCiphertextFFI *ct,
    int_signed_scalar_mul_high<Torus> *mem_ptr, Torus *const *ksks,
