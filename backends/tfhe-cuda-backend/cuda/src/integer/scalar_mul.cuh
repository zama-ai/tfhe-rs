...
__host__ void host_integer_radix_scalar_mul_high_kb(
    ...
    CudaModulusSwitchNoiseReductionKeyFFI const *ms_noise_reduction_key,
    void *const *bsks, const CudaScalarDivisorFFI *scalar_properties) {

  if (scalar_properties->is_multiplier_zero) {
  ...
  if (scalar_properties->active_bits_multiplier != (uint32_t)0 &&
      !scalar_properties->is_abs_multiplier_one &&
      tmp_ffi->num_radix_blocks != 0) {

    if (scalar_properties->is_multiplier_pow2) {
      ...
      host_integer_scalar_mul_radix<Torus>(
          streams, gpu_indexes, gpu_count, tmp_ffi,
          scalar_properties->decomposed_multiplier,
          ...
          scalar_properties->num_scalars_multiplier);
    }
  }
