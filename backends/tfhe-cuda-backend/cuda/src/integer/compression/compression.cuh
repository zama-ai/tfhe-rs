...
__host__ void host_extract(cudaStream_t stream, uint32_t gpu_index,
  ...
  auto initial_out_len = 0;
  if (total_lwe_bodies_count % polynomial_size == 0)
    initial_out_len = glwe_dimension * polynomial_size + polynomial_size;
  else if (glwe_index == num_glwes - 1)
    initial_out_len = glwe_dimension * polynomial_size +
                      (total_lwe_bodies_count % polynomial_size);
  else
    initial_out_len = glwe_dimension * polynomial_size + polynomial_size;
