...
__host__ void host_extract(cudaStream_t stream, uint32_t gpu_index,
  ...
  auto lwe_per_glwe = array_in->lwe_per_glwe;

  auto glwe_ciphertext_size = (glwe_dimension + 1) * polynomial_size;

  auto num_glwes = (total_lwe_bodies_count + lwe_per_glwe - 1) / lwe_per_glwe;

  if (lwe_per_glwe != polynomial_size) {
    PANIC("Error: lwe_per_glwe should be equal to polynomial_size");
  }

  // Compressed length of the compressed GLWE we want to extract
  auto body_count = 0;
  auto initial_out_len = glwe_dimension * polynomial_size;
  if (total_lwe_bodies_count % polynomial_size == 0)
    body_count = lwe_per_glwe;
  else if (glwe_index == num_glwes - 1)
    body_count = total_lwe_bodies_count % lwe_per_glwe;
  else
    body_count = lwe_per_glwe;

  initial_out_len += body_count;
