...
void cuda_boolean_bitnot_64_async(CudaStreamsFFI streams,
                                  ...
                                  void *const *ksks) {
  host_boolean_bitnot_async<uint64_t>(CudaStreams(streams), lwe_array,
...
void cuda_bitnot_ciphertext_64(CudaStreamsFFI streams,
  ...
  host_bitnot_async<uint64_t>(cuda_streams, radix_ciphertext, ct_message_modulus,
...
void cuda_integer_bitop_inplace_64_async(
    ...
    void *const *bsks, void *const *ksks) {
  // In-place variant: lwe_array_inout op= lwe_array_2, no aliasing check needed
  host_bitop_async<uint64_t>(CudaStreams(streams), lwe_array_inout, lwe_array_inout,
