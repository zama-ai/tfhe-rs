...
__host__ void host_radix_blocks_rotate_right(
    CudaStreams streams, CudaRadixCiphertextFFI *dst,
...
__host__ void integer_radix_apply_univariate_lookup_table_kb(
  ...
  if (active_gpus.count() == 1) {
    ...
    multi_gpu_scatter_lwe_async<Torus>(
        active_gpus, lwe_array_in_vec,
        (Torus *)lwe_array_in->ptr, lut->h_lwe_indexes_in,
        lut->using_trivial_lwe_indexes, lut->active_gpus.count(), num_radix_blocks,
        big_lwe_dimension + 1);

    /// Apply KS to go from a big LWE dimension to a small LWE dimension
    execute_keyswitch_async<Torus>(active_gpus,
    ...
    multi_gpu_gather_lwe_async<Torus>(active_gpus,
...
__host__ void integer_radix_apply_many_univariate_lookup_table_kb(
  ...
  if (active_gpus.count() == 1) {
    ...
    multi_gpu_gather_many_lut_lwe_async<Torus>(
        active_gpus, (Torus *)lwe_array_out->ptr,
        lwe_after_pbs_vec, lut->h_lwe_indexes_out,
...
__host__ void integer_radix_apply_bivariate_lookup_table_kb(
  ...
  host_pack_bivariate_blocks<Torus>(streams, lwe_array_pbs_in,
...
void host_compute_shifted_blocks_and_states(
  ...
  integer_radix_apply_many_univariate_lookup_table_kb<Torus>(streams, shifted_blocks_and_states, lwe_array,
  ...
  copy_radix_ciphertext_slice_async<Torus>(
      streams.stream(0), streams.gpu_index(0), block_states, 0, num_radix_blocks,
      shifted_blocks_and_states, 0, num_radix_blocks);
  // This works because num_many_lut is hard coded to 2
  copy_radix_ciphertext_slice_async<Torus>(
      streams.stream(0), streams.gpu_index(0), shifted_blocks, 0, num_radix_blocks,
...
void host_resolve_group_carries_sequentially(
  ...
  if (num_groups > 1) {
    ...
    copy_radix_ciphertext_slice_async<Torus>(streams.stream(0), streams.gpu_index(0),
    ...
    for (int i = 0; i < num_loops; i++) {
      ...
      as_radix_ciphertext_slice<Torus>(&shifted_group_resolved_carries,
                                       ...
                                       blocks_to_solve + 1);
      integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, &shifted_group_resolved_carries,
      ...
      copy_radix_ciphertext_slice_async<Torus>(
          streams.stream(0), streams.gpu_index(0), resolved_carries, last_resolved_pos + 1,
...
void host_compute_prefix_sum_hillis_steele(
  ...
  for (int step = 0; step < num_steps; step++) {
    ...
    integer_radix_apply_bivariate_lookup_table_kb<Torus>(streams, &cur_blocks, &cur_blocks, prev_blocks,
...
void host_compute_propagation_simulators_and_group_carries(
  ...
  host_radix_cumulative_sum_in_groups<Torus>(streams.stream(0), streams.gpu_index(0),
                                             propagation_cum_sums, block_states,
                                             num_radix_blocks, group_size);

  auto luts_array_second_step = mem->luts_array_second_step;
  integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, propagation_cum_sums,
      propagation_cum_sums, bsks, ksks, ms_noise_reduction_key,
      luts_array_second_step, num_radix_blocks);

  host_integer_radix_scalar_addition_inplace<Torus>(streams, propagation_cum_sums,
  ...
  if (mem->use_sequential_algorithm_to_resolve_group_carries) {
    ...
    host_resolve_group_carries_sequentially(streams, resolved_carries, grouping_pgns,
    ...
    host_compute_prefix_sum_hillis_steele<Torus>(streams, &shifted_resolved_carries,
...
void host_compute_shifted_blocks_and_borrow_states(
  ...
  integer_radix_apply_many_univariate_lookup_table_kb<Torus>(streams, shifted_blocks_and_borrow_states,
  ...
  copy_radix_ciphertext_slice_async<Torus>(
      streams.stream(0), streams.gpu_index(0), borrow_states, 0, num_radix_blocks,
...
void host_full_propagate_inplace(
  ...
  for (int i = 0; i < num_blocks; i++) {
    ...
    execute_keyswitch_async<Torus>(
        streams.subset_first_gpu(), (Torus *)(mem_ptr->tmp_small_lwe_vector->ptr),
    ...
    copy_radix_ciphertext_slice_async<Torus>(
        streams.stream(0), streams.gpu_index(0), mem_ptr->tmp_small_lwe_vector, 1, 2,
    ...
    if (i < num_blocks - 1) {
      ...
      host_addition<Torus>(streams.stream(0), streams.gpu_index(0), &next_input_block,
...
__host__ void extract_n_bits(
  ...
  if (effective_num_radix_blocks / num_radix_blocks > 0) {
    for (uint i = 1; i < effective_num_radix_blocks / num_radix_blocks; i++) {
      copy_radix_ciphertext_slice_async<Torus>(
          streams.stream(0), streams.gpu_index(0), lwe_array_out, i * num_radix_blocks,
          (i + 1) * num_radix_blocks, lwe_array_in, 0, num_radix_blocks);
    }
  }
  integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, lwe_array_out, lwe_array_out, bsks, ksks,
...
__host__ void reduce_signs(
  ...
  copy_radix_ciphertext_slice_async<Torus>(streams.stream(0), streams.gpu_index(0), signs_a,
  ...
  if (num_sign_blocks > 2) {
    ...
    while (num_sign_blocks > 2) {
      pack_blocks<Torus>(streams.stream(0), streams.gpu_index(0), signs_b, signs_a,
                         num_sign_blocks, message_modulus);
      integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, signs_a, signs_b, bsks, ksks,
          ms_noise_reduction_key, lut, num_sign_blocks / 2);

      if (num_sign_blocks % 2 == 1)
        copy_radix_ciphertext_slice_async<Torus>(
            streams.stream(0), streams.gpu_index(0), signs_a, num_sign_blocks / 2,
  ...
  if (num_sign_blocks == 2) {
    ...
    pack_blocks<Torus>(streams.stream(0), streams.gpu_index(0), signs_b, signs_a,
                       num_sign_blocks, message_modulus);
    integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, signs_array_out, signs_b, bsks, ksks,
    ...
    integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, signs_array_out, signs_a, bsks, ksks,
        ms_noise_reduction_key, lut, 1);
  }
}
...
uint64_t scratch_cuda_apply_univariate_lut_kb(
  ...
  uint64_t size_tracker = 0;
  *mem_ptr = new int_radix_lut<Torus>(streams, params,
...
void host_apply_univariate_lut_kb(
  ...
  integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, radix_lwe_out, radix_lwe_in, bsks, ksks,
...
void host_apply_many_univariate_lut_kb(
  ...
  integer_radix_apply_many_univariate_lookup_table_kb<Torus>(streams, radix_lwe_out, radix_lwe_in, bsks, ksks,
...
void host_apply_bivariate_lut_kb(
  ...
  integer_radix_apply_bivariate_lookup_table_kb<Torus>(streams, radix_lwe_out, radix_lwe_in_1,
...
uint64_t scratch_cuda_propagate_single_carry_kb_inplace(
  ...
  uint64_t size_tracker = 0;
  *mem_ptr = new int_sc_prop_memory<Torus>(streams, params, num_radix_blocks, requested_flag,
...
void host_propagate_single_carry(
  ...
  if (requested_flag == outputFlag::FLAG_CARRY && carry_out == nullptr)
    PANIC("Cuda error: when requesting FLAG_CARRY, carry_out must be a valid "
          "pointer")
  if (uses_carry == 1) {
    host_addition<Torus>(streams.stream(0), streams.gpu_index(0), lwe_array, lwe_array,
                         input_carries, 1, params.message_modulus,
                         params.carry_modulus);
  }

  // Step 1
  host_compute_shifted_blocks_and_states<Torus>(streams, lwe_array, mem->shifted_blocks_state_mem,
  ...
  if (requested_flag == outputFlag::FLAG_CARRY) {
    copy_radix_ciphertext_slice_async<Torus>(
        streams.stream(0), streams.gpu_index(0), &output_flag, 0, 1, block_states,
        num_radix_blocks - 1, num_radix_blocks);
  }
  // Step 2
  host_compute_propagation_simulators_and_group_carries<Torus>(streams, block_states, params,
  ...
  if (requested_flag == outputFlag::FLAG_OVERFLOW ||
    ...
    as_radix_ciphertext_slice<Torus>(
        ...
        num_radix_blocks - 1, num_radix_blocks);
    host_addition<Torus>(streams.stream(0), streams.gpu_index(0), &output_flag, &output_flag,
  ...
  if (requested_flag == outputFlag::FLAG_CARRY) {
    ...
    copy_radix_ciphertext_slice_async<Torus>(
        streams.stream(0), streams.gpu_index(0), prepared_blocks, num_radix_blocks,
        num_radix_blocks + 1, &output_flag, 0, 1);
    integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, mem->output_flag, prepared_blocks,
    ...
    copy_radix_ciphertext_slice_async<Torus>(
        streams.stream(0), streams.gpu_index(0), carry_out, 0, 1, mem->output_flag,
        num_radix_blocks, num_radix_blocks + 1);
  } else {
    auto message_extract = mem->lut_message_extract;
    integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, lwe_array, prepared_blocks, bsks, ksks,
...
void host_add_and_propagate_single_carry(
  ...
  host_addition<Torus>(streams.stream(0), streams.gpu_index(0), lhs_array, lhs_array,
  ...
  // Step 1
  host_compute_shifted_blocks_and_states<Torus>(streams, lhs_array, mem->shifted_blocks_state_mem,
      bsks, ksks, ms_noise_reduction_key, lut_stride, num_many_lut);
  auto block_states = mem->shifted_blocks_state_mem->block_states;
  if (requested_flag == outputFlag::FLAG_OVERFLOW) {
    auto lut_overflow_prep = mem->lut_overflow_flag_prep;
    integer_radix_apply_bivariate_lookup_table_kb<Torus>(streams, &output_flag, mem->last_lhs,
  ...
  if (requested_flag == outputFlag::FLAG_OVERFLOW ||
    ...
    integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, lhs_array, prepared_blocks, bsks, ksks,
...
uint64_t scratch_cuda_integer_overflowing_sub(
  ...
  uint64_t size_tracker = 0;
  *mem_ptr = new int_borrow_prop_memory<Torus>(streams, params, num_radix_blocks,
...
void host_single_borrow_propagate(
  ...
  if (uses_input_borrow == 1) {
    host_unchecked_sub_with_correcting_term<Torus>(
        streams.stream(0), streams.gpu_index(0), lwe_array, lwe_array, input_borrow, 1,
        message_modulus, carry_modulus);
  }
  // Step 1
  host_compute_shifted_blocks_and_borrow_states<Torus>(streams, lwe_array,
  ...
  copy_radix_ciphertext_slice_async<Torus>(
      streams.stream(0), streams.gpu_index(0), mem->overflow_block, 0, 1, borrow_states,
      num_radix_blocks - 1, num_radix_blocks);

  // Step 2
  host_compute_propagation_simulators_and_group_carries<Torus>(streams, borrow_states, params,
      mem->prop_simu_group_carries_mem, bsks, ksks, ms_noise_reduction_key,
  ...
  host_integer_radix_add_scalar_one_inplace<Torus>(streams, prepared_blocks, message_modulus,
      carry_modulus);
  ...
  if (compute_overflow == outputFlag::FLAG_OVERFLOW) {
    ...
    as_radix_ciphertext_slice<Torus>(
        &shifted_simulators, mem->prop_simu_group_carries_mem->simulators,
        num_radix_blocks - 1, num_radix_blocks);
    host_addition<Torus>(streams.stream(0), streams.gpu_index(0), mem->overflow_block,
  ...
  cuda_event_record(mem->incoming_events[0], streams.stream(0), streams.gpu_index(0));
  for (int j = 0; j < mem->active_gpus.count(); j++) {
  ...
  if (compute_overflow == outputFlag::FLAG_OVERFLOW) {
    ...
    integer_radix_apply_univariate_lookup_table_kb<Torus>(
        mem->sub_streams_1, overflow_block,
  ...
  host_negation<Torus>(
      mem->sub_streams_2[0], streams.gpu_index(0), (Torus *)resolved_carries->ptr,
      (Torus *)resolved_carries->ptr, big_lwe_dimension, num_groups);

  host_radix_sum_in_groups<Torus>(
      mem->sub_streams_2[0], streams.gpu_index(0), prepared_blocks, prepared_blocks,
      resolved_carries, num_radix_blocks, mem->group_size);

  auto message_extract = mem->lut_message_extract;
  integer_radix_apply_univariate_lookup_table_kb<Torus>(
      mem->sub_streams_2, lwe_array, prepared_blocks,
...
__host__ void integer_radix_apply_noise_squashing_kb(
  ...
  pack_blocks<InputTorus>(streams.stream(0), streams.gpu_index(0), lwe_array_pbs_in,
                          lwe_array_in, lwe_array_in->num_radix_blocks,
                          params.message_modulus);

  // Since the radix ciphertexts are packed, we have to use the num_radix_blocks
  // from the output ct
  auto active_gpus = streams.active_gpu_subset(lwe_array_out->num_radix_blocks);
  if (active_gpus.count() == 1) {
    ...
    multi_gpu_scatter_lwe_async<InputTorus>(
        ...
        lut->using_trivial_lwe_indexes, lut->active_gpus.count(),
    ...
    execute_pbs_128_async<__uint128_t>(
        active_gpus, lwe_after_pbs_vec, lut->lut_vec,
