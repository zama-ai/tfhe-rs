 ...
 * input_1 (lhs) by computing lhs + (z - rhs), where z = ceil(degree /
 * message_modulus) * message_modulus is a correcting term added to each block
 * body (encoded as w = z * delta) to prevent plaintext underflow. No carry
...
/** @brief Subtracts a plaintext vector from a radix-ciphertext by modifying
 * only the body of each block. Noise levels are propagated unchanged. This is a
 * fully levelled subtraction and no PBS is performed.
 * @param output destination radix-ciphertext
 * @param lwe_input input radix-ciphertext
 * @param d_plaintext_input GPU array of plaintexts, one per ciphertext block
 * @param h_plaintext_degrees CPU array of plaintext degrees, one per block
