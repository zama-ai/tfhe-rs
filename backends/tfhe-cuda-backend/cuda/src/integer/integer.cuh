...
__host__ void integer_radix_apply_univariate_lookup_table_kb(
  ...
  if (active_gpus.count() == 1) {
    ...
    cuda_event_record(lut->event_scatter_in, streams.stream(0),
                      streams.gpu_index(0));
    for (int j = 1; j < active_gpus.count(); j++) {
    ...
    multi_gpu_scatter_lwe_async<Torus>(
        active_gpus, lwe_array_in_vec, (Torus *)lwe_array_in->ptr,
        lut->lwe_indexes_in, lut->using_trivial_lwe_indexes,
        lut->lwe_aligned_vec, lut->active_gpus.count(), num_radix_blocks,
        big_lwe_dimension + 1);
    POP_RANGE()
    /// Apply KS to go from a big LWE dimension to a small LWE dimension
    execute_keyswitch_async<Torus>(
        active_gpus, lwe_after_ks_vec, lwe_trivial_indexes_vec,
    ...
    /// dimension to a big LWE dimension
    execute_pbs_async<Torus, Torus>(
        active_gpus, lwe_after_pbs_vec, lwe_trivial_indexes_vec, lut->lut_vec,
        lut->lut_indexes_vec, lwe_after_ks_vec, lwe_trivial_indexes_vec, bsks,
        ms_noise_reduction_key, lut->buffer, glwe_dimension,
        small_lwe_dimension, polynomial_size, pbs_base_log, pbs_level,
        grouping_factor, num_radix_blocks, pbs_type, num_many_lut, lut_stride);
    ...
    multi_gpu_gather_lwe_async<Torus>(
        active_gpus, (Torus *)lwe_array_out->ptr, lwe_after_pbs_vec,
...
__host__ void integer_radix_apply_bivariate_lookup_table_kb(
  ...
  if (active_gpus.count() == 1) {
    ...
    PUSH_RANGE("scatter")
    multi_gpu_scatter_lwe_async<Torus>(
        active_gpus, lwe_array_in_vec, (Torus *)lwe_array_pbs_in->ptr,
        lut->lwe_indexes_in, lut->using_trivial_lwe_indexes,
        lut->lwe_aligned_vec, lut->active_gpus.count(), num_radix_blocks,
        big_lwe_dimension + 1);
    POP_RANGE()
...
__host__ void reduce_signs(
  ...
  auto active_gpus = mem_ptr->active_gpus;
...
uint64_t scratch_cuda_apply_univariate_lut_kb(
  ...
  *(*mem_ptr)->get_degree(0) = lut_degree;
  auto active_gpus = streams.active_gpu_subset(num_radix_blocks);
  (*mem_ptr)->broadcast_lut(active_gpus);
  POP_RANGE()
  return size_tracker;
}
...
__host__ void integer_radix_apply_noise_squashing_kb(
  ...
  if (active_gpus.count() == 1) {
    ...
    multi_gpu_scatter_lwe_async<InputTorus>(
        active_gpus, lwe_array_in_vec, (InputTorus *)lwe_array_pbs_in->ptr,
        lut->lwe_indexes_in, lut->using_trivial_lwe_indexes,
        lut->lwe_aligned_scatter_vec, lut->active_gpus.count(),
    ...
    execute_pbs_async<uint64_t, __uint128_t>(
        active_gpus, lwe_after_pbs_vec, lwe_trivial_indexes_vec, lut->lut_vec,
        lwe_trivial_indexes_vec, lwe_after_ks_vec, lwe_trivial_indexes_vec,
        bsks, ms_noise_reduction_key, lut->pbs_buffer, glwe_dimension,
        small_lwe_dimension, polynomial_size, pbs_base_log, pbs_level,
        grouping_factor, lwe_array_out->num_radix_blocks, params.pbs_type, 0,
        0);
    ...
    multi_gpu_gather_lwe_async<__uint128_t>(
        active_gpus, (__uint128_t *)lwe_array_out->ptr, lwe_after_pbs_vec,
