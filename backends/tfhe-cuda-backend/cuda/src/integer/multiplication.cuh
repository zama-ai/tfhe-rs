...
__host__ void host_integer_partial_sum_ciphertexts_vec_kb(
  ...
  if (terms->num_radix_blocks == 0) {
    mem_ptr->allocated_luts_message_carry = false;
    return;
  }
  ...
  // Since setup_lookup_tables is called in the host the release of memory for
  // the lut has to be done here as well
  if (!mem_ptr->mem_reuse) {
    if (mem_ptr->allocated_luts_message_carry) {
      mem_ptr->luts_message_carry->release(streams, gpu_indexes, gpu_count);
      delete mem_ptr->luts_message_carry;
    }
  }
}
