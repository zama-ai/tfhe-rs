...
__host__ uint64_t scratch_cuda_integer_partial_sum_ciphertexts_vec_kb(
  ...
  *mem_ptr = new int_sum_ciphertexts_vec_memory<Torus>(streams, params, num_blocks_in_radix,
...
__host__ void host_integer_partial_sum_ciphertexts_vec_kb(
  ...
  if (num_radix_in_vec == 2) {
    ...
    as_radix_ciphertext_slice<Torus>(&terms_slice, terms, num_radix_blocks,
                                     2 * num_radix_blocks);
    host_addition<Torus>(streams.stream(0), streams.gpu_index(0), radix_lwe_out, terms,
  ...
  mem_ptr->setup_lookup_tables(streams,
  ...
  while (needs_processing) {
    ...
    prepare_new_columns_and_pbs_indexes<<<1, num_radix_blocks, 0, streams.stream(0)>>>(
    ...
    if (active_gpus.count() == 1) {
      ...
      cuda_memcpy_async_to_cpu(
          ...
          total_ciphertexts * sizeof(Torus), streams.stream(0), streams.gpu_index(0));
      cuda_memcpy_async_to_cpu(
  ...
  if (mem_ptr->reduce_degrees_for_single_carry_propagation) {
    ...
    if (active_gpus.count() == 1) {
      ...
      cuda_memcpy_async_to_cpu(
          h_lwe_indexes_in_pinned, luts_message_carry->lwe_indexes_in,
          num_blocks_in_apply_lut * sizeof(Torus), streams.stream(0), streams.gpu_index(0));
      cuda_memcpy_async_to_cpu(
          h_lwe_indexes_out_pinned, luts_message_carry->lwe_indexes_out,
          num_blocks_in_apply_lut * sizeof(Torus), streams.stream(0), streams.gpu_index(0));
      cuda_synchronize_stream(streams.stream(0), streams.gpu_index(0));
      ...
      integer_radix_apply_univariate_lookup_table_kb<Torus>(
          active_gpus, current_blocks, radix_lwe_out,
...
__host__ void host_integer_mult_radix_kb(
  ...
  if (is_bool_right) {
    zero_out_if<Torus>(streams, radix_lwe_out,
  ...
  integer_radix_apply_bivariate_lookup_table_kb<Torus>(streams, block_mul_res, block_mul_res,
  ...
  host_integer_partial_sum_ciphertexts_vec_kb<Torus>(streams, radix_lwe_out, vector_result_sb, bsks,
