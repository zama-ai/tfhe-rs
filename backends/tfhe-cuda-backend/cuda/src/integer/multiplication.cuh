...
__global__ inline void debug_kernel(uint32_t **columns,
                                    uint32_t *columns_counter, uint64_t *blocks,
                                    int L, int N) {
  ...
  for (int i = 0; i < L; i++) {
    ...
    for (int j = 0; j < columns_counter[i]; j++) {
      size_t block_id = columns[i][j];
      printf("%lu ", blocks[block_id * 2049 + 2048]);
    }
    printf("\n");
  }
}
...
__host__ void host_integer_partial_sum_ciphertexts_vec_kb(
  ...
  while (needs_processing) {
    ...
    debug_kernel<<<1, 1>>>(d_new_columns, d_new_columns_counter,
