...
__host__ uint64_t scratch_cuda_integer_radix_shift_and_rotate_kb(
  ...
  *mem_ptr = new int_shift_and_rotate_buffer<Torus>(streams, shift_type, is_signed, params,
...
__host__ void host_integer_radix_shift_and_rotate_kb_inplace(
  ...
  extract_n_bits<Torus>(streams, bits, lwe_array, bsks,
  ...
  extract_n_bits<Torus>(streams, shift_bits, lwe_shift,
  ...
  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0), input_bits_a,
                                     bits);
  for (int d = 0; d < max_num_bits_that_tell_shift; d++) {
    ...
    copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0), input_bits_b,
                                       input_bits_a);
    auto rotations = 1 << d;
    ...
    case RIGHT_SHIFT:
      ...
      if (input_bits_b->num_radix_blocks != total_nb_bits)
        PANIC("Cuda error: incorrect number of blocks")
      host_radix_blocks_rotate_left<Torus>(streams,
                                           rotated_input, input_bits_b,
                                           rotations, total_nb_bits);
    ...
    // control_bit|b|a
    host_pack_bivariate_blocks<Torus>(streams, mux_inputs, mux_lut->lwe_indexes_out,
    ...
    host_add_the_same_block_to_all_blocks<Torus>(
        streams.stream(0), streams.gpu_index(0), mux_inputs, mux_inputs, &shift_bit,
        mem->params.message_modulus, mem->params.carry_modulus);

    // we have
    // control_bit|b|a
    integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, input_bits_a, mux_inputs, bsks, ksks,
  ...
  for (int i = 0; i < num_radix_blocks; i++) {
    ...
    copy_radix_ciphertext_slice_async<Torus>(
        streams.stream(0), streams.gpu_index(0), lwe_array, i, i + 1, input_bits_a,
  ...
  for (int i = bits_per_block - 2; i >= 0; i--) {
    ...
    for (int j = 0; j < num_radix_blocks; j++) {
      ...
      host_addition<Torus>(streams.stream(0), streams.gpu_index(0), &block, &block,
    ...
    integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, lwe_array, lwe_array, bsks, ksks,
