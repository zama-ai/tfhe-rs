...
__host__ void integer_radix_unsigned_scalar_difference_check_kb(
  ...
  if (num_scalar_blocks == 0) {
    ...
    as_radix_ciphertext_slice<Torus>(&lwe_array_msb_out,
                                     mem_ptr->tmp_lwe_array_out, 1,
                                     num_radix_blocks + 1);
    ...
    //////////////
    // lsb
    Torus *lhs = (Torus *)diff_buffer->tmp_packed->ptr;
    Torus *rhs = lhs + num_radix_blocks / 2 * big_lwe_size;

    pack_blocks<Torus>(lsb_streams[0], gpu_indexes[0], lhs,
                       (Torus *)lwe_array_in->ptr, big_lwe_dimension,
    ...
    pack_blocks<Torus>(streams[0], gpu_indexes[0], lhs,
                       (Torus *)lwe_array_in->ptr, big_lwe_dimension,
                       num_lsb_radix_blocks, message_modulus);
...
__host__ void integer_radix_signed_scalar_difference_check_kb(
  ...
  if (num_scalar_blocks == 0) {
    ...
    as_radix_ciphertext_slice<Torus>(
        &sign_block, &msb, num_msb_radix_blocks - 1, msb.num_radix_blocks);
    integer_radix_apply_bivariate_lookup_table_kb<Torus>(
    ...
    scalar_compare_radix_blocks_kb<Torus>(lsb_streams, gpu_indexes, gpu_count,
                                          lwe_array_ct_out,
                                          diff_buffer->tmp_packed, rhs, mem_ptr,
...
__host__ void host_integer_radix_scalar_equality_check_kb(
  ...
  auto lsb = (Torus *)lwe_array_in->ptr;
  CudaRadixCiphertextFFI msb_in;
  as_radix_ciphertext_slice<Torus>(&msb_in, lwe_array_in, num_lsb_radix_blocks,
                                   lwe_array_in->num_radix_blocks);

  auto lwe_array_lsb_out = (Torus *)mem_ptr->tmp_lwe_array_out->ptr;
  ...
  if (num_halved_scalar_blocks > 0) {
    auto packed_blocks = (Torus *)mem_ptr->tmp_packed_input->ptr;
    auto packed_scalar =
