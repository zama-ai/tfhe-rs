...
__host__ uint64_t scratch_integer_unsigned_scalar_div_radix(
  ...
  *mem_ptr = new int_unsigned_scalar_div_mem<Torus>(streams, params, num_radix_blocks,
...
__host__ void host_integer_unsigned_scalar_div_radix(
  ...
  if (scalar_divisor_ffi->is_divisor_pow2) {
    host_integer_radix_logical_scalar_shift_kb_inplace<Torus>(streams, numerator_ct,
  ...
  if (scalar_divisor_ffi->divisor_has_more_bits_than_numerator) {
    copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0), numerator_ct,
                                       mem_ptr->tmp_ffi);
    return;
  }
  ...
  if (scalar_divisor_ffi->is_chosen_multiplier_geq_two_pow_numerator) {
    ...
    host_integer_radix_scalar_mul_high_kb<Torus>(streams, numerator_cpy,
        mem_ptr->scalar_mul_high_mem, ksks, ms_noise_reduction_key, bsks,
        scalar_divisor_ffi);

    host_sub_and_propagate_single_carry<Torus>(streams, numerator_ct, numerator_cpy, nullptr,
        nullptr, mem_ptr->sub_and_propagate_mem, bsks, ksks,
        ms_noise_reduction_key, FLAG_NONE, (uint32_t)0);

    host_integer_radix_logical_scalar_shift_kb_inplace<Torus>(streams, numerator_ct, (uint32_t)1,
        mem_ptr->logical_scalar_shift_mem, bsks, ksks, ms_noise_reduction_key,
        numerator_ct->num_radix_blocks);

    host_add_and_propagate_single_carry<Torus>(streams, numerator_ct, numerator_cpy, nullptr,
...
__host__ uint64_t scratch_integer_signed_scalar_div_radix_kb(
  ...
  *mem_ptr = new int_signed_scalar_div_mem<Torus>(streams, params, num_radix_blocks,
      scalar_divisor_ffi, allocate_gpu_memory, size_tracker);
...
__host__ void host_integer_signed_scalar_div_radix_kb(
  ...
  if (scalar_divisor_ffi->is_abs_divisor_one) {
    if (scalar_divisor_ffi->is_divisor_negative) {
      ...
      host_integer_radix_negation<Torus>(streams, tmp, numerator_ct,
  ...
  if (scalar_divisor_ffi->is_divisor_pow2) {
    copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0), tmp,
                                       numerator_ct);

    host_integer_radix_arithmetic_scalar_shift_kb_inplace<Torus>(streams, tmp,
        scalar_divisor_ffi->chosen_multiplier_num_bits - 1,
        mem_ptr->arithmetic_scalar_shift_mem, bsks, ksks,
        ms_noise_reduction_key);

    host_integer_radix_logical_scalar_shift_kb_inplace<Torus>(streams, tmp,
        numerator_bits - scalar_divisor_ffi->chosen_multiplier_num_bits,
        mem_ptr->logical_scalar_shift_mem, bsks, ksks, ms_noise_reduction_key,
        tmp->num_radix_blocks);

    host_add_and_propagate_single_carry<Torus>(streams, tmp, numerator_ct, nullptr, nullptr,
    ...
    host_integer_radix_signed_scalar_mul_high_kb<Torus>(streams, tmp, mem_ptr->scalar_mul_high_mem,
        ksks, scalar_divisor_ffi, ms_noise_reduction_key, bsks);

    host_integer_radix_arithmetic_scalar_shift_kb_inplace<Torus>(streams, tmp, scalar_divisor_ffi->shift_post,
        mem_ptr->arithmetic_scalar_shift_mem, bsks, ksks,
        ms_noise_reduction_key);

    CudaRadixCiphertextFFI *xsign = mem_ptr->xsign_ffi;
    copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0), xsign,
                                       numerator_ct);

    host_integer_radix_arithmetic_scalar_shift_kb_inplace<Torus>(streams, xsign, numerator_bits - 1,
        mem_ptr->arithmetic_scalar_shift_mem, bsks, ksks,
        ms_noise_reduction_key);

    host_sub_and_propagate_single_carry<Torus>(streams, tmp, xsign, nullptr, nullptr,
  ...
  if (scalar_divisor_ffi->is_divisor_negative) {
    host_integer_radix_negation<Torus>(streams, numerator_ct, tmp,
        mem_ptr->params.message_modulus, mem_ptr->params.carry_modulus,
        numerator_ct->num_radix_blocks);
  } else {
    copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0), numerator_ct,
...
__host__ uint64_t scratch_integer_unsigned_scalar_div_rem_radix(
  ...
  uint64_t size_tracker = 0;
  *mem_ptr = new int_unsigned_scalar_div_rem_buffer<Torus>(streams, params, num_radix_blocks,
...
__host__ void host_integer_unsigned_scalar_div_rem_radix(
  ...
  host_integer_unsigned_scalar_div_radix(streams, quotient_ct, mem_ptr->unsigned_div_mem,
      bsks, ksks, ms_noise_reduction_key, scalar_divisor_ffi);

  if (scalar_divisor_ffi->is_divisor_pow2) {

    copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0), remainder_ct,
                                       numerator_ct);
    ...
    if (!scalar_divisor_ffi->is_divisor_zero) {
      ...
      if (!scalar_divisor_ffi->is_abs_divisor_one &&
        ...
        host_integer_scalar_mul_radix<Torus>(streams, remainder_ct, decomposed_divisor,
    ...
    host_sub_and_propagate_single_carry(streams, numerator_ct, remainder_ct, nullptr,
        nullptr, mem_ptr->sub_and_propagate_mem, bsks, ksks,
        ms_noise_reduction_key, FLAG_NONE, (uint32_t)0);
...
__host__ uint64_t scratch_integer_signed_scalar_div_rem_radix(
  ...
  *mem_ptr = new int_signed_scalar_div_rem_buffer<Torus>(streams, params, num_radix_blocks,
      scalar_divisor_ffi, active_bits_divisor, allocate_gpu_memory,
      size_tracker);
...
__host__ void host_integer_signed_scalar_div_rem_radix(
  ...
  host_integer_signed_scalar_div_radix_kb(streams, quotient_ct, mem_ptr->signed_div_mem,
  ...
  if (!scalar_divisor_ffi->is_divisor_negative &&
    ...
    host_integer_radix_logical_scalar_shift_kb_inplace(streams, remainder_ct,
        scalar_divisor_ffi->ilog2_divisor, mem_ptr->logical_scalar_shift_mem,
