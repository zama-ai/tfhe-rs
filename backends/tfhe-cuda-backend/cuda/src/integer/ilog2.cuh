...
__host__ void host_integer_prepare_count_of_consecutive_bits(
  ...
  host_apply_univariate_lut_kb<Torus>(streams, tmp,
  ...
  host_compute_prefix_sum_hillis_steele<uint64_t>(streams, ciphertext, tmp, mem_ptr->biv_lut_mem,
...
__host__ uint64_t scratch_integer_count_of_consecutive_bits(
  ...
  *mem_ptr = new int_count_of_consecutive_bits_buffer<Torus>(streams, params, num_radix_blocks,
...
__host__ void host_integer_count_of_consecutive_bits(
  ...
  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0), ct_prepared,
                                     input_ct);

  // Prepare count of consecutive bits
  //
  host_integer_prepare_count_of_consecutive_bits(streams, ct_prepared, mem_ptr->prepare_mem, bsks,
  ...
  host_integer_partial_sum_ciphertexts_vec_kb<Torus>(streams, output_ct, cts, bsks, ksks,
      ms_noise_reduction_key, mem_ptr->sum_mem, counter_num_blocks,
      ct_prepared->num_radix_blocks);

  host_propagate_single_carry<Torus>(streams, output_ct,
...
__host__ uint64_t scratch_integer_ilog2(
  ...
  *mem_ptr = new int_ilog2_buffer<Torus>(streams, params, input_num_blocks,
...
__host__ void host_integer_ilog2(
  ...
  host_integer_prepare_count_of_consecutive_bits<Torus>(streams, mem_ptr->ct_in_buffer,
  ...
  //
  host_integer_partial_sum_ciphertexts_vec_kb<Torus>(streams, mem_ptr->sum_output_not_propagated,
  ...
  host_integer_partial_sum_ciphertexts_vec_kb<Torus>(streams, output_ct, mem_ptr->sum_input_cts, bsks,
      ksks, ms_noise_reduction_key, mem_ptr->sum_mem,
      mem_ptr->counter_num_blocks, 3);

  host_full_propagate_inplace<Torus>(streams, output_ct, mem_ptr->final_propagate_mem,
