...
__host__ uint64_t scratch_cuda_integer_abs_kb(
  ...
  if (is_signed) {
    *mem_ptr = new int_abs_buffer<Torus>(streams,
                                         params, num_blocks,
...
__host__ void host_integer_abs_kb(
  ...
  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0), mask, ct);

  host_integer_radix_arithmetic_scalar_shift_kb_inplace<Torus>(streams, mask, num_bits_in_ciphertext - 1,
  ...
  host_propagate_single_carry<Torus>(streams, ct, nullptr, nullptr, mem_ptr->scp_mem,
      bsks, ksks, ms_noise_reduction_key, requested_flag, uses_carry);

  host_integer_radix_bitop_kb<Torus>(streams, ct, mask,
