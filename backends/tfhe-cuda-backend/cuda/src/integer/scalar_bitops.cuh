...
__host__ void host_integer_radix_scalar_bitop_kb(
  ...
  if (num_clear_blocks == 0) {
    if (op == SCALAR_BITAND) {
      ...
      if (input != output)
        copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0), output,
                                           input);
    }
  } else {
    ...
    cuda_memcpy_async_gpu_to_gpu(lut->get_lut_indexes(0, 0), clear_blocks,
                                 num_clear_blocks * sizeof(Torus), streams.stream(0), streams.gpu_index(0));
    lut->broadcast_lut(streams);

    integer_radix_apply_univariate_lookup_table_kb<Torus>(streams, output, input, bsks, ksks,
