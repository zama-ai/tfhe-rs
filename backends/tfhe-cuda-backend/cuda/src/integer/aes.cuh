...
__host__ __forceinline__ void vectorized_sbox_flush(
  ...
  integer_radix_apply_univariate_lookup_table_kb<Torus>(
      streams, gpu_indexes, gpu_count, data, data, bsks, ksks,
      ms_noise_reduction_key, mem->sbox_flush_lut, data->num_radix_blocks);
}
...
__host__ __forceinline__ void vectorized_scalar_add_one_flush(
  ...
  host_integer_radix_add_scalar_one_inplace<Torus>(
      ...
      mem->params.carry_modulus);
  vectorized_sbox_flush<Torus>(streams, gpu_indexes, gpu_count, data, mem, bsks,
...
__host__ void vectorized_sbox_byte(
  ...
  for (int i = 0; i < 22; ++i)
    as_radix_ciphertext_slice<Torus>(&y[i], mem->sbox_vars_buffer,
                                     i * num_blocks, (i + 1) * num_blocks);
  for (int i = 0; i < 68; ++i)
    as_radix_ciphertext_slice<Torus>(&t[i], mem->sbox_vars_buffer,
                                     (22 + i) * num_blocks,
                                     (23 + i) * num_blocks);
  for (int i = 0; i < 18; ++i)
    as_radix_ciphertext_slice<Torus>(&z[i], mem->sbox_vars_buffer,
                                     (22 + 68 + i) * num_blocks,
                                     (23 + 68 + i) * num_blocks);

#define VEC_XOR(out, a, b)                                                     \
  vectorized_aes_xor<Torus>(streams, gpu_indexes, gpu_count, mem, out, a, b)
#define VEC_AND(out, a, b)                                                     \
  vectorized_aes_and<Torus>(streams, gpu_indexes, gpu_count, out, a, b, mem,   \
                            bsks, ksks, ms_noise_reduction_key)
#define VEC_FLUSH(data)                                                        \
  vectorized_sbox_flush<Torus>(streams, gpu_indexes, gpu_count, data, mem,     \
                               bsks, ksks, ms_noise_reduction_key)
  ...
  VEC_FLUSH(&y[18]);
  VEC_AND(&t[2], &y[15], &y[12]);
  VEC_AND(&t[3], &y[3], &y[6]);
  VEC_XOR(&t[4], &t[3], &t[2]);
  VEC_AND(&t[5], &x[7], &y[4]);
  VEC_XOR(&t[6], &t[5], &t[2]);
  VEC_AND(&t[7], &y[13], &y[16]);
  VEC_AND(&t[8], &y[1], &y[5]);
  VEC_XOR(&t[9], &t[8], &t[7]);
  VEC_AND(&t[10], &y[2], &y[7]);
  VEC_XOR(&t[11], &t[10], &t[7]);
  VEC_AND(&t[12], &y[9], &y[11]);
  VEC_AND(&t[13], &y[14], &y[17]);
  VEC_XOR(&t[14], &t[13], &t[12]);
  VEC_AND(&t[15], &y[8], &y[10]);
  ...
  VEC_FLUSH(&t[25]);
  VEC_AND(&t[26], &t[21], &t[23]);
  VEC_XOR(&t[27], &t[24], &t[26]);
  VEC_FLUSH(&t[27]);
  VEC_AND(&t[28], &t[25], &t[27]);
  ...
  VEC_FLUSH(&t[35]);
  VEC_AND(&t[36], &t[24], &t[35]);
  VEC_XOR(&t[37], &t[36], &t[34]);
  VEC_XOR(&t[38], &t[27], &t[36]);
  VEC_FLUSH(&t[38]);
  VEC_AND(&t[39], &t[38], &t[29]);
  ...
  VEC_FLUSH(&t[45]);
  VEC_AND(&z[0], &y[15], &t[44]);
  VEC_AND(&z[1], &y[6], &t[37]);
  VEC_AND(&z[2], &t[33], &x[7]);
  VEC_AND(&z[3], &y[16], &t[43]);
  VEC_AND(&z[4], &y[1], &t[40]);
  VEC_AND(&z[5], &t[29], &y[7]);
  VEC_AND(&z[6], &t[42], &y[11]);
  VEC_AND(&z[7], &y[17], &t[45]);
  VEC_AND(&z[8], &y[10], &t[41]);
  VEC_AND(&z[9], &y[12], &t[44]);
  VEC_AND(&z[10], &y[3], &t[37]);
  VEC_AND(&z[11], &t[33], &y[4]);
  VEC_AND(&z[12], &y[13], &t[43]);
  VEC_AND(&z[13], &y[5], &t[40]);
  VEC_AND(&z[14], &t[29], &y[2]);
  VEC_AND(&z[15], &t[42], &y[9]);
  VEC_AND(&z[16], &t[45], &y[14]);
  VEC_AND(&z[17], &t[41], &y[8]);
  ...
  for (int i = 0; i < 8; i++)
    as_radix_ciphertext_slice<Torus>(&s[i], mem->tmp_byte_buffer_2,
                                     i * num_blocks, (i + 1) * num_blocks);
  CudaRadixCiphertextFFI tmp_bit;
  as_radix_ciphertext_slice<Torus>(&tmp_bit, mem->tmp_byte_buffer_1, 0,
                                   num_blocks);
...
vectorized_shift_rows(cudaStream_t const *streams, uint32_t const *gpu_indexes,
  ...
  CudaRadixCiphertextFFI *tmp_full_state_bitsliced = mem->sbox_vars_buffer;
...
__host__ void vectorized_mix_columns(
  ...
  for (int col = 0; col < 4; ++col) {
    ...
    as_radix_ciphertext_slice<Torus>(&col_copy_buffer, mem->tmp_byte_buffer_1,
                                     0, 32 * num_blocks);
    for (int i = 0; i < 32; ++i) {
    ...
    for (int i = 0; i < 4; i++) {
      as_radix_ciphertext_slice<Torus>(
          &b_mul2_tmp_buffers[i], mem->tmp_byte_buffer_2, (i * 8) * num_blocks,
          (i * 8 + 8) * num_blocks);
      for (int j = 0; j < 8; j++) {
    ...
    vectorized_aes_flush<Torus>(streams, gpu_indexes, gpu_count,
                                mem->tmp_byte_buffer_2, mem, bsks, ksks,
                                ms_noise_reduction_key);

    CudaRadixCiphertextFFI b0_mul2_copy_buffer;
    as_radix_ciphertext_slice<Torus>(
        &b0_mul2_copy_buffer, mem->tmp_byte_buffer_2, (4 * 8) * num_blocks,
        (4 * 8 + 8) * num_blocks);
...
__host__ void vectorized_aes_encrypt_inplace(
    ...
    CudaRadixCiphertextFFI const *round_keys_bitsliced, uint32_t num_blocks,
    int_aes_encrypt_buffer<Torus> *mem, void *const *bsks, Torus *const *ksks,
    CudaModulusSwitchNoiseReductionKeyFFI const *ms_noise_reduction_key) {

  auto xor_round_key = [&](int round_idx) {
    for (int i = 0; i < 128; i++) {
      CudaRadixCiphertextFFI k_slice;
      as_radix_ciphertext_slice<Torus>(
          &k_slice, (CudaRadixCiphertextFFI *)round_keys_bitsliced,
          (round_idx * 128 + i) * num_blocks,
          (round_idx * 128 + i + 1) * num_blocks);

      CudaRadixCiphertextFFI s_slice;
      as_radix_ciphertext_slice<Torus>(&s_slice, all_states_bitsliced,
                                       i * num_blocks, (i + 1) * num_blocks);

      vectorized_aes_xor<Torus>(streams, gpu_indexes, gpu_count, mem, &s_slice,
                                &s_slice, &k_slice);
    }
  };
  ...
  for (int round = 1; round <= 10; ++round) {
    ...
    if (round != 10) {
      ...
      for (int i = 0; i < 128; i++) {
        as_radix_ciphertext_slice<Torus>(&s_bits_for_mix_cols[i],
                                         all_states_bitsliced, i * num_blocks,
                                         (i + 1) * num_blocks);
      }
      vectorized_mix_columns<Torus>(streams, gpu_indexes, gpu_count,
                                    s_bits_for_mix_cols, num_blocks, mem, bsks,
                                    ksks, ms_noise_reduction_key);
      vectorized_aes_flush<Torus>(streams, gpu_indexes, gpu_count,
                                  all_states_bitsliced, mem, bsks, ksks,
                                  ms_noise_reduction_key);
...
__host__ void host_integer_aes_ctr_encrypt(
  ...
  CudaRadixCiphertextFFI *initial_states = mem->tmp_initial_states_buffer;
  ...
  CudaRadixCiphertextFFI *transposed_states = mem->tmp_transposed_states_buffer;
  ...
  CudaRadixCiphertextFFI *tiled_key_buffer = mem->tmp_tiled_key_buffer;
  CudaRadixCiphertextFFI *transposed_keys = mem->tmp_transposed_keys_buffer;
  ...
  for (uint32_t i = 0; i < 11; ++i) {
    ...
    for (uint32_t block = 0; block < num_blocks; ++block) {
      ...
      as_radix_ciphertext_slice<Torus>(&tile_slice, tiled_key_buffer,
                                       block * block_size,
                                       (block + 1) * block_size);
      copy_radix_ciphertext_async<Torus>(streams[0], gpu_indexes[0],
    ...
    CudaRadixCiphertextFFI transposed_key_slice;
    as_radix_ciphertext_slice<Torus>(&transposed_key_slice, transposed_keys,
                                     i * block_size * num_blocks,
                                     (i + 1) * block_size * num_blocks);

    transpose_blocks_to_bitsliced<Torus>(
        streams[0], gpu_indexes[0], &transposed_key_slice, tiled_key_buffer,
        num_blocks, block_size);
  }

  vectorized_aes_encrypt_inplace<Torus>(
      streams, gpu_indexes, gpu_count, transposed_states, transposed_keys,
