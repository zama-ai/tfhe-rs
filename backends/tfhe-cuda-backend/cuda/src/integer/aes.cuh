...
#define AND(out, a, b)                                                         \
  ...
  xor_op(&tmp_bit, &t[62], mem_ptr->trivial_1_bit);
  fhe_flush(streams, gpu_indexes, gpu_count, &tmp_bit, mem_ptr, bsks, ksks,
            ms_noise_reduction_key);
  XOR(&s[6], &t[56], &tmp_bit);

  xor_op(&tmp_bit, &t[60], mem_ptr->trivial_1_bit);
  fhe_flush(streams, gpu_indexes, gpu_count, &tmp_bit, mem_ptr, bsks, ksks,
            ms_noise_reduction_key);
  XOR(&s[7], &t[48], &tmp_bit);

  xor_op(&tmp_bit, &s[3], mem_ptr->trivial_1_bit);
  fhe_flush(streams, gpu_indexes, gpu_count, &tmp_bit, mem_ptr, bsks, ksks,
            ms_noise_reduction_key);
  XOR(&s[1], &t[64], &tmp_bit);

  xor_op(&tmp_bit, &t[67], mem_ptr->trivial_1_bit);
  fhe_flush(streams, gpu_indexes, gpu_count, &tmp_bit, mem_ptr, bsks, ksks,
            ms_noise_reduction_key);
