...
__host__ void host_unsigned_integer_div_rem_block_by_block_2_2(
  ...
  host_radix_blocks_rotate_right_async<Torus>(streams.get_ith(0), mem_ptr->d3,
                                        mem_ptr->tmp_gpu_0, 1,
                                        mem_ptr->tmp_gpu_0->num_radix_blocks);
  set_zero_radix_ciphertext_slice_async<Torus>(
      streams.stream(0), streams.gpu_index(0), mem_ptr->d3, 0, 1);
  host_sub_and_propagate_single_carry_async(streams.get_ith(0), mem_ptr->d3,
  ...
  for (int block_index = num_blocks - 1; block_index >= 0; block_index--) {
    ...
    auto cmp_f = [&](CudaStreams streams, size_t gpu_index,
      ...
      if (d_msb.num_radix_blocks == 0) {
        ...
        GPU_ASSERT(radix_params.big_lwe_dimension ==
                   ...
                   "lwe_dimension");
        host_negation_async<Torus>(streams.stream(gpu_index),
    ...
    // used as a bitor
    host_bitop_async(streams.get_ith(0), o3, o3, mem_ptr->cmp_1, mem_ptr->bitor_mem_1,
               &bsks[0], &ksks[0]);
    // used as a bitor
    host_bitop_async(streams.get_ith(1), o2, o2, mem_ptr->cmp_2, mem_ptr->bitor_mem_2,
               &bsks[1], &ksks[1]);
    // used as a bitor
    host_bitop_async(streams.get_ith(2), o1, o1, mem_ptr->cmp_3, mem_ptr->bitor_mem_3,
               &bsks[2], &ksks[2]);
    ...
    host_negation_async<Torus>(streams.stream(0), streams.gpu_index(0), c3, c3, 1,
    ...
    GPU_ASSERT(
        ...
        "Cuda error: big_lwe_dimension must match ciphertexts' lwe_dimension");
    host_negation_async<Torus>(streams.stream(1), streams.gpu_index(1), c2, c2, 1,
                         radix_params.message_modulus,
                         radix_params.carry_modulus);
    host_addition_plaintext_scalar_async<Torus>(
    ...
    GPU_ASSERT(
        ...
        "Cuda error: big_lwe_dimension must match ciphertexts' lwe_dimension");
    host_negation_async<Torus>(streams.stream(2), streams.gpu_index(2), c1, c1, 1,
                         radix_params.message_modulus,
                         radix_params.carry_modulus);
    host_addition_plaintext_scalar_async<Torus>(
    ...
    auto conditional_update = [&](CudaStreams streams, size_t gpu_index,
      ...
      host_add_the_same_block_to_all_blocks_async<Torus>(streams.stream(gpu_index),
                                                   streams.gpu_index(gpu_index),
                                                   rx, rx, cx, 4, 4);
    ...
    host_addition_async<Torus>(streams.stream(0), streams.gpu_index(0), rem_gpu_0,
    ...
    host_addition_async<Torus>(streams.stream(0), streams.gpu_index(0), q3_gpu_0,
...
__host__ void host_unsigned_integer_div_rem(
  ...
  host_addition_async<Torus>(streams.stream(0), streams.gpu_index(0), remainder,
...
__host__ void host_integer_div_rem(
  ...
  if (is_signed) {
    ...
    host_integer_abs_async<Torus>(int_mem_ptr->sub_streams_1, positive_numerator,
                            bsks, ksks, int_mem_ptr->abs_mem_1, true);
    host_integer_abs_async<Torus>(int_mem_ptr->sub_streams_2, positive_divisor, bsks,
    ...
    host_propagate_single_carry_async<Torus>(int_mem_ptr->sub_streams_1,
    ...
    host_propagate_single_carry_async<Torus>(int_mem_ptr->sub_streams_2,
    ...
    host_cmux_async<Torus>(int_mem_ptr->sub_streams_2, remainder, &numerator_sign,
