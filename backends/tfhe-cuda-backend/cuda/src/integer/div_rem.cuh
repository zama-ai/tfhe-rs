...
__host__ void host_integer_div_rem_kb(
    cuda_stream_t *stream, Torus *quotient, Torus *remainder,
    Torus *numerator, Torus *divisor, void *bsk, uint64_t *ksk,
    int_div_rem_memory<uint64_t> *mem_ptr, uint32_t num_blocks) {

  auto radix_params = mem_ptr->params;

  auto big_lwe_size = radix_params.big_lwe_dimension + 1;
  auto big_lwe_size_bytes = big_lwe_size * sizeof
                            (Torus);
  ...
  Torus *numerator_block_stack = (Torus *)cuda_malloc_async(radix_size_bytes,
                                                            stream);
  Torus *interesting_remainder1 = (Torus *)cuda_malloc_async(radix_size_bytes, stream);
  Torus *interesting_remainder2 = (Torus *)cuda_malloc_async
      (radix_size_bytes, stream);
  Torus *interesting_divisor = (Torus *)cuda_malloc_async(radix_size_bytes, stream);
  Torus *divisor_ms_blocks = (Torus *)cuda_malloc_async(radix_size_bytes, stream);

  Torus *merged_interesting_remainder = (Torus *)cuda_malloc_async(radix_size_bytes, stream);
  ...
  //luts
  int_radix_lut<Torus> *merge_overflow_flags_luts = new int_radix_lut<Torus>(stream, radix_params,
                                                              num_bits_in_message, num_bits_in_message * num_blocks,
                                                              true);
  int_radix_lut <Torus> *masking_lut = new int_radix_lut<Torus>(stream, radix_params,
                                                               1,
                                                               num_blocks,
                                                               true);
  ...
  for (int i = 0; i < num_bits_in_message; i++) {
    ...
    generate_device_accumulator_bivariate<Torus>(
        stream, cur_lut, radix_params.glwe_dimension, radix_params
                                                          .polynomial_size,
  ...
  for (int i = total_bits - 1; i >= 0; i--) {
    ...
    cuda_memcpy_async_gpu_to_gpu(interesting_remainder1, remainder1,
                                 big_lwe_size_bytes * last_non_trivial_block, stream);
    cuda_memcpy_async_gpu_to_gpu(interesting_remainder2, remainder2,
                                 big_lwe_size_bytes * last_non_trivial_block, stream);
    cuda_memcpy_async_gpu_to_gpu(interesting_divisor, divisor,
                                 big_lwe_size_bytes * last_non_trivial_block, stream);
    uint32_t ms_start_index = (msb_bit_set + 1) / num_bits_in_message *
                              big_lwe_size;
    cuda_memcpy_async_gpu_to_gpu(divisor_ms_blocks, &divisor[ms_start_index],
                                 radix_size_bytes - ms_start_index *
                                     sizeof(Torus), stream);
    ...
    if (((msb_bit_set + 1) % num_bits_in_message)) {
      ...
      generate_device_accumulator<Torus>(
          ...
          radix_params.polynomial_size, radix_params.message_modulus, radix_params.carry_modulus,
          lut_f_masking);
      ...
      integer_radix_apply_univariate_lookup_table_kb(
          stream, &interesting_divisor[last_block_index], &interesting_divisor[last_block_index], bsk, ksk, 1,
          masking_lut);
    }
    ...
    if ( (msb_bit_set + 1) / num_bits_in_message != (num_blocks - 1) ) {
      ...
      integer_radix_apply_univariate_lookup_table_kb(
          stream, divisor_ms_blocks, divisor_ms_blocks, bsk, ksk, 1,
          masking_lut);
    }
    ...
    if (numerator_block_stack_size) {
      ...
      cuda_memcpy_async_gpu_to_gpu(&interesting_remainder1[big_lwe_size * interesting_remainder1_size],
                                   &numerator_block_stack[
                                       (numerator_block_stack_size - 1) *
                                       big_lwe_size],
                                   big_lwe_size_bytes, stream);
      numerator_block_stack_size--;
      interesting_remainder1_size++;
      host_integer_radix_logical_scalar_shift_kb_inplace(stream,
                                                         interesting_remainder1, 1, mem_ptr->shift_mem, bsk, ksk, num_blocks);
      ...
      if (pos_in_block != 0) {
          ...
          cuda_memcpy_async_gpu_to_gpu(&numerator_block_stack[big_lwe_size * numerator_block_stack_size],
                                       &interesting_remainder1[
                                           (interesting_remainder1_size - 1) *
                                           big_lwe_size],
                                       big_lwe_size_bytes, stream);
          numerator_block_stack_size++;
      }
    ...
    host_integer_radix_logical_scalar_shift_kb_inplace(stream,
                                                       interesting_remainder2, 1, mem_ptr->shift_mem, bsk, ksk, num_blocks);
    ...
    host_addition(stream, merged_interesting_remainder, merged_interesting_remainder,
                  interesting_remainder2, radix_params.big_lwe_dimension,
                  num_blocks);
...
//                                    cur_quotient,
//                                    overflowed, merged_interesting_remainder,
//                                    interesting_divisor,
...
//                                    num_blocks);
  ...
  cuda_memcpy_async_gpu_to_gpu(quotient,
                               cur_quotient,
                               radix_size_bytes, stream);
