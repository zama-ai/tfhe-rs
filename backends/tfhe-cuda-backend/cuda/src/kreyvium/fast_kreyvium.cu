...
void cuda_fast_kreyvium_init_async(
  ...
  host_fast_kreyvium_init_async<uint64_t>(CudaStreams(streams), buffer, a_reg, b_reg,
...
void cuda_fast_kreyvium_step_async(
    ...
    void *const *ksks) {
  auto buffer = (int_fast_kreyvium_buffer<uint64_t> *)mem_ptr;
  host_fast_kreyvium_step_async<uint64_t>(CudaStreams(streams), keystream_output,
