...
__host__ void kreyvium_init(CudaStreams streams,
  ...
  // k.reverse();
  reverse_bitsliced_radix_inplace_kreyvium<Torus>(streams, s->shift_workspace,
                                                  s->k_reg, 128, N);
...
__host__ void kreyvium_compute_64_steps_stateful(
  ...
  *k_offset = (*k_offset + 64) % 128;
  *iv_offset = (*iv_offset + 64) % 128;
...
__host__ void host_kreyvium_init_stateful(
  ...
  *iv_offset = 0;
  ...
  slice_reg_batch_kreyvium<Torus>(&src_key_slice, key_bitsliced, 0, 128, N);
  CudaRadixCiphertextFFI dest_k_reg_slice;
  slice_reg_batch_kreyvium<Torus>(&dest_k_reg_slice, k_reg, 0, 128, N);
  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0),
                                     &dest_k_reg_slice, &src_key_slice);

  // a[0..93] = key[35..128]
  CudaRadixCiphertextFFI k_source_for_a;
  slice_reg_batch_kreyvium<Torus>(&k_source_for_a, k_reg, 35, 93, N);
  CudaRadixCiphertextFFI dest_a_slice;
  slice_reg_batch_kreyvium<Torus>(&dest_a_slice, a_reg, 0, 93, N);
  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0),
                                     &dest_a_slice, &k_source_for_a);

  // k.reverse();
  reverse_bitsliced_radix_inplace_kreyvium<Torus>(
      streams, mem->ws->shift_workspace, k_reg, 128, N);

  // iv = iv_bits.to_vec();
  CudaRadixCiphertextFFI src_iv_slice;
  slice_reg_batch_kreyvium<Torus>(&src_iv_slice, iv_bitsliced, 0, 128, N);
  CudaRadixCiphertextFFI dest_iv_reg_slice;
  slice_reg_batch_kreyvium<Torus>(&dest_iv_reg_slice, iv_reg, 0, 128, N);
  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0),
                                     &dest_iv_reg_slice, &src_iv_slice);

  // b[0..84] = iv[44..128]
  CudaRadixCiphertextFFI iv_source_for_b;
  slice_reg_batch_kreyvium<Torus>(&iv_source_for_b, iv_reg, 44, 84, N);
  CudaRadixCiphertextFFI dest_b_slice;
  slice_reg_batch_kreyvium<Torus>(&dest_b_slice, b_reg, 0, 84, N);
  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0),
                                     &dest_b_slice, &iv_source_for_b);

  // c[67..111] = iv[0..44]
  CudaRadixCiphertextFFI iv_source_for_c;
  slice_reg_batch_kreyvium<Torus>(&iv_source_for_c, iv_reg, 0, 44, N);
  CudaRadixCiphertextFFI dest_c_iv_part;
  slice_reg_batch_kreyvium<Torus>(&dest_c_iv_part, c_reg, 67, 44, N);
  copy_radix_ciphertext_async<Torus>(streams.stream(0), streams.gpu_index(0),
                                     &dest_c_iv_part, &iv_source_for_c);

  // iv.reverse();
  reverse_bitsliced_radix_inplace_kreyvium<Torus>(
      streams, mem->ws->shift_workspace, iv_reg, 128, N);
