...
kreyvium_compute_64_steps(CudaStreams streams, int_kreyvium_buffer<Torus> *mem,
  ...
  slice_reg_batch_kreyvium<Torus>(&a65_slice, s->a_reg, 2, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&a92_slice, s->a_reg, 29, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&a91_slice, s->a_reg, 28, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&a90_slice, s->a_reg, 27, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&a68_slice, s->a_reg, 5, BATCH, N);

  CudaRadixCiphertextFFI b68_slice, b83_slice, b82_slice, b81_slice, b77_slice;
  slice_reg_batch_kreyvium<Torus>(&b68_slice, s->b_reg, 5, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&b83_slice, s->b_reg, 20, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&b82_slice, s->b_reg, 19, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&b81_slice, s->b_reg, 18, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&b77_slice, s->b_reg, 14, BATCH, N);

  CudaRadixCiphertextFFI c65_slice, c110_slice, c109_slice, c108_slice,
      c86_slice;
  slice_reg_batch_kreyvium<Torus>(&c65_slice, s->c_reg, 2, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&c110_slice, s->c_reg, 47, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&c109_slice, s->c_reg, 46, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&c108_slice, s->c_reg, 45, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&c86_slice, s->c_reg, 23, BATCH, N);

  CudaRadixCiphertextFFI k_slice, iv_slice;
  slice_reg_batch_kreyvium<Torus>(&k_slice, s->k_reg, 64, BATCH, N);
  slice_reg_batch_kreyvium<Torus>(&iv_slice, s->iv_reg, 64, BATCH, N);
  ...
  kreyvium_xor(streams, mem, s->new_a, s->temp_t3, &a68_slice);
  kreyvium_xor(streams, mem, s->new_a, s->new_a, &and_res_a);

  kreyvium_xor(streams, mem, s->new_b, s->temp_t1, &b77_slice);
  kreyvium_xor(streams, mem, s->new_b, s->new_b, &and_res_b);
  kreyvium_xor(streams, mem, s->new_b, s->new_b, &iv_slice);

  kreyvium_xor(streams, mem, s->new_c, s->temp_t2, &c86_slice);
  kreyvium_xor(streams, mem, s->new_c, s->new_c, &and_res_c);

  if (output_dest != nullptr) {
    kreyvium_xor(streams, mem, output_dest, s->temp_t1, s->temp_t2);
    kreyvium_xor(streams, mem, output_dest, output_dest, s->temp_t3);
  }
  ...
  if (output_dest != nullptr) {
    ...
    reverse_bitsliced_radix_inplace_kreyvium<Torus>(streams, mem, output_dest,
                                                    64);
  }
}
