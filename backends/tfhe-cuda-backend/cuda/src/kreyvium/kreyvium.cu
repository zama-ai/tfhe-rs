...
void cuda_kreyvium_init_async(
  ...
  auto buffer = (int_kreyvium_stateful_buffer<uint64_t> *)mem_ptr;
  host_kreyvium_init_stateful<uint64_t>(
...
void cuda_kreyvium_step_async(
    ...
    void *const *ksks) {
  auto buffer = (int_kreyvium_stateful_buffer<uint64_t> *)mem_ptr;
  host_kreyvium_step_stateful<uint64_t>(
...
uint64_t scratch_cuda_kreyvium_init_async(
  ...
  int_radix_params params(pbs_type, glwe_dimension, polynomial_size,
                          ...
                          grouping_factor, message_modulus, carry_modulus,
                          noise_reduction_type);
  return scratch_cuda_kreyvium_stateful_encrypt<uint64_t>(
      CudaStreams(streams), (int_kreyvium_stateful_buffer<uint64_t> **)mem_ptr,
      params, allocate_gpu_memory, num_inputs);
}

void cleanup_cuda_kreyvium_init(CudaStreamsFFI streams, int8_t **mem_ptr_void) {
  int_kreyvium_stateful_buffer<uint64_t> *mem_ptr =
      (int_kreyvium_stateful_buffer<uint64_t> *)(*mem_ptr_void);
  mem_ptr->release(CudaStreams(streams));
  delete mem_ptr;
  *mem_ptr_void = nullptr;
}
