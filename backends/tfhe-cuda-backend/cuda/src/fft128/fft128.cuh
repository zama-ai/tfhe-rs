...
// ----------------------------------------------------------------------------
// The transforms the host-driven TBC flavor uses.
//
...
__device__ void negacyclic_forward_fft_f128_relaxed(double *buffer) {
  ...
  constexpr Index HALF_DEGREE = params::degree >> 1;
  constexpr Index STRIDE = params::degree / params::opt;
  double *dt_re_hi = buffer + 0 * params::degree;
  double *dt_re_lo = buffer + 1 * params::degree;
  double *dt_im_hi = buffer + 2 * params::degree;
  double *dt_im_lo = buffer + 3 * params::degree;
...
#pragma unroll
  ...
  // per blind-rotation iteration in the host-driven TBC kernel. Barrier
...
#pragma unroll
  ...
  // Level 6 reads slot t ^ 32 from another warp and the paired store below
  // writes slots 2t, 2t + 1; Part 2 only syncs within a warp, so a block-wide
...
negacyclic_backward_fft_f128_relaxed(double *buffer, double2 acc_re_hi,
                                     ...
                                     double2 acc_im_lo, G &group) {
  constexpr Index BUTTERFLY_DEPTH = params::opt >> 1;
  constexpr Index LOG2_DEGREE = params::log2_degree;
  constexpr Index DEGREE = params::degree;
  constexpr Index HALF_DEGREE = params::degree >> 1;
  constexpr Index STRIDE = params::degree / params::opt;
  double *dt_re_hi = buffer + 0 * DEGREE;
  double *dt_re_lo = buffer + 1 * DEGREE;
  double *dt_im_hi = buffer + 2 * DEGREE;
  double *dt_im_lo = buffer + 3 * DEGREE;
