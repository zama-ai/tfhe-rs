...
__global__ void __launch_bounds__(params::degree / params::opt,
    ...
    device_programmable_bootstrap_step_two_128_halfhalf(
        ...
        uint64_t device_memory_size_per_block, bool relaxed_default_requested) {
...
__host__ uint64_t scratch_programmable_bootstrap_128_halfhalf(
  ...
  // Under TFHE_RS_GPU_PBS128_RELAXED_DEFAULT the step kernels configured below
  // read the interleaved twiddle table, which is filled at runtime. It is built
  // here, as in scratch_programmable_bootstrap_128, so that no launch of those
  // kernels can precede it on this GPU.
...
uint64_t scratch_cuda_programmable_bootstrap_128_halfhalf_vector(
  ...
  // Latency-sized batches would otherwise auto-select CG. The matrix needs
  // DEFAULT when RELAXED_DEFAULT or FORCE_DEFAULT is set.
  const bool force_default = is_relaxed_default_pbs128_requested() ||
                             is_force_default_pbs128_requested();
  ...
  if (max_shared_memory < partial_sm) {
    device_programmable_bootstrap_step_two_128_halfhalf<
        ...
        <<<grid, thds, 0, stream>>>(
            ...
            d_mem, full_dm, relaxed_default_requested);
  } else if (max_shared_memory < full_sm) {
    device_programmable_bootstrap_step_two_128_halfhalf<
        ...
        <<<grid, thds, partial_sm, stream>>>(
            lwe_array_out, group_bsk, global_accumulator, global_join_buffer,
            group_local_iter, polynomial_size, level_mask_g, level_body_g,
            d_mem, partial_dm, relaxed_default_requested);
  } else {
    device_programmable_bootstrap_step_two_128_halfhalf<
        ...
        <<<grid, thds, full_sm, stream>>>(
            lwe_array_out, group_bsk, global_accumulator, global_join_buffer,
            group_local_iter, polynomial_size, level_mask_g, level_body_g,
            d_mem, 0, relaxed_default_requested);
  }
  check_cuda_error(cudaGetLastError());
}
  ...
  if constexpr (params::degree == 2048) {
    if (level_mask_g == 2 && level_body_g == 2) {
      launch_step_two_128_halfhalf<params, last_iter, 2, 2>(
          ...
          max_shared_memory, relaxed_default_requested);
      return;
    }
