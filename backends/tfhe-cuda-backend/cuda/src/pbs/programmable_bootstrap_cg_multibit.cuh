...
template <typename Torus, class params, sharedMemDegree SMD>
__global__ void device_multi_bit_programmable_bootstrap_cg_accumulate_pack(
    Torus *lwe_array_out, const Torus *__restrict__ lwe_output_indexes,
    const Torus *__restrict__ lut_vector,
    const Torus *__restrict__ lut_vector_indexes,
    const Torus *__restrict__ lwe_array_in,
    ...
    uint32_t grouping_factor, uint32_t lwe_offset, uint32_t lwe_chunk_size,
    uint32_t keybundle_size_per_input, int8_t *device_mem,
    uint64_t device_memory_size_per_block, uint32_t blockIdz_offset) {
  ...
  extern __shared__ int8_t sharedmem[];
  int8_t *selected_memory;
  const int global_blockIdz = blockIdx.z + blockIdz_offset;

  if constexpr (SMD == FULLSM) {
    selected_memory = sharedmem;
  } else {
    int block_index = blockIdx.x + blockIdx.y * gridDim.x +
                      global_blockIdz * gridDim.x * gridDim.y;
    selected_memory = &device_mem[block_index * device_memory_size_per_block];
  }
  ...
  // this block is operating, in the case of batch bootstraps
  const Torus *block_lwe_array_in =
      &lwe_array_in[lwe_input_indexes[global_blockIdz] * (lwe_dimension + 1)];

  const Torus *block_lut_vector =
      &lut_vector[lut_vector_indexes[global_blockIdz] * params::degree *
                  (glwe_dimension + 1)];

  double2 *block_join_buffer =
      &join_buffer[global_blockIdz * level_count * (glwe_dimension + 1) *
                   params::degree / 2];

  Torus *global_slice =
      global_accumulator +
      (blockIdx.y + global_blockIdz * (glwe_dimension + 1)) * params::degree;

  const double2 *keybundle = keybundle_array +
                             // select the input
                             global_blockIdz * keybundle_size_per_input;
  ...
  if (lwe_offset + lwe_chunk_size >= (lwe_dimension / grouping_factor)) {
    auto block_lwe_array_out =
        &lwe_array_out[lwe_output_indexes[global_blockIdz] *
                           (glwe_dimension * polynomial_size + 1) +
                       blockIdx.y * polynomial_size];
...
__host__ void scratch_cg_multi_bit_programmable_bootstrap(
  ...
  if (max_shared_memory < partial_sm_cg_accumulate) {
    ...
    cudaFuncSetCacheConfig(
        ...
        cudaFuncCachePreferShared);
    check_cuda_error(cudaFuncSetAttribute(
        device_multi_bit_programmable_bootstrap_cg_accumulate_pack<
            Torus, params, NOSM>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, 0));
    cudaFuncSetCacheConfig(
        device_multi_bit_programmable_bootstrap_cg_accumulate_pack<
...
template <typename Torus, class params>
__host__ void execute_cg_external_product_loop_pack(
    cudaStream_t stream, uint32_t gpu_index, Torus *lut_vector,
    Torus *lut_vector_indexes, Torus *lwe_array_in, Torus *lwe_input_indexes,
    Torus *lwe_array_out, Torus *lwe_output_indexes,
    pbs_buffer<Torus, MULTI_BIT> *buffer, uint32_t num_samples,
  ...
  uint32_t blockIdz_offset = samples_to_process * pack_iteration;
  ...
  kernel_args[17] = &keybundle_size_per_input;
  kernel_args[18] = &d_mem;
  kernel_args[20] = &blockIdz_offset;

  dim3 grid_accumulate(level_count, glwe_dimension + 1, samples_to_process);
  dim3 thds(polynomial_size / params::opt, 1, 1);

  if (max_shared_memory < partial_dm) {
    kernel_args[19] = &full_dm;
    check_cuda_error(cudaLaunchCooperativeKernel(
        (void *)device_multi_bit_programmable_bootstrap_cg_accumulate_pack<
            Torus, params, NOSM>,
        grid_accumulate, thds, (void **)kernel_args, 0, stream));
  } else if (max_shared_memory < full_dm) {
    kernel_args[19] = &partial_dm;
...
__host__ void host_cg_multi_bit_programmable_bootstrap(
  ...
  // to be developed
  const int num_packs = 4;
  // Generating streams in naive way, later on i will think where should I add
  // this. Also, this might create an overhead, check with the profiler
  cudaStream_t pbs_streams[4];
  for (int i = 0; i < num_packs; ++i) {
    cudaStreamCreate(&pbs_streams[i]);
  }
  for (uint32_t lwe_offset = 0; lwe_offset < (lwe_dimension / grouping_factor);
    ...
    for (int pack_iteration = 0; pack_iteration < num_packs; pack_iteration++) {
      ...
      execute_compute_keybundle_pack<Torus, params>(
          pbs_streams[pack_iteration], gpu_index, lwe_array_in,
          lwe_input_indexes, bootstrapping_key, buffer, num_samples,
          lwe_dimension, glwe_dimension, polynomial_size, grouping_factor,
          base_log, level_count, max_shared_memory, lwe_chunk_size, lwe_offset,
          pack_iteration, num_packs);
    }
    for (int pack_iteration = 0; pack_iteration < num_packs; pack_iteration++) {
      // Accumulate
      execute_cg_external_product_loop_pack<Torus, params>(
          pbs_streams[pack_iteration], gpu_index, lut_vector,
          lut_vector_indexes, lwe_array_in, lwe_input_indexes, lwe_array_out,
          lwe_output_indexes, buffer, num_samples, lwe_dimension,
          glwe_dimension, polynomial_size, grouping_factor, base_log,
          level_count, lwe_chunk_size, max_shared_memory, lwe_offset,
          pack_iteration, num_packs);
    }
    for (int i = 0; i < num_packs; ++i) {
      cudaStreamSynchronize(pbs_streams[i]);
    }
  }
  for (int i = 0; i < num_packs; ++i) {
    cudaStreamDestroy(pbs_streams[i]);
  }
}
