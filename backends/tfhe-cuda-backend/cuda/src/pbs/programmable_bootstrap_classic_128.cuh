...
__global__ void __launch_bounds__(params::degree / params::opt)
  ...
  if constexpr (first_iter) {
    ...
    // Put "b" in [0, 2N[
    InputTorus b_hat = 0;
    InputTorus e_mms = centered_binary_modulus_switch_body_correction_to_add(
  ...
  modulus_switch<InputTorus>(block_lwe_array_in[lwe_iteration], a_hat,
                             params::log2_degree + 1); // 2 * log_modulus);
...
__global__ void device_programmable_bootstrap_cg_128(
  ...
  modulus_switch<InputTorus>(block_lwe_array_in[lwe_dimension], b_hat,
                             log_modulus);
  b_hat += centered_binary_modulus_switch_body_correction_to_add(
      block_lwe_array_in, lwe_dimension, log_modulus);
