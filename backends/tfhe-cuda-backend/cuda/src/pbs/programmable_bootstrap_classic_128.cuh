 ...
 *
 * Bit-exact against the variant it replaces (see
 * decompose_and_compress_next_level_128 for why the digits match). Nothing is
 * reassociated and no register state has to be copied, because each digit
 * extraction consumes the rotated accumulator in precisely the order needed.
 *
...
__global__ void __launch_bounds__(params::degree / params::opt,
  ...
  constexpr auto log_modulus = params::log2_degree + 1;
  if constexpr (first_iter) {
    // First iteration: build the accumulator from the LUT and persist it.
    InputTorus b_hat = 0;
    InputTorus correction = 0;
    if (noise_reduction_type == PBS_MS_REDUCTION_T::CENTERED) {
      correction = centered_binary_modulus_switch_body_correction_to_add(
...
__host__ uint64_t scratch_programmable_bootstrap_128(
  ...
  // which is filled at runtime. This is the single point where it is built: the
  // buffer this function returns is what selects those kernels, so no launch of
  // them can precede this call on this GPU. Building it here rather than in the
...
bool supports_distributed_shared_memory_on_classic_programmable_bootstrap_128(
  ...
  if (max_shared_memory < minimum_sm) {
    // If we cannot allocate all the necessary memory for the TBC implementation
...
__global__ void __launch_bounds__(params::degree / params::opt, 2)
    device_programmable_bootstrap_host_driven_tbc_128(
        __uint128_t *lwe_array_out, const __uint128_t *__restrict__ lut_vector,
        const InputTorus *__restrict__ lwe_array_in,
        const double *__restrict__ bootstrapping_key,
...
#pragma unroll 1
  ...
  mul_ggsw_glwe_in_fourier_domain_128_host_driven_tbc<
...
// Raises the shared-memory limit on one (is_first_iter, is_last_iter)
// instantiation.
template <typename InputTorus, class params, bool is_first_iter,
          bool is_last_iter, uint32_t base_log, uint32_t level_count,
          uint32_t glwe_dimension>
__host__ void configure_one_host_driven_tbc_128() {
  check_cuda_error(cudaFuncSetAttribute(
      device_programmable_bootstrap_host_driven_tbc_128<
          InputTorus, params, is_first_iter, is_last_iter, base_log,
          level_count, glwe_dimension>,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      get_buffer_size_host_driven_tbc_128<params, InputTorus, level_count>()));
}

template <typename InputTorus, class params, uint32_t base_log,
          uint32_t level_count, uint32_t glwe_dimension>
__host__ void configure_host_driven_tbc_128() {
  configure_one_host_driven_tbc_128<InputTorus, params, true, false, base_log,
                                    level_count, glwe_dimension>();
  configure_one_host_driven_tbc_128<InputTorus, params, false, false, base_log,
                                    level_count, glwe_dimension>();
  configure_one_host_driven_tbc_128<InputTorus, params, false, true, base_log,
                                    level_count, glwe_dimension>();
  configure_one_host_driven_tbc_128<InputTorus, params, true, true, base_log,
                                    level_count, glwe_dimension>();
}
...
template <typename InputTorus, class params, bool is_first_iter,
          bool is_last_iter, uint32_t base_log, uint32_t level_count,
          uint32_t glwe_dimension>
__host__ void execute_host_driven_tbc_128(
  ...
  auto kernel = device_programmable_bootstrap_host_driven_tbc_128<
      InputTorus, params, is_first_iter, is_last_iter, base_log, level_count,
      glwe_dimension>;
...
__host__ void host_programmable_bootstrap_host_driven_tbc_128(
    cudaStream_t stream, uint32_t gpu_index, __uint128_t *lwe_array_out,
    __uint128_t const *lut_vector, InputTorus const *lwe_array_in,
  ...
  static_assert(params::degree == PBS128_SNS_POLYNOMIAL_SIZE,
                "the host-driven TBC flavor is only compiled for the "
                "noise-squashing polynomial size");
  PANIC_IF_FALSE(polynomial_size == PBS128_SNS_POLYNOMIAL_SIZE &&
                     base_log_rt == base_log && level_count_rt == level_count &&
                 ...
                 "128-bit programmable bootstrap only supports the "
                 "noise-squashing parameters (N=%u, k=%u, l=%u, Bg=2^%u) on a "
                 "GPU with distributed shared memory.",
                 PBS128_SNS_POLYNOMIAL_SIZE, PBS128_SNS_GLWE_DIMENSION,
                 PBS128_SNS_LEVEL_COUNT, PBS128_SNS_BASE_LOG);
  ...
  for (uint32_t i = 0; i < lwe_dimension; i++) {
    ...
    if (is_first && is_last)
      execute_host_driven_tbc_128<InputTorus, params, true, true, base_log,
      ...
      execute_host_driven_tbc_128<InputTorus, params, true, false, base_log,
                                  level_count, glwe_dimension_ct>(
          stream, gpu_index, lwe_array_out, lut_vector, lwe_array_in,
          bootstrapping_key, global_accumulator, input_lwe_ciphertext_count,
          lwe_dimension, i, noise_reduction_type);
    else if (is_last)
      execute_host_driven_tbc_128<InputTorus, params, false, true, base_log,
                                  level_count, glwe_dimension_ct>(
          stream, gpu_index, lwe_array_out, lut_vector, lwe_array_in,
          bootstrapping_key, global_accumulator, input_lwe_ciphertext_count,
          lwe_dimension, i, noise_reduction_type);
    else
      execute_host_driven_tbc_128<InputTorus, params, false, false, base_log,
                                  level_count, glwe_dimension_ct>(
          stream, gpu_index, lwe_array_out, lut_vector, lwe_array_in,
          bootstrapping_key, global_accumulator, input_lwe_ciphertext_count,
          lwe_dimension, i, noise_reduction_type);
...
__host__ bool has_support_to_cuda_programmable_bootstrap_host_driven_tbc_128(
  ...
  if (polynomial_size != PBS128_SNS_POLYNOMIAL_SIZE ||
      glwe_dimension != PBS128_SNS_GLWE_DIMENSION ||
      level_count != PBS128_SNS_LEVEL_COUNT)
    return false;
  if (!cuda_check_support_thread_block_clusters())
    return false;
  return max_shared_memory >=
         get_buffer_size_host_driven_tbc_128<params, InputTorus,
                                             PBS128_SNS_LEVEL_COUNT>();
}
#else
...
uint64_t scratch_cuda_programmable_bootstrap_128_vector(
  ...
  if (is_relaxed_pbs128_requested()) {
    ...
    return scratch_programmable_bootstrap_host_driven_tbc_128<
        InputTorus, Degree<PBS128_SNS_POLYNOMIAL_SIZE>>(
...
__host__ void execute_step_one_128_all_levels(
    ...
    PBS_MS_REDUCTION_T noise_reduction_type,
    uint32_t input_lwe_ciphertext_count, uint32_t lwe_dimension,
    uint32_t glwe_dimension, uint32_t polynomial_size, int8_t *d_mem,
...
__host__ void host_programmable_bootstrap_tbc_128(
  ...
  PANIC_IF_FALSE(can_use_tbc,
                 ...
                 "bootstrap for 128 bits inputs is only supported for the "
                 "noise squash parameters (N=2048, l=3, Bg=2^24) and if the "
                 "GPU supports distributed shared memory.");
  // With SM each block corresponds to either the mask or body, no need to
  // duplicate data for each
  auto max_shared_memory = cuda_get_max_shared_memory(gpu_index);
  cuda_set_device(gpu_index);
