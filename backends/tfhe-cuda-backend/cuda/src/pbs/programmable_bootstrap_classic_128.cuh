...
uint64_t scratch_cuda_programmable_bootstrap_128_vector(
  ...
  // The single-iteration TBC flavor uses relaxed arithmetic and is
  ...
  if (is_relaxed_pbs128_requested()) {
    ...
    PANIC_IF_FALSE(
        ...
        "which is only tuned for the noise-squashing parameters (N = 2048, "
        "k = 2, l = 3) on a GPU with distributed shared memory.");
