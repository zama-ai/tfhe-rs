...
__device__ void
mul_ggsw_glwe_in_fourier_domain_128(double *fft, double *join_buffer,
  ...
  for (int j = 0; j < glwe_dimension + 1; j++) {
    ...
    auto buffer_slice = get_join_buffer_element_128<G>(blockIdx.z, idx, group,
  ...
  for (int l = 0; l < level_count; l++) {
    auto cur_src_acc = get_join_buffer_element_128<G>(l, blockIdx.y, group,
                                                  join_buffer, polynomial_size,
                                                  glwe_dimension, support_dsm);

    polynomial_accumulate_in_fourier_domain_128<params>(fft, cur_src_acc, l == 0);
  }
...
__global__ void device_programmable_bootstrap_cg_128(
  ...
  const Torus *block_lut_vector =
      &lut_vector[blockIdx.x * params::degree *
          (glwe_dimension + 1)];
...
//  printf("cuda_check_support_cooperative_groups\n");
  ...
  if (max_shared_memory < partial_sm) {
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        ...
        (void *)device_programmable_bootstrap_cg_128<__uint128_t, params, NOSM>, thds, 0);
  } else if (max_shared_memory < full_sm) {
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &max_active_blocks_per_sm,
        (void *)device_programmable_bootstrap_cg_128<__uint128_t, params, PARTIALSM>,
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &max_active_blocks_per_sm,
        (void *)device_programmable_bootstrap_cg_128<__uint128_t, params, FULLSM>, thds,
        full_sm);
  }
