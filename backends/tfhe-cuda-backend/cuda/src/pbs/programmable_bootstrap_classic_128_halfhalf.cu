...
uint64_t scratch_cuda_programmable_bootstrap_128_halfhalf_async(
  ...
  // not carry. It shares TFHE_RS_GPU_PBS128_RELAXED with the vanilla PBS-128
  // flavor, so one switch turns relaxed arithmetic on for every 128-bit
  // bootstrap a process runs.
  if (is_relaxed_pbs128_requested()) {
    ...
    PANIC_IF_FALSE(
        is_supported,
        "Cuda error (classical PBS128 halfhalf): TFHE_RS_GPU_PBS128_RELAXED "
...
#else
    PANIC("Cuda error (classical PBS128 halfhalf): TFHE_RS_GPU_PBS128_RELAXED "
          "asks for the relaxed arithmetic flavor of the 128-bit halfhalf "
...
// Test-only entry point for the relaxed arithmetic flavor. It bypasses the
// TFHE_RS_GPU_PBS128_RELAXED opt-in so the test binary can exercise the flavor
// without the env var, which is read once per process and would otherwise
// order-couple the tests in one binary. The production path keeps the opt-in.
