...
public:
  ...
  __device__ void decompose_and_compress_next_polynomial(double2 *result,
    ...
    for (int i = 0; i < params::opt / 2; i++) {
      ...
      signed_shift_right<T>(*input1, base_log); // Update state
      signed_shift_right<T>(*input2, base_log); // Update state
  ...
  __device__ void decompose_and_compress_next_polynomial_128(double *result,
    ...
    for (int i = 0; i < params::opt / 2; i++) {
      ...
      signed_shift_right<T>(*input1, base_log);
      signed_shift_right<T>(*input2, base_log);
...
__device__ Torus decompose_one(Torus &state, Torus mask_mod_b, int base_log) {
  ...
  signed_shift_right<Torus>(state, base_log);
