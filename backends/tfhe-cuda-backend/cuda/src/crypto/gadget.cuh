...
rotate_and_decompose_first_level_128(double *result, uint64_t *state,
  ...
  static_assert(level_count >= 1 && base_log * (level_count - 1) <= 64,
                "after the first level the decomposer state holds base_log * "
                "(level_count - 1) significant bits and must fit a uint64_t");
