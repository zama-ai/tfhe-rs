...
__host__ void host_packing_keyswitch_lwe_list_to_glwe(
  ...
  // Shared memory requirement is 4096, 8192, and 16384 bytes respectively for 32, 64, and 128-bit Torus elements
    // We want to keep this as a sanity check
  uint32_t shared_mem_size = get_shared_mem_size_tgemm<Torus>();
  if (shared_mem_size > 1024 * sizeof(Torus))
