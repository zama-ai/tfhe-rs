...
__global__ void decompose_vectorize(Torus const *lwe_in,
                               Torus *lwe_out, 
    ...
    auto val_idx = (blockIdx.y + threadIdx.y) * lwe_dimension_in + blockIdx.x * blockDim.x + threadIdx.x;
    Torus a_i = lwe_in[val_idx];

    a_i = round_to_closest_multiple(a_i, base_log, level_count);
    
    Torus state = a_i >> (sizeof(Torus) * 8 - base_log * level_count);
    Torus mod_b_mask = (1ll << base_log) - 1ll;
    lwe_out[val_idx] = decompose_one<Torus>(state, mod_b_mask, base_log);
}
...
template <typename Torus> 
  void polynomial_accumulate_monic_monomial_mul_many(Torus* in_glwe_buffer,
    Torus* out_glwe_buffer, 
    uint32_t num_glwes, 
    uint32_t polynomial_size, 
    uint32_t glwe_dimension
)
{
    uint32_t poly_id = blockIdx.y * blockDim.y + threadIdx.y;
    uint32_t degree = poly_id;
    uint32_t coeffIdx = blockIdx.x * blockDim.x + threadIdx.x;

    auto in_poly = in_glwe_buffer + poly_id * polynomial_size * (glwe_dimension + 1);
    auto out_result = out_glwe_buffer + poly_id * polynomial_size * (glwe_dimension + 1);
    polynomial_accumulate_monic_monomial_mul<Torus>(out_result, 
              in_poly, 
              degree,
              coeffIdx,
              polynomial_size, 
...
__host__ void host_fast_packing_keyswitch_lwe_list_to_glwe (
  ...
  Torus* decomp_lwe_in = (Torus*)cuda_malloc_async(sizeof(Torus) * lwe_dimension_in * num_lwes, stream,
        gpu_index);
...
//decompose LWEs
    ...
    //don't decompose LWE body - the LWE has lwe_size + 1 elements. The last element, the body is ignored
    //by rounding down the number of blocks 
    dim3 grid_decomp(lwe_dimension_in / BLOCK_SIZE_DECOMP, num_lwes / BLOCK_SIZE_DECOMP);
    dim3 threads_decomp(BLOCK_SIZE_DECOMP, BLOCK_SIZE_DECOMP);

    //decompose inplace (assume levels == 1)
    decompose_vectorize<Torus, TorusVec><<<grid_decomp, threads_decomp, 0, stream>>>(
        lwe_array_in, decomp_lwe_in, lwe_dimension_in, num_lwes, base_log, level_count
    );
    check_cuda_error(cudaGetLastError());

    //gemm to ks the individual LWEs to GLWEs
    dim3 grid_gemm(num_lwes / BLOCK_SIZE_GEMM, lwe_dimension_in / BLOCK_SIZE_GEMM);
    dim3 threads_gemm(THREADS_GEMM, THREADS_GEMM);
    tgemmVectorize<Torus, TorusVec><<<grid_gemm, threads_gemm, BLOCK_SIZE_GEMM * THREADS_GEMM * 2 * sizeof(Torus), stream>>>(
        num_lwes, lwe_dimension_in, glwe_accumulator_size, decomp_lwe_in, fp_ksk_array, d_mem
    );
    check_cuda_error(cudaGetLastError());

    // should we include the mask in the rotation ??
    dim3 grid_rotate(num_lwes / BLOCK_SIZE_DECOMP, polynomial_size / BLOCK_SIZE_DECOMP);
    dim3 threads_rotate(BLOCK_SIZE_DECOMP, BLOCK_SIZE_DECOMP);    
    //rotate the GLWEs
    polynomial_accumulate_monic_monomial_mul_many<Torus> (d_mem, 
                d_tmp_glwe_array_out,
                num_lwes,
                polynomial_size, 
                glwe_dimension);
    check_cuda_error(cudaGetLastError());

     dim3 grid_accumulate(polynomial_size * (glwe_dimension  + 1) / BLOCK_SIZE_DECOMP);
  ...
  cuda_drop_async(decomp_lwe_in, stream, gpu_index);
  check_cuda_error(cudaGetLastError());
}
