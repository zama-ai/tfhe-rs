...
void execute_keyswitch_async(
    ...
    const std::vector<Torus *> &fp_tmp_buffer, uint64_t numKsTmpSamples = 0) {
  ...
  for (uint i = 0; i < streams.count(); i++) {
    ...
    if (num_samples_on_gpu >= 144) {
      GPU_ASSERT(fp_tmp_buffer.size() >= streams.count(),
                 "GEMM KS Buffers %d were not initialized for this amount of "
                 "streams, %d",
                 fp_tmp_buffer.size(), streams.count());

      if (numKsTmpSamples) {
        GPU_ASSERT(num_samples_on_gpu <= numKsTmpSamples,
                   "Temp buffer ks not big enough");
      }
...
uint64_t scratch_cuda_keyswitch(cudaStream_t stream, uint32_t gpu_index,
  ...
  *fp_ks_buffer = new ks_mem();
  (*fp_ks_buffer)->buffer = (int8_t *)cuda_malloc_with_size_tracking_async(
...
void cleanup_cuda_keyswitch(cudaStream_t stream, uint32_t gpu_index,
                            ks_mem *fp_ks_buffer, bool allocate_gpu_memory) {
  cuda_drop_with_size_tracking_async(fp_ks_buffer->buffer, stream, gpu_index,
                                     allocate_gpu_memory);
}
