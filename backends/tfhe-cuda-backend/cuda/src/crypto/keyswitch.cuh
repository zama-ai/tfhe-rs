...
template <typename Torus>
__host__ void scratch_fp_keyswitch_lwe_list_to_glwe(
...
template <typename Torus>
__device__ void fp_keyswitch_lwe_ciphertext_into_glwe_ciphertext(
  ...
  if (tid < glwe_size * polynomial_size) {
    ...
    local_glwe_chunk[shmem_index] = SEL(
        0, lwe_in[lwe_dimension_in], tid == glwe_dimension * polynomial_size);
    ...
    for (int i = 0; i < lwe_dimension_in; i++) {
      ...
      for (int j = 0; j < level_count; j++) {
        ...
        local_glwe_chunk[shmem_index] -=
            decomposed * ksk_glwe_chunk[shmem_index];
      }
    }
...
fp_keyswitch_lwe_list_to_glwe(Torus *glwe_array_out, Torus *lwe_array_in,
  ...
  // KS LWE to GLWE
  fp_keyswitch_lwe_ciphertext_into_glwe_ciphertext(
...
__host__ void host_fp_keyswitch_lwe_list_to_glwe(
  ...
  auto shared_mem = sizeof(Torus) * num_threads;
  ...
  accumulate_glwes<<<num_blocks, threads, shared_mem, stream>>>(
