...
template <typename Torus>
__global__ void improve_noise_modulus_switch_inplace(
    Torus *array, const Torus *zeros, int lwe_size, int num_zeros,
    double input_variance, double r_sigma, double bound, uint32_t log_modulus) {
  ...
  Torus input_element1 = array[threadIdx.x + blockIdx.x * lwe_size];

  Torus input_element2 =
      threadIdx.x + blockDim.x < lwe_size
          ? array[threadIdx.x + blockDim.x + blockIdx.x * lwe_size]
          : 0;
  ...
  // If the noise is less than the bound we can return
  ...
  for (int index = 0; index < num_zeros; index++) {
    ...
    if (found)
      array[threadIdx.x + blockIdx.x * lwe_size] = zero_element1;

    if (found && (threadIdx.x + blockDim.x) < lwe_size) {
      array[threadIdx.x + blockDim.x + blockIdx.x * lwe_size] = zero_element2;
    }
    __syncthreads();
...
template <typename Torus>
__host__ void host_improve_noise_modulus_switch_inplace(
    cudaStream_t stream, uint32_t gpu_index, Torus *array, const Torus *zeros,
    int lwe_size, int num_lwes, int num_zeros, double input_variance,
  ...
  improve_noise_modulus_switch_inplace<uint64_t>
      <<<num_blocks, num_threads, 0, stream>>>(array, zeros, lwe_size,
                                               num_zeros, input_variance,
