...
#include <immintrin.h>
#define IFFT16_HAVE_AVX512 1
#endif

// The transform's twiddle factors live in GPU constant memory, defined once in
// the backend (src/fft/fft16x4x16_twiddles.cu) and declared here with the same
// `extern __device__ __constant__` signatures. Rather than duplicating the
// ~1500 hex-float constants on the host, the test copies these exact device
// tables down to host memory at runtime (cudaMemcpyFromSymbol below), so the
// CPU reference and the GPU kernel provably use the identical bytes. The
// inverse transform uses the conjugates of the forward twiddles (built on the
// fly from these same tables).
//
...
// INVERSE transform (the inverse FFT used by the specialized 2_2_params PBS).
//
// This is the mirror image of test_cpu_gpu_fft16x4x16.cpp (the forward test):
// it carries its own host CPU reference implementation of the inverse transform
// (below) and reuses the exact device twiddle tables. None of it leaks into the
// backend itself.
//
// On every run it draws a FRESH set of random complex inputs, transforms them
// with both the GPU kernel (cuda_backward_fft16x4x16_async) and the CPU
// reference, and asserts the two results are IDENTICAL BIT FOR BIT (not merely
...
//
// The inverse transform is: IFFT16 -> conjugate inter-stage twiddles -> 16x64
// transpose -> IFFT4x4 -> conjugate compact twiddles -> 4x4 transpose -> IFFT16
// -> fused negacyclic post-twist (untwist). The 1/N scaling that the production
// PBS folds into the bootstrapping key is NOT applied here (this is a pure
// inverse FFT + untwist), matching the forward test which carries no 1/N.
//
...
static inline double2 c_conj(double2 a) { return mk(a.x, -a.y); }
// Multiply by exp(+i*pi/2) = +i: (x, y) -> (-y, x).
static inline double2 c_mul_iexp_1_4(double2 a) { return mk(-a.y, a.x); }

// Bit-reversal table for the 16-element FFT (register index -> natural order).
static const int BITREV16[16] = {0, 8, 4, 12, 2, 10, 6, 14,
                                 1, 9, 5, 13, 3, 11, 7, 15};

// --- Butterfly primitives (inverse, sign = +1) — direct ports of
// fft16x4x16.cuh (IFFT2 / IFFT2_mul_iexp_1_4 / IFFT4 / IFFT16).
static inline void ifft2(double2 *a, int i, int j) {
  double2 c0 = a[i];
  a[i] = c_add(c0, a[j]);
  a[j] = c_sub(c0, a[j]);
}
// IFFT2 fused with a * exp(+i*pi/2) twist on the second output.
static inline void ifft2_mul_iexp_1_4(double2 *a, int i, int j) {
  double2 c0 = a[i];
  double a1_x = a[j].x;
  a[i].x = c0.x - a[j].y;
  a[i].y = c0.y + a1_x;
...
static inline void ifft4(double2 *a, int i0, int i1, int i2, int i3) {
  ifft2(a, i0, i2);
  ifft2(a, i1, i3);
  ifft2(a, i0, i1);
  ifft2_mul_iexp_1_4(a, i2, i3);
}

// 16-point inverse DFT held in 16 registers (4x4 Cooley-Tukey grid), in place.
// Output is in bit-reversed order: a[j] holds coefficient BITREV16[j].
static inline void ifft16(double2 *a) {
  const double2 d_iexp_1_16 = mk(D_COS_PI_8, D_SIN_PI_8);
  const double2 d_iexp_3_16 = mk(D_SIN_PI_8, D_COS_PI_8);
  const double2 d_iexp_9_16 = mk(-D_COS_PI_8, -D_SIN_PI_8);
  const double2 d_iexp_1_8 = mk(1.0, 1.0);  // post-multiply by 1/sqrt(2)
  const double2 d_iexp_3_8 = mk(-1.0, 1.0); // post-multiply by 1/sqrt(2)
  ...
  a[5] = c_scale(c_mul(a[5], d_iexp_1_8), D_SQRT1_2);
  a[6] = c_mul_iexp_1_4(a[6]);
  a[7] = c_scale(c_mul(a[7], d_iexp_3_8), D_SQRT1_2);
  a[9] = c_mul(a[9], d_iexp_1_16);
  a[10] = c_scale(c_mul(a[10], d_iexp_1_8), D_SQRT1_2);
  a[11] = c_mul(a[11], d_iexp_3_16);
  a[13] = c_mul(a[13], d_iexp_3_16);
  a[14] = c_scale(c_mul(a[14], d_iexp_3_8), D_SQRT1_2);
  a[15] = c_mul(a[15], d_iexp_9_16);
...
static inline void ifft4x4(double2 *a) {
  ifft4(a, 0, 1, 2, 3);
  ifft4(a, 4, 5, 6, 7);
  ifft4(a, 8, 9, 10, 11);
  ifft4(a, 12, 13, 14, 15);
...
// --- Per-thread twiddle layers (inverse: conjugate of the forward twiddles).

// Inter-stage twiddles from g_tw_1024, conjugated (matches
// mul_itwiddles_16_cached): a[j] *= conj(g_tw_1024[BITREV16[j] - 1][tid]).
static inline void mul_itwiddles_16(double2 *a, int tid) {
  for (int j = 1; j < 16; j++) {
    int r = BITREV16[j] - 1;
    a[j] = c_mul(a[j], c_conj(g_tw_1024[r][tid]));
  }
}

// Compact twiddle layer, conjugated (matches mul_itwiddles_4x4_regs): the three
// active columns come from conj(g_tw_1024[{7,3,11}][4 * lo4]).
static inline void mul_itwiddles_4x4(double2 *a, int tid) {
  int lo4 = tid & 15;
  double2 w1 = c_conj(g_tw_1024[7][4 * lo4]);
  double2 w2 = c_conj(g_tw_1024[3][4 * lo4]);
  double2 w3 = c_conj(g_tw_1024[11][4 * lo4]);
...
// Fused negacyclic post-twist / untwist (matches
// apply_negacyclic_post_twist_16x64_stage_fused). After the inverse FFT, a[j]
// holds the time value at position BITREV16[j]*64 + tid, so it is multiplied by
// ψ_{tid + BITREV16[j]*64}. The half-table reflection at k >= 512 is resolved
// statically per register (BITREV16[j] < 8 <=> k < 512).
static inline void post_twist(double2 *a, int tid) {
  for (int j = 0; j < 16; j++) {
    int r = BITREV16[j];
    if (r < 8) {
      double2 t = g_twisting_twiddles[tid + r * 64];
      a[j] = c_mul(a[j], t);
    } else {
      double2 t = g_twisting_twiddles[1024 - (tid + r * 64)];
      a[j] = c_mul(a[j], mk(-t.y, -t.x));
    }
  }
}

// --- Shared-memory transposes. Identical to the forward path (permute is pure
// data movement, transform-agnostic) — the GPU inverse core reuses the same
// permute_16x64_optimized / permute_4x4_optimized. Pure data movement, no
// arithmetic, so they cannot perturb any bit pattern.

// 16x64 transpose (matches permute_16x64_optimized).
...
// 4x4 block transpose (matches permute_4x4_optimized).
...
// Inverse FFT16x4x16 for a single polynomial (host reference).
//
...
// consumes. Applies the fused negacyclic post-twist and writes the time-domain
// result in NATURAL order, matching cuda_backward_fft16x4x16_async.
static void cpu_backward_fft16x4x16(const double2 *input, double2 *output) {
  ...
  // Inverse core (matches FFT16x4x16_inv_optimized_for_pbs).
  for (int tid = 0; tid < THREADS; tid++) {
    ifft16(regs[tid]);
    mul_itwiddles_16(regs[tid], tid);
  }
  ...
  for (int tid = 0; tid < THREADS; tid++) {
    ifft4x4(regs[tid]);
    mul_itwiddles_4x4(regs[tid], tid);
  }
  permute_4x4(regs, smem.data());
  for (int tid = 0; tid < THREADS; tid++) {
    ifft16(regs[tid]);
    post_twist(regs[tid], tid);
  }

  // Store: un-scramble bit-reversed registers to natural time-domain order.
...
// ===========================================================================
//  AVX-512 host reference — vectorized across the tid dimension (lane = tid, 8
//  tids per __m512d, struct-of-arrays layout, which is exactly the GPU's
//  within-FFT parallelism). Every op is a lane-wise IEEE-754 correctly-rounded
//  double operation, so each lane executes the same sequence as the scalar path
//  for its tid — hence bit-identical to both the scalar reference and the GPU
//  kernel. The two inter-stage transposes are folded into the loads of the next
//  phase (pure data movement, no arithmetic).
// ===========================================================================
#ifdef IFFT16_HAVE_AVX512
namespace avx512 {

#define IFFT16_AVX_TARGET __attribute__((target("avx512f,avx512dq")))
...
// twiddle tables (g_tw_1024 / g_twisting_twiddles) the scalar path uses. All
// inter-stage twiddles are pre-conjugated (inverse transform); post-twist holds
// the fused untwist multipliers.
struct Tw {
  double tw16_re[REGS][THREADS];
  double tw16_im[REGS][THREADS];
  double tw4_re[REGS][THREADS];
  double tw4_im[REGS][THREADS];
  double post_re[REGS][THREADS];
  double post_im[REGS][THREADS];
};

static void build_tw(Tw &t) {
  std::memset(&t, 0, sizeof(t));
  for (int tid = 0; tid < THREADS; tid++) {
    // mul_itwiddles_16: a[j] *= conj(g_tw_1024[BITREV16[j] - 1][tid]).
    for (int j = 1; j < 16; j++) {
      double2 w = g_tw_1024[BITREV16[j] - 1][tid];
      t.tw16_re[j][tid] = w.x;
      t.tw16_im[j][tid] = -w.y;
    }
    // mul_itwiddles_4x4: three columns from conj(g_tw_1024[{7,3,11}][4*lo4]).
    int lo4 = tid & 15;
    ...
    // Negacyclic post-twist: ψ_{tid + BITREV16[j]*64}, with the half-table
    // reflection ((-t.y, -t.x)) resolved per register at BITREV16[j] >= 8.
    for (int j = 0; j < 16; j++) {
      int r = BITREV16[j];
      if (r < 8) {
        double2 w = g_twisting_twiddles[tid + r * 64];
        t.post_re[j][tid] = w.x;
        t.post_im[j][tid] = w.y;
      } else {
        double2 w = g_twisting_twiddles[1024 - (tid + r * 64)];
        t.post_re[j][tid] = -w.y;
        t.post_im[j][tid] = -w.x;
      }
    }
...
IFFT16_AVX_TARGET static inline void cmul(__m512d are, __m512d aim, __m512d bre,
...
IFFT16_AVX_TARGET static inline void cmul_c(__m512d are, __m512d aim,
                                            double cre, double cim,
                                            __m512d &ore, __m512d &oim) {
  cmul(are, aim, _mm512_set1_pd(cre), _mm512_set1_pd(cim), ore, oim);
}
// IEEE sign-bit flip (matches unary -, including the -0.0 edge case).
IFFT16_AVX_TARGET static inline __m512d vnegate(__m512d x) {
  return _mm512_xor_pd(x, _mm512_set1_pd(-0.0));
}

IFFT16_AVX_TARGET static inline void ifft2(__m512d *re, __m512d *im, int i,
                                           int j) {
...
// Inverse of fft2_mul_exp_1_4 (multiply by +i on the second output):
//   a0 = (c0.x - a1.y, c0.y + a1.x),  a1 = (c0.x + a1.y, c0.y - a1.x).
IFFT16_AVX_TARGET static inline void
ifft2_mul_iexp_1_4(__m512d *re, __m512d *im, int i, int j) {
  __m512d c0re = re[i], c0im = im[i], a1re = re[j], a1im = im[j];
  re[i] = _mm512_sub_pd(c0re, a1im);
  im[i] = _mm512_add_pd(c0im, a1re);
  re[j] = _mm512_add_pd(c0re, a1im);
  im[j] = _mm512_sub_pd(c0im, a1re);
}
IFFT16_AVX_TARGET static inline void ifft4(__m512d *re, __m512d *im, int i0,
                                           int i1, int i2, int i3) {
  ifft2(re, im, i0, i2);
  ifft2(re, im, i1, i3);
  ifft2(re, im, i0, i1);
  ifft2_mul_iexp_1_4(re, im, i2, i3);
}

IFFT16_AVX_TARGET static void ifft16(__m512d *re, __m512d *im) {
  ifft4(re, im, 0, 4, 8, 12);
  ifft4(re, im, 1, 5, 9, 13);
  ifft4(re, im, 2, 6, 10, 14);
  ifft4(re, im, 3, 7, 11, 15);
  ...
  ifft4(re, im, 0, 1, 2, 3);
  ifft4(re, im, 4, 5, 6, 7);
  ifft4(re, im, 8, 9, 10, 11);
  ifft4(re, im, 12, 13, 14, 15);
}

IFFT16_AVX_TARGET static inline void ifft4x4(__m512d *re, __m512d *im) {
  ifft4(re, im, 0, 1, 2, 3);
  ifft4(re, im, 4, 5, 6, 7);
  ifft4(re, im, 8, 9, 10, 11);
  ifft4(re, im, 12, 13, 14, 15);
...
// Phase A: deinterleave load from input -> IFFT16 -> mul_itwiddles_16, writing
// SoA to a. (No pre-twist on the inverse path.)
IFFT16_AVX_TARGET static void phase_a(const Tw &tw, const double *input,
...
// Phase B: folded permute_16x64 load from a -> IFFT4x4 -> mul_itwiddles_4x4,
// writing SoA to b.
IFFT16_AVX_TARGET static void phase_b(const Tw &tw,
...
// Phase C: folded permute_4x4 gather from b -> IFFT16 -> negacyclic post-twist
// -> interleaved store to output in natural time-domain order.
IFFT16_AVX_TARGET static void phase_c(const Tw &tw,
                                      const double b_re[REGS][THREADS],
  ...
  for (int g = 0; g < THREADS / 8; g++) {
    ...
    ifft16(vre, vim);
    for (int j = 0; j < REGS; j++)
      cmul(vre[j], vim[j], _mm512_loadu_pd(&tw.post_re[j][base]),
           _mm512_loadu_pd(&tw.post_im[j][base]), vre[j], vim[j]);
    for (int j = 0; j < REGS; j++) {
      int f = BITREV16[j] * 64 + base; // time index of lane 0
...
// AVX-512 inverse FFT16x4x16 for a single polynomial. input/output are
...
// the phases use.
IFFT16_AVX_TARGET static void backward(const Tw &tw, const double2 *input,
                                       double2 *output) {
  ...
  phase_c(tw, b_re, b_im, out);
}

} // namespace avx512
#endif // IFFT16_HAVE_AVX512

} // namespace

// FFT16x4x16 is specialized for N = 2048; run a batch of samples per
// invocation.
constexpr size_t IFFT16_POLYNOMIAL_SIZE = 2048;
constexpr int IFFT16_SAMPLES = 100;

TEST(CpuGpuIFFT16x4x16, backward_matches_cpu_bitwise) {
  ...
  const size_t half = IFFT16_POLYNOMIAL_SIZE / 2; // 1024 complex coeffs
  const size_t total = half * (size_t)IFFT16_SAMPLES;
  ...
  cuda_backward_fft16x4x16_async(stream, gpu_index, d_input, d_output,
                                 IFFT16_POLYNOMIAL_SIZE, IFFT16_SAMPLES);
  ...
  for (int p = 0; p < IFFT16_SAMPLES; p++)
    cpu_backward_fft16x4x16(&h_input[(size_t)p * half],
                            &h_cpu[(size_t)p * half]);
