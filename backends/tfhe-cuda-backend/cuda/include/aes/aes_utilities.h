...
/**
 * The fixed GF(2) maps of a round, precomputed as index tables: per output
 * bit, the list of input bits to sum, padded with AES_LINEAR_NO_TERM. That
 * is all device_aes_linear_combination needs, so a permutation and a five
 * term sum are the same single launch and neither costs a bootstrap. Pure
 * bit positions, hence built once whatever num_aes_inputs is.
 */
