...
static constexpr uint32_t AES_LUT_CTR_KGP = 1;
static constexpr uint32_t AES_LUT_CTR_SELECT = 2;
static constexpr uint32_t AES_LUT_CTR_SUM = 3;
static constexpr uint32_t AES_NUM_STATE_LUTS = 4;
 ...
 * state-wide flushes and the three functions of the CTR adder.
 */
template <typename Torus> struct int_aes_lut_buffers {
  ...
  int_aes_lut_buffers(CudaStreams streams, const int_radix_params &params,
    ...
    std::function<Torus(Torus)> kgp_lambda = [](Torus x) -> Torus {
      return x == 0 ? 0 : (x == 1 ? 2 : 1);
    };

    std::function<Torus(Torus, Torus)> select_lambda = [](Torus hi,
                                                          Torus lo) -> Torus {
      Torus h = hi > 2 ? 2 : hi;
      Torus l = lo > 2 ? 2 : lo;
      return h == 2 ? l : h;
    ...
    std::function<Torus(Torus, Torus)> sum_lambda = [](Torus p,
                                                       Torus c) -> Torus {
      return (p == 2 ? 1 : 0) ^ (c == 1 ? 1 : 0);
    };

    auto active_streams_state_lut = streams.active_gpu_subset(
        AES_STATE_BITS * num_aes_inputs, params.pbs_type);
    this->state_lut->generate_and_broadcast_lut(
        active_streams_state_lut, {AES_LUT_FLUSH, AES_LUT_CTR_KGP},
        {flush_lambda, kgp_lambda}, LUT_0_FOR_ALL_BLOCKS);
...
template <typename Torus> struct int_aes_main_workspaces {
  ...
  int_aes_main_workspaces(CudaStreams streams, const int_radix_params &params,
    ...
    if (scope == aes_buffer_scope::FULL_ENCRYPTION &&
        sbox_input_blocks < AES_STATE_BITS)
      sbox_input_blocks = AES_STATE_BITS;
