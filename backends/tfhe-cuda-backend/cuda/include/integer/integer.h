...
typedef struct {
  // divisor-related attributes
  uint64_t const *divisor_has_at_least_one_set;
  uint64_t const *decomposed_divisor;
  uint32_t const num_scalars_divisor;
  uint32_t const active_bits_divisor;
  ...
  bool const is_divisor_wider_than_numerator;

  // multiplier-related attributes
  uint64_t const *multiplier_has_at_least_one_set;
  uint64_t const *decomposed_multiplier;
  uint32_t const num_scalars_multiplier;
  uint32_t const active_bits_multiplier;
  uint32_t const ilog2_multiplier;
  uint64_t const shift_pre;
  uint32_t const shift_post;
  uint32_t const multiplier_length;

  bool const is_multiplier_zero;
  bool const is_abs_multiplier_one;
  bool const is_multiplier_negative;
  bool const is_multiplier_pow2;
  bool const is_multiplier_wider_than_numerator;
  bool const is_multiplier_geq_numerator_magnitude;
