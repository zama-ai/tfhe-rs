...
// Strong typedefs for cryptographic parameter types (C++ internal).
// Use these inside C++/CUDA code to avoid accidental argument transposition.
struct LweDimension {
  uint32_t value;
};
struct GlweDimension {
  uint32_t value;
};
struct PolynomialSize {
  uint32_t value;
};
struct DecompositionBaseLog {
  uint32_t value;
};
struct DecompositionLevelCount {
  uint32_t value;
};

// C++-internal parameter struct for a LWE bootstrap key.
struct CudaLweBootstrapKeyParams {
...
// Converts a CudaLweBootstrapKeyParamsFFI into the strongly-typed C++ form.
inline CudaLweBootstrapKeyParams crypto_params(CudaLweBootstrapKeyParamsFFI p) {
  return CudaLweBootstrapKeyParams{
      LweDimension{p.input_lwe_dimension},    GlweDimension{p.glwe_dimension},
      PolynomialSize{p.polynomial_size},      DecompositionBaseLog{p.base_log},
      DecompositionLevelCount{p.level_count},
  };
}
