...
template <typename Torus> struct int_aes_encrypt_buffer {
  ...
  // A trivial ciphertext encrypting the constant value 1 as a single bit.
  // Used for homomorphic NOT operations (XOR with 1).
  //
  CudaRadixCiphertextFFI *trivial_1_bit;

  // A trivial ciphertext encrypting the constant 0x1B as an 8-bit value.
  // This is the specific constant required for the MixColumns operation.
  //
  CudaRadixCiphertextFFI *trivial_0x1b_byte;

  Torus *d_trivial_scalars;

  CudaRadixCiphertextFFI *tmp_byte_1;
  CudaRadixCiphertextFFI *tmp_byte_2;
  CudaRadixCiphertextFFI *tmp_byte_3;
  ...
  int_aes_encrypt_buffer(cudaStream_t const *streams,
    ...
    // Pre-computes the bivariate LUT for the bitwise AND operation.
    //
    this->and_lut =
    ...
    // Pre-computes a LUT for the flush operation.
    //
    ...
    // Prepare trivial ciphertexts for constants.
    //
    ...
    // Create a trivial ciphertext encrypting the value 1.
    //
    ...
    // Create a trivial ciphertext encrypting the constant 0x1B.
    // The bit array is in little-endian format.
    //
    ...
    cuda_memcpy_async_to_gpu(d_trivial_scalars, h_trivial_scalars,
                             num_bits_in_byte * sizeof(Torus), streams[0],
                             gpu_indexes[0]);
    this->trivial_0x1b_byte = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams[0], gpu_indexes[0], this->trivial_0x1b_byte, num_bits_in_byte,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    set_trivial_radix_ciphertext_async<Torus>(
        streams[0], gpu_indexes[0], this->trivial_0x1b_byte, d_trivial_scalars,
        h_trivial_scalars, num_bits_in_byte, params.message_modulus,
        params.carry_modulus);

    free(h_trivial_scalars);

    // Allocate all necessary temporary buffers on the GPU.
    // These will be reused across different AES layers to avoid repeated
    // memory allocations.
    //
    ...
    create_zero_radix_ciphertext_async<Torus>(
        ...
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    this->tmp_byte_3 = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams[0], gpu_indexes[0], this->tmp_byte_3, num_bits_in_byte,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
  ...
  void release(cudaStream_t const *streams, uint32_t const *gpu_indexes,
    ...
    release_radix_ciphertext_async(streams[0], gpu_indexes[0],
                                   this->trivial_0x1b_byte,
                                   allocate_gpu_memory);
    delete this->trivial_0x1b_byte;
    this->trivial_0x1b_byte = nullptr;
    ...
    release_radix_ciphertext_async(streams[0], gpu_indexes[0], this->tmp_byte_3,
                                   allocate_gpu_memory);
    delete this->tmp_byte_3;
    this->tmp_byte_3 = nullptr;
