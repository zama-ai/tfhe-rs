...
template <typename Torus> struct int_radix_lut {
  ...
  int_radix_lut(CudaStreams streams, int_radix_params params, uint32_t num_luts,
    ...
    if (active_streams.count() > 1) {
      ...
      for (int i = 0; i < active_streams.count(); i++) {
        event_scatter_out[i] = cuda_create_event(active_streams.gpu_index(i));
      }
    }
    ...
    multi_gpu_alloc_lwe_async(streams, lwe_array_in_vec, num_radix_blocks,
                              params.big_lwe_dimension + 1, size_tracker,
                              allocate_gpu_memory);
    multi_gpu_alloc_lwe_async(streams, lwe_after_ks_vec, num_radix_blocks,
                              params.small_lwe_dimension + 1, size_tracker,
                              allocate_gpu_memory);
    multi_gpu_alloc_lwe_async(streams, lwe_after_pbs_vec, num_radix_blocks,
                              params.big_lwe_dimension + 1, size_tracker,
                              allocate_gpu_memory);
    multi_gpu_alloc_array_async(streams, lwe_trivial_indexes_vec,
  ...
  int_radix_lut(CudaStreams streams, int_radix_params params, uint32_t num_luts,
    ...
    multi_gpu_alloc_array_async(active_streams, lwe_trivial_indexes_vec,
                                num_radix_blocks, size_tracker,
                                allocate_gpu_memory);
    multi_gpu_copy_array_from_cpu_async(active_streams, lwe_trivial_indexes_vec,
  ...
  void release(CudaStreams streams) {
    ...
    if (!mem_reuse) {
      ...
      if (active_streams.count() > 1) {
        ...
        for (uint i = 0; i < active_streams.count(); i++) {
          cuda_event_destroy(event_scatter_out[i], active_streams.gpu_index(i));
        }
        cuda_event_destroy(event_scatter_in, active_streams.gpu_index(0));
        cuda_event_destroy(event_broadcast, active_streams.gpu_index(0));
...
template <typename InputTorus> struct int_noise_squashing_lut {
  ...
  int_noise_squashing_lut(CudaStreams streams, int_radix_params params,
    ...
    multi_gpu_alloc_lwe_async<InputTorus>(
        active_streams, lwe_after_ks_vec, num_radix_blocks,
        params.small_lwe_dimension + 1, size_tracker, allocate_gpu_memory);
    multi_gpu_alloc_lwe_async<__uint128_t>(
        active_streams, lwe_after_pbs_vec, num_radix_blocks,
        params.big_lwe_dimension + 1, size_tracker, allocate_gpu_memory);
    multi_gpu_alloc_array_async<InputTorus>(
        active_streams, lwe_trivial_indexes_vec, num_radix_blocks, size_tracker,
        allocate_gpu_memory);
    cuda_synchronize_stream(streams.stream(0), streams.gpu_index(0));
...
template <typename Torus> struct int_aes_encrypt_buffer {
  ...
  int_aes_encrypt_buffer(CudaStreams streams, const int_radix_params params,
    ...
    this->and_lut = new int_radix_lut<Torus>(
        streams, params, 1, 18 * this->num_blocks * sbox_parallel_instances,
        allocate_gpu_memory, size_tracker);
    ...
    auto active_streams_and_lut = streams.active_gpu_subset(
        18 * this->num_blocks * sbox_parallel_instances);
    this->and_lut->broadcast_lut(active_streams_and_lut);

    this->flush_lut = new int_radix_lut<Torus>(
        streams, params, 1, single_block_state_size * this->num_blocks,
        allocate_gpu_memory, size_tracker);
    ...
    auto active_streams_flush_lut =
        streams.active_gpu_subset(single_block_state_size * this->num_blocks);
    this->flush_lut->broadcast_lut(active_streams_flush_lut);

    this->carry_lut =
        new int_radix_lut<Torus>(streams, params, 1, this->num_blocks,
                                 allocate_gpu_memory, size_tracker);
    ...
    auto active_streams_carry_lut = streams.active_gpu_subset(this->num_blocks);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0),
        this->mix_columns_col_copy_buffer, 32 * this->num_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

    this->mix_columns_mul_workspace_buffer = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0),
        this->mix_columns_mul_workspace_buffer, 40 * this->num_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

    this->tmp_bit_buffer = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), this->tmp_bit_buffer, 1,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), this->tmp_carry_buffer, 1,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

    this->tmp_sum_buffer = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), this->tmp_sum_buffer, 1,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0),
        this->vec_trivial_b_bits_buffer, this->num_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    this->d_counter_bits_buffer = (Torus *)cuda_malloc_with_size_tracking_async(
        this->num_blocks * sizeof(Torus), streams.stream(0),
        streams.gpu_index(0), size_tracker, allocate_gpu_memory);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0),
        this->initial_states_and_jit_key_workspace, this->num_blocks * 128,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

    this->main_bitsliced_states_buffer = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0),
        this->main_bitsliced_states_buffer, this->num_blocks * 128,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
