...
template <typename Torus> struct int_shifted_blocks_and_states_memory {
  ...
  int_shifted_blocks_and_states_memory(
    ...
    shifted_blocks_and_states = (Torus *)cuda_malloc_async(
        lut_count * num_radix_blocks * big_lwe_size_bytes, streams[0],
        gpu_indexes[0]);
    cuda_memset_async(shifted_blocks_and_states, 0,
                      lut_count * num_radix_blocks * big_lwe_size_bytes,
                      streams[0], gpu_indexes[0]);
...
template <typename Torus> struct int_prop_simu_group_carries_memory {
  ...
  // needed for the divison to update the lut indexes
...
template <typename Torus> struct int_fast_sc_prop_memory {
  uint32_t lut_count;
  uint32_t lut_stride;
  ...
  int_fast_sc_prop_memory(cudaStream_t const *streams,
    ...
    lut_count = 2; // many luts apply 2 luts
    uint32_t box_size = polynomial_size / block_modulus;
    lut_stride = (block_modulus / lut_count) * box_size;
    ...
    active_gpu_count = get_active_gpu_count(2 * num_radix_blocks, gpu_count);
    sub_streams_1 =
...
template <typename Torus> struct int_fast_borrow_prop_memory {
  uint32_t lut_count;
  uint32_t lut_stride;
  ...
  Torus **second_indexes_for_div;
  Torus **scalars_for_div;
  uint32_t active_gpu_count;
  cudaStream_t *sub_streams_1;
  cudaStream_t *sub_streams_2;
  ...
  bool is_in_div = false;
  uint32_t max_indexes_to_erase = 0;

  uint32_t compute_overflow;
  int_fast_borrow_prop_memory(cudaStream_t const *streams,
                              uint32_t const *gpu_indexes, uint32_t gpu_count,
                              int_radix_params params,
                              uint32_t num_radix_blocks,
    ...
    outgoing_events2 =
        (cudaEvent_t *)malloc(active_gpu_count * sizeof(cudaEvent_t));
    for (uint j = 0; j < active_gpu_count; j++) {
      cudaEventCreate(&incoming_events[j]);
      cudaEventCreate(&outgoing_events1[j]);
      cudaEventCreate(&outgoing_events2[j]);
    }
  };

  void create_indexes_for_division(cudaStream_t const *streams,
                                   uint32_t const *gpu_indexes,
                                   uint32_t num_blocks) {

    is_in_div = true;
    max_indexes_to_erase = num_blocks;
    bool use_seq = prop_simu_group_carries_mem
                       ->use_sequential_algorithm_to_resolver_group_carries;
    first_indexes_for_div = (Torus **)malloc(num_blocks * sizeof(Torus *));
    second_indexes_for_div = (Torus **)malloc(num_blocks * sizeof(Torus *));
    scalars_for_div = (Torus **)malloc(num_blocks * sizeof(Torus *));
    ...
    for (int nb = 1; nb <= num_blocks; nb++) {
      cudaMalloc((void **)&first_indexes_for_div[nb - 1], nb * sizeof(Torus));
      ...
      cuda_memcpy_async_to_gpu(first_indexes_for_div[nb - 1], h_lut_indexes,
                               nb * sizeof(Torus), streams[0], gpu_indexes[0]);
    }
    // Extra indexes for the luts in second step
    for (int nb = 1; nb <= num_blocks; nb++) {
      cudaMalloc((void **)&second_indexes_for_div[nb - 1], nb * sizeof(Torus));
      cudaMalloc((void **)&scalars_for_div[nb - 1], nb * sizeof(Torus));
      ...
      cuda_memcpy_async_to_gpu(second_indexes_for_div[nb - 1], h_lut_indexes,
                               nb * sizeof(Torus), streams[0], gpu_indexes[0]);
      cuda_memcpy_async_to_gpu(scalars_for_div[nb - 1], h_scalar,
                               nb * sizeof(Torus), streams[0], gpu_indexes[0]);
    }
    free(h_lut_indexes);
    free(h_scalar);
  ...
  void update_lut_indexes(cudaStream_t const *streams,
                          uint32_t const *gpu_indexes,
                          uint32_t new_num_blocks) {
    assert(max_indexes_to_erase >= new_num_blocks);
    shifted_blocks_borrow_state_mem->update_lut_indexes(
        streams, gpu_indexes, first_indexes_for_div[new_num_blocks - 1],
        new_num_blocks);
    prop_simu_group_carries_mem->update_lut_indexes(
        streams, gpu_indexes, second_indexes_for_div[new_num_blocks - 1],
        scalars_for_div[new_num_blocks - 1], new_num_blocks);
  }
  void release(cudaStream_t const *streams, uint32_t const *gpu_indexes,
               uint32_t gpu_count) {
    ...
    if (is_in_div) {
      for (int i = 0; i < max_indexes_to_erase; i++) {
        cuda_drop_async(first_indexes_for_div[i], streams[0], gpu_indexes[0]);
        cuda_drop_async(second_indexes_for_div[i], streams[0], gpu_indexes[0]);
        cuda_drop_async(scalars_for_div[i], streams[0], gpu_indexes[0]);
      }
      free(first_indexes_for_div);
      free(second_indexes_for_div);
      free(scalars_for_div);
    }
    // release sub streams
    for (uint i = 0; i < active_gpu_count; i++) {
      cuda_destroy_stream(sub_streams_1[i], gpu_indexes[i]);
    ...
    // release events
    for (uint j = 0; j < active_gpu_count; j++) {
      cudaEventDestroy(incoming_events[j]);
...
template <typename Torus> struct int_mul_memory {
  ...
  int_fast_sc_prop_memory<Torus> *fast_sc_prop_mem;
  ...
  void release(cudaStream_t const *streams, uint32_t const *gpu_indexes,
    ...
    fast_sc_prop_mem->release(streams, gpu_indexes, gpu_count);

    delete luts_array;
    delete sum_ciphertexts_mem;
    delete fast_sc_prop_mem;
  }
};
...
template <typename Torus> struct int_div_rem_memory {
  ...
  int_fast_borrow_prop_memory<Torus> *overflow_sub_mem;
  ...
  int_div_rem_memory(cudaStream_t const *streams, uint32_t const *gpu_indexes,
    ...
    overflow_sub_mem = new int_fast_borrow_prop_memory<Torus>(
        streams, gpu_indexes, gpu_count, params, num_blocks, compute_overflow,
        true);
    overflow_sub_mem->create_indexes_for_division(streams, gpu_indexes,
                                                  num_blocks);
