...
template <typename Torus> struct int_sum_ciphertexts_vec_memory {
  ...
  int_sum_ciphertexts_vec_memory(cudaStream_t const *streams,
    ...
    d_degrees = (uint64_t *)cuda_malloc_async(
        max_total_blocks_in_vec * sizeof(uint64_t), streams[0], gpu_indexes[0]);

    d_pbs_counters = (uint32_t *)cuda_malloc_async(3 * sizeof(uint32_t),
                                                   streams[0], gpu_indexes[0]);

    auto setup_columns = [num_blocks_in_radix, max_num_radix_in_vec, streams,
                          gpu_indexes](uint32_t **&columns,
                                       uint32_t *&columns_data,
                                       uint32_t *&columns_counter) {
      columns_data = (uint32_t *)cuda_malloc_async(
      ...
      columns = (uint32_t **)cuda_malloc_async(
          num_blocks_in_radix * sizeof(uint32_t *), streams[0], gpu_indexes[0]);
    ...
    generate_device_accumulator<Torus>(streams[0], gpu_indexes[0], message_acc,
                                       luts_message_carry->get_degree(0),
                                       luts_message_carry->get_max_degree(0),
                                       params.glwe_dimension,
                                       params.polynomial_size, message_modulus,
                                       params.carry_modulus, lut_f_message);
    generate_device_accumulator<Torus>(streams[0], gpu_indexes[0], carry_acc,
                                       luts_message_carry->get_degree(1),
                                       luts_message_carry->get_max_degree(1),
                                       params.glwe_dimension,
                                       params.polynomial_size, message_modulus,
                                       params.carry_modulus, lut_f_carry);
  ...
  void release(cudaStream_t const *streams, uint32_t const *gpu_indexes,
               uint32_t gpu_count) {
    cuda_drop_async(d_degrees, streams[0], gpu_indexes[0]);
    cuda_drop_async(d_pbs_counters, streams[0], gpu_indexes[0]);

    cuda_drop_async(d_columns_data, streams[0], gpu_indexes[0]);
    cuda_drop_async(d_columns_counter, streams[0], gpu_indexes[0]);
    cuda_drop_async(d_columns, streams[0], gpu_indexes[0]);

    cuda_drop_async(d_new_columns_data, streams[0], gpu_indexes[0]);
    cuda_drop_async(d_new_columns_counter, streams[0], gpu_indexes[0]);
    cuda_drop_async(d_new_columns, streams[0], gpu_indexes[0]);
