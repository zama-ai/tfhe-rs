...
template <typename Torus> struct int_radix_lut {
  ...
  int_radix_lut(CudaStreams streams, int_radix_params params, uint32_t num_luts,
    ...
    active_gpus = streams.active_gpu_subset(num_radix_blocks);

    cuda_synchronize_stream(streams.stream(0), streams.gpu_index(0));
    for (uint i = 0; i < active_gpus.count(); i++) {
      cuda_set_device(active_gpus.gpu_index(i));
      int8_t *gpu_pbs_buffer;
      auto num_blocks_on_gpu = std::max(
          THRESHOLD_MULTI_GPU,
          get_num_inputs_on_gpu(num_radix_blocks, i, active_gpus.count()));

      uint64_t size = 0;
      execute_scratch_pbs<Torus>(
          streams.stream(i), streams.gpu_index(i), &gpu_pbs_buffer, params.glwe_dimension,
    ...
    for (uint i = 0; i < active_gpus.count(); i++) {
      auto lut = (Torus *)cuda_malloc_with_size_tracking_async(
          num_luts * lut_buffer_size, streams.stream(i), streams.gpu_index(i), size_tracker,
          allocate_gpu_memory);
      auto lut_indexes = (Torus *)cuda_malloc_with_size_tracking_async(
          lut_indexes_size, streams.stream(i), streams.gpu_index(i), size_tracker,
          allocate_gpu_memory);
    ...
    lwe_indexes_in = (Torus *)cuda_malloc_with_size_tracking_async(
        num_radix_blocks * sizeof(Torus), streams.stream(0), streams.gpu_index(0),
    ...
    multi_gpu_alloc_lwe_async(active_gpus,
    ...
    multi_gpu_alloc_lwe_async(active_gpus,
                              ...
                              params.big_lwe_dimension + 1, size_tracker,
                              allocate_gpu_memory);
    multi_gpu_alloc_array_async(active_gpus,
                                lwe_trivial_indexes_vec, num_radix_blocks,
                                size_tracker, allocate_gpu_memory);
    cuda_synchronize_stream(streams.stream(0), streams.gpu_index(0));
    multi_gpu_copy_array_async(active_gpus,
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), tmp_lwe_before_ks, num_radix_blocks,
  ...
  int_radix_lut(CudaStreams streams, int_radix_params params, uint32_t num_luts,
                ...
                bool allocate_gpu_memory, uint64_t &size_tracker) : residence_streams(streams) {
  ...
  // Broadcast luts from device residence_streams.gpu_index(0) to all destination gpus
  void broadcast_lut(CudaStreams streams) {
    ...
    cuda_synchronize_stream(streams.stream(0), streams.gpu_index(0));
    for (uint i = 0; i < active_gpus.count(); i++) {
      if (residence_streams.gpu_index(i) != streams.gpu_index(0)) {
        auto dst_lut = lut_vec[i];
        auto dst_lut_indexes = lut_indexes_vec[i];
        cuda_memcpy_with_size_tracking_async_gpu_to_gpu(
            dst_lut, src_lut, num_luts * lut_size * sizeof(Torus), streams.stream(i), streams.gpu_index(i), gpu_memory_allocated);
        cuda_memcpy_with_size_tracking_async_gpu_to_gpu(
...
//    free(this->gpu_indexes);
    for (uint i = 0; i < active_gpus.count(); i++) {
      cuda_drop_with_size_tracking_async(lut_vec[i], streams.stream(i), streams.gpu_index(i),
                                         gpu_memory_allocated);
      cuda_drop_with_size_tracking_async(lut_indexes_vec[i], streams.stream(i), streams.gpu_index(i), gpu_memory_allocated);
    }

    cuda_drop_with_size_tracking_async(lwe_indexes_in, streams.stream(0), streams.gpu_index(0), gpu_memory_allocated);
    cuda_drop_with_size_tracking_async(lwe_indexes_out, streams.stream(0), streams.gpu_index(0), gpu_memory_allocated);
    cuda_drop_with_size_tracking_async(lwe_trivial_indexes, streams.stream(0), streams.gpu_index(0), gpu_memory_allocated);
    ...
    if (!mem_reuse) {
      ...
      for (int i = 0; i < buffer.size(); i++) {
        ...
        case MULTI_BIT:
          cleanup_cuda_multi_bit_programmable_bootstrap(
              streams.stream(i), streams.gpu_index(i), &buffer[i]);
          break;
        case CLASSICAL:
          cleanup_cuda_programmable_bootstrap(streams.stream(i), streams.gpu_index(i),
                                              &buffer[i]);
          break;
        default:
      ...
      if (gpu_memory_allocated) {
        multi_gpu_release_async(streams, lwe_array_in_vec);
        multi_gpu_release_async(streams, lwe_after_ks_vec);
        multi_gpu_release_async(streams, lwe_after_pbs_vec);
        multi_gpu_release_async(streams, lwe_trivial_indexes_vec);
        for (uint i = 0; i < active_gpus.count(); i++)
          cuda_synchronize_stream(streams.stream(i), streams.gpu_index(i));
      }
...
template <typename InputTorus> struct int_noise_squashing_lut {
  ...
  int_noise_squashing_lut(CudaStreams streams,
                          int_radix_params params,
                          uint32_t input_glwe_dimension,
                          uint32_t input_polynomial_size,
...
//    std::memcpy(gpu_indexes, input_gpu_indexes, gpu_count * sizeof(uint32_t));
    ...
    for (uint i = 0; i < active_gpus.count(); i++) {
      ...
      int8_t *gpu_pbs_buffer;
      uint64_t size = 0;
      execute_scratch_pbs_128(streams.stream(i), streams.gpu_index(i), &gpu_pbs_buffer,
    ...
    lwe_indexes_in = (InputTorus *)cuda_malloc_with_size_tracking_async(
        num_radix_blocks * sizeof(InputTorus), streams.stream(0), streams.gpu_index(0),
        size_tracker, allocate_gpu_memory);
    ...
    multi_gpu_alloc_array_async<InputTorus>(
        active_gpus, lwe_trivial_indexes_vec,
    ...
    if (allocate_gpu_memory) {
      ...
      for (uint i = 0; i < active_gpus.count(); i++) {
        auto lut = (__uint128_t *)cuda_malloc_with_size_tracking_async(
            lut_buffer_size, streams.stream(i), streams.gpu_index(i), size_tracker,
            allocate_gpu_memory);
        lut_vec.push_back(lut);
        cuda_synchronize_stream(streams.stream(i), streams.gpu_index(i));
      }
    ...
    create_zero_radix_ciphertext_async<InputTorus>(
        streams.stream(0), streams.gpu_index(0), tmp_lwe_before_ks, original_num_blocks,
    ...
    for (uint i = 0; i < active_gpus.count(); i++) {
      ...
      generate_device_accumulator<__uint128_t>(
          streams.stream(i), streams.gpu_index(i), squash_lut, degrees, max_degrees,
  ...
  void release(CudaStreams streams) {
    ...
    for (int i = 0; i < pbs_buffer.size(); i++) {
      cleanup_cuda_programmable_bootstrap_128(streams.stream(i), streams.gpu_index(i),
                                              &pbs_buffer[i]);
      cuda_synchronize_stream(streams.stream(i), streams.gpu_index(i));
    }
...
template <typename Torus> struct int_bit_extract_luts_buffer {
  ...
  // With offset
  int_bit_extract_luts_buffer(CudaStreams streams,
    ...
    for (int i = 0; i < bits_per_block; i++) {
      ...
      generate_device_accumulator<Torus>(
          streams.stream(0), streams.gpu_index(0), lut->get_lut(0, i), lut->get_degree(i),
    ...
    cuda_memcpy_with_size_tracking_async_to_gpu(
        ...
        num_radix_blocks * bits_per_block * sizeof(Torus), streams.stream(0), streams.gpu_index(0), allocate_gpu_memory);
    lut->broadcast_lut(streams);
    ...
    lut->set_lwe_indexes(streams.stream(0), streams.gpu_index(0), h_lwe_indexes_in,
                         h_lwe_indexes_out);
  ...
  int_bit_extract_luts_buffer(CudaStreams streams,
                              ...
                              uint32_t num_radix_blocks,
                              bool allocate_gpu_memory, uint64_t &size_tracker)
      : int_bit_extract_luts_buffer(streams, params,
...
template <typename Torus> struct int_shift_and_rotate_buffer {
  ...
  int_shift_and_rotate_buffer(CudaStreams streams,
    ...
    bit_extract_luts = new int_bit_extract_luts_buffer<Torus>(streams, params, bits_per_block,
        num_radix_blocks, allocate_gpu_memory, size_tracker);
    bit_extract_luts_with_offset_2 = new int_bit_extract_luts_buffer<Torus>(streams, params, bits_per_block, 2,
        num_radix_blocks, allocate_gpu_memory, size_tracker);

    mux_lut = new int_radix_lut<Torus>(streams, params,
                                       1, bits_per_block * num_radix_blocks,
                                       allocate_gpu_memory, size_tracker);
    cleaning_lut = new int_radix_lut<Torus>(streams,
                                            params, 1, num_radix_blocks,
                                            allocate_gpu_memory, size_tracker);

    tmp_bits = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), tmp_bits, bits_per_block * num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    cleaning_lut->broadcast_lut(streams);
  }

  void release(CudaStreams streams) {
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_bits,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_shift_bits,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_rotated,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_input_bits_a,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_input_bits_b,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_mux_inputs,
                                   gpu_memory_allocated);
...
template <typename Torus> struct int_fullprop_buffer {
  ...
  int_fullprop_buffer(CudaStreams streams, int_radix_params params,
    ...
    generate_device_accumulator<Torus>(
        streams.stream(0), streams.gpu_index(0), lut_buffer_message, message_degree,
    ...
    cuda_memcpy_with_size_tracking_async_to_gpu(
        lwe_indexes, h_lwe_indexes, lwe_indexes_size, streams.stream(0), streams.gpu_index(0), allocate_gpu_memory);
...
template <typename Torus> struct int_overflowing_sub_memory {
  ...
  int_overflowing_sub_memory(CudaStreams streams,
    ...
    generates_or_propagates = (Torus *)cuda_malloc_with_size_tracking_async(
        num_radix_blocks * big_lwe_size_bytes, streams.stream(0), streams.gpu_index(0),
        size_tracker, allocate_gpu_memory);
    ...
    cuda_memset_with_size_tracking_async(
        ...
        streams.stream(0), streams.gpu_index(0), allocate_gpu_memory);
    cuda_memset_with_size_tracking_async(
        step_output, 0, num_radix_blocks * big_lwe_size_bytes, streams.stream(0), streams.gpu_index(0), allocate_gpu_memory);
    ...
    // create lut objects
    luts_array = new int_radix_lut<Torus>(streams,
                                          params, 2, num_radix_blocks,
                                          allocate_gpu_memory, size_tracker);
    luts_borrow_propagation_sum = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
    ...
    generate_device_accumulator<Torus>(
        streams.stream(0), streams.gpu_index(0), lut_does_block_generate_or_propagate,
    ...
    generate_device_accumulator_bivariate<Torus>(
        streams.stream(0), streams.gpu_index(0), luts_borrow_propagation_sum->get_lut(0, 0),
    ...
    luts_array->broadcast_lut(streams);
    luts_borrow_propagation_sum->broadcast_lut(streams);
    message_acc->broadcast_lut(streams);
  }

  void release(CudaStreams streams) {
    cuda_drop_with_size_tracking_async(generates_or_propagates, streams.stream(0), streams.gpu_index(0), gpu_memory_allocated);
    cuda_drop_with_size_tracking_async(step_output, streams.stream(0), streams.gpu_index(0),
                                       gpu_memory_allocated);
...
template <typename Torus> struct int_sum_ciphertexts_vec_memory {
  ...
  void setup_index_buffers(CudaStreams streams,
                           uint64_t &size_tracker) {

    d_degrees = (uint64_t *)cuda_malloc_with_size_tracking_async(
        max_total_blocks_in_vec * sizeof(uint64_t), streams.stream(0), streams.gpu_index(0),
    ...
    auto setup_columns =
        [num_blocks_in_radix, max_num_radix_in_vec, streams](uint32_t **&columns, uint32_t *&columns_data,
          ...
          columns_data = (uint32_t *)cuda_malloc_with_size_tracking_async(
              ...
              streams.stream(0), streams.gpu_index(0), size_tracker, gpu_memory_allocated);
          columns_counter = (uint32_t *)cuda_malloc_with_size_tracking_async(
              num_blocks_in_radix * sizeof(uint32_t), streams.stream(0), streams.gpu_index(0), size_tracker, gpu_memory_allocated);
          cuda_memset_with_size_tracking_async(
  ...
  void setup_lookup_tables(CudaStreams streams,
                           uint32_t num_radix_in_vec,
    ...
    if (!mem_reuse) {
      ...
      if (total_ciphertexts > 0 ||
        ...
        luts_message_carry =
            new int_radix_lut<Torus>(streams, params, 2,
    ...
    if (allocated_luts_message_carry) {
      ...
      luts_message_carry->broadcast_lut(streams);
    }
  }
  int_sum_ciphertexts_vec_memory(
      CudaStreams streams, int_radix_params params, uint32_t num_blocks_in_radix,
      uint32_t max_num_radix_in_vec,
      bool reduce_degrees_for_single_carry_propagation,
      bool allocate_gpu_memory, uint64_t &size_tracker) {
    this->params = params;
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), current_blocks, max_total_blocks_in_vec,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    small_lwe_vector = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), small_lwe_vector, max_total_blocks_in_vec,
        params.small_lwe_dimension, size_tracker, allocate_gpu_memory);
  }
  ...
  void release(CudaStreams streams) {
    cuda_drop_with_size_tracking_async(d_degrees, streams.stream(0), streams.gpu_index(0),
                                       gpu_memory_allocated);
    cuda_drop_with_size_tracking_async(d_columns_data, streams.stream(0), streams.gpu_index(0), gpu_memory_allocated);
    cuda_drop_with_size_tracking_async(d_columns_counter, streams.stream(0), streams.gpu_index(0), gpu_memory_allocated);
    cuda_drop_with_size_tracking_async(d_columns, streams.stream(0), streams.gpu_index(0),
                                       gpu_memory_allocated);

    cuda_drop_with_size_tracking_async(d_new_columns_data, streams.stream(0), streams.gpu_index(0), gpu_memory_allocated);
    cuda_drop_with_size_tracking_async(d_new_columns_counter, streams.stream(0), streams.gpu_index(0), gpu_memory_allocated);
    cuda_drop_with_size_tracking_async(d_new_columns, streams.stream(0), streams.gpu_index(0), gpu_memory_allocated);

    if (!mem_reuse) {
      release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), current_blocks,
                                     gpu_memory_allocated);
      release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0),
...
template <typename Torus> struct int_seq_group_prop_memory {
  ...
  int_seq_group_prop_memory(CudaStreams streams,
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), group_resolved_carries, grouping_size,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

    int num_seq_luts = grouping_size - 1;
    Torus *h_seq_lut_indexes = (Torus *)malloc(num_seq_luts * sizeof(Torus));
    lut_sequential_algorithm = new int_radix_lut<Torus>(streams, params, num_seq_luts, num_seq_luts,
        allocate_gpu_memory, size_tracker);
    ...
    lut_sequential_algorithm->broadcast_lut(streams);
...
template <typename Torus> struct int_hs_group_prop_memory {
  ...
  int_hs_group_prop_memory(CudaStreams streams,
...
template <typename Torus> struct int_shifted_blocks_and_states_memory {
  ...
  int_shifted_blocks_and_states_memory(
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), shifted_blocks, num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    luts_array_first_step = new int_radix_lut<Torus>(streams, params, num_luts_first_step,
    ...
    generate_many_lut_device_accumulator<Torus>(
        streams.stream(0), streams.gpu_index(0), first_block_lut, first_block_lut_degrees,
    ...
    for (int lut_id = 1; lut_id < grouping_size; lut_id++) {
      ...
      generate_many_lut_device_accumulator<Torus>(
          streams.stream(0), streams.gpu_index(0), lut, lut_degrees, lut_max_degree,
          glwe_dimension, polynomial_size, message_modulus, carry_modulus,
    ...
    generate_many_lut_device_accumulator<Torus>(
        streams.stream(0), streams.gpu_index(0), last_block_lut, last_block_lut_degrees,
    ...
    cuda_memcpy_with_size_tracking_async_to_gpu(
        lut_indexes, h_lut_indexes, lut_indexes_size, streams.stream(0), streams.gpu_index(0), allocate_gpu_memory);
    // Do I need to do something else for the multi-gpu?

    luts_array_first_step->broadcast_lut(streams);
  };
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0),
                                   ...
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), shifted_blocks,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), block_states,
                                   gpu_memory_allocated);
...
template <typename Torus> struct int_prop_simu_group_carries_memory {
  ...
  int_prop_simu_group_carries_memory(
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), propagation_cum_sums, num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), prepared_blocks, num_radix_blocks + 1,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    resolved_carries = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), resolved_carries, num_groups + 1,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    scalar_array_cum_sum = (Torus *)cuda_malloc_with_size_tracking_async(
        ...
        size_tracker, allocate_gpu_memory);
    cuda_memset_with_size_tracking_async(
        scalar_array_cum_sum, 0, num_radix_blocks * sizeof(Torus), streams.stream(0), streams.gpu_index(0), allocate_gpu_memory);
    ...
    luts_array_second_step = new int_radix_lut<Torus>(streams, params, num_luts_second_step,
        num_radix_blocks, allocate_gpu_memory, size_tracker);
    ...
    generate_device_accumulator<Torus>(
        streams.stream(0), streams.gpu_index(0), luts_array_second_step->get_lut(0, lut_id),
        luts_array_second_step->get_degree(lut_id),
        luts_array_second_step->get_max_degree(lut_id), glwe_dimension,
        polynomial_size, message_modulus, carry_modulus,
    ...
    cuda_memcpy_with_size_tracking_async_to_gpu(
        second_lut_indexes, h_second_lut_indexes, lut_indexes_size, streams.stream(0), streams.gpu_index(0), allocate_gpu_memory);

    cuda_memcpy_with_size_tracking_async_to_gpu(
        scalar_array_cum_sum, h_scalar_array_cum_sum,
        num_radix_blocks * sizeof(Torus), streams.stream(0), streams.gpu_index(0),
        allocate_gpu_memory);
    luts_array_second_step->broadcast_lut(streams);

    if (use_sequential_algorithm_to_resolve_group_carries) {

      seq_group_prop_mem = new int_seq_group_prop_memory<Torus>(streams, params, grouping_size,
          big_lwe_size_bytes, allocate_gpu_memory, size_tracker);

    } else {
      hs_group_prop_mem = new int_hs_group_prop_memory<Torus>(streams, params, num_groups,
          big_lwe_size_bytes, allocate_gpu_memory, size_tracker);
    }
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), simulators,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), grouping_pgns,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), prepared_blocks,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), resolved_carries,
                                   gpu_memory_allocated);
    cuda_drop_with_size_tracking_async(scalar_array_cum_sum, streams.stream(0), streams.gpu_index(0), gpu_memory_allocated);
...
template <typename Torus> struct int_sc_prop_memory {
  ...
  int_sc_prop_memory(CudaStreams streams, int_radix_params params,
    ...
    shifted_blocks_state_mem = new int_shifted_blocks_and_states_memory<Torus>(streams, params, num_radix_blocks, num_many_lut,
        grouping_size, allocate_gpu_memory, size_tracker);

    prop_simu_group_carries_mem = new int_prop_simu_group_carries_memory<Torus>(streams, params, num_radix_blocks,
    ...
    lut_message_extract = new int_radix_lut<Torus>(streams, params, num_luts_message_extract,
    ...
    generate_device_accumulator<Torus>(
        streams.stream(0), streams.gpu_index(0), lut_message_extract->get_lut(0, 0),
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), output_flag, num_radix_blocks + 1,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    if (requested_flag == outputFlag::FLAG_OVERFLOW) {
      ...
      create_zero_radix_ciphertext_async<Torus>(
          streams.stream(0), streams.gpu_index(0), last_lhs, 1, params.big_lwe_dimension,
          size_tracker, allocate_gpu_memory);
      create_zero_radix_ciphertext_async<Torus>(
          streams.stream(0), streams.gpu_index(0), last_rhs, 1, params.big_lwe_dimension,
          size_tracker, allocate_gpu_memory);
      ...
      generate_device_accumulator_bivariate<Torus>(
          streams.stream(0), streams.gpu_index(0), lut_overflow_flag_prep->get_lut(0, 0),
      ...
      lut_overflow_flag_prep->broadcast_lut(streams);
    }
    ...
    if (requested_flag == outputFlag::FLAG_OVERFLOW) { // Overflow case
      ...
      generate_device_accumulator<Torus>(
          streams.stream(0), streams.gpu_index(0), lut_message_extract->get_lut(0, 1),
      ...
      cuda_memcpy_with_size_tracking_async_to_gpu(
          ...
          (num_radix_blocks + 1) * sizeof(Torus), streams.stream(0), streams.gpu_index(0),
          allocate_gpu_memory);
    }
    ...
    lut_message_extract->broadcast_lut(streams);
  };
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), output_flag,
                                   gpu_memory_allocated);
    ...
    if (requested_flag == outputFlag::FLAG_OVERFLOW) { // In case of overflow
      release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), last_lhs,
                                     gpu_memory_allocated);
      release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), last_rhs,
                                     gpu_memory_allocated);
...
template <typename Torus> struct int_shifted_blocks_and_borrow_states_memory {
  ...
  int_shifted_blocks_and_borrow_states_memory(
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), shifted_blocks_and_borrow_states,
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), borrow_states, num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), shifted_blocks,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), borrow_states,
                                   gpu_memory_allocated);
...
template <typename Torus> struct int_borrow_prop_memory {
  ...
  bool gpu_memory_allocated;
  int_borrow_prop_memory(CudaStreams streams,
                         int_radix_params params, uint32_t num_radix_blocks,
    ...
    shifted_blocks_borrow_state_mem =
        new int_shifted_blocks_and_borrow_states_memory<Torus>(streams, params, num_radix_blocks,
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), overflow_block, 1, params.big_lwe_dimension,
        size_tracker, allocate_gpu_memory);

    lut_message_extract = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
        allocate_gpu_memory, size_tracker);
    // lut for the first block in the first grouping
    auto f_message_extract = [message_modulus](Torus block) -> Torus {
      return (block >> 1) % message_modulus;
    ...
    if (compute_overflow) {
      lut_borrow_flag = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
          allocate_gpu_memory, size_tracker);
      // lut for the first block in the first grouping
      ...
      generate_device_accumulator<Torus>(
          streams.stream(0), streams.gpu_index(0), lut_borrow_flag->get_lut(0, 0),
          lut_borrow_flag->get_degree(0), lut_borrow_flag->get_max_degree(0),
          glwe_dimension, polynomial_size, message_modulus, carry_modulus,
          f_borrow_flag, gpu_memory_allocated);

      lut_borrow_flag->broadcast_lut(streams);
    }

    active_gpus = streams.active_gpu_subset(num_radix_blocks);
    sub_streams_1 = active_gpus.clone();
    sub_streams_2 = active_gpus.clone();

    incoming_events =
        (cudaEvent_t *)malloc(active_gpus.count() * sizeof(cudaEvent_t));
    outgoing_events1 =
        (cudaEvent_t *)malloc(active_gpus.count() * sizeof(cudaEvent_t));
    outgoing_events2 =
        (cudaEvent_t *)malloc(active_gpus.count() * sizeof(cudaEvent_t));
    for (uint j = 0; j < active_gpus.count(); j++) {
      incoming_events[j] = cuda_create_event(active_gpus.gpu_index(j));
      outgoing_events1[j] = cuda_create_event(active_gpus.gpu_index(j));
      outgoing_events2[j] = cuda_create_event(active_gpus.gpu_index(j));
    }
  };

  // needed for the division to update the lut indexes
  void update_lut_indexes(CudaStreams streams,
    ...
    prop_simu_group_carries_mem->update_lut_indexes(
        streams, second_indexes_for_div, scalars_for_div,
        new_num_blocks);
  }
  void release(CudaStreams streams) {
    ...
    prop_simu_group_carries_mem->release(streams);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), overflow_block,
                                   gpu_memory_allocated);
    ...
    // release events
    for (uint j = 0; j < active_gpus.count(); j++) {
      cuda_event_destroy(incoming_events[j], active_gpus.gpu_index(j));
      cuda_event_destroy(outgoing_events1[j], active_gpus.gpu_index(j));
      cuda_event_destroy(outgoing_events2[j], active_gpus.gpu_index(j));
    }
    free(incoming_events);
    free(outgoing_events1);
    free(outgoing_events2);

      sub_streams_1.destroy();
      sub_streams_2.destroy();
  };
};
...
template <typename Torus> struct int_zero_out_if_buffer {
  ...
  int_zero_out_if_buffer(CudaStreams streams,
                         int_radix_params params, uint32_t num_radix_blocks,
                         bool allocate_gpu_memory, uint64_t &size_tracker) {
    gpu_memory_allocated = allocate_gpu_memory;
    this->params = params;
    ...
    true_streams = active_gpus.clone();
      false_streams = active_gpus.clone();
  }
  void release(CudaStreams streams) {
    ...
    cuda_synchronize_stream(streams.stream(0), streams.gpu_index(0));
    delete tmp;
      true_streams.destroy();
      false_streams.destroy();
  }
};
...
template <typename Torus> struct int_mul_memory {
  ...
  int_mul_memory(CudaStreams streams, int_radix_params params,
    ...
    if (boolean_mul) {
      ...
      zero_out_predicate_lut = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
          allocate_gpu_memory, size_tracker);
      generate_device_accumulator_bivariate<Torus>(
          streams.stream(0), streams.gpu_index(0), zero_out_predicate_lut->get_lut(0, 0),
      ...
      zero_out_predicate_lut->broadcast_lut(streams);

      zero_out_mem = new int_zero_out_if_buffer<Torus>(streams, params, num_radix_blocks,
          allocate_gpu_memory, size_tracker);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), vector_result_sb, 2 * total_block_count,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    block_mul_res = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), block_mul_res, 2 * total_block_count,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    small_lwe_vector = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), small_lwe_vector, 2 * total_block_count,
        params.small_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    // generate accumulators
    generate_device_accumulator_bivariate<Torus>(
        streams.stream(0), streams.gpu_index(0), lsb_acc, luts_array->get_degree(0),
        luts_array->get_max_degree(0), glwe_dimension, polynomial_size,
        message_modulus, carry_modulus, lut_f_lsb, gpu_memory_allocated);
    generate_device_accumulator_bivariate<Torus>(
        streams.stream(0), streams.gpu_index(0), msb_acc, luts_array->get_degree(1),
    ...
    sum_ciphertexts_mem = new int_sum_ciphertexts_vec_memory<Torus>(streams, params, num_radix_blocks,
        2 * num_radix_blocks, vector_result_sb, small_lwe_vector, luts_array,
        true, allocate_gpu_memory, size_tracker);
    uint32_t uses_carry = 0;
    uint32_t requested_flag = outputFlag::FLAG_NONE;
    sc_prop_mem = new int_sc_prop_memory<Torus>(streams, params, num_radix_blocks,
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), vector_result_sb,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), block_mul_res,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), small_lwe_vector,
                                   gpu_memory_allocated);
...
template <typename Torus> struct int_logical_scalar_shift_buffer {
  ...
  int_logical_scalar_shift_buffer(
      CudaStreams streams, SHIFT_OR_ROTATE_TYPE shift_type,
      int_radix_params params, uint32_t num_radix_blocks,
      bool allocate_gpu_memory, uint64_t &size_tracker) {
    gpu_memory_allocated = allocate_gpu_memory;
    this->shift_type = shift_type;
    ...
    tmp_rotated = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), tmp_rotated, max_amount_of_pbs + 2,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    for (int s_w_b = 1; s_w_b < num_bits_in_block; s_w_b++) {
      auto cur_lut_bivariate = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
          allocate_gpu_memory, size_tracker);
...
template <typename Torus> struct int_arithmetic_scalar_shift_buffer {
  ...
  int_arithmetic_scalar_shift_buffer(
    ...
    active_gpus = streams.active_gpu_subset(1);
    ...
    local_streams_1 =
        (cudaStream_t *)malloc(active_gpus.count() * sizeof(cudaStream_t));
    local_streams_2 =
        (cudaStream_t *)malloc(active_gpus.count() * sizeof(cudaStream_t));
    for (uint j = 0; j < active_gpus.count(); j++) {
      local_streams_1[j] = cuda_create_stream(active_gpus.gpu_index(j));
      local_streams_2[j] = cuda_create_stream(active_gpus.gpu_index(j));
    }
    this->shift_type = shift_type;
    this->params = params;
    ...
    tmp_rotated = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), tmp_rotated, num_radix_blocks + 3,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    generate_device_accumulator<Torus>(
        streams.stream(0), streams.gpu_index(0), padding_block_lut_univariate->get_lut(0, 0),
    ...
    padding_block_lut_univariate->broadcast_lut(streams);
    ...
    for (int s_w_b = 1; s_w_b < num_bits_in_block; s_w_b++) {
      auto shift_blocks_lut_bivariate = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
          allocate_gpu_memory, size_tracker);
      ...
      generate_device_accumulator_bivariate<Torus>(
          streams.stream(0), streams.gpu_index(0), shift_blocks_lut_bivariate->get_lut(0, 0),
      ...
      shift_blocks_lut_bivariate->broadcast_lut(streams);
  ...
  void release(CudaStreams streams) {
    for (uint j = 0; j < active_gpus.count(); j++) {
      cuda_destroy_stream(local_streams_1[j], active_gpus.gpu_index(j));
      cuda_destroy_stream(local_streams_2[j], active_gpus.gpu_index(j));
    }
...
template <typename Torus> struct int_cmux_buffer {
  ...
  int_cmux_buffer(CudaStreams streams,
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), buffer_in, 2 * num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), buffer_out, 2 * num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), condition_array, 2 * num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    predicate_lut = new int_radix_lut<Torus>(streams,
                                             params, 2, 2 * num_radix_blocks,
                                             allocate_gpu_memory, size_tracker);

    message_extract_lut = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
        allocate_gpu_memory, size_tracker);
    ...
    generate_device_accumulator<Torus>(
        streams.stream(0), streams.gpu_index(0), message_extract_lut->get_lut(0, 0),
    ...
    cuda_memcpy_with_size_tracking_async_to_gpu(
        ...
        2 * num_radix_blocks * sizeof(Torus), streams.stream(0), streams.gpu_index(0),
        allocate_gpu_memory);

    predicate_lut->broadcast_lut(streams);
    message_extract_lut->broadcast_lut(streams);
  }
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), buffer_in,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), buffer_out,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), condition_array,
                                   gpu_memory_allocated);
    cuda_synchronize_stream(streams.stream(0), streams.gpu_index(0));
    delete buffer_in;
    delete buffer_out;
...
template <typename Torus> struct int_are_all_block_true_buffer {
  ...
  int_are_all_block_true_buffer(CudaStreams streams,
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), tmp_block_accumulated, max_chunks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    is_max_value->broadcast_lut(streams);
  }

  void release(CudaStreams streams) {
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_out,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0),
...
template <typename Torus> struct int_comparison_eq_buffer {
  ...
  int_comparison_eq_buffer(CudaStreams streams,
                           COMPARISON_TYPE op, int_radix_params params,
                           uint32_t num_radix_blocks, bool allocate_gpu_memory,
                           uint64_t &size_tracker) {
    gpu_memory_allocated = allocate_gpu_memory;
    ...
    are_all_block_true_buffer = new int_are_all_block_true_buffer<Torus>(streams, op, params, num_radix_blocks,
        allocate_gpu_memory, size_tracker);
    ...
    operator_lut = new int_radix_lut<Torus>(streams,
                                            params, 1, num_radix_blocks,
                                            allocate_gpu_memory, size_tracker);
    ...
    operator_lut->broadcast_lut(streams);
    ...
    is_non_zero_lut = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
        allocate_gpu_memory, size_tracker);
    ...
    is_non_zero_lut->broadcast_lut(streams);

    // Scalar may have up to num_radix_blocks blocks
    scalar_comparison_luts = new int_radix_lut<Torus>(streams, params, total_modulus,
        num_radix_blocks, allocate_gpu_memory, size_tracker);
    ...
    for (int i = 0; i < total_modulus; i++) {
      ...
      generate_device_accumulator<Torus>(
          streams.stream(0), streams.gpu_index(0), scalar_comparison_luts->get_lut(0, i),
    ...
    scalar_comparison_luts->broadcast_lut(streams);
  }
...
template <typename Torus> struct int_tree_sign_reduction_buffer {
  ...
  int_tree_sign_reduction_buffer(
      CudaStreams streams, std::function<Torus(Torus)> operator_f,
      int_radix_params params, uint32_t num_radix_blocks,
      bool allocate_gpu_memory, uint64_t &size_tracker) {
    gpu_memory_allocated = allocate_gpu_memory;
    this->params = params;
    ...
    create_zero_radix_ciphertext_async<Torus>(
        ...
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    // LUTs
    tree_inner_leaf_lut = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
        allocate_gpu_memory, size_tracker);
    ...
    tree_last_leaf_scalar_lut =
        new int_radix_lut<Torus>(streams, params, 1, 1,
                                 allocate_gpu_memory, size_tracker);
    generate_device_accumulator_bivariate<Torus>(
        streams.stream(0), streams.gpu_index(0), tree_inner_leaf_lut->get_lut(0, 0),
    ...
    tree_inner_leaf_lut->broadcast_lut(streams);
  }

  void release(CudaStreams streams) {
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_x,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_y,
                                   gpu_memory_allocated);
...
template <typename Torus> struct int_comparison_diff_buffer {
  ...
  int_comparison_diff_buffer(CudaStreams streams,
                             COMPARISON_TYPE op, int_radix_params params,
                             uint32_t num_radix_blocks,
                             bool allocate_gpu_memory, uint64_t &size_tracker) {
    gpu_memory_allocated = allocate_gpu_memory;
    ...
    tree_buffer = new int_tree_sign_reduction_buffer<Torus>(streams, operator_f, params, num_radix_blocks,
        allocate_gpu_memory, size_tracker);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        ...
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    // LUTs
    reduce_signs_lut = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
        allocate_gpu_memory, size_tracker);
  }

  void release(CudaStreams streams) {
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_packed,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_signs_a,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_signs_b,
                                   gpu_memory_allocated);
...
template <typename Torus> struct int_comparison_buffer {
  ...
  int_comparison_buffer(CudaStreams streams,
                        COMPARISON_TYPE op, int_radix_params params,
    ...
    msb_streams =
        (cudaStream_t *)malloc(active_gpus.count() * sizeof(cudaStream_t));
    for (uint j = 0; j < active_gpus.count(); j++) {
      lsb_streams[j] = cuda_create_stream(active_gpus.gpu_index(j));
      msb_streams[j] = cuda_create_stream(active_gpus.gpu_index(j));
    }

    // +1 to have space for signed comparison
    tmp_lwe_array_out = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), tmp_lwe_array_out, num_radix_blocks + 1,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

    tmp_packed_input = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), tmp_packed_input, 2 * num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

    // Block comparisons
    tmp_block_comparisons = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), tmp_block_comparisons, num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

    // Cleaning LUT
    identity_lut = new int_radix_lut<Torus>(streams,
                                            params, 1, num_radix_blocks,
                                            allocate_gpu_memory, size_tracker);
    ...
    identity_lut->broadcast_lut(streams);
    ...
    is_zero_lut = new int_radix_lut<Torus>(streams,
                                           params, 1, num_radix_blocks,
                                           allocate_gpu_memory, size_tracker);
    ...
    is_zero_lut->broadcast_lut(streams);

    switch (op) {
    case COMPARISON_TYPE::MAX:
    case COMPARISON_TYPE::MIN:
      cmux_buffer = new int_cmux_buffer<Torus>(streams,
          [op](Torus x) -> Torus {
    ...
    case COMPARISON_TYPE::LE:
      diff_buffer = new int_comparison_diff_buffer<Torus>(streams, op, params, num_radix_blocks,
          allocate_gpu_memory, size_tracker);
    case COMPARISON_TYPE::EQ:
    case COMPARISON_TYPE::NE:
      eq_buffer = new int_comparison_eq_buffer<Torus>(streams, op, params, num_radix_blocks,
          allocate_gpu_memory, size_tracker);
      break;
    default:
    ...
    if (is_signed) {
      ...
      signed_lut->broadcast_lut(streams);
    }
  }
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_packed_input,
                                   gpu_memory_allocated);
    ...
    for (uint j = 0; j < active_gpus.count(); j++) {
      cuda_destroy_stream(lsb_streams[j], active_gpus.gpu_index(j));
      cuda_destroy_stream(msb_streams[j], active_gpus.gpu_index(j));
    }
    free(lsb_streams);
    free(msb_streams);
  }
...
template <typename Torus> struct unsigned_int_div_rem_memory {
  ...
  void init_temporary_buffers(CudaStreams streams,
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), numerator_block_stack, num_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    interesting_remainder2 = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), interesting_remainder2, num_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    interesting_divisor = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), interesting_divisor, num_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), cleaned_merged_interesting_remainder,
        num_blocks, params.big_lwe_dimension, size_tracker,
        allocate_gpu_memory);
    tmp_1 = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), tmp_1, num_blocks, params.big_lwe_dimension,
        size_tracker, allocate_gpu_memory);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), interesting_remainder1, num_blocks + 1,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), overflow_sum, 1, params.big_lwe_dimension,
        size_tracker, allocate_gpu_memory);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), at_least_one_upper_block_is_non_zero, 1,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
  }

  // initialize lookup tables for div_rem operation
  void init_lookup_tables(CudaStreams streams,
                          uint32_t num_blocks, bool allocate_gpu_memory,
                          uint64_t &size_tracker) {
    ...
    for (int i = 0; i < params.message_modulus - 1; i++) {
      ...
      masking_luts_1[i] =
          new int_radix_lut<Torus>(streams, params, 1,
                                   1, allocate_gpu_memory, size_tracker);
      masking_luts_2[i] = new int_radix_lut<Torus>(streams, params, 1, num_blocks,
          allocate_gpu_memory, size_tracker);
    ...
    generate_device_accumulator_bivariate_with_factor<Torus>(
        ...
        gpu_memory_allocated);
    zero_out_if_overflow_did_not_happen[0]->broadcast_lut(streams);
    generate_device_accumulator_bivariate_with_factor<Torus>(
        streams.stream(0), streams.gpu_index(0),
        ...
        gpu_memory_allocated);
    zero_out_if_overflow_did_not_happen[1]->broadcast_lut(streams);
    ...
    generate_device_accumulator_bivariate_with_factor<Torus>(
        ...
        gpu_memory_allocated);
    zero_out_if_overflow_happened[0]->broadcast_lut(streams);
    generate_device_accumulator_bivariate_with_factor<Torus>(
        streams.stream(0), streams.gpu_index(0),
        ...
        gpu_memory_allocated);
    zero_out_if_overflow_happened[1]->broadcast_lut(streams);
    ...
    for (int i = 0; i < num_bits_in_message; i++) {
      ...
      merge_overflow_flags_luts[i]->broadcast_lut(streams);
    }
  }

  unsigned_int_div_rem_memory(CudaStreams streams,
                              int_radix_params params, uint32_t num_blocks,
                              bool allocate_gpu_memory,
                              uint64_t &size_tracker) {
    gpu_memory_allocated = allocate_gpu_memory;
    active_gpus = streams.active_gpu_subset(2 * num_blocks);

    this->params = params;
    shift_mem_1 = new int_logical_scalar_shift_buffer<Torus>(streams, SHIFT_OR_ROTATE_TYPE::LEFT_SHIFT,
        params, 2 * num_blocks, allocate_gpu_memory, size_tracker);

    shift_mem_2 = new int_logical_scalar_shift_buffer<Torus>(streams, SHIFT_OR_ROTATE_TYPE::LEFT_SHIFT,
        params, 2 * num_blocks, allocate_gpu_memory, size_tracker);

    uint32_t compute_overflow = 1;
    overflow_sub_mem = new int_borrow_prop_memory<Torus>(streams, params, num_blocks, compute_overflow,
        allocate_gpu_memory, size_tracker);
    uint32_t group_size = overflow_sub_mem->group_size;
    bool use_seq = overflow_sub_mem->prop_simu_group_carries_mem
                       ->use_sequential_algorithm_to_resolve_group_carries;
    create_indexes_for_overflow_sub(streams, num_blocks,
                                    group_size, use_seq, allocate_gpu_memory,
                                    size_tracker);

    comparison_buffer = new int_comparison_buffer<Torus>(streams, COMPARISON_TYPE::NE, params,
        num_blocks, false, allocate_gpu_memory, size_tracker);

    init_lookup_tables(streams, num_blocks,
                       allocate_gpu_memory, size_tracker);
    init_temporary_buffers(streams, num_blocks,
                           allocate_gpu_memory, size_tracker);

    sub_streams_1 = active_gpus.clone();
    sub_streams_2 = active_gpus.clone();
    sub_streams_3 = active_gpus.clone();
    sub_streams_4 = active_gpus.clone();
  }

  void create_indexes_for_overflow_sub(CudaStreams streams,
    ...
    for (int nb = 1; nb <= num_blocks; nb++) {
      first_indexes_for_overflow_sub[nb - 1] =
          (Torus *)cuda_malloc_with_size_tracking_async(
              nb * sizeof(Torus), streams.stream(0), streams.gpu_index(0), size_tracker,
              allocate_gpu_memory);
      ...
      cuda_memcpy_with_size_tracking_async_to_gpu(
          ...
          nb * sizeof(Torus), streams.stream(0), streams.gpu_index(0), allocate_gpu_memory);
    }
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), remainder1,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), remainder2,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0),
                                   numerator_block_stack, gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0),
                                   numerator_block_1, gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_radix,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0),
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), new_remainder,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0),
                                   subtraction_overflowed,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), did_not_overflow,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), overflow_sum,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0),
                                   overflow_sum_radix, gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_1,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0),
    ...
    sub_streams_1.destroy();
    sub_streams_2.destroy();
    sub_streams_3.destroy();
    sub_streams_4.destroy();
...
template <typename Torus> struct int_bitop_buffer {
  ...
  int_bitop_buffer(CudaStreams streams, BITOP_TYPE op, int_radix_params params,
    ...
    case BITOR:
    case BITXOR:
      lut = new int_radix_lut<Torus>(streams, params, 1,
                                     num_radix_blocks, allocate_gpu_memory,
                                     size_tracker);
      {
        ...
        generate_device_accumulator_bivariate<Torus>(
            streams.stream(0), streams.gpu_index(0), lut->get_lut(0, 0), lut->get_degree(0),
    ...
    default:
      // Scalar OP
      lut = new int_radix_lut<Torus>(streams, params,
...
template <typename Torus> struct int_scalar_mul_buffer {
  ...
  int_scalar_mul_buffer(CudaStreams streams,
                        int_radix_params params, uint32_t num_radix_blocks,
    ...
    if (num_ciphertext_bits * num_radix_blocks >= num_radix_blocks + 2)
      logical_scalar_shift_buffer = new int_logical_scalar_shift_buffer<Torus>(streams, LEFT_SHIFT, params, num_radix_blocks,
    ...
    if (num_ciphertext_bits > 0) {
      sum_ciphertexts_vec_mem = new int_sum_ciphertexts_vec_memory<Torus>(streams, params, num_radix_blocks,
...
template <typename Torus> struct int_abs_buffer {
  ...
  int_abs_buffer(CudaStreams streams, int_radix_params params,
    ...
    this->params = params;
    this->allocate_gpu_memory = allocate_gpu_memory;
    arithmetic_scalar_shift_mem = new int_arithmetic_scalar_shift_buffer<Torus>(streams, SHIFT_OR_ROTATE_TYPE::RIGHT_SHIFT,
        params, num_radix_blocks, allocate_gpu_memory, size_tracker);
    uint32_t requested_flag = outputFlag::FLAG_NONE;
    uint32_t uses_carry = 0;
    scp_mem = new int_sc_prop_memory<Torus>(streams, params, num_radix_blocks,
        requested_flag, uses_carry, allocate_gpu_memory, size_tracker);
    bitxor_mem = new int_bitop_buffer<Torus>(streams, BITOP_TYPE::BITXOR, params,
        num_radix_blocks, allocate_gpu_memory, size_tracker);
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), mask,
...
template <typename Torus> struct int_div_rem_memory {
  ...
  int_div_rem_memory(CudaStreams streams, int_radix_params params,
    ...
    gpu_memory_allocated = allocate_gpu_memory;
    this->active_gpus = streams.active_gpu_subset(num_blocks);
    this->params = params;
    this->is_signed = is_signed;

    unsigned_mem = new unsigned_int_div_rem_memory<Torus>(streams, params, num_blocks,
        allocate_gpu_memory, size_tracker);
    ...
    if (is_signed) {
      ...
      abs_mem_1 = new int_abs_buffer<Torus>(streams,
                                            params, num_blocks,
                                            allocate_gpu_memory, size_tracker);
      abs_mem_2 = new int_abs_buffer<Torus>(streams,
                                            params, num_blocks,
                                            allocate_gpu_memory, size_tracker);
      uint32_t requested_flag = outputFlag::FLAG_NONE;
      uint32_t uses_carry = 0;
      scp_mem_1 = new int_sc_prop_memory<Torus>(streams, params, num_blocks, requested_flag,
          uses_carry, allocate_gpu_memory, size_tracker);
      scp_mem_2 = new int_sc_prop_memory<Torus>(streams, params, num_blocks, requested_flag,
          uses_carry, allocate_gpu_memory, size_tracker);
      ...
      cmux_quotient_mem = new int_cmux_buffer<Torus>(streams, quotient_predicate_lut_f, params,
          num_blocks, allocate_gpu_memory, size_tracker);
      cmux_remainder_mem = new int_cmux_buffer<Torus>(streams, remainder_predicate_lut_f, params,
          num_blocks, allocate_gpu_memory, size_tracker);
      // init temporary memory buffers
      positive_numerator = new CudaRadixCiphertextFFI;
      create_zero_radix_ciphertext_async<Torus>(
          streams.stream(0), streams.gpu_index(0), positive_numerator, num_blocks,
          params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
      ...
      create_zero_radix_ciphertext_async<Torus>(
          streams.stream(0), streams.gpu_index(0), negated_remainder, num_blocks,
          params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
      ...
      // init sub streams
      sub_streams_1 = active_gpus.clone(); //cudaStream_t *)malloc(gpu_count * sizeof(cudaStream_t));
      sub_streams_2 = active_gpus.clone(); // (cudaStream_t *)malloc(gpu_count * sizeof(cudaStream_t));
      ...
      generate_device_accumulator_bivariate<Torus>(
          streams.stream(0), streams.gpu_index(0), compare_signed_bits_lut->get_lut(0, 0),
      ...
      compare_signed_bits_lut->broadcast_lut(streams);
    }
  }
...
template <typename Torus> struct int_scalar_mul_high_buffer {
  ...
  int_scalar_mul_high_buffer(CudaStreams streams,
    ...
    this->logical_scalar_shift_mem = new int_logical_scalar_shift_buffer<Torus>(streams, RIGHT_SHIFT, params,
        2 * num_radix_blocks, allocate_gpu_memory, size_tracker);

    this->scalar_mul_mem = new int_scalar_mul_buffer<Torus>(streams, params, 2 * num_radix_blocks,
...
template <typename Torus> struct int_sub_and_propagate {
  ...
  int_sub_and_propagate(CudaStreams streams,
                        const int_radix_params params,
                        uint32_t num_radix_blocks, uint32_t requested_flag_in,
                        bool allocate_gpu_memory, uint64_t &size_tracker) {
    ...
    this->sc_prop_mem = new int_sc_prop_memory<Torus>(streams, params, num_radix_blocks,
        requested_flag_in, (uint32_t)0, allocate_gpu_memory, size_tracker);

    this->neg_rhs_array = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), neg_rhs_array, num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
  }
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), neg_rhs_array,
                                   allocate_gpu_memory);
...
template <typename Torus> struct int_extend_radix_with_sign_msb_buffer {
  ...
  int_extend_radix_with_sign_msb_buffer(
    ...
    if (num_additional_blocks != 0) {
      this->lut = new int_radix_lut<Torus>(streams,
                                           params, 1, num_radix_blocks,
                                           allocate_gpu_memory, size_tracker);
      ...
      create_zero_radix_ciphertext_async<Torus>(
          streams.stream(0), streams.gpu_index(0), last_block, 1, params.big_lwe_dimension,
          size_tracker, allocate_gpu_memory);
  ...
  void release(CudaStreams streams) {
    ...
    if (last_block != nullptr) {
      release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), last_block,
                                     allocate_gpu_memory);
      delete last_block;
    }
    if (padding_block != nullptr) {
      release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), padding_block,
                                     allocate_gpu_memory);
...
template <typename Torus> struct int_unsigned_scalar_div_mem {
  ...
  int_unsigned_scalar_div_mem(CudaStreams streams,
    ...
    if (!scalar_divisor_ffi->is_abs_divisor_one) {
      if (scalar_divisor_ffi->is_divisor_pow2) {
        ...
        logical_scalar_shift_mem = new int_logical_scalar_shift_buffer<Torus>(streams, RIGHT_SHIFT, params,
            num_radix_blocks, allocate_gpu_memory, size_tracker);
        ...
        logical_scalar_shift_mem = new int_logical_scalar_shift_buffer<Torus>(streams, RIGHT_SHIFT, params,
            num_radix_blocks, allocate_gpu_memory, size_tracker);
        scalar_mul_high_mem = new int_scalar_mul_high_buffer<Torus>(streams, params, num_radix_blocks,
            scalar_divisor_ffi->active_bits, allocate_gpu_memory, size_tracker);
        scp_mem = new int_sc_prop_memory<Torus>(streams, params, num_radix_blocks,
            FLAG_NONE, (uint32_t)0, allocate_gpu_memory, size_tracker);
        sub_and_propagate_mem = new int_sub_and_propagate<Torus>(streams, params, num_radix_blocks,
  ...
  void release(CudaStreams streams) {
    ...
    if (tmp_ffi != nullptr) {
      release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_ffi,
                                     allocate_gpu_memory);
      delete tmp_ffi;
    }
  }
...
template <typename Torus> struct int_signed_scalar_mul_high_buffer {
  ...
  int_signed_scalar_mul_high_buffer(
    ...
    this->extend_radix_mem = new int_extend_radix_with_sign_msb_buffer<Torus>(streams, params, num_radix_blocks,
        num_radix_blocks, allocate_gpu_memory, size_tracker);
  }
...
template <typename Torus> struct int_signed_scalar_div_mem {
  ...
  int_signed_scalar_div_mem(CudaStreams streams,
                            const int_radix_params params,
                            uint32_t num_radix_blocks,
                            const CudaScalarDivisorFFI *scalar_divisor_ffi,
                            const bool allocate_gpu_memory,
    ...
    if (!scalar_divisor_ffi->chosen_multiplier_has_more_bits_than_numerator) {
      ...
      if (scalar_divisor_ffi->is_abs_divisor_one &&
        ...
        arithmetic_scalar_shift_mem =
            new int_arithmetic_scalar_shift_buffer<Torus>(streams, RIGHT_SHIFT, params,
                num_radix_blocks, allocate_gpu_memory, size_tracker);
        ...
        if (scalar_divisor_ffi->is_divisor_pow2) {
          ...
          create_zero_radix_ciphertext_async<Torus>(
              streams.stream(0), streams.gpu_index(0), xsign_ffi, num_radix_blocks,
              params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

          scalar_mul_high_mem = new int_signed_scalar_mul_high_buffer<Torus>(streams, params, num_radix_blocks,
  ...
  void release(CudaStreams streams) {
    ...
    if (xsign_ffi != nullptr) {
      release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), xsign_ffi,
                                     allocate_gpu_memory);
      delete xsign_ffi;
    }
  }
...
template <typename Torus> struct int_unsigned_scalar_div_rem_buffer {
  ...
  int_unsigned_scalar_div_rem_buffer(
    ...
    this->unsigned_div_mem = new int_unsigned_scalar_div_mem<Torus>(streams, params, num_radix_blocks,
        scalar_divisor_ffi, allocate_gpu_memory, size_tracker);

    if (scalar_divisor_ffi->is_divisor_pow2) {
      this->bitop_mem = new int_bitop_buffer<Torus>(streams, SCALAR_BITAND, params,
          num_radix_blocks, allocate_gpu_memory, size_tracker);
    } else {
      if (!scalar_divisor_ffi->is_divisor_zero &&
          !scalar_divisor_ffi->is_abs_divisor_one && num_radix_blocks != 0) {
        this->scalar_mul_mem = new int_scalar_mul_buffer<Torus>(streams, params, num_radix_blocks,
            active_bits_divisor, allocate_gpu_memory, true, size_tracker);
      }
      this->sub_and_propagate_mem = new int_sub_and_propagate<Torus>(streams, params, num_radix_blocks, FLAG_NONE,
          allocate_gpu_memory, size_tracker);
    }
  }
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), numerator_ct,
                                   allocate_gpu_memory);
    delete numerator_ct;
...
template <typename Torus> struct int_signed_scalar_div_rem_buffer {
  ...
  int_signed_scalar_div_rem_buffer(
    ...
    this->signed_div_mem = new int_signed_scalar_div_mem<Torus>(streams, params, num_radix_blocks,
        scalar_divisor_ffi, allocate_gpu_memory, size_tracker);

    this->scp_mem = new int_sc_prop_memory<Torus>(streams, params, num_radix_blocks, FLAG_NONE,
    ...
    if (!scalar_divisor_ffi->is_divisor_negative &&
      ...
      this->logical_scalar_shift_mem =
          new int_logical_scalar_shift_buffer<Torus>(streams, LEFT_SHIFT, params,
              num_radix_blocks, allocate_gpu_memory, size_tracker);
...
template <typename Torus> struct int_prepare_count_of_consecutive_bits_buffer {
  ...
  int_prepare_count_of_consecutive_bits_buffer(
    ...
    this->univ_lut_mem = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
        allocate_gpu_memory, size_tracker);
    this->biv_lut_mem = new int_radix_lut<Torus>(streams, params, 1, num_radix_blocks,
        allocate_gpu_memory, size_tracker);
    ...
    if (allocate_gpu_memory) {
      univ_lut_mem->broadcast_lut(streams);
    }
    ...
    if (allocate_gpu_memory) {
      biv_lut_mem->broadcast_lut(streams);
    }
  ...
  void release(CudaStreams streams) {
    ...
    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), tmp_ct,
                                   allocate_gpu_memory);
    delete tmp_ct;
  }
};
...
template <typename Torus> struct int_count_of_consecutive_bits_buffer {
  ...
  int_count_of_consecutive_bits_buffer(
    ...
    this->prepare_mem = new int_prepare_count_of_consecutive_bits_buffer<Torus>(streams, params, num_radix_blocks, direction,
        bit_value, allocate_gpu_memory, size_tracker);

    this->cts = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), cts, counter_num_blocks * num_radix_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

    this->sum_mem = new int_sum_ciphertexts_vec_memory<Torus>(streams, params, counter_num_blocks,
        num_radix_blocks, true, allocate_gpu_memory, size_tracker);

    this->propagate_mem = new int_sc_prop_memory<Torus>(streams, params, counter_num_blocks, 0, 0,
        allocate_gpu_memory, size_tracker);
  }

  void release(CudaStreams streams) {

    release_radix_ciphertext_async(streams.stream(0), streams.gpu_index(0), ct_prepared,
                                   allocate_gpu_memory);
    delete ct_prepared;
    ct_prepared = nullptr;
...
template <typename Torus> struct int_grouped_oprf_memory {
  ...
  int_grouped_oprf_memory(CudaStreams streams,
                          int_radix_params params,
    ...
    this->luts = new int_radix_lut<Torus>(streams, params, message_bits_per_block,
        num_blocks, allocate_gpu_memory, size_tracker);

    this->plaintext_corrections = new CudaRadixCiphertextFFI;
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), this->plaintext_corrections, num_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    for (uint32_t random_bit = 1; random_bit <= message_bits_per_block;
      ...
      generate_device_accumulator_no_encoding<Torus>(
          streams.stream(0), streams.gpu_index(0), luts->get_lut(0, lut_index), degree,
    ...
    cuda_memcpy_async_to_gpu(this->plaintext_corrections->ptr, h_corrections,
                             num_blocks * lwe_size * sizeof(Torus), streams.stream(0), streams.gpu_index(0));
...
template <typename Torus> struct int_ilog2_buffer {
  ...
  int_ilog2_buffer(CudaStreams streams, const int_radix_params params,
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), this->ct_in_buffer, input_num_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);

    this->prepare_mem = new int_prepare_count_of_consecutive_bits_buffer<Torus>(streams, params, input_num_blocks, Leading,
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), this->sum_input_cts, sum_input_total_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), this->sum_output_not_propagated,
        counter_num_blocks, params.big_lwe_dimension, size_tracker,
        allocate_gpu_memory);

    this->lut_message_not = new int_radix_lut<Torus>(streams, params, 1, counter_num_blocks,
        allocate_gpu_memory, size_tracker);
    ...
    generate_device_accumulator(
        streams.stream(0), streams.gpu_index(0), this->lut_message_not->get_lut(0, 0),
    ...
    this->lut_carry_not = new int_radix_lut<Torus>(streams, params, 1, counter_num_blocks,
        allocate_gpu_memory, size_tracker);
    ...
    generate_device_accumulator(
        streams.stream(0), streams.gpu_index(0), this->lut_carry_not->get_lut(0, 0),
    ...
    create_zero_radix_ciphertext_async<Torus>(
        streams.stream(0), streams.gpu_index(0), this->carry_blocks_not, counter_num_blocks,
        params.big_lwe_dimension, size_tracker, allocate_gpu_memory);
    ...
    this->final_propagate_mem =
        new int_fullprop_buffer<Torus>(streams, params,
                                       allocate_gpu_memory, size_tracker);
  }
