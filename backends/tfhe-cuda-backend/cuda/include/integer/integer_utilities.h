...
template <typename Torus> struct unsigned_int_div_rem_2_2_memory {
  ...
  void init_lookup_tables(cudaStream_t const *streams,
    ...
    zero_out_if_not_1_lut_1 =
        new int_radix_lut<Torus>(&streams[0], &gpu_indexes[0], 1, params, 1,
    ...
    for (int j = 0; j < 2; j++) {
      generate_device_accumulator<Torus>(
          streams[lut_gpu_indexes[j]], gpu_indexes[lut_gpu_indexes[j]],
          luts[j]->get_lut(0, 0), luts[j]->get_degree(0),
          luts[j]->get_max_degree(0), params.glwe_dimension,
          params.polynomial_size, params.message_modulus, params.carry_modulus,
  ...
  unsigned_int_div_rem_2_2_memory(cudaStream_t const *streams,
    ...
    comparison_buffer_1 = new int_comparison_buffer<Torus>(
        &streams[0], &gpu_indexes[0], 1, COMPARISON_TYPE::EQ, params,
  ...
  void scatter_indexes_for_overflowing_sub(
      cudaStream_t const *streams, uint32_t const *gpu_indexes,
      size_t gpu_index, Torus ***first_indexes_ptr, Torus ***second_indexes_ptr,
      Torus ***scalars_ptr, uint32_t num_blocks, bool allocate_gpu_memory,
      uint64_t &size_tracker) {
  ...
  void release(cudaStream_t const *streams, uint32_t const *gpu_indexes,
    ...
    overflow_sub_mem_1->release(&streams[0], &gpu_indexes[0], 1);
    overflow_sub_mem_2->release(&streams[1], &gpu_indexes[1], 1);
    overflow_sub_mem_3->release(&streams[2], &gpu_indexes[2], 1);
    ...
    quotient_lut_1->release(&streams[2], &gpu_indexes[2], gpu_count);
    ...
    release_radix_ciphertext_async(streams[0], gpu_indexes[0], low3,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams[3], gpu_indexes[3], rem0,
                                   gpu_memory_allocated);
    ...
    release_radix_ciphertext_async(streams[2], gpu_indexes[2], remainder_gpu_2,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams[3], gpu_indexes[3], remainder_gpu_3,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams[2], gpu_indexes[2],
                                   comparison_blocks_3, gpu_memory_allocated);
    release_radix_ciphertext_async(streams[1], gpu_indexes[1],
    ...
    release_radix_ciphertext_async(streams[3], gpu_indexes[3], c0,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams[2], gpu_indexes[2], q1,
                                   gpu_memory_allocated);
    release_radix_ciphertext_async(streams[1], gpu_indexes[1], q2,
                                   gpu_memory_allocated);
