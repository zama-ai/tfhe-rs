...
// Round constants pulled back through M = SR . M' so the e-xor layers can
// absorb them. Derived, not transcribed, so they cannot drift from RC_V2.
...
// Index of the e-xor term each output bit of Mk reads: output bit 4r + j takes
...
template <typename Torus> struct int_prince_key_prep_buffer {
  ...
  cuda_index_table_owner<uint32_t> maps;
  cuda_index_table_owner<Torus> lut_indexes;

  radix_gather_map map_dup;      // 64 <- 32, K=1 (each u2 twice)
  radix_gather_map map_bw_sum3;  // 64 <- 64, K=3 (IPERM-addressed parity)
  radix_gather_map map_mid_sum3; // 64 <- 64, K=3 (direct parity)
  cuda_index_table<Torus> lut_idx_keybit; // 64 entries, slots 1/0 alternating
  ...
  int_prince_key_prep_buffer(CudaStreams streams,
    ...
    maps.create(streams, allocate_gpu_memory, size_tracker, map_dup, NUM_BITS,
                1, [](uint32_t o, uint32_t) { return o >> 1; });
...
// slots: [0, 4) parity, one per output weight 8, 4, 2, 1, then one group per
// distinct S-box table.
template <typename Torus> struct int_prince_buffer {
  ...
  // high/low slots it is generated with; only the final layer departs
  // from it.
  ...
  radix_gather_map map_key_tile;     // 32N <- 32,  K=1 (broadcast over lanes)
  radix_gather_map map_tile64;       // 64N <- 64,  K=1 (broadcast over lanes)
  radix_gather_map map_transpose_in; // 32N <- 32N, K=1 (instance -> word major)
  radix_gather_map
      map_transpose_out;         // 32N <- 32N, K=1 (word -> instance major)
  radix_gather_map map_pair;     // 16N <- 32N, K=2 (hi/lo pair combine)
  radix_gather_map map_stage_x4; // 64N <- 16N, K=1 (replicate each word x4)
  radix_gather_map map_stage_x2; // 32N <- 16N, K=1 (replicate each word x2)
  radix_gather_map map_perm_pair_mid; // 32N <- 64N, K=2 (FHE_M_PERM + pair)
  radix_gather_map map_mperm_comb;    // 16N <- 64N, K=4 (FHE_M_PERM + comb)
  radix_gather_map map_fw_sum3;       // 64N <- 64N, K=3 (bridge+perm composed)
  radix_gather_map map_mid_sum3;      // 64N <- 64N, K=3 (bridge composed)
  radix_gather_map map_bw_sum3;       // 64N <- 64N, K=3 (IPERM composed)
  radix_gather_map map_comb4_id; // 16N <- 64N, K=4 (4 weighted bits -> nibble)

  cuda_index_table<Torus> lut_idx_fw_sbox[5];
  cuda_index_table<Torus> lut_idx_mid_in;
  cuda_index_table<Torus> lut_idx_mid_out;
  cuda_index_table<Torus> lut_idx_bw_sbox[5];
  cuda_index_table<Torus> lut_idx_par_fw;
  cuda_index_table<Torus> lut_idx_par_bw;
  cuda_index_table<Torus> lut_idx_par_mid;
  ...
  int_prince_buffer(CudaStreams streams, const int_radix_params &params,
    ...
    auto build_raw_map =
        [&](radix_gather_map &map, uint32_t num_out_blocks, uint32_t num_terms,
