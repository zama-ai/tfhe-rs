...
struct pbs_buffer_128<InputTorus, PBS_TYPE::MULTI_BIT> {
  ...
  pbs_buffer_128(cudaStream_t stream, uint32_t gpu_index,
    ...
    // #if CUDA_ARCH >= 900
    //     uint64_t full_sm_tbc_accumulate =
    //         get_buffer_size_full_sm_tbc_multibit_programmable_bootstrap<__uint128_t>(
    //             polynomial_size);
    //     uint64_t partial_sm_tbc_accumulate =
    //         get_buffer_size_partial_sm_tbc_multibit_programmable_bootstrap<__uint128_t>(
    //             polynomial_size);
    //     uint64_t minimum_sm_tbc =
    //         get_buffer_size_sm_dsm_plus_tbc_multibit_programmable_bootstrap<__uint128_t>(
    //             polynomial_size);
    //     auto num_blocks_acc_tbc = num_blocks_acc_cg;
    // #endif
    ...
    case PBS_VARIANT::DEFAULT:
      ...
      // #if CUDA_ARCH >= 900
      //     case TBC:
      //       // There is a minimum amount of memory we need to run the TBC
      //       PBS, which
      //       // is minimum_sm_tbc. We know that minimum_sm_tbc bytes are
      //       available
      //       // because otherwise the previous check would have redirected
      //       // computation to some other variant. If over that we don't have
      //       more
      //       // partial_sm_tbc_accumulate bytes, TBC PBS will run on NOSM. If
      //       we have
      //       // partial_sm_tbc_accumulate but not full_sm_tbc_accumulate
      //       bytes, it
      //       // will run on PARTIALSM. Otherwise, FULLSM.
      //       //
      //       // NOSM mode actually requires minimum_sm_tbc shared memory
      //       bytes.
      //
      //       // Accumulator TBC
      ...
      //             gpu_index, size_tracker, allocate_gpu_memory);
      //       else if (max_shared_memory < full_sm_tbc_accumulate +
      //       minimum_sm_tbc)
      //         d_mem_acc_tbc = (int8_t *)cuda_malloc_with_size_tracking_async(
  ...
  void release(cudaStream_t stream, uint32_t gpu_index) {
    ...
    case CG:
      ...
      //     case TBC:
      //       if (d_mem_acc_tbc)
      //         cuda_drop_with_size_tracking_async(d_mem_acc_tbc, stream,
      //         gpu_index,
      //                                            gpu_memory_allocated);
      //       break;
      // #endif
