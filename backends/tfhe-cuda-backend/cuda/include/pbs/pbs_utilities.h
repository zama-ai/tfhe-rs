...
struct pbs_buffer_128<InputTorus, PBS_TYPE::CLASSICAL>
  ...
  pbs_buffer_128(cudaStream_t stream, uint32_t gpu_index,
      ...
      // Only the variants that carry the accumulator across launches allocate
      // it; release() frees it when it is non-null, so it must start null.
  ...
  void release(cudaStream_t stream, uint32_t gpu_index) override {
    ...
    // and the member has no default initializer, so this must stay a variant
