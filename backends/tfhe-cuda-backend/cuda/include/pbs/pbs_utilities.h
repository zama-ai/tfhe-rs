...
template <typename Torus> struct pbs_buffer<Torus, PBS_TYPE::CLASSICAL> {
  ...
  pbs_buffer(cudaStream_t stream, uint32_t gpu_index, uint32_t lwe_dimension,
             ...
             PBS_MS_REDUCTION_T noise_reduction_type, uint64_t &size_tracker) : noise_reduction_type(noise_reduction_type) {
    ...
    this->temp_lwe_array_in = (Torus *)cuda_malloc_with_size_tracking_async(
        ...
        stream, gpu_index, size_tracker, noise_reduction_type == PBS_MS_REDUCTION_T::DRIFT);
    switch (pbs_variant) {
...
struct pbs_buffer_128<InputTorus, PBS_TYPE::CLASSICAL> {
  ...
  pbs_buffer_128(cudaStream_t stream, uint32_t gpu_index,
                 ...
                 uint64_t &size_tracker)  : noise_reduction_type(noise_reduction_type) {
    gpu_memory_allocated = allocate_gpu_memory;
    cuda_set_device(gpu_index);
    this->pbs_variant = pbs_variant;
