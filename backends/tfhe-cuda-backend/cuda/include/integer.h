...
template <typename Torus> struct int_radix_lut {
  ...
  std::vector<Torus *> lut_array;
  std::vector<Torus *> lut_indexes_array;
  ...
  int_radix_lut(cudaStream_t *streams, uint32_t *gpu_indexes,
    ...
    if (allocate_gpu_memory) {
      ...
      for (uint i = 0; i < active_gpu_count; i++) {
        ...
        lut_array.push_back(lut);
        lut_indexes_array.push_back(lut_indexes);
      }
  ...
  int_radix_lut(cudaStream_t *streams, uint32_t *gpu_indexes,
    ...
    // LUT is used as a trivial encryption and must be initialized outside
    // this constructor
    active_gpu_count = std::min(num_radix_blocks, gpu_count);
    for (uint i = 0; i < active_gpu_count; i++) {
      auto lut = (Torus *)cuda_malloc_async(num_luts * lut_buffer_size,
                                            streams[0], gpu_indexes[0]);
  ...
  Torus *get_lut_indexes(uint32_t gpu_index, size_t ind) {
    auto lut_indexes = lut_indexes_array[gpu_index];
...
#pragma omp parallel for num_threads(active_gpu_count)
    for (uint i = 0; i < active_gpu_count; i++) {
      if (i != src_gpu_idx) {
        ...
        auto dst_lut_indexes = lut_indexes_array[i];
        cuda_memcpy_async_gpu_to_gpu(dst_lut, src_lut,
                                     num_luts * lut_buffer_size, streams[i],
                                     gpu_indexes[i]);
  ...
  void release(cudaStream_t *streams, uint32_t *gpu_indexes,
               uint32_t gpu_count) {
    for (uint i = 0; i < active_gpu_count; i++) {
      cuda_drop_async(lut_indexes_array[i], streams[i], gpu_indexes[i]);
      cuda_drop_async(lut_array[i], streams[i], gpu_indexes[i]);
    }
...
template <typename Torus> struct int_bit_extract_luts_buffer {
  ...
  int_bit_extract_luts_buffer(cudaStream_t *streams, uint32_t *gpu_indexes,
    ...
    if (allocate_gpu_memory) {
      ...
      // Broadcast through all active gpus
      lut->broadcast_lut(streams, gpu_indexes, 0);
    }
  }
  ...
  int_logical_scalar_shift_buffer(cudaStream_t *streams, uint32_t *gpu_indexes,
    ...
    if (allocate_gpu_memory) {
      ...
      for (int s_w_b = 1; s_w_b < num_bits_in_block; s_w_b++) {
        ...
        // right shift
        generate_device_accumulator_bivariate<Torus>(
            streams[0], gpu_indexes[0], cur_lut_bivariate->lut_array[0],
            params.glwe_dimension, params.polynomial_size,
            params.message_modulus, params.carry_modulus, shift_lut_f);
        cur_lut_bivariate->broadcast_lut(streams, gpu_indexes, 0);
        lut_buffers_bivariate.push_back(cur_lut_bivariate);
  ...
  int_arithmetic_scalar_shift_buffer(cudaStream_t *streams,
    ...
    if (allocate_gpu_memory) {
      ...
      for (int s_w_b = 1; s_w_b < num_bits_in_block; s_w_b++) {
        ...
        generate_device_accumulator<Torus>(
            streams[0], gpu_indexes[0],
            shift_last_block_lut_univariate->lut_array[0],
            params.glwe_dimension, params.polynomial_size,
      ...
      generate_device_accumulator<Torus>(
          streams[0], gpu_indexes[0],
          padding_block_lut_univariate->lut_array[0], params.glwe_dimension,
          params.polynomial_size, params.message_modulus, params.carry_modulus,
          padding_block_lut_f);
      ...
      for (int s_w_b = 1; s_w_b < num_bits_in_block; s_w_b++) {
        ...
        generate_device_accumulator_bivariate<Torus>(
            streams[0], gpu_indexes[0],
            shift_blocks_lut_bivariate->lut_array[0], params.glwe_dimension,
...
template <typename Torus> struct int_tree_sign_reduction_buffer {
  ...
  int_tree_sign_reduction_buffer(cudaStream_t *streams, uint32_t *gpu_indexes,
    ...
    if (allocate_gpu_memory) {
      ...
      tree_last_leaf_lut->broadcast_lut(streams, gpu_indexes, 0);
      tree_last_leaf_scalar_lut->broadcast_lut(streams, gpu_indexes, 0);
    }
  }
...
template <typename Torus> struct int_div_rem_memory {
  ...
  void init_lookup_tables(cudaStream_t *streams, uint32_t *gpu_indexes,
    ...
    for (int i = 0; i < params.message_modulus - 1; i++) {
      ...
      Torus *luts[2] = {masking_luts_1[i]->get_lut(0, 0),
                        masking_luts_2[i]->get_lut(0, 0)};
      ...
      masking_luts_1[i]->broadcast_lut(streams, gpu_indexes, 0);
      masking_luts_2[i]->broadcast_lut(streams, gpu_indexes, 0);
    }
    ...
    Torus *luts[2] = {message_extract_lut_1->get_lut(0, 0),
                      message_extract_lut_2->get_lut(0, 0)};
    for (int j = 0; j < 2; j++) {
      generate_device_accumulator<Torus>(
          streams[0], gpu_indexes[0], luts[j], params.glwe_dimension,
          params.polynomial_size, params.message_modulus, params.carry_modulus,
    ...
    message_extract_lut_1->broadcast_lut(streams, gpu_indexes, 0);
    message_extract_lut_2->broadcast_lut(streams, gpu_indexes, 0);
