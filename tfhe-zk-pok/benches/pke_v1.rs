...
mod gpu {
    use super::*;
    use tfhe_zk_pok::gpu::{prove_pke_gpu, verify_pke_gpu};
    ...
    pub fn bench_pke_v1_verify_gpu(c: &mut Criterion) {
        ...
        for (params, param_name) in [
            ...
            for load in [ComputeLoad::Proof, ComputeLoad::Verify] {
                ...
                bench_group.bench_function(&bench_id, |b| {
                    b.iter(|| {
                        verify_pke_gpu(&proof, (&public_param, &public_commit), &metadata).unwrap();
                    })
                });
...
#[cfg(feature = "gpu-experimental")]
criterion_main!(benches_pke_v1_gpu);
