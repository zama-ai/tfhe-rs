...
/// Generates the vector `[0, y, y^2, y^3, ...]` from y
/// `out` should be initialized with 0
...
pub(crate) enum PkeV2HashMode {
    ...
    /// - generates only y1 as a hash and derives y = [0, y1, y1^2,...]
    /// - only hash R in phi
...
impl PkeV2HashMode {
    ...
    /// to `output` slice and a byte representation is returned
...
impl<'a> OmegaHash<'a> {
    ...
    pub(super) fn gen_delta<Zp: FieldOps>(self) -> ([Zp; 7], DeltaHash<'a>) {
        ...
        let delta_bytes = mode.gen_scalars(&mut delta, &self.delta_hash_input());
