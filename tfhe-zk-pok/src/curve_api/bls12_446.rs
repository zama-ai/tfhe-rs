...
fn fq2_to_normal_limbs(fq2: &crate::curve_446::Fq2) -> ([u64; 7], [u64; 7]) {
    ...
    let mut c0_limbs = [0u64; 7];
    let mut c1_limbs = [0u64; 7];
    for (i, limb) in c0_bigint.as_ref().iter().take(7).enumerate() {
        c0_limbs[i] = *limb;
    }
    for (i, limb) in c1_bigint.as_ref().iter().take(7).enumerate() {
        c1_limbs[i] = *limb;
    }
    (c0_limbs, c1_limbs)
}
...
mod g2 {
    ...
    impl From<&G2Affine> for zk_cuda_backend::G2Affine {
        fn from(value: &G2Affine) -> Self {
            ...
            let xy = value.inner.xy().unwrap_or((zero, zero));
            let x_c0_limbs = {
                let bigint = xy.0.c0.into_bigint();
                let mut limbs = [0u64; 7];
                for (i, limb) in bigint.as_ref().iter().take(7).enumerate() {
                    limbs[i] = *limb;
                }
            ...
            let x_c1_limbs = {
                let bigint = xy.0.c1.into_bigint();
                let mut limbs = [0u64; 7];
                for (i, limb) in bigint.as_ref().iter().take(7).enumerate() {
                    limbs[i] = *limb;
                }
            ...
            let y_c0_limbs = {
                let bigint = xy.1.c0.into_bigint();
                let mut limbs = [0u64; 7];
                for (i, limb) in bigint.as_ref().iter().take(7).enumerate() {
                    limbs[i] = *limb;
                }
            ...
            let y_c1_limbs = {
                let bigint = xy.1.c1.into_bigint();
                let mut limbs = [0u64; 7];
                for (i, limb) in bigint.as_ref().iter().take(7).enumerate() {
                    limbs[i] = *limb;
                }
