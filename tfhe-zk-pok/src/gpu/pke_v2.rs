...
//!
//! `prove_gpu` duplicates the logic of [`crate::proofs::pke_v2::prove_impl`] but
//! replaces every `multi_mul_scalar` call with the GPU-accelerated
//! [`super::g1_msm_gpu`] / [`super::g2_msm_gpu`].  `verify_gpu` duplicates
//! [`crate::proofs::pke_v2::verify_impl`] and the two pairing-check helpers
//! (`pairing_check_two_steps`, `pairing_check_batched`) with MSM sites similarly
//! replaced.
...
///
/// Identical to [`crate::proofs::pke_v2::prove`] but dispatches MSM to the GPU
/// via [`super::g1_msm_gpu`] / [`super::g2_msm_gpu`].
pub fn prove_gpu(
...
///
/// Identical to [`crate::proofs::pke_v2::verify`] but dispatches MSM to the GPU
/// via [`super::g1_msm_gpu`] / [`super::g2_msm_gpu`].
...
pub fn verify_gpu(
    ...
    run_in_pool(|| verify_gpu_impl(proof, public, metadata, pairing_mode))
}

// ---------------------------------------------------------------------------
// prove_gpu_impl – GPU variant of proofs::pke_v2::prove_impl
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// verify_gpu_impl – GPU variant of proofs::pke_v2::verify_impl
// ---------------------------------------------------------------------------
...
fn verify_gpu_impl(
    ...
    match pairing_mode {
        VerificationPairingMode::TwoSteps => pairing_check_two_steps_gpu(
            proof,
            &public.0.g_lists,
            n,
            d,
        ...
        VerificationPairingMode::Batched => pairing_check_batched_gpu(
            proof,
            &public.0.g_lists,
            n,
            d,
...
// ---------------------------------------------------------------------------
// pairing_check_two_steps_gpu
// ---------------------------------------------------------------------------

#[allow(clippy::too_many_arguments)]
fn pairing_check_two_steps_gpu(
    proof: &Proof<Bls12_446>,
...
// ---------------------------------------------------------------------------
// pairing_check_batched_gpu
// ---------------------------------------------------------------------------

#[allow(clippy::too_many_arguments)]
fn pairing_check_batched_gpu(
    proof: &Proof<Bls12_446>,
    g_lists: &GroupElements<Bls12_446>,
    n: usize,
    d: usize,
