...
//!
//! `prove_gpu` duplicates the logic of [`crate::proofs::pke::prove_impl`] but
//! replaces every `multi_mul_scalar` call with the GPU-accelerated
//! [`super::g1_msm_gpu`] / [`super::g2_msm_gpu`].  `verify_gpu` simply
//! delegates to the CPU verifier since v1 verification contains no MSM calls.
...
///
/// Identical to [`crate::proofs::pke::prove`] but dispatches MSM to the GPU via
...
pub fn verify_gpu(
    ...
    crate::proofs::pke::verify(proof, public, metadata)
}

// ---------------------------------------------------------------------------
// prove_gpu_impl – GPU variant of proofs::pke::prove_impl
// ---------------------------------------------------------------------------
...
fn prove_gpu_impl(
    ...
    let c_y = g.mul_scalar(gamma_y)
        + super::g1_msm_gpu(&g_list.0[n - big_d..n], &scalars, select_gpu_for_msm());
    ...
    if load == ComputeLoad::Proof {
        ...
        let c_hat_t = super::g2_msm_gpu(&g_hat_list.0, &t.0, select_gpu_for_msm());
        let scalars = (1..n + 1)
            .map(|i| {
                let i = n + 1 - i;
        ...
        let c_h = super::g1_msm_gpu(&g_list.0[..n], &scalars, select_gpu_for_msm());
        ...
        let pi_kzg = g.mul_scalar(q[0])
            + super::g1_msm_gpu(&g_list.0[..n - 1], &q[1..n], select_gpu_for_msm());
