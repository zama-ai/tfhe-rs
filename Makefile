...
clippy_gpu: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" clippy \
		--features=boolean,shortint,integer,internal-keycache,gpu,experimental-gpu-zk,pbs-stats,extended-types,zk-pok \
		--all-targets \
...
check_gpu: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" check \
		--features=boolean,shortint,integer,internal-keycache,gpu,experimental-gpu-zk,pbs-stats \
		--all-targets \
		-p tfhe
...
clippy_gpu_hpu: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" clippy \
		--features=boolean,shortint,integer,internal-keycache,gpu,experimental-gpu-zk,hpu,pbs-stats,extended-types,zk-pok \
		--all-targets \
		-p tfhe -- --no-deps -D warnings
...
clippy_rustdoc_gpu: install_rs_check_toolchain
	...
	CARGO_TERM_QUIET=true CLIPPYFLAGS="-D warnings" RUSTDOCFLAGS="--no-run --test-builder ./scripts/clippy_driver.sh -Z unstable-options" \
		cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" test --doc \
		--features=boolean,shortint,integer,zk-pok,pbs-stats,strings,experimental,gpu,experimental-gpu-zk \
		-p tfhe -- --nocapture
...
build_c_api_gpu: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_CHECK_TOOLCHAIN) build --profile $(CARGO_PROFILE) \
		--features=boolean-c-api,shortint-c-api,high-level-c-api,zk-pok,extended-types,gpu,experimental-gpu-zk \
		-p tfhe
...
test_zk_pok_gpu:
	RUSTFLAGS="$(RUSTFLAGS)" cargo test --profile $(CARGO_PROFILE) \
		-p tfhe-zk-pok --features experimental,experimental-gpu -- gpu
...
test_integer_zk_gpu: install_rs_check_toolchain
	RUSTFLAGS="$(RUSTFLAGS)" cargo $(CARGO_RS_BUILD_TOOLCHAIN) test --profile release_lto_off \
		--features=integer,zk-pok,gpu,experimental-gpu-zk -p tfhe -- \
		integer::gpu::zk::
...
bench_msm_zk_gpu: install_rs_check_toolchain
	...
	cargo $(CARGO_RS_CHECK_TOOLCHAIN) bench \
	--bench zk-msm \
	--features=gpu,experimental-gpu-zk,zk-pok -p tfhe-benchmark --profile release_lto_off --
...
bench_integer_zk_gpu: install_rs_check_toolchain
	...
	--features=integer,internal-keycache,gpu,experimental-gpu-zk,pbs-stats,zk-pok -p tfhe-benchmark --profile release_lto_off --
...
bench_tfhe_zk_pok_gpu: install_rs_check_toolchain
	...
	cargo $(CARGO_RS_CHECK_TOOLCHAIN) bench \
	--package tfhe-zk-pok \
	--features=experimental-gpu --profile release_lto_off
