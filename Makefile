...
clippy_backward_compat_data: install_rs_check_toolchain # the toolchain is selected with toolchain.toml
	RUSTFLAGS="$(RUSTFLAGS)" cargo "$(CARGO_RS_CHECK_TOOLCHAIN)" -Z unstable-options \
		-C utils/tfhe-backward-compat-data clippy --all-targets \
		-- --no-deps -D warnings
...
test_backward_compatibility_ci: install_rs_build_toolchain clone_backward_compat_data
...
test_backward_compatibility: test_backward_compatibility_ci
...
clone_backward_compat_data:
	./scripts/clone_backward_compat_data.sh $(BACKWARD_COMPAT_DATA_DIR)
