...
check_typos: install_typos_checker
	@git ls-files ":!*.png" ":!*.cbor" ":!*.bcode" ":!*.ico" ":!*/twiddles.cu" ":!*/fft16x4x16_twiddles.cuh" ":!*.hpu" | typos --file-list - && echo "No typos found"
...
.PHONY: bench_hlapi_erc7984_multi_group_gpu # Runs ERC7984 bench in several processes (an equal share of gpus for each) and aggregates results
bench_hlapi_erc7984_multi_group_gpu: install_rs_check_toolchain
	...
	NUM_GROUPS=$${__TFHE_RS_BENCH_MULTI_PROC_GROUPS:-8}; \
	...
	for i in $$(seq 0 $$((NUM_GROUPS - 1))); do \
		...
		echo "Starting benchmark group $$i with CUDA_VISIBLE_DEVICES=$$GPU_LIST on cores $$CORE_LIST"; \
		(cd tfhe-benchmark && CUDA_VISIBLE_DEVICES=$$GPU_LIST CUDA_DEVICE_MAX_CONNECTIONS=32 CARGO_TARGET_DIR=target_p$$i RAYON_NUM_THREADS=$$CORES_PER_GROUP __TFHE_RS_BENCH_TYPE=$(BENCH_TYPE) __TFHE_RS_PARAM_TYPE=$(BENCH_PARAM_TYPE) __TFHE_RS_BENCH_GPU_PROCESS_COUNT=$$NUM_GROUPS \
