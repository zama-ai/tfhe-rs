...
.PHONY: test_zk_wasm_x86_compat_client_ci
test_zk_wasm_x86_compat_client_ci: check_nvm_installed
	source ~/.nvm/nvm.sh && \
	nvm install $(NODE_VERSION) && \
	nvm use $(NODE_VERSION) && \
	$(MAKE) test_zk_wasm_x86_compat_client

.PHONY: test_zk_wasm_x86_compat_client # Check compatibility between wasm client pkg and x86_64 proofs
test_zk_wasm_x86_compat_client: build_node_js_api_client
	cd tfhe/tests/zk_wasm_x86_test && npm install
	RUSTFLAGS="$(RUSTFLAGS)" cargo test --profile $(CARGO_PROFILE) \
		-p tfhe --test zk_wasm_x86_test --features=integer,zk-pok
