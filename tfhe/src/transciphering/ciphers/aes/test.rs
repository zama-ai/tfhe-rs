...
use crate::transciphering::ciphers::aes::{
    AesFheState, AesPlainKey, AesPlainStream, ExpandedAesFheKey,
};
...
fn gen_random_key_iv() -> (u64, u128, u128) {
...
fn fhe_sbox_exhaustive() {
    ...
    let mismatches: Vec<(u8, u8, u8)> = (0usize..256)
        .into_par_iter()
        .filter_map(|b| {
            let b = b as u8;
            ...
            let expected = AES_SBOX[b as usize];
...
fn aes_fhe_known_answer() {
    ...
    let fhe_key = ExpandedAesFheKey::new(&sks, &enc_key);
...
fn aes_fhe_matches_plain_random() {
    let (cks, sks) = gen_keys(PARAM);
    let (seed, key, iv) = gen_random_key_iv();
...
fn aes_transcipher_round_trip() {
    let (cks, sks) = gen_keys(PARAM);
    let (_, key, iv) = gen_random_key_iv();
