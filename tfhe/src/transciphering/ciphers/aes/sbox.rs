...
fn top_sbox(sks: &ServerKey, flush_lut: &LookupTable<Vec<u64>>, byte: SboxByte) -> SboxTop {
    ...
    let t9 = flush_xor(sks, flush_lut, &t7, u7);

    let [t19, t13, t20, t22] = flush_xor_batch(
        sks,
        flush_lut,
        [(&t9, u3), (&t3, &t4), (&t9, u0), (&t9, u6)],
    );

    let t23 = flush_xor(sks, flush_lut, &t22, &t2);

    let t6_pre = sks.unchecked_add(u4, &t13);
    let (t6, t14) = join(
        || flush_xor(sks, flush_lut, &t6_pre, u5),
        || flush_xor(sks, flush_lut, &t6_pre, u1),
    );

    let (t8, t10) = join(
        || flush_xor(sks, flush_lut, &t6, u7),
        || flush_xor(sks, flush_lut, &t6, &t7),
    );

    let t15 = flush_xor(sks, flush_lut, &t14, &t1);

    let [t17, t27, t24, t16] = flush_xor_batch(
        sks,
        flush_lut,
        [(u7, &t15), (&t10, &t15), (&t10, &t2), (&t7, &t15)],
    );
