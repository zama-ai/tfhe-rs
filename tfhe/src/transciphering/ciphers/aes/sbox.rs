...
fn top_sbox(sks: &ServerKey, flush_lut: &LookupTable<Vec<u64>>, byte: SboxByte) -> SboxTop {
    ...
    let [t4, t3, t1, t2] =
        flush_xor_batch(sks, flush_lut, [(u3, u5), (u0, u6), (u0, u3), (u0, u5)]);
    ...
    let t9 = flush_xor(sks, flush_lut, &t7, u7); // T9 = U7 + T7

    let [t19, t13, t20, t22] = flush_xor_batch(
        sks,
        flush_lut,
        [
            (&t9, u3),  // T19 = T7 + T18 = T9 + U3
            (&t3, &t4), // T13 = T3 + T4
            (&t9, u0),  // T20 = T1 + T19 = T9 + U0
            (&t9, u6),  // T22 = T7 + T21 = T9 + U6
        ],
    );
    ...
    let (t6, t14) = join(
        || flush_xor(sks, flush_lut, &t6_pre, u5), // T6  = T1 + T5
        || flush_xor(sks, flush_lut, &t6_pre, u1), // T14 = T6 + T11
    );

    let (t8, t10) = join(
        || flush_xor(sks, flush_lut, &t6, u7),  // T8  = U7 + T6
        || flush_xor(sks, flush_lut, &t6, &t7), // T10 = T6 + T7
    );

    let t15 = flush_xor(sks, flush_lut, &t14, &t1); // T15 = T5 + T11

    let [t17, t27, t24, t16] = flush_xor_batch(
        sks,
        flush_lut,
        [
            (u7, &t15),   // T17 = T9 + T16
    ...
    let (t26, t25) = join(
        ...
        || flush_xor(sks, flush_lut, u0, &t16),  // T25 = T20 + T17
    );
...
fn middle_sbox(
    ...
    let ((m27, m25), m24) = join(
        || {
            join(
                ...
                || sks.unchecked_bitand(&m22, &m20),      // M25 = M22 x M20
            )
        },
    ...
    let (m29, m30) = join(
        || sks.unchecked_bitand(&m28, &m27), // M29 = M28 x M27
        || sks.unchecked_bitand(&m26, &m24), // M30 = M26 x M24
    );
