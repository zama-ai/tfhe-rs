...
pub(super) fn encrypt_block(sks: &ServerKey, state: &mut [Ciphertext; 128], key: &AesFheRoundKeys) {
    debug_assert!(
        state.iter().all(|c| c.noise_level() == NoiseLevel::ZERO),
        "encrypt_block expects a freshly built trivial state (noise 0) on entry"
    );

    let round_keys = key.round_keys();
    let flush_lut = key.flush_lut();

    xor_assign(sks, state, &round_keys[0].key);
    for erk in &round_keys[1..10] {
        sub_bytes(sks, state, flush_lut);
        flush_state(sks, state, flush_lut);
        shift_rows(state);
        mix_columns(sks, flush_lut, state);
        flush_state(sks, state, flush_lut);
        xor_assign(sks, state, &erk.key);
        flush_state(sks, state, flush_lut);
    }
    sub_bytes(sks, state, flush_lut);
    shift_rows(state);
    xor_assign(sks, state, &round_keys[10].key);
    flush_state(sks, state, flush_lut);
}
