...
/// AES-128 IV / initial CTR counter, held as a 128-bit integer.
///
/// Unlike [`AesPlainKey`] (a byte container fed to the block cipher), an IV is
/// genuinely a number: CTR mode increments it as a big-endian integer
/// (`iv + block_index`). `AesIv` is therefore a plain `u128` newtype; the
/// big-endian (NIST) byte convention is applied only at the `[u8; 16]` /
/// `[bool; 128]` construction boundaries.
#[derive(Clone, Copy)]
