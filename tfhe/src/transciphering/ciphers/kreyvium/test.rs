...
fn kreyvium_test_plain() {
    ...
    for KreyviumTestVector { key, iv, expected } in cases {
        ...
        let mut kreyvium = KreyviumPlainStream::new(key_bytes.into(), iv_bytes);
...
fn kreyvium_fhe_keystream_known_answer(params: ClassicPBSParameters) {
    ...
    let cipher_key: KreyviumFheKey = key_bits.map(|x| client_key.encrypt(x)).into();
...
fn kreyvium_test_round_trip() {
    ...
    for iter in 0..N_ITER {
        ...
        let cipher_key: KreyviumFheKey = key_bits.map(|b| client_key.encrypt(b as u64)).into();
