...
fn kreyvium_fhe_keystream_known_answer(params: ClassicPBSParameters) {
    ...
    // Each keystream Ciphertext encodes the bit in its low bit only
...
fn kreyvium_seek_fhe() {
    ...
    let decrypt_bits = |fhe: &FheKeyStream| -> Vec<u8> {
    ...
    assert_eq!(decrypt_bits(&mid), ref_keystream[8..16]);
    ...
    assert_eq!(k.current_counter(), 128);
    assert_eq!(decrypt_bits(&mid_again), ref_keystream[8..16]);
}
