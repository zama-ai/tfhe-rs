...
use super::{
    KreyviumFheKey, KreyviumRound, KreyviumRoundInput, KreyviumRoundOutput, KreyviumState,
    KreyviumStream,
};
...
impl KreyviumPlainKey {
    ...
    pub(super) fn expand(self) -> [bool; 128] {
        unpack_bits_lsb_first(&self.bits)
    }

    /// Encrypt this key bit-by-bit under `client_key`, producing the FHE-side
    /// key consumed by [`KreyviumFheStream`](super::KreyviumFheStream).
...
fn unpack_bits_lsb_first(bytes: &[u8; 16]) -> [bool; 128] {
...
impl KreyviumPlainStream {
    ...
    pub fn new(key: impl Into<KreyviumPlainKey>, iv: impl Into<KreyviumIV>) -> Self {
        let mut state = KreyviumPlainState::new(key.into(), iv.into());
        state.warmup(&());
...
impl KreyviumPlainState {
    pub fn new(key: KreyviumPlainKey, iv: KreyviumIV) -> Self {
        let mut key = key.expand();
        let mut iv = iv.expand();
