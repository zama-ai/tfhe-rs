...
fn worker(
    ...
    // Invariant relied upon by the coordinator:
    // Workers must return errors and not panic to allow the coordinator to the
    // gracefully end the execution when a worker notified it of an error.
    //
    // So we put the execution in a catch unwind to respect the invariant.
    // Note that by default allocation errors are aborts, not panics
    loop {
        ...
        // catch all panics whether intentional or not so that the
        ...
        .map_err(|payload| CpuError::ExecutionError {
            node_index: id.0,
            op: op_name,
            // Formatting the payload is a second, separate `catch_unwind`
            // so that a misbehaving payload cannot take the worker down.
            message: std::panic::catch_unwind(AssertUnwindSafe(|| {
                panic_payload_to_string(payload)
            }))
            .unwrap_or_else(|_| "panic while formatting the panic payload".to_string()),
        });
...
pub(crate) fn execute_graph(
    ...
    std::thread::scope(|s| {
        ...
        'main: loop {
            ...
            // `recv` fails only once every worker has dropped its sender,
            // i.e. all workers exited while outputs are still missing.
            //
            // Workers either exit after the coordinator dropped
            // (so we already quit the loop).
            //
            // Or if every worker exited via panic outside the catch_unwind
            // they have, which given that all their code is in a catch unwind
            // seems extremely unlikely, so if the .expect is triggered here
            // some dark magic made things go extremely wrong.
            let DoneOp { id, result } = work_done_receiver
                .recv()
                .expect("all workers exited before the graph finished executing");
