...
fn test_signed_div_compare_multiwidth_gpu() {
    ...
    let num_threads: usize = std::env::var("TFHE_TEST_THREADS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(4);

    let setup_fns: [fn() -> crate::ClientKey; 2] = [setup_classical_gpu, setup_multibit_gpu];
        ...
        let handles: Vec<_> = (0..num_threads)
            .map(|_| {
                std::thread::spawn(move || {
                    let client_key = setup_fn();
                    super::test_case_div_compare_multiwidth(&client_key);
                })
            })
            .collect();
        for h in handles {
