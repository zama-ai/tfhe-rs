...
fn test_compact_public_key_gpu() {
    ...
    let b: FheInt8 = expanded.get(1).unwrap().unwrap();

    let clear_a: i8 = a.decrypt(&client_key);
    assert_eq!(clear_a, -1i8);
    let clear_b: i8 = b.decrypt(&client_key);
    assert_eq!(clear_b, -123i8);
}
