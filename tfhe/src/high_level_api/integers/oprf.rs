...
mod test {
    ...
    const EXPECTED_NUM_INPUT_RANDOM_BITS: (u64, u64) = (2, 4);
    const MAX_DISTANCE: (f64, f64) = (0.7, 0.1);
    const EXCLUDED_UPPER_BOUNDS: [u64; 4] = [3, 5, 6, 7];
    ...
    fn test_uniformity_generate_oblivious_pseudo_random_custom_range_cpu() {
        ...
        for (expected_num_input_random_bits, max_distance, excluded_upper_bounds) in [
            (
                EXPECTED_NUM_INPUT_RANDOM_BITS.0,
                MAX_DISTANCE.0,
                EXCLUDED_UPPER_BOUNDS,
            ),
            (
                EXPECTED_NUM_INPUT_RANDOM_BITS.1,
                MAX_DISTANCE.1,
                EXCLUDED_UPPER_BOUNDS,
            ),
        ] {
    ...
    mod gpu {
        ...
        use crate::{CompressedServerKey, FheInt128, FheUint32, FheUint64, GpuIndex};
