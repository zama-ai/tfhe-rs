...
impl<Id: FheUintId> FheUint<Id> {
    ...
    ///
    /// If the range is a power of 2, the distribution will be uniform (for any `max_distance`) and
    /// the cost smaller.
    ///
    /// ```rust
    /// use tfhe::prelude::FheDecrypt;
...
impl RangeForRandom {
    pub fn new_from_excluded_upper_bound(excluded_upper_bound: u64) -> Self {
        Self {
            excluded_upper_bound,
        }
    }
...
mod test {
    ...
    fn test_uniformity_scalar_mul_shift() {
        ...
        // the distribution is indistinguishable from the uniform with athe given sample count
    ...
    fn test_uniformity_generate_oblivious_pseudo_random_custom_range() {
        let base_sample_count: usize = 1_000;
        ...
        for max_distance in [0.7, 0.1] {
            for excluded_upper_bound in [3, 5, 6, 7, 9] {
                let num_input_random_bits = num_input_random_bits_for_max_distance(
                    excluded_upper_bound,
                    max_distance,
                    message_modulus,
                ...
                if max_distance > 0.5 {
                    assert_eq!(num_input_random_bits, 2);
                } else {
                    assert_eq!(num_input_random_bits, 4);
                }
