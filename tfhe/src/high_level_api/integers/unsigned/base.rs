    ...
    pub fn trailing_zeros(&self) -> super::FheUint32 {
        global_state::with_internal_keys(|key| match key {
            ...
            InternalServerKey::Hpu(device) => {
                ...
                let (opcode, proto) = {
                    let asm_iop = &hpu_asm::IOP_TRAIL0;
                    (
                        asm_iop.opcode(),
                        &asm_iop.format().expect("Unspecified IOP format").proto,
                    )
    ...
    pub fn trailing_ones(&self) -> super::FheUint32 {
        global_state::with_internal_keys(|key| match key {
            ...
            InternalServerKey::Hpu(device) => {
                ...
                let (opcode, proto) = {
                    let asm_iop = &hpu_asm::IOP_TRAIL1;
                    (
                        asm_iop.opcode(),
                        &asm_iop.format().expect("Unspecified IOP format").proto,
                    )
    ...
    pub fn count_ones(&self) -> super::FheUint32 {
        global_state::with_internal_keys(|key| match key {
            ...
            InternalServerKey::Hpu(device) => {
                ...
                let (opcode, proto) = {
                    let asm_iop = &hpu_asm::IOP_COUNT0;
                    (
                        asm_iop.opcode(),
                        &asm_iop.format().expect("Unspecified IOP format").proto,
                    )
    ...
    pub fn count_zeros(&self) -> super::FheUint32 {
        global_state::with_internal_keys(|key| match key {
            ...
            InternalServerKey::Hpu(device) => {
                ...
                let (opcode, proto) = {
                    let asm_iop = &hpu_asm::IOP_COUNT1;
                    (
                        asm_iop.opcode(),
                        &asm_iop.format().expect("Unspecified IOP format").proto,
                    )
        ...
        global_state::with_internal_keys(|key| {
            match key {
                ...
                InternalServerKey::Cuda(cuda_key) => {
                    ...
                    self.ciphertext
                        .as_gpu_mut(streams)
                        .re_randomize(rerand_key, seed, streams)?;
                }
                #[cfg(feature = "hpu")]
                InternalServerKey::Hpu(_device) => {
