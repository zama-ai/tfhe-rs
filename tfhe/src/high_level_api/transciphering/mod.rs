        ...
        T: HlExpandable + Tagged,
    {
        try_with_internal_keys(|keys| match keys {
            Some(InternalServerKey::Cpu(cpu_key)) => {
                cpu_transcipher(self, input.integer(), cpu_key, None)
            }
            #[cfg(feature = "gpu")]
...
impl TranscipherSession {
    fn new_cpu(inner: crate::transciphering::TranscipherSession, tag: Tag) -> Self {
        Self {
...
impl Tagged for TranscipherSession {
    fn tag(&self) -> &Tag {
        &self.tag
    }
...
impl From<crate::transciphering::TranscipherSession> for TranscipherSession {
    fn from(inner: crate::transciphering::TranscipherSession) -> Self {
        Self::new_cpu(inner, Tag::default())
    }
}
...
impl HlTranscipherer for TranscipherSession {
        ...
        T: HlExpandable + Tagged,
    {
        ...
        // A session built straight from a lower-layer session carries no tag of
        // its own, so fall back to the server key's, like every other HL op.
        let tag = (!tag.is_empty()).then_some(&*tag);
        try_with_internal_keys(|keys| match (inner, keys) {
            (InnerTranscipherSession::Cpu(inner), Some(InternalServerKey::Cpu(cpu_key))) => {
                cpu_transcipher(inner, input.integer(), cpu_key, tag)
            }
            #[cfg(feature = "gpu")]
            (InnerTranscipherSession::Gpu(inner), Some(InternalServerKey::Cuda(cuda_key))) => {
                gpu_transcipher::<T>(inner, input.integer(), cuda_key, tag)
            }
...
/// `tag` is the session's own tag; without one the server key's is used.
    ...
    T: Expandable + Tagged,
{
    ...
    out.tag_mut().set_data(tag.unwrap_or(&cpu_key.tag).data());
    Ok(out)
}
    ...
    T: CudaExpandable + Tagged,
{
    ...
    let mut out = T::from_expanded_blocks(blocks, kind)?;
    out.tag_mut().set_data(tag.unwrap_or(&cuda_key.tag).data());
    Ok(out)
}
...
mod test {
    ...
    fn test_using_otp() {
        ...
        let fhe_kreyv_key = KreyviumFheKey::new_random(seed).unwrap();
        let mut rng = rand::thread_rng();
        let iv_bits: [bool; 128] = std::array::from_fn(|_| rng.gen());
    ...
    /// The pad picks up the tag of the key set it was generated under, keeps it
    /// across the wire, and stamps it on what it transciphers.
    #[test]
    fn test_tag_propagation() {
        ...
        const OTHER_KEY_SET: u64 = 0x00BA_DBED;
        ...
        // Server: the OPRF-generated pad inherits the server key's tag.
        let fhe_mask = OneTimePadFheSecretMask::new_random(Seed(0), 64).unwrap();
        assert_eq!(fhe_mask.tag().as_u64(), KEY_SET);

        // The tag survives both wire formats.
        let serialized = bincode::serialize(&fhe_mask).unwrap();
        let fhe_mask: OneTimePadFheSecretMask = bincode::deserialize(&serialized).unwrap();
        assert_eq!(fhe_mask.tag().as_u64(), KEY_SET);
        ...
        // Retag the pad so it no longer matches the server key: the output must
        // follow the pad, since that is what decides which client key decrypts.
        fhe_mask.tag_mut().set_u64(OTHER_KEY_SET);

        let mut sym = OneTimePadPlainState::new(fhe_mask.decrypt(&client_key));
        let sym_cipher = sym.try_encrypt(42u64).unwrap();

        let mut fhe_stream = TranscipherSession::one_time_pad(fhe_mask).unwrap();
        assert_eq!(fhe_stream.tag().as_u64(), OTHER_KEY_SET);

        let transciphered: FheUint64 = fhe_stream.transcipher(&sym_cipher).unwrap();
        assert_eq!(transciphered.tag().as_u64(), OTHER_KEY_SET);
