        ...
        match self {
            ...
            Self::Cuda(inner) => {
                let cpu_data = with_cuda_internal_keys(|keys| {
                    ...
                    inner.to_compressed_squashed_noise_ciphertext_list(streams)
                });
...
impl InnerCompressedSquashedNoiseCiphertextList {
    ...
    fn move_to_device(&mut self, target_device: crate::Device) -> Result<(), crate::Error> {
        let new_value = match (&self, target_device) {
            ...
            (Self::Cpu(cpu_ct), crate::Device::CudaGpu) => {
                let cuda_ct = with_cuda_internal_keys(|keys| {
                    let streams = &keys.streams;
                    CudaCompressedSquashedNoiseCiphertextList::from_compressed_squashed_noise_ciphertext_list(
                        cpu_ct, streams,
                    )
                });
            ...
            (Self::Cpu(_), crate::Device::Hpu) => {
                return Err(crate::error!(
                    "HPU device does not support noise squashed ciphertext compression"
                ));
            }
