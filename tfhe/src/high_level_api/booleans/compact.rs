...
pub struct CompactFheBoolConformanceParams {
    shortint_params: CiphertextConformanceParams,
}

impl CompactFheBoolConformanceParams {
    fn new(mut shortint_params: CiphertextConformanceParams) -> Self {
        shortint_params.degree = Degree::new(1);
        Self { shortint_params }
    }
}

impl<P> From<P> for CompactFheBoolConformanceParams
where
    P: Into<PBSParameters>,
{
    fn from(params: P) -> Self {
        Self::new(params.into().to_shortint_conformance_param())
    }
}

impl From<&ServerKey> for CompactFheBoolConformanceParams {
    fn from(sk: &ServerKey) -> Self {
        Self::new(sk.key.pbs_key().key.conformance_params())
    }
}

impl ParameterSetConformant for CompactFheBool {
    type ParameterSet = CompactFheBoolConformanceParams;

    fn is_conformant(&self, params: &CompactFheBoolConformanceParams) -> bool {
