...
use crate::high_level_api::traits::{FheEq, Flip, IfThenElse, ScalarIfThenElse, Tagged};
    ...
    fn flip(&self, lhs: &FheInt<Id>, rhs: &FheInt<Id>) -> (Self::Output, Self::Output) {
        ...
        global_state::with_internal_keys(|sks| match sks {
            InternalServerKey::Cpu(cpu_sks) => {
                ...
                (
                    FheInt::new(a, cpu_sks.tag.clone()),
                    FheInt::new(b, cpu_sks.tag.clone()),
                )
            }
            #[cfg(feature = "gpu")]
            InternalServerKey::Cuda(_) => rayon::join(
                || self.if_then_else(rhs, lhs),
                || self.if_then_else(lhs, rhs),
            ),
            #[cfg(feature = "hpu")]
            InternalServerKey::Hpu(_device) => {
    ...
    fn flip(&self, lhs: &FheUint<Id>, rhs: &FheUint<Id>) -> (Self::Output, Self::Output) {
        ...
        global_state::with_internal_keys(|sks| match sks {
            InternalServerKey::Cpu(cpu_sks) => {
                ...
                (
                    FheUint::new(a, cpu_sks.tag.clone()),
                    FheUint::new(b, cpu_sks.tag.clone()),
                )
            }
            #[cfg(feature = "gpu")]
            InternalServerKey::Cuda(_) => rayon::join(
