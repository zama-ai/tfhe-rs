...
mod cpu {
    ...
    use crate::{
        CompactFheBoolConformanceParams, CompactFheBoolListConformanceParams,
        CompressedFheBoolConformanceParams, FheBoolConformanceParams,
    };
    use rand::random;
    ...
    fn test_safe_deserialize_conformant_compact_fhe_bool() {
        ...
        let params = CompactFheBoolConformanceParams::from(&server_key);
        let deserialized_a =
            safe_deserialize_conformant::<CompactFheBool>(serialized.as_slice(), 1 << 20, &params)
                .unwrap();

        assert!(deserialized_a.is_conformant(&CompactFheBoolConformanceParams::from(block_params)));
