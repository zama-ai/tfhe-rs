...
mod strings {
    use super::*;
    use crate::high_level_api::strings::ascii::{AsciiDevice, FheAsciiString};
    use crate::integer::ciphertext::Compressible;

    impl HlCompressible for FheAsciiString {
        fn compress_into(self, messages: &mut Vec<(ToBeCompressed, DataKind)>) {
            match self.inner {
...
impl CompressedCiphertextListBuilder {
        ...
        T: HlCompressible,
    {
        ...
        // `compress_into` should only push elements at the end of the builder
        assert!(size_before <= size_after);
        for _ in size_before..size_after {
            self.rerandomization_metadata.push(meta.clone())
        }
        self
    }
