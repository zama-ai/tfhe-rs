...
pub mod zk {
    ...
    pub struct ProvenCompactCiphertextList {
        pub(in crate::high_level_api) inner: InnerProvenCompactCiphertextList,
    ...
    impl serde::Serialize for InnerProvenCompactCiphertextList {
            ...
            S: Serializer,
        {
            match self {
                ...
                #[cfg(feature = "gpu")]
                Self::Cuda(inner) => inner.h_proved_lists.serialize(serializer),
            }
        }
    }
      ...
      <crate::integer::ciphertext::ProvenCompactCiphertextList as VersionizeOwned>::VersionedOwned;
        fn versionize(&self) -> Self::Versioned<'_> {
            match self {
                ...
                #[cfg(feature = "gpu")]
                Self::Cuda(inner) => inner.h_proved_lists.clone().versionize_owned(),
            }
        }
    }
      ...
      <crate::integer::ciphertext::ProvenCompactCiphertextList as VersionizeOwned>::VersionedOwned;
        fn versionize_owned(self) -> Self::VersionedOwned {
            match self {
                ...
                #[cfg(feature = "gpu")]
                Self::Cuda(inner) => inner.h_proved_lists.versionize_owned(),
            }
        }
    }
    ...
    impl ProvenCompactCiphertextList {
        ...
        pub fn len(&self) -> usize {
            match &self.inner {
                InnerProvenCompactCiphertextList::Cpu(inner) => inner.len(),
                #[cfg(feature = "gpu")]
                InnerProvenCompactCiphertextList::Cuda(inner) => inner.h_proved_lists.len(),
            }
        }
        ...
        pub fn get_kind_of(&self, index: usize) -> Option<crate::FheTypes> {
            match &self.inner {
                ...
                InnerProvenCompactCiphertextList::Cuda(inner) => inner
                    ...
                    .get_kind_of(index)
                    .and_then(|data_kind| {
                        crate::FheTypes::from_data_kind(
                            data_kind,
                            inner.h_proved_lists.message_modulus(),
                        )
                    }),
            }
        }
        ...
        pub fn verify(
            ...
            match &self.inner {
                InnerProvenCompactCiphertextList::Cpu(inner) => {
                    inner.verify(crs, &pk.key.key, metadata)
                }
                #[cfg(feature = "gpu")]
                InnerProvenCompactCiphertextList::Cuda(inner) => {
                    inner.h_proved_lists.verify(crs, &pk.key.key, metadata)
                }
            }
        }
            ...
            global_state::try_with_internal_keys(|maybe_keys| match maybe_keys {
                ...
                Some(InternalServerKey::Cpu(cpu_key)) => match &self.inner {
                    ...
                    InnerProvenCompactCiphertextList::Cuda(_) => {
                        Err(crate::Error::new("Expected a Cuda server key".to_string()))
                    }
                },
                #[cfg(feature = "gpu")]
                Some(InternalServerKey::Cuda(gpu_key)) => match &self.inner {
                    ...
                    InnerProvenCompactCiphertextList::Cpu(_) => {
                        Err(crate::Error::new("Expected a CPU server key".to_string()))
                    }
                },
            })
        }
    ...
    impl ParameterSetConformant for ProvenCompactCiphertextList {
        ...
        fn is_conformant(&self, parameter_set: &Self::ParameterSet) -> bool {
            ...
            match inner {
                InnerProvenCompactCiphertextList::Cpu(inner) => inner.is_conformant(parameter_set),
                #[cfg(feature = "gpu")]
                InnerProvenCompactCiphertextList::Cuda(inner) => {
                    inner.h_proved_lists.is_conformant(parameter_set)
                }
            }
        }
    }
...
impl CompactCiphertextListBuilder {
    ...
    pub fn build_with_proof_packed(
        ...
        crate::high_level_api::global_state::try_with_internal_keys(|keys| {
            match keys {
            Some(InternalServerKey::Cpu(_)) => self
                .inner
                .build_with_proof_packed(crs, metadata, compute_load)
                .map(|proved_list| ProvenCompactCiphertextList {
                    inner: crate::high_level_api::compact_list::zk::InnerProvenCompactCiphertextList::Cpu(
                        proved_list,
                    ),
                    tag: self.tag.clone(),
                }),
            ...
            Some(InternalServerKey::Cuda(_)) => {
                ...
                let gpu_proven_ct = with_thread_local_cuda_streams(|streams| {
                    if let crate::high_level_api::compact_list::zk::InnerProvenCompactCiphertextList::Cpu(
                        inner,
                    ) = cpu_proven_ct.inner
                    {
                        Ok(
                        ...
                        // Handle other cases or panic if this should never happen
                        Err("Expected CPU variant".to_string())
                    }
                });

                Ok(ProvenCompactCiphertextList {
                    inner: crate::high_level_api::compact_list::zk::InnerProvenCompactCiphertextList::Cuda(
                        gpu_proven_ct.unwrap(),
                    ),
                    tag: self.tag.clone(),
                })
            }
