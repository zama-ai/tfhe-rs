...
impl CompactCiphertextList {
    ...
    pub fn expand_with_key(
        &self,
        sks: &InternalServerKey,
    ) -> crate::Result<CompactCiphertextListExpander> {
        match (&self.inner, sks) {
            (InnerCompactCiphertextList::Cpu(cpu_inner), InternalServerKey::Cpu(cpu_key)) => {
            ...
            #[cfg(feature = "gpu")]
            (InnerCompactCiphertextList::Cpu(cpu_inner), InternalServerKey::Cuda(cuda_key)) => {
            ...
            #[cfg(feature = "gpu")]
            (InnerCompactCiphertextList::Cuda(gpu_inner), InternalServerKey::Cpu(cpu_key)) => {
                // CUDA data, CPU key case
                // We copy data to CPU and then expand it
                let cpu_inner = with_thread_local_cuda_streams(|streams| {
            ...
            #[cfg(feature = "gpu")]
            (InnerCompactCiphertextList::Cuda(gpu_inner), InternalServerKey::Cuda(cuda_key)) => {
    ...
    pub fn expand(&self) -> crate::Result<CompactCiphertextListExpander> {
        ...
        global_state::try_with_internal_keys(|maybe_keys| match maybe_keys {
            ...
            Some(internal_key) => self.expand_with_key(internal_key),
        })
    }
}
                ...
                match maybe_keys {
                    ...
                    Some(InternalServerKey::Cpu(cpu_key)) => match &self.inner {
                        ...
                        InnerProvenCompactCiphertextList::Cuda(_) => {
                            Err(crate::Error::new("Tried expanding a ProvenCompactCiphertextList on Cuda device, but the set ServerKey is a CPU ServerKey".to_string()))
                        }
                    },
                    #[cfg(feature = "gpu")]
                    Some(InternalServerKey::Cuda(gpu_key)) => match &self.inner {
                        ...
                        InnerProvenCompactCiphertextList::Cpu(_) => {
                            Err(crate::Error::new("Tried expanding a ProvenCompactCiphertextList on the CPU, but the set ServerKey is a CudaServerKey".to_string()))
                        }
                    },
                }
            })
            ...
            match keys {
                ...
                Some(InternalServerKey::Cuda(_)) => {
                    ...
                    let gpu_proven_ct = with_thread_local_cuda_streams(|streams| {
                                CudaProvenCompactCiphertextList::from_proven_compact_ciphertext_list(
                                    &cpu_proven_ct.inner.on_cpu(), streams,
                            )
                    });
...
mod tests {
    ...
    fn test_compact_list_with_casting() {
        ...
        let expander = compact_list
            .expand_with_key(&InternalServerKey::Cpu(sk))
            .unwrap();
