...
use crate::prelude::{CastFrom, CastInto, Tagged};
pub use cpu::{
...
fn fhe_bool_dot_product<F, FheType, Clear>(
    bools: &[FheBool],
    clears: &[Clear],
    num_bits: u32,
    create_fhe_value: F,
...
where
    F: FnOnce(Vec<crate::shortint::Ciphertext>, Tag) -> FheType,
    FheType: Tagged,
    Clear: Numeric
        ...
        + AddAssign<Clear>,
{
    global_state::with_internal_keys(|keys| match keys {
        InternalServerKey::Cpu(cpu_key) => {
            ...
            create_fhe_value(radix.blocks, cpu_key.tag.clone())
        }
        #[cfg(feature = "gpu")]
    ...
    Id: FheUintId,
{
    ...
    fn dot_product(bools: &[FheBool], clears: &[Clear]) -> Self {
        fhe_bool_dot_product(bools, clears, Id::num_bits() as u32, |blocks, tag| {
