...
use crate::high_level_api::array::{FheBackendArray, FheBackendArraySlice, IOwnedArray};
...
where
    ...
    for<'a> FheBackendArraySlice<'a, Backend, Id>:
        Copy + BitAnd<FheBackendArraySlice<'a, Backend, Id>, Output = FheBackendArray<Backend, Id>>,
...
where
    ...
    for<'a, 'b> Array::Slice<'a>: Copy + BitOr<Array::Slice<'a>, Output = Array>,
    // for the 2 tested slice/array ops
    for<'a, 'b> Array::Slice<'a>: Copy + BitOr<&'b Array, Output = Array>,
    for<'a, 'b> &'b Array: Copy + BitOr<Array::Slice<'a>, Output = Array>,
{
    let num_values = 5;
    let clear_lhs = draw_random_values::<Clear>(num_values);
    let clear_rhs = draw_random_values::<Clear>(num_values);
...
where
    Standard: Distribution<Clear>,
    Clear: BitXor<Clear, Output = Clear> + Copy + Eq + Debug,
    Array: IOwnedArray
        + for<'a> FheTryEncrypt<&'a [Clear], ClientKey>
        + FheDecrypt<Vec<Clear>>
    ...
    // Bounds for slicing tests
    for<'a, 'b> Array::Slice<'a>: Copy + BitXor<Array::Slice<'a>, Output = Array>,
    // for the 2 tested slice/array ops
    for<'a, 'b> Array::Slice<'a>: Copy + BitXor<&'b Array, Output = Array>,
{
    let num_values = 5;
    let clear_lhs = draw_random_values::<Clear>(num_values);
    let clear_rhs = draw_random_values::<Clear>(num_values);
...
where
    ...
    Array: IOwnedArray
        + for<'a> FheTryEncrypt<&'a [Clear], ClientKey>
        + FheDecrypt<Vec<Clear>>
        + for<'a> BitAnd<&'a [Clear], Output = Array>,
    for<'a> &'a Array: BitAnd<&'a [Clear], Output = Array>,
    for<'a, 'b> Array::Slice<'a>: Copy + BitAnd<&'b [Clear], Output = Array>,
{
    let num_values = 5;
    let clear_lhs = draw_random_values::<Clear>(num_values);
    let clear_rhs = draw_random_values::<Clear>(num_values);
