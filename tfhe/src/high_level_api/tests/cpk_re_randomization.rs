...
fn test_re_rand() {
    ...
    // Case where we want to compute FheUint64 + FheUint64 and re-randomize those inputs
    {
        ...
        let a_re_rand = a.re_randomize(&cpk, seed_gen.next_seed().unwrap()).unwrap();

        let b_re_rand = b.re_randomize(&cpk, seed_gen.next_seed().unwrap()).unwrap();
    ...
    // Case where we want to compute FheInt8 + FheInt8 and re-randomize those inputs
    {
        ...
        let a: FheInt8 = list.get(0).unwrap().unwrap();
        let b: FheInt8 = list.get(1).unwrap().unwrap();
    ...
    // Case where we want to compute FheBool && FheBool and re-randomize those inputs
    {
        for clear_a in [false, true] {
            for clear_b in [false, true] {
                ...
                let a: FheBool = list.get(0).unwrap().unwrap();
                let b: FheBool = list.get(1).unwrap().unwrap();
