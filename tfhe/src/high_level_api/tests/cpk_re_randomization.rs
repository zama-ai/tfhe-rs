...
fn test_compact_list_re_rand() {
    ...
    // Also test expand_and_re_randomize_without_verification
    {
        ...
        let expander = compact_list.expand_without_verification().unwrap();
...
mod gpu {
    ...
    fn test_compact_list_re_rand_cpu_gpu_compatibility() {
        ...
        let mut cpu_list = compact_list.clone();
        ...
        // Re-randomize on CPU
        cpu_list.re_randomize(&cpk, cpu_seed).unwrap();
        ...
        assert!(cpu_list.inner.on_cpu() == &gpu_on_cpu);
    }
}
