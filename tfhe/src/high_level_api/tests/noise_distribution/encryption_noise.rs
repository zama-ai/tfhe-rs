...
fn test_noise_check_secret_key_encryption_noise_tuniform() {
    let params_as_ap_params = AtomicPatternParameters::Standard(
        TEST_PARAM_MESSAGE_2_CARRY_2_KS_PBS_TUNIFORM_2M128.into(),
    );

    let modulus_as_f64 = if params_as_ap_params.ciphertext_modulus().is_native_modulus() {
    ...
    let encryption_noise = params_as_ap_params.encryption_noise_distribution();
    ...
    let expected_encryption_noise = match params_as_ap_params.encryption_key_choice() {
        shortint::EncryptionKeyChoice::Big => params_as_ap_params.glwe_noise_distribution(),
        shortint::EncryptionKeyChoice::Small => params_as_ap_params.lwe_noise_distribution(),
    };

    assert_eq!(encryption_noise, expected_encryption_noise);

    let config = ConfigBuilder::with_custom_parameters(params_as_ap_params).build();
