...
/// and one strictly above its balance, which makes it rejected. Both branches consume the same
/// single draw, so the transaction stream stays the same whichever is taken.
///
