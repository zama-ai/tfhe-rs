...
impl crate::shortint::oprf::CompressedOprfServerKey {
    pub(super) fn generate_with_pre_seeded_generator<Gen>(
        ...
        client_key: &crate::shortint::ClientKey,
        generator: &mut EncryptionRandomGenerator<Gen>,
    ) -> Self
    where
        Gen: ByteRandomGenerator + ParallelByteRandomGenerator,
        ...
        Gen: ByteRandomGenerator,
    {
        ...
        match &self.inner {
            CompressedOprfBootstrappingKey::Classic { seeded_bsk } => {
                generator.skip(seeded_bsk.decompression_fork_config(Uniform).byte_count());
            }
            CompressedOprfBootstrappingKey::MultiBit { seeded_bsk, .. } => {
                generator.skip(seeded_bsk.decompression_fork_config(Uniform).byte_count());
            }
        }
    }
...
impl crate::integer::oprf::CompressedOprfServerKey {
    pub(super) fn generate_with_pre_seeded_generator<Gen>(
        private_key: &crate::integer::oprf::OprfPrivateKey,
        ...
        Gen: ByteRandomGenerator + ParallelByteRandomGenerator,
    {
        ExpandedOprfServerKey::from_raw_parts(
            self.0.decompress_with_pre_seeded_generator(generator),
        )
    }
    ...
    where
        Gen: ByteRandomGenerator,
    {
        self.0.advance_generator(generator);
    }
}
...
impl crate::transciphering::CompressedTranscipheringServerKey {
    ...
    where
        Gen: ByteRandomGenerator,
    {
        self.oprf_key().advance_generator(generator);
    }
}
