...
use crate::backward_compatibility::xof_key_set::{
    CompressedXofKeySetVersions, XofDerivationModeVersions, XofGenerationParamsVersions,
};
...
/// How a [XofSeed] is consumed to produce a random stream:
/// which Aes variant and the byte the generator starts at.
#[derive(Copy, Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Versionize)]
#[versionize(XofDerivationModeVersions)]
pub enum XofDerivationMode {
    /// Current default: stream derived with Aes-256, generator starts at the first byte.
    Aes256,
    /// Stream derived with Aes-128, generator starts at the first byte.
    LegacyAes128,
    /// Stream derived with Aes-128, replicating the tfhe-rs=1.5.4 csprng off by one.
    LegacyAes128SecondByte,
}

/// A [XofSeed] and the [XofDerivationMode] it is meant to be consumed with.
///
/// Use [`XofGenerationParams::new_aes256`] for the current default.
/// The legacy modes exist to reproduce streams generated by older tfhe-rs versions.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Versionize)]
#[versionize(XofGenerationParamsVersions)]
pub struct XofGenerationParams {
    pub seed: XofSeed,
    pub mode: XofDerivationMode,
}

impl XofGenerationParams {
    pub fn new(seed: XofSeed, mode: XofDerivationMode) -> Self {
        Self { seed, mode }
    }

    /// Consumes the seed with the current default mode, [XofDerivationMode::Aes256].
    pub fn new_aes256(seed: XofSeed) -> Self {
        Self {
            seed,
            mode: XofDerivationMode::Aes256,
        }
    }

    /// Applies the same derivation mode to another seed.
    fn with_seed(self, seed: XofSeed) -> Self {
        Self {
            seed,
            mode: self.mode,
        }
...
impl From<XofGenerationParams> for AesCtrParams {
    fn from(val: XofGenerationParams) -> Self {
        let XofGenerationParams { seed, mode } = val;
        match mode {
            XofDerivationMode::Aes256 => Self {
                seed: SeedKind::xof_aes256(seed),
                first_index: TableIndex::FIRST,
            },
            XofDerivationMode::LegacyAes128 => Self {
                seed: SeedKind::xof_aes128(seed),
                first_index: TableIndex::FIRST,
            },
            XofDerivationMode::LegacyAes128SecondByte => Self {
                seed: SeedKind::xof_aes128(seed),
...
impl CompressedXofKeySet {
    ...
    /// and consumes the seed with the default Aes256-CTR generator.
    ///
    /// If a different configuration is needed, see [Self::generate_with_separators]
    ///
    ...
    pub fn generate(
        ...
        Self::generate_with_separators(
            config,
            XofGenerationParams::new_aes256(private_seed),
            public_separator,
            security_bits,
            max_norm_hwt,
            tag,
    ...
    pub fn generate_with_separators(
        config: Config,
        private_seed: XofGenerationParams,
        ...
        let mut private_generator =
            RandomGenerator::<DefaultRandomGenerator>::new(private_seed.clone());
        ...
        let public_seed =
            private_seed.with_seed(XofSeed::new(public_seed_bytes, public_seed_separator));
    ...
    /// Generates the CompressedXofKeySet, using an already seeded generator for the noise
    pub fn generate_with_pre_seeded_generator<G>(
        pub_seed: XofGenerationParams,
        ck: &ClientKey,
