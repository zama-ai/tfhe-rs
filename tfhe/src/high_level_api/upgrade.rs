...
///
/// let (intermediates_a, upgraded_a) = upgrader.upgrade(&a, ck2.tag(), Device::Cpu).unwrap();
/// // The upgrade was direct
/// assert!(intermediates_a.is_empty());
///
/// let (intermediates_b, upgraded_b) = upgrader.upgrade(&b, ck2.tag(), Device::Cpu).unwrap();
/// assert!(intermediates_b.is_empty());
///
/// set_server_key(sk2);
///
...
impl UpgradeKeyChain {
        ...
        T: HlExpandable + Tagged + Clone,
    {
        ...
        let (mut intermediates, last) = self.apply_upgrade_path(&ct, &upgrade_path[1..]);
    ...
    fn apply_upgrade_path<T>(&self, ct: &T, upgrade_path: &[usize]) -> (Vec<T>, T)
    where
        T: Tagged + Clone,
        KeySwitchingKey: FheKeyswitch<T>,
    {
...
mod tests {
    ...
    use crate::prelude::*;
    #[cfg(feature = "gpu")]
    use crate::shortint::parameters::current_params::V1_1_PARAM_GPU_MULTI_BIT_GROUP_4_MESSAGE_2_CARRY_2_KS_PBS_TUNIFORM_2M128;
    #[cfg(feature = "gpu")]
    use crate::shortint::parameters::v1_1::V1_1_COMP_PARAM_GPU_MULTI_BIT_GROUP_4_MESSAGE_2_CARRY_2_KS_PBS_TUNIFORM_2M128;
    ...
    fn test_graph() {
        ...
        // First key set only has CPU compute params
        ...
        // Second Keyset has CPU and GPU params
        ...
        // Finding a path from from self to self returns an empty path
        // (as opposed to not finding a path which returns None)
        assert_eq!(graph.find_upgrade_path(node_1, node_1).unwrap(), vec![]);
