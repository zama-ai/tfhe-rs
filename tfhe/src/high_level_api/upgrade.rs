...
impl DecompressionUpgradeKey {
    pub fn new(
        ...
        let glwe_secret_key = match &cks_out.key.key.key.atomic_pattern {
            AtomicPatternClientKey::Standard(ap) => &ap.glwe_secret_key,
            AtomicPatternClientKey::KeySwitch32(ap) => &ap.glwe_secret_key,
        };
            ...
            ShortintEngine::with_thread_local_mut(|engine| ShortintBootstrappingKey::Classic {
                bsk: engine.new_classic_bootstrapping_key(
                    &private_compression_key
                        ...
                        .as_lwe_secret_key(),
                    glwe_secret_key,
                    cks_out.key.key.key.parameters().glwe_noise_distribution(),
                    params.br_base_log,
                    params.br_level,
                    cks_out.key.key.key.parameters().ciphertext_modulus(),
                ),
                modulus_switch_noise_reduction_key: None,
            });

        let glwe_decompression_key = crate::shortint::list_compression::DecompressionKey {
            blind_rotate_key,
...
pub struct ParamDescriptor {
    // The tag these params are associated with
    tag: Tag,
    // The kind of ciphertext these params are for
    kind: CipherKind,
    // The device meant for these params
    device: Device,
}

impl ParamDescriptor {
    pub fn new(tag: &Tag, kind: CipherKind, device: Device) -> Self {
        Self {
            tag: tag.clone(),
            kind,
            device,
...
#[derive(Debug, PartialEq, Eq)]
struct Node(ParamDescriptor);

impl Node {
    fn new(tag: &Tag, kind: CipherKind, device: Device) -> Self {
...
impl UpgradeGraph {
    ...
    fn add_path(&mut self, node_in: NodeId, node_out: NodeId, key_index: usize) {
        self.adjacency[node_in.0].push(Vertex {
            out_index: node_out,
            data: key_index,
        });
    }
...
impl UpgradeKey {
    fn input_cipher_kind(&self) -> CipherKind {
        match self {
            Self::Keyswitch(_) => CipherKind::Compute,
            Self::Decompress(_) => CipherKind::Compressed,
        }
    }
...
impl UpgradeKeyChain {
    ...
    pub fn add_key_set_gpu(&mut self, sks: &crate::CudaServerKey) {
        let node = Node::new(sks.tag(), CipherKind::Compute, Device::CudaGpu);
        let _compute_node_id = self.graph.get_or_insert_node(node);

        if sks.key.compression_key.is_some() {
            let node = Node::new(sks.tag(), CipherKind::Compressed, Device::CudaGpu);
    ...
    pub fn add_upgrade_key(&mut self, key: impl Into<UpgradeKey>) -> crate::Result<()> {
        ...
        self.graph
            .add_path(node_in_idx, node_out_idx, self.upgrade_keys.len());
    ...
    pub fn upgrade<T>(
        ...
        dest_tag: &Tag,
        dest_device: Device,
    ) -> crate::Result<(Vec<T>, T)>
    where
        T: Tagged + Clone,
        KeySwitchingKey: FheKeyswitch<T>,
    {
        let input_node = Node::new(ct.tag(), CipherKind::Compute, Device::Cpu);
        let input_node_id = self
            .graph
            .index_of_node(&input_node)
            .ok_or_else(|| crate::error!("Input parameters have no matching point"))?;

        let output_node = Node::new(dest_tag, CipherKind::Compute, dest_device);
        ...
        Ok(self.follow_upgrade_path(ct, &upgrade_path))
    }
    ...
    {
        let input_node = Node::new(&input.tag, CipherKind::Compressed, Device::Cpu);
        let input_node_id = self
            .graph
            .index_of_node(&input_node)
            .ok_or_else(|| crate::error!("Input parameters have no matching point"))?;
        ...
        if upgrade_path.len() == 1 {
            return Ok((Vec::new(), ct));
        }

        let (mut intermediates, last) = self.follow_upgrade_path(&ct, &upgrade_path[1..]);
        intermediates.insert(0, ct);

        Ok((intermediates, last))
    }

    // Follows the upgrade path
    //
    // NOTE: only keyswitch are allowed in the upgrade path
    fn follow_upgrade_path<T>(&self, ct: &T, upgrade_path: &[usize]) -> (Vec<T>, T)
    where
        T: Tagged + Clone,
        KeySwitchingKey: FheKeyswitch<T>,
    {
        if upgrade_path.is_empty() {
            return (Vec::new(), ct.clone());
        }
        ...
        let last = intermediates.pop().unwrap();
        (intermediates, last)
    }
}
...
mod tests {
    ...
    use crate::prelude::*;
    #[cfg(feature = "gpu")]
    use crate::shortint::parameters::test_params::TEST_COMP_PARAM_GPU_MULTI_BIT_GROUP_4_MESSAGE_2_CARRY_2_KS_PBS_TUNIFORM_2M128;
    #[cfg(feature = "gpu")]
    ...
    fn test_keychain_upgrade() {
        ...
        // We add an upgrade path, we add an upgrade path to form a chain
        ...
        //
        // This also tests that upgrading to from latest to latest is ok
        for i in 0..num_key_sets {
            ...
            let (intermediates_a, upgraded_a) = upgrader
                .upgrade(&a, &Tag::from(num_key_sets - 1), Device::Cpu)
                .unwrap();
            // We expect to hop from keyset to keyset
            assert_eq!(
                intermediates_a.len(),
                num_key_sets.saturating_sub(i + 2) as usize
            );

            let (intermediates_b, upgraded_b) = upgrader
        ...
        // So we test that this path is taken
        {
            ...
            let (intermediates, upgraded_a) = upgrader
                ...
                .unwrap();
            assert!(intermediates.is_empty()); // We expect the conversion to be direct
            let (intermediates, upgraded_b) = upgrader
                .upgrade_from_compressed::<FheUint32>(
                    &list,
                    1,
                    key_sets.last().map(|x| x.1.tag()).unwrap(),
        ...
        for i in 1..num_key_sets as usize - 1 {
            ...
            assert_eq!(
                intermediates.len(),
                1 + num_key_sets.saturating_sub((i + 2) as u64) as usize
            );
            let (intermediates, upgraded_b) = upgrader
                .upgrade_from_compressed::<FheUint32>(
                    &list,
