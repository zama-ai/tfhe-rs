...
impl TryFrom<SerializedKind> for FheTypes {
    ...
    fn try_from(kind: SerializedKind) -> crate::Result<Self> {
        match kind {
            ...
            SerializedKind::String { .. } => Ok(Self::AsciiString),
        }
    }
}

#[derive(Serialize, Deserialize, Debug, Copy, Clone, Versionize)]
#[versionize(SerializedKindVersions)]
pub enum SerializedKind {
    Bool,
    Uint { num_bits: u32 },
    Int { num_bits: u32 },
    String { n_chars: u32, padded: bool },
}
...
impl SerializedKind {
    fn from_data_kind(value: DataKind, msg_modulus: MessageModulus) -> Self {
        match value {
            ...
            DataKind::String { n_chars, padded } => Self::String { n_chars, padded },
        }
    }

    fn to_data_kind(self, msg_modulus: MessageModulus) -> DataKind {
        match self {
            Self::Bool => DataKind::Boolean,
            Self::Uint { .. } => DataKind::Unsigned(self.num_blocks(msg_modulus) as usize),
            Self::Int { .. } => DataKind::Signed(self.num_blocks(msg_modulus) as usize),
            Self::String { n_chars, padded } => DataKind::String { n_chars, padded },
        }
    }
    ...
    pub(crate) fn num_blocks(self, message_modulus: MessageModulus) -> u32 {
        match self {
            Self::Bool => 1,
            Self::Uint { num_bits } => num_bits.div_ceil(message_modulus.0.ilog2()),
            Self::Int { num_bits } => num_bits.div_ceil(message_modulus.0.ilog2()),
            Self::String { n_chars, .. } => {
                let blocks_per_char = 7u32.div_ceil(message_modulus.0.ilog2());
                n_chars * blocks_per_char
            }
        }
    }
}
