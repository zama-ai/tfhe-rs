...
#[no_mangle]
pub unsafe extern "C" fn shortint_bc_gen_server_key(
    client_key: *const super::ShortintClientKey,
    result_server_key: *mut *mut ShortintServerKey,
) -> c_int {
    catch_panic(|| {
        ...
        *result_server_key = std::ptr::null_mut();
        ...
        let server_key = shortint::server_key::ServerKey::bc_new(&client_key.0);
...
#[no_mangle]
pub unsafe extern "C" fn shortint_bc_server_key_create_trivial(
    server_key: *const ShortintServerKey,
    value_to_trivially_encrypt: u64,
    result: *mut *mut ShortintCiphertext,
) -> c_int {
    catch_panic(|| {
        ...
        let heap_allocated_ciphertext = Box::new(ShortintCiphertext(
            server_key.0.bc_create_trivial(value_to_trivially_encrypt),
        ));
