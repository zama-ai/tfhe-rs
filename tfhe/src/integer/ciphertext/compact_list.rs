...
impl IntegerUnpackingToShortintCastingModeHelper {
    ...
    pub fn generate_sanitize_without_unpacking_functions(
        ...
        for data_kind in infos {
            ...
            match data_kind {
                DataKind::Boolean => {
                    functions.extend(std::iter::repeat_n(
                        Some(vec![self.msg_extract_bool.as_ref()]),
                        block_count,
                    ));
                }
                DataKind::String { n_chars, .. } => {
                    let blocks_per_char = 7u32.div_ceil(self.message_modulus.0.ilog2());
                    for _ in 0..*n_chars {
                        for _ in 0..blocks_per_char - 1 {
                            functions.push(Some(vec![self.msg_extract.as_ref()]));
                        }
                        functions.push(Some(vec![self.msg_extract_last_char_block.as_ref()]));
                    }
                }
                _ => {
                    functions.extend(std::iter::repeat_n(
                        Some(vec![self.msg_extract.as_ref()]),
                        block_count,
                    ));
                }
            }
        ...
        for data_kind in infos {
            ...
            match data_kind {
                DataKind::Boolean => {
                    let lut = sks
                        .key
                        .generate_lookup_table(self.msg_extract_bool.as_ref());
                    functions.extend(std::iter::repeat_n(lut, block_count));
                }
                DataKind::String { n_chars, .. } => {
                    let blocks_per_char = 7u32.div_ceil(self.message_modulus.0.ilog2());
                    for _ in 0..*n_chars {
                        let message_extract_lut =
                            sks.key.generate_lookup_table(self.msg_extract.as_ref());
                        functions.extend(std::iter::repeat_n(
                            message_extract_lut,
                            blocks_per_char.saturating_sub(1) as usize,
                        ));
                        functions.push(
                            sks.key
                                .generate_lookup_table(self.msg_extract_last_char_block.as_ref()),
                        );
                    }
                }
                _ => {
                    let message_extract_lut =
                        sks.key.generate_lookup_table(self.msg_extract.as_ref());
                    functions.extend(std::iter::repeat_n(message_extract_lut, block_count));
                }
            }
        }
