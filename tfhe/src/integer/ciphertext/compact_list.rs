...
struct IntegerUnpackingToShortintCastingModeHelper {
    ...
    /// strings
    blocks_per_char: Option<NonZeroUsize>,
}
...
impl IntegerUnpackingToShortintCastingModeHelper {
    pub fn new(message_modulus: MessageModulus) -> Self {
        ...
        // value stored in ASCII_CHAR_BITS bits (currently 8), so the upper bits of the last block
        // must be sanitized to 0.
        let bits_of_last_char_block = blocks_per_char.map_or(0, |blocks_per_char| {
            ASCII_USEFUL_BITS
                .saturating_sub((blocks_per_char.get() as u32 - 1) * message_modulus.0.ilog2())
        });
    ...
    fn blocks_per_char(&self) -> crate::Result<NonZeroUsize> {
