...
/// Utility function for HpeDevice initialisation
pub fn init_device(device: &HpuDevice, server_key: CompressedServerKey) {
            ...
            let ksk = server_key.key.key_switching_key.decompress_into_lwe_keyswitch_key();
            let hpu_ksk = ksk.as_view().into_with(params.clone());

            // Upload them on Hpu and configure internal Fw/Lut
            device.init(hpu_bsk, hpu_ksk, crate::core_crypto::hpu::glwe_lookuptable::create_hpu_lookuptable)
        }
