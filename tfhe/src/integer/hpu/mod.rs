use crate::{core_crypto::hpu::from_with::IntoWith, shortint::ClassicPBSParameters};
...
pub fn init_device(device: &HpuDevice, server_key: CompressedServerKey) -> crate::Result<()> {
    ...
    let bsk = match server_key.key.bootstrapping_key {
        crate::shortint::server_key::ShortintCompressedBootstrappingKey::Classic {
            ...
            if tfhe_params.lwe_dimension != bsk.input_lwe_dimension() {
                return Err("BootstrappingKey has imcompatible input_lwe_dimension".into());
            }
            if tfhe_params.glwe_dimension.to_glwe_size() != bsk.glwe_size() {
                return Err("BootstrappingKey has imcompatible glwe_size".into());
            }
            if tfhe_params.polynomial_size != bsk.polynomial_size() {
                return Err("BootstrappingKey has imcompatible polynomial size".into());
            }
            if tfhe_params.pbs_base_log != bsk.decomposition_base_log() {
                return Err("BootstrappingKey has imcompatible decomposition_base_log".into());
            }
            if tfhe_params.pbs_level != bsk.decomposition_level_count() {
                return Err("BootstrappingKey has imcompatible decomposition_level_count".into());
            }
            if tfhe_params.ciphertext_modulus != bsk.ciphertext_modulus() {
                return Err("BootstrappingKey has imcompatible ciphertext_modulus".into());
            }
            Ok(bsk)
        }
    ...
    if tfhe_params.ks_base_log != ksk.decomposition_base_log() {
        return Err("KeyswitchingKey has imcompatible decomposition_base_log".into());
    }
    if tfhe_params.ks_level != ksk.decomposition_level_count() {
        return Err("KeyswitchingKey has imcompatible decomposition_level_count".into());
    }
