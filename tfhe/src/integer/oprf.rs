...
impl ServerKey {
    ...
    pub fn par_generate_oblivious_pseudo_random_unsigned_custom_range(
        ...
        let num_bits_output = num_blocks_output * self.message_modulus().0;
        assert!((excluded_upper_bound as f64) < 2_f64.powi(num_bits_output as i32));

        let post_mul_log2_range =
            num_input_random_bits as f64 + (excluded_upper_bound as f64).log2();

        let num_random_blocks =
            (num_input_random_bits as f64 / message_bits_count as f64).ceil() as usize;

        let num_blocks = (post_mul_log2_range / message_bits_count as f64).ceil() as usize;

        let mut deterministic_seeder = DeterministicSeeder::<DefaultRandomGenerator>::new(seed);

        let seeds: Vec<Seed> = (0..num_random_blocks)
            .map(|_| deterministic_seeder.seed())
            .collect();

        let mut in_blocks = seeds
            .into_par_iter()
            .enumerate()
            .map(|(i, seed)| {
                let i = i as u64;
                ...
                // if we generate 5 bits of noise in n blocks of 2 bits, the third (i=2) block
                // must have only one bit of random
                let random_bits_count = if num_input_random_bits < (i + 1) * message_bits_count {
                ...
                self.key
                    .generate_oblivious_pseudo_random(seed, random_bits_count)
            })
            .collect::<Vec<_>>();

        in_blocks.resize(num_blocks, self.key.create_trivial(0));

        let input = RadixCiphertext::from(in_blocks);

        let before_shift = self.scalar_mul_parallelized(&input, excluded_upper_bound);
...
pub(crate) mod test {
    ...
    fn oprf_test_almost_uniformity_ci_run_filter() {
        ...
        let random_input_upper_bound = 2_f64.powi(num_input_random_bits as i32);

        let mut density = vec![0_usize; excluded_upper_bound];

        for i in 0..1 << num_input_random_bits {
            let index = ((i * excluded_upper_bound) as f64 / random_input_upper_bound) as usize;
        ...
        let theoretical_pdf: Vec<f64> = density
            .iter()
            .map(|count| *count as f64 / random_input_upper_bound)
            .collect();
