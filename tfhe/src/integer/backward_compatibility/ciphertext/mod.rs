...
impl Upgrade<DataKind> for DataKindV0 {
    ...
    fn upgrade(self) -> Result<DataKind, Self::Error> {
        match self {
            ...
            Self::String { n_chars, padded } => NonZero::new(n_chars)
                .ok_or_else(|| crate::error!("DataKind::String requires non-zero char count"))
                .map(|n_chars| DataKind::String { n_chars, padded }),
        }
    }
}
