...
/// Normalize a rust bound object, and check that it is valid for the source integer
pub(crate) fn parse_bounds<B, R>(
    range: &R,
    nb_bits: usize,
...
impl ServerKey {
    ...
    where
        ...
        B: CastFrom<usize> + CastInto<usize> + Copy,
    {
        let (start_block, end_block) = parse_bounds(&range, ctxt.blocks.len())?;
        Ok(self.scalar_blockslice_aligned(ctxt, start_block, end_block))
    }
        ...
        B: CastFrom<usize> + CastInto<usize> + Copy,
    {
        ...
        let (start, end) = parse_bounds(&range, block_width * ctxt.blocks.len())?;

        let slice_width = end - start;
        ...
        let mut sliced = if start % block_width != 0 {
            ...
            let mut sliced =
                self.scalar_blockslice_aligned(ctxt, start / block_width, end / block_width);
            if slice_width % block_width != 0 {
                let last_block =
                    self.bitslice_remainder(ctxt, end / block_width, slice_width % block_width);
                sliced.blocks.push(last_block);
            }
