    ...
    P: Into<TestParameters>,
{
    ...
    for fn_index in 0..get_long_test_iterations() {
        ...
        if binary_ops_range.contains(&i) {
            ...
            assert_eq!(
                res, res_1,
                "{idx}: Determinism check failed on unary op {fn_name} with clear input {}.",
                operand.p
            );
            ...
            assert_eq!(
                ...
                "{idx}: Invalid result on unary op {fn_name} with clear input {} at iteration {fn_index}.",
                operand.p
            );
            ...
            assert!(
                ...
                "{idx}: Noise level greater than nominal value on overflow for op {fn_name}",
            );
            // Determinism check
            let (res_1, overflow_1) = overflowing_op_executor.execute((&lhs.c, &rhs.c));
            assert_eq!(
                res, res_1,
                "{idx}: Determinism check failed on binary op {fn_name} with clear inputs {} and {}.",
                lhs.p, rhs.p
            );
            assert_eq!(
                overflow, overflow_1,
                "{idx}: Determinism check failed on binary op {fn_name} with clear inputs {} and {} on the overflow.",
                lhs.p, rhs.p
            );
            ...
            assert_eq!(
                decrypted_res, expected_res,
                "{idx}: Invalid result on op {fn_name} with clear inputs {} and {} at iteration {fn_index}.",
                lhs.p, rhs.p
            );
            assert_eq!(
                decrypted_overflow, expected_overflow,
                "{idx}: Invalid overflow on op {fn_name} with clear inputs {} and {}.",
                lhs.p, rhs.p
            );
            ...
            assert!(
                ...
                "{idx}: Noise level greater than nominal value on op {fn_name}",
            );
            // Determinism check
            ...
            let res_ct: RadixCiphertext =
                sks.cast_to_unsigned(res.into_radix::<RadixCiphertext>(1, &sks), NB_CTXT_LONG_RUN);
            datagen.put_op_result_random_side(expected_res as u64, &res_ct, fn_name, idx);

            // Correctness check
            assert_eq!(
                decrypted_res, expected_res,
                "{idx}: Invalid result on binary op {fn_name} with clear inputs {} and {} at iteration {fn_index}.",
                lhs.p, rhs.p
            );
            ...
            assert_eq!(
                ...
                "{idx}: Invalid result on op {fn_name} with clear inputs {}, {} at iteration {fn_index}",
                lhs.p, rhs.p
            );
            assert_eq!(
            ...
            assert_eq!(
                res, res_1,
                "{idx}: Determinism check failed on op {fn_name} with clear input {}.",
                operand.p
            );
            ...
            assert_eq!(
                decrypted_res, expected_res,
                "Invalid result on op {fn_name} with clear input {} at iteration {fn_index}.",
                operand.p
            );
        }
    }
