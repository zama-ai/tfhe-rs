    ...
    T: IntegerRadixCiphertext,
{
    ...
    fn flip_parallelized(
        ...
        for mut vec in results_for_a {
            a_blocks_if_not_cond.push(vec.pop().unwrap());
            a_blocks_if_cond.push(vec.pop().unwrap());
        }
        ...
        for mut vec in results_for_b {
            b_blocks_if_cond.push(vec.pop().unwrap());
            b_blocks_if_not_cond.push(vec.pop().unwrap());
        }
        ...
        rayon::join(
            || {
                a_blocks_if_cond
                    .par_iter_mut()
                    .zip(b_blocks_if_not_cond.par_iter())
                    .for_each(|(lhs, rhs)| {
            ...
            || {
                a_blocks_if_not_cond
                    .par_iter_mut()
                    .zip(b_blocks_if_cond.par_iter())
                    .for_each(|(lhs, rhs)| {
                        self.key.unchecked_add_assign(lhs, rhs);
                        self.key.apply_lookup_table_assign(lhs, &clean_lut);
                    });
