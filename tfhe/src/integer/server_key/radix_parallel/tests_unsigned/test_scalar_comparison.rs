...
macro_rules! define_scalar_comparison_test_functions {
    ($comparison_name:ident, $clear_type:ty) => {
        ::paste::paste!{
            fn [<integer_unchecked_scalar_ $comparison_name _parallelized_ $clear_type:lower>]<P>(param: P) where P: Into<AtomicPatternParameters>{
                let num_tests = 1;
            ...
            fn [<integer_smart_scalar_ $comparison_name _parallelized_ $clear_type:lower>]<P>(param: P) where P: Into<AtomicPatternParameters>{
                let num_tests = 1;
            ...
            fn [<integer_default_scalar_ $comparison_name _parallelized_ $clear_type:lower>]<P>(param: P) where P: Into<AtomicPatternParameters>{
                let num_tests = 1;
...
fn integer_extensive_trivial_default_scalar_comparisons(
