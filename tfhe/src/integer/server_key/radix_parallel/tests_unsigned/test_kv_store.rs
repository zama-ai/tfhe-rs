    ...
    >,
{
    ...
    // Exercise three store sizes that cover distinct paths in the GPU fold-sum
    // schedule for 2_2 params (max_noise = 5, L = 2, extra = 1):
    //
...
pub fn default_kv_store_contains_test<P, T1>(params: P, mut kv_store_contains_key: T1)
where
    P: Into<TestParameters>,
    T1: for<'a> FunctionExecutor<
        (&'a KVStore<KeyType, RadixCiphertext>, &'a RadixCiphertext),
