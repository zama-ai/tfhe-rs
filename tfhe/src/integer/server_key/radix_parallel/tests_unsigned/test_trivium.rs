    ...
    E: for<'a> FunctionExecutor<(&'a RadixCiphertext, &'a RadixCiphertext, usize), RadixCiphertext>,
{
    ...
    let ct_key = RadixCiphertext::from(
        key.iter()
            .map(|&bit| cks.encrypt_one_block(bit))
            .collect::<Vec<_>>(),
    );
    let ct_iv = RadixCiphertext::from(
        iv.iter()
            .map(|&bit| cks.encrypt_one_block(bit))
            .collect::<Vec<_>>(),
    );
    ...
    let num_steps = 512;
    let output_radix = executor.execute((&ct_key, &ct_iv, num_steps));

    let decrypted_bits: Vec<u8> = output_radix
        .blocks
        .iter()
    ...
    E: for<'a> FunctionExecutor<(&'a RadixCiphertext, &'a RadixCiphertext, usize), RadixCiphertext>,
{
    ...
    for i in 0..num_runs {
        ...
        let output_radix = executor.execute((&ct_key, &ct_iv, num_steps));
        let fhe_output: Vec<u8> = output_radix
            .blocks
            .iter()
            .map(|block| cks.decrypt_one_block(block) as u8)
            .collect();
