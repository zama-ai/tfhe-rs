    ...
    E: for<'a> FunctionExecutor<(&'a RadixCiphertext, &'a RadixCiphertext, usize), RadixCiphertext>,
{
    ...
    let cpu_key = cks.encrypt_bits_from_u64_slice_to_radix_ciphertext(&key);
    let cpu_iv = cks.encrypt_bits_from_u64_slice_to_radix_ciphertext(&iv);

    let num_steps = 512;
    let output_radix = executor.execute((&cpu_key, &cpu_iv, num_steps));

    let decrypted_bits = cks.decrypt_bits_from_radix_ciphertext_to_vec_u8(&output_radix);
    ...
    E: for<'a> FunctionExecutor<(&'a RadixCiphertext, &'a RadixCiphertext, usize), RadixCiphertext>,
{
    ...
    for i in 0..num_runs {
        ...
        let cpu_key = cks.encrypt_bits_from_u64_slice_to_radix_ciphertext(&key_bits_u64);
        let cpu_iv = cks.encrypt_bits_from_u64_slice_to_radix_ciphertext(&iv_bits_u64);
        ...
        let output_radix = executor.execute((&cpu_key, &cpu_iv, num_steps));
        let fhe_output = cks.decrypt_bits_from_radix_ciphertext_to_vec_u8(&output_radix);
