...
use crate::integer::server_key::radix::slice::parse_bounds;
...
fn slice_reference_impl<T, B>(value: u64, bounds: B, modulus: u64) -> u64
where
    B: RangeBounds<T>,
    T: CastFrom<usize> + Copy,
    usize: CastFrom<T>,
{
    let (start, end) = parse_bounds(&bounds, modulus.ilog2() as usize).unwrap();
    let bin: String = format!("{value:064b}").chars().rev().collect();

    let out_bin: String = bin[start..end].chars().rev().collect();
