...
pub(crate) fn clear_prince_encrypt(m: u64, k0: u64, k1: u64) -> u64 {
    let mut state = m ^ k0;
    for (round, rc) in PV2_RC.iter().enumerate().skip(1).take(5) {
        state = m_layer(sub_nibbles(state, &PV2_SBOX)) ^ rc ^ round_key(round, k0, k1);
    }
    ...
    for (round, rc) in PV2_RC.iter().enumerate().skip(6).take(5) {
        state = sub_nibbles(
            inv_m_layer(state ^ rc ^ round_key(round, k0, k1)),
            &PV2_INV_SBOX,
        );
    }
    state ^ PV2_RC[11] ^ k1
...
pub(crate) fn clear_prince_decrypt(c: u64, k0: u64, k1: u64) -> u64 {
    ...
    for (round, rc) in PV2_RC.iter().enumerate().skip(6).take(5).rev() {
        state = m_layer(sub_nibbles(state, &PV2_SBOX)) ^ rc ^ round_key(round, k0, k1);
    }
    state = m_prime(sub_nibbles(state, &PV2_SBOX) ^ PV2_BETA ^ k1) ^ k0;
    state = sub_nibbles(state, &PV2_INV_SBOX);
    for (round, rc) in PV2_RC.iter().enumerate().skip(1).take(5).rev() {
        state = sub_nibbles(
            inv_m_layer(state ^ rc ^ round_key(round, k0, k1)),
            &PV2_INV_SBOX,
        );
