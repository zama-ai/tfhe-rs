...
use crate::integer::{BooleanBlock, ClientKey, CompressedServerKey, RadixCiphertext, ServerKey};
...
///
/// Everything derives from a single seed:
/// * The keys and the encryptions (through a seeded [`ShortintEngine`] installed on the test
///   thread)
/// * And the clear values, input states and dirtying amounts (through `rng`).
pub(crate) struct TestContext {
    params: TestParameters,
    /// Used for encrypting and decrypting every ciphertext flowing through the tests
    cks: ClientKey,
    /// Source each backend should use to get its own version of the server key
    compressed_sks: CompressedServerKey,
    /// The harness's own CPU server key, used to prepare inputs (trivial encryptions,
    /// carries, noise); never to run the operation under test.
    sks: Arc<ServerKey>,
    seed: u128,
...
impl TestContext {
    ...
    pub(crate) fn new(params: TestParameters, seed: u128) -> Self {
        ...
        let mut sks = compressed_sks.decompress();
    ...
    /// The key material a backend executor sets itself up from.
    pub(crate) fn compressed_server_key(&self) -> &CompressedServerKey {
        &self.compressed_sks
    }
    ...
    ClearInput::Input: TestInput<Clear = ClearInput>,
{
        ...
        Output: TestOutput,
    {
        ...
        'widths: for n_blocks in self.block_counts.iter().copied() {
...
impl<'a, E, ClearF> CaseRunner<'a, E, ClearF> {
        ...
        O: TestOutput,
    {
        ...
        let mut failures = CaseFailures::default();
...
///
/// Stamps each failure with the index of the output being validated.
#[derive(Default)]
pub(crate) struct CaseFailures {
    output_index: usize,
...
/// Failing cases are collected instead of panicking on the first one.
/// But possibly, a test could fail a lot, so we chose to 'truncate' the number of
/// reported errors
const MAX_REPORTED_FAILING_CASES: usize = 20;
...
impl Report {
    ...
    /// Whether enough failing cases were collected to stop running new cases.
    fn is_capped(&self) -> bool {
        self.failing_cases.len() >= MAX_REPORTED_FAILING_CASES
    }
}
...
impl std::fmt::Display for Report {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if self.is_capped() {
            writeln!(
                f,
                "(stopped after {MAX_REPORTED_FAILING_CASES} failing cases)"
            )?;
        }

        // Summary by kind, most frequent first
        let mut by_kind: Vec<(&'static str, usize)> = vec![];
        for failure in self.failing_cases.iter().flat_map(|c| c.failures.iter()) {
            let label = failure.kind.label();
...
impl InputState for RadixState {
    ...
    fn all_combinations() -> Vec<Self> {
        vec![Self::Clean, Self::FullNoise, Self::Saturated]
    }
}
...
/// e.g using `Scalar<u64>` when building a test for a scalar test, means the value of the
/// random scalar used will be in the range of a u64 (but stored on a Uint)
#[derive(Copy, Clone, PartialEq, Eq)]
pub(crate) struct Scalar<T> {
    value: Uint,
...
/// [`scalar_edge_values`]).
pub(crate) fn default_scalar_fixed_cases<T: ScalarType>(radix_bits: u32) -> Vec<(Uint, Scalar<T>)> {
    let radix_edges = [
        Uint::zero(radix_bits),
        Uint::one(radix_bits),
        Uint::max_value(radix_bits),
    ];
...
/// Scalar values worth testing against a radix of `radix_bits` bits: zero, one, the radix
...
fn scalar_edge_values<T: ScalarType>(radix_bits: u32) -> Vec<Scalar<T>> {
    let type_max = Uint::mask(T::BITS);
    let radix_max = Uint::mask(radix_bits);
    let mut values = vec![0, 1, radix_max, type_max];
    for past_radix in [
        radix_max.wrapping_add(1),
        radix_max.wrapping_add(2),
        radix_max.wrapping_mul(2).wrapping_add(1),
    ] {
        if radix_max < type_max && past_radix <= type_max {
            values.push(past_radix);
        }
    }
    values.sort_unstable();
    values.dedup();
    values.into_iter().map(Scalar::new).collect()
}

impl<T: ScalarType> TestClearInput for Scalar<T> {
    type Input = T;
    ...
    fn generate_random(rng: &mut dyn RngCore, n_blocks: u32, ctx: &TestContext) -> Self {
        ...
        let value = if radix_bits < T::BITS && rng.gen_bool(0.5) {
            ...
            rng.gen_range(1u128 << (bit_length - 1)..=Uint::mask(bit_length))
        } else {
            rng.gen_range(0..=Uint::mask(radix_bits))
        };
        Self::new(value)
    }
}
...
pub(crate) trait TestOutput: PartialEq {
    ...
    fn validate(&self, expected: Self::Clear, ctx: &TestContext, failures: &mut CaseFailures);
}
