...
impl ServerKey {
        ...
        C: Container<Element = c64> + Sync,
    {
        ...
        // We are going to generate all bits/blocks in one go with the same seed
        // and then chunk them into keys
        let random_bits_needed = data.len() as u64 * key_num_bits;
        let all_blocks =
            oprf_key
                .key
                .generate_oblivious_pseudo_random_bits(seed, random_bits_needed, &self.key);

        let mut keys = Vec::with_capacity(data.len());

        // manual chunking to be able to move the blocks from one vec to another
        let mut blocks_iter = all_blocks.into_iter();
        for _ in 0..data.len() {
            let mut key_blocks = Vec::with_capacity(key_num_blocks as usize);
            for block in blocks_iter.by_ref().take(key_num_blocks as usize) {
                key_blocks.push(block);
            }

            keys.push(RadixCiphertext::from(key_blocks));
        }
