...
impl ServerKey {
    ...
    // Let's suppose we have a 64 bits input and the input is not 0 (this case works a bit differently)
    // The result is on 6 bits grouped by 2 (b5_b4, b3_b2, b1_b0)
    //
    // b5_b4 is the index of the first (starting from MSB) group of 16 bits which has (at least) one bit equal to 1
    // (example, if b5_b4=0, then leading_zeros < 16 thus (at least) one of the 16 MSB is not 0)
    // (example, if b5_b4=1, then 16 <= leading_zeros < 32 thus the 16 MSB are all 0 and (at least) one the one of the 16 next MSB is not 0)
    // In this group of 16 bits:
    //  - b3_b2 is is the index of the first (starting from MSB) group of 4 bits which has (at least) one bit equal to 1
    ...
    // We compute the "b1_b0" of each group of 4 bits, and a boolean flag to know if (at least) of of the bits are not 0.
    //
    // We then group by 16 bits.
    // We compute the "b3_b2" of each group. It is the index of the first (starting from MSB) group of 4 with a non zero flag.
    // We select its "b1_b0" among the 4 computed in the previous step.
    // We now have "b3_b2_b1_b0" for each group of 16.
    // We also need a flag to know
    //
    ...
    pub(crate) fn unchecked_count_consecutive_bits_2_2_unsigned(
        ...
        while state.len() != 1 {
            ...
            state = match direction {
                ...
                // - get a reverse index without changing the order
                // - align chunk on the most significant blocks (if the last chunk does not have 4 elements)
    ...
    fn pack_blocks_by_pair(
        ...
        match chunk {
            ...
            [chunk0] => {
                ...
                // This padding value will only have an impact on the final result if all other bits are equal to the given bit_value
                // In this case, we want to pad with bits equal to the bit_value
                // so the result is flag = 0, digits= [0, ..., 0] as specified
    ...
    // The input is expected to be (at most) 4 binary flags, scaled by 0, 1, 2, 3 (0_0_0_f, 0_0_f_0, 0_f_0_0, f_0_0_0)
    // The new flag is 0 if all input flags are 0, 1 otherwise
    ...
    fn select_old_digit_lut(
        ...
        self.key.generate_lookup_table(|x: u64| {
            ...
            // The input of the resulting LUT is supposed to have the old/new digits in carry/message depending on up_scale_digits
    ...
    // build a lut
    // the input is expected to the sum of 4 binary flags, scaled by 0, 1, 2, 3 (0_0_0_f0 + 0_0_f1_0 + 0_f2_0_0 + f3_0_0_0 = f3_f2_f1_f0)
    // if all the flags are 0, the result if 0
    // otherwise, the result is the index of the first of these flag which is not 0, starting by f0.
    // this function is used in all cases (trailing/leading zeros/ones) in all steps expect the first one
...
mod test {
    ...
    #[test]
    fn test_unchecked_new_leading_zeroes_trivial_input_param_message_2_carry_2() {
