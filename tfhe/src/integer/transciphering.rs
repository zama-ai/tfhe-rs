...
pub enum IntegerStreamKind {
    Unsigned,
    Signed,
    Boolean,
}

impl IntegerStreamKind {
...
pub struct IntegerStreamCiphertext {
    ...
    kind: IntegerStreamKind,
}

impl IntegerStreamCiphertext {
    pub fn into_raw_parts(self) -> (StreamCiphertext, IntegerStreamKind) {
        (self.inner, self.kind)
    }

    pub fn from_raw_parts(inner: StreamCiphertext, kind: IntegerStreamKind) -> Self {
        Self { inner, kind }
    }
    ...
    pub fn kind(&self) -> IntegerStreamKind {
        self.kind
    }
...
impl<C: StreamCipher + ?Sized> IntegerStreamCipher for C {
    ...
    fn encrypt_bool(&mut self, input: bool) -> IntegerStreamCiphertext {
        IntegerStreamCiphertext {
            ...
            kind: IntegerStreamKind::Boolean,
        }
    }
}
...
pub enum IntegerTranscipherError {
    ...
    KindMismatch {
        expected: IntegerStreamKind,
        got: IntegerStreamKind,
    },
}
...
impl<T: Transcipherer> IntegerTranscipherer for T {
    fn transcipher_radix(
        ...
        check_kind(input, IntegerStreamKind::Unsigned)?;
    ...
    fn transcipher_signed_radix(
        ...
        check_kind(input, IntegerStreamKind::Signed)?;
        let blocks = self.transcipher(&sks.key, &input.inner)?;
    ...
    fn transcipher_bool(
        ...
        check_kind(input, IntegerStreamKind::Boolean)?;
