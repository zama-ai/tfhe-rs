...
pub struct BlockDecomposer<T> {
    ...
    num_bits_valid: u32,
    // This value (0 or 1) will be used to pad the last block
    // when the number of bits in the block is not a multiple of T::BITS
    padding_bit: Option<u32>,
    limit: Option<T>,
}
    ...
    T: Decomposable,
{
    ...
    pub fn with_padding_bit(value: T, bits_per_block: u32, padding_bit: u32) -> Self {
    ...
    T: Decomposable,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.num_bits_valid < self.num_bits_in_mask {
            ...
            // intoduced by the shift, to a specific value. if one was provided.
            if let Some(padding_bit) = self.padding_bit {
                ...
                let padding_bit = T::cast_from(padding_bit);
...
mod tests {
    ...
    fn test_bit_block_decomposer_3() {
        ...
        let value = -1i8;
        let blocks = BlockDecomposer::with_padding_bit(value, bits_per_block, 0)
            .iter_as::<u64>()
            .collect::<Vec<_>>();
