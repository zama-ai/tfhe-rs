...
pub struct CudaKeySwitchingKey<'keys> {
    pub key_switching_key: CudaLweKeyswitchKey<u64>, // TODO: Change it to pub(crate)
    pub(crate) dest_server_key: &'keys CudaServerKey,
    pub(crate) src_server_key: Option<&'keys CudaServerKey>,
...
impl<'keys> CudaKeySwitchingKey<'keys> {
    ...
    // TODO: We cannot implement a from_ method since there is no conversion between ServerKey to
    // CudaServerKey.
    // pub fn from_key_switching_key(key_switching_key: &KeySwitchingKey, streams: &CudaStreams) ->
    // Self {
    //
    //     let h_key_switching_key = key_switching_key.key.as_view().key_switching_key_material
    //         .key_switching_key;
    //     let d_key_switching_key =
    // CudaLweKeyswitchKey::from_lwe_keyswitch_key(&h_key_switching_key,                        
    // &streams);     let d_sk = CudaServerKey::n
    //
    //     CudaKeySwitchingKey{
    //         key_switching_key: d_key_switching_key
    //         dest_server_key: key_switching_key.key.as_view().dest_server_key,
    //         src_server_key: None,
