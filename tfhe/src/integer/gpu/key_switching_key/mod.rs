...
impl<'keys> CudaKeySwitchingKey<'keys> {
    ...
    {
        let destination_key = params.destination_key;
        let pbs_order = output_key_pair.1.pbs_order;

        match pbs_order {
            PBSOrder::KeyswitchBootstrap => {
                assert_eq!(destination_key, EncryptionKeyChoice::Small)
            }
            PBSOrder::BootstrapKeyswitch => {
                assert_eq!(destination_key, EncryptionKeyChoice::Big)
            }
        };
