...
pub(crate) fn cuda_backend_get_rerand_size_on_gpu(
    ...
    let size_tracker = unsafe {
        scratch_cuda_rerand_64_async(
            streams.ffi(),
            std::ptr::addr_of_mut!(mem_ptr),
            u32::try_from(ksk_params.input_lwe_dimension).unwrap(),
            u32::try_from(ksk_params.output_lwe_dimension).unwrap(),
            u32::try_from(ksk_params.level_count).unwrap(),
            u32::try_from(ksk_params.base_log).unwrap(),
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn cuda_backend_get_rerand_without_keyswitch_size_on_gpu(
    streams: &CudaStreams,
    message_modulus: MessageModulus,
    carry_modulus: CarryModulus,
    lwe_dimension: LweDimension,
