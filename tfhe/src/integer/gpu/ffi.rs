...
>(
    ...
    // Re-randomization parameters. When `apply_rerand` is false the backend ignores these, so we
    // pass sentinel values and null pointers. When true, `zero_lwes` carries the encryptions of
    // zero and `rerand_keyswitch_key` selects the keyswitching mode (same layout as
    ...
    let (rerand_ksk_params, rerand_mode, zero_lwes_ptr) = if apply_rerand {
        ...
        let (rerand_ksk_params, rerand_mode) = match rerand_keyswitch_key {
            ...
            None => {
                // Without keyswitch, encryptions of zero already live at the radix block
                // dimension. Only input_lwe_dimension is read; the other fields are unused.
