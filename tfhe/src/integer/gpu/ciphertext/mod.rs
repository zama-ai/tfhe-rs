...
impl CudaUnsignedRadixCiphertext {
    ...
    pub unsafe fn duplicate_async(&self, stream: &CudaStream) -> Self {
        let lwe_ciphertext_count = self.ciphertext.d_blocks.lwe_ciphertext_count();
        let ciphertext_modulus = self.ciphertext.d_blocks.ciphertext_modulus();

        let mut d_ct = CudaVec::new_async(self.ciphertext.d_blocks.0.d_vec.len(), stream);
        d_ct.copy_from_gpu_async(&self.ciphertext.d_blocks.0.d_vec, stream);
        ...
        Self {
            ciphertext: CudaRadixCiphertext {
                d_blocks,
                info: self.ciphertext.info.clone(),
            },
        }
    }
    ...
    pub fn is_equal(&self, other: &Self, stream: &CudaStream) -> bool {
        let self_size = self.ciphertext.d_blocks.0.d_vec.len();
        let other_size = other.ciphertext.d_blocks.0.d_vec.len();
