...
impl CudaCompressionKey {
    ...
    pub fn compress_ciphertexts_into_list(
        ...
        (output_glwe, info, self.storage_log_modulus)
    }
}
...
impl CudaDecompressionKey {
    pub fn unpack(
        ...
        let compression_glwe_dimension = packed_list.0.glwe_dimension();
        let compression_polynomial_size = packed_list.0.polynomial_size();
        let lwe_ciphertext_count = LweCiphertextCount(end_block_index - start_block_index + 1);
        ...
        let storage_log_modulus = packed_list.2;
