...
/// - [CudaStreams::synchronize] __must__ be called after this function
/// as soon as synchronization is required
pub unsafe fn generate_last_block_inner_propagation_assign_async<T: UnsignedInteger, B: Numeric>(
    streams: &CudaStreams,
    result: &mut CudaVec<T>,
    lhs: &CudaVec<T>,
    rhs: &CudaVec<T>,
    ...
    let op_id: i8 = if op == SignedOperation::Addition {
        1
    } else {
        -1
    };
    scratch_cuda_generate_last_block_inner_propagation_kb_64_inplace(
        streams.ptr.as_ptr(),
        streams.gpu_indexes.as_ptr(),
        streams.len() as u32,
        op_id as i8,
    ...
    cuda_generate_last_block_inner_propagation_kb_64_inplace(
        streams.ptr.as_ptr(),
        streams.gpu_indexes.as_ptr(),
        streams.len() as u32,
        result.as_mut_c_ptr(0),
