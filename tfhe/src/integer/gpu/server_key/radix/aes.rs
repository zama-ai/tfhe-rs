...
mod test {
    ...
    fn test_gpu_sbox() {
        ...
        for &p in &samples {
            ...
            assert_eq!(y, S_BOX[p as usize], "SBox({:#04x})", p);
        }
    }
    ...
    fn z_test_gpu_encrypt_aes() {
        ...
        (0..number_of_outputs).into_par_iter().for_each(|i| {
            let streams = CudaStreams::new_single_gpu(GpuIndex::new(gpu_index));

            let ctr = iv.wrapping_add(i as u128);

            let p_bits = u128_to_bits(ctr);
            let ct_radix_cpu = encrypt_bits(&cks, &p_bits);
            ...
            if i < nist_expected_outputs.len() {
                let result_radix_cpu = d_state_to_encrypt.to_radix_ciphertext(&streams);
