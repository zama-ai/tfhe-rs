...
macro_rules! define_gpu_signed_scalar_comparison_test_functions {
    ($comparison_name:ident, $clear_type:ty) => {
        ::paste::paste!{
            fn [<integer_signed_unchecked_scalar_ $comparison_name _ $clear_type>]<P>(param: P) where P: Into<AtomicPatternParameters> {
                let num_tests = 1;
            ...
            fn [<integer_signed_default_scalar_ $comparison_name $clear_type>]<P>(param: P) where P: Into<AtomicPatternParameters> {
                let num_tests = 10;
