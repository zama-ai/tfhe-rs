...
impl CudaServerKey {
    ...
    pub(crate) fn convert_unsigned_radix_ciphertext_to_selectors(
        ...
        for i in 0..num_blocks {
            ...
            let mut slice_out = radix_ct
                ...
                .unwrap();
            unsafe { slice_out.copy_from_gpu_async(&slice_in, streams, 0) };
        ...
        Clear: Decomposable + CastInto<usize>,
    {
        ...
        let blocks_cmps =
            unsafe { self.apply_many_lookup_table_async(ct.as_ref(), &luts, streams) };
        ...
        possible_input_values
            .map(|input_value| {
                ...
                let mut d_ct_res: CudaUnsignedRadixCiphertext =
                    unsafe { self.create_trivial_zero_radix_async(num_blocks, streams) };
                ...
                for (block_index, block_value) in cmps.iter().enumerate() {
                    ...
                    let src_slice = copy_ct
                        ...
                        .as_mut_slice(block_index * lwe_size..lwe_size * (block_index + 1), 0)
                        .unwrap();
                    unsafe { dest_slice.copy_from_gpu_async(&src_slice, streams, 0) };
                }
            ...
            .map(|(selector, output_value)| {
                ...
                let blocks = decomposed_value
                    ...
                    .flat_map(|chunk_of_packed_value| {
                        ...
                        unsafe {
                            self.apply_many_lookup_table_async(
                                &selector.0.ciphertext,
                                &luts,
                                streams,
                            )
                        }
        ...
        T: CudaIntegerRadixCiphertext,
    {
                ...
                match &self.bootstrapping_key {
                    CudaBootstrappingKey::Classic(d_bsk) => {
                        apply_univariate_lut_kb_async(
                            streams,
                            &mut aggregated_mut_slice,
                            &aggregated_slice,
                            identity_lut.acc.as_ref(),
                            &d_bsk.d_vec,
        ...
        let mut message_ct: T =
            unsafe { self.create_trivial_zero_radix_async(num_ct_blocks, streams) };
        let mut message_mut_slice = message_ct
            .as_mut()
            .d_blocks
            .0
            .d_vec
        ...
        let mut carry_mut_slice = carry_ct
            .as_mut()
            .d_blocks
            .0
            .d_vec
        ...
        unsafe {
            ...
            match &self.bootstrapping_key {
                CudaBootstrappingKey::Classic(d_bsk) => {
                    apply_univariate_lut_kb_async(
                        streams,
                        &mut message_mut_slice,
                        &aggregated_slice,
                        message_extract_lut.acc.as_ref(),
                        &d_bsk.d_vec,
                        &self.key_switching_key.d_vec,
                        self.key_switching_key
                            .output_key_lwe_size()
        ...
        let mut output_ct: T =
            unsafe { self.create_trivial_zero_radix_async(num_ct_blocks * 2, streams) };

        // unpacked_blocks
        for index in 0..num_ct_blocks {
            let mut output_mut_slice1 = output_ct
                .as_mut()
                .d_blocks
                .0
                .d_vec
            ...
            unsafe { output_mut_slice1.copy_from_gpu_async(&message_mut_slice, streams, 0) };
        }
        for index in 0..num_ct_blocks {
            let mut output_mut_slice2 = output_ct
                .as_mut()
                .d_blocks
                .0
                .d_vec
            ...
            unsafe { output_mut_slice2.copy_from_gpu_async(&carry_mut_slice, streams, 0) };
        }
        streams.synchronize();
        output_ct.as_mut().info = output_ct.as_ref().info.after_aggregate_one_hot_vector();
        output_ct
    }
