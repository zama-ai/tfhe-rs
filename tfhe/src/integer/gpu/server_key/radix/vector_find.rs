...
impl CudaServerKey {
    ...
    pub fn unchecked_match_value<Clear, T>(
        &self,
        ct: &T,
    ...
    // /// - Input values are not required to span all possible values that `ct` could hold.
    // ///
    // /// - The output radix has a number of blocks that depends on the maximum possible output
    // value ///   from the `MatchValues`
    // ///
    // /// Returns a boolean block that encrypts `true` if the input `ct`
    // /// matched one of the possible inputs
    ...
    // /// - If the encrypted value is not in the encrypted slice, the returned index is 0
    // pub fn index_of_parallelized<T>(&self, cts: &[T], value: &T) -> (RadixCiphertext,
    // BooleanBlock) where
    //     T: IntegerRadixCiphertext,
    // {
    //     let mut tmp_cts;
    ...
    // /// Returns the encrypted index of the _first_ occurrence of encrypted `value` in the
    // ciphertext /// slice also, it returns an encrypted boolean that is `true` if the
    // encrypted value was /// found.
    // ///
    // /// # Notes
    // ///
    // /// - If the encrypted value is not in the clear slice, the returned index is 0
    ...
    fn compute_equality_selectors<T, Iter, Clear>(&self, ct: &T, possible_input_values: Iter) -> u32
        ...
        Clear: Decomposable + CastInto<usize>,
    {
        ...
        let blocks_cmps = ct
            .blocks()
            .par_iter()
            .map(|block| self.apply_many_lookup_table(block, &luts))
            .collect::<Vec<_>>();
        ...
        possible_input_values
            .map(|input_value| {
                ...
                CudaBooleanBlock::new_unchecked(self.are_all_comparisons_block_true(cmps))
            })
            .collect::<Vec<_>>();*/
    ...
    // ///
    // /// The elements of the resulting vector are zero if the corresponding BooleanBlock encrypted
    // 0, /// otherwise it encrypts the associated clear value.
    // ///
    ...
    //             let decomposed_value = BlockDecomposer::new(output_value, 2 *
    // num_bits_in_message)                 .take(num_blocks.div_ceil(2))
    //                 .collect::<Vec<_>>();
    ...
    //                     .blocks_mut()
    //                     .par_iter_mut()
    //                     .for_each(|block| self.key.apply_lookup_table_assign(block,
    // &identity_lut));                 result
    //             })
    ...
    // /// This function will return a Vec of Ciphertext where at most one encryption of 1 is
    // present ///
