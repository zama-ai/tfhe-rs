...
fn sanity_check_encrypt_dp_ks_pbs_gpu(meta_params: MetaParameters, filename_suffix: &str) {
    ...
    let mut results: Vec<(LweCiphertext<Vec<u64>>, LweCiphertext<Vec<u64>>)> = Vec::new();
    ...
    let res_cond = results.iter().fold(true, |acc, (lhs, rhs)| {
        acc && lhs.as_view() == rhs.as_view()
    });
...
fn noise_check_encrypt_dp_ks_ms_noise_gpu(meta_params: MetaParameters, filename_suffix: &str) {
    ...
    let (after_ms_is_ok, variance_cond, mean_cond) = mean_and_variance_check(
    ...
    let before_ms_normality_cond = before_ms_normality.null_hypothesis_is_valid;

    let pass = before_ms_normality_cond && after_ms_is_ok;

    let noise_check = TestResult::DpKsMsNoiseCheckResult(Box::new(DpKsMsNoiseCheckResult::new(
        variance_cond,
        mean_cond,
        before_ms_normality_cond,
    )));
