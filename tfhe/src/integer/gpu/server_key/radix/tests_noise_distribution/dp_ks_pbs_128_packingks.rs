...
use crate::integer::gpu::server_key::radix::tests_noise_distribution::utils::noise_simulation::CudaPackingKeyswitchKey128;
...
fn sanity_check_encrypt_dp_ks_standard_pbs128_packing_ks_gpu(meta_params: MetaParameters) {
    ...
    let cuda_packing_key = CudaPackingKeyswitchKey128::new(&cuda_noise_squashing_compression_key);
    ...
    P: Into<AtomicPatternParameters> + Copy,
{
    ...
    let cpu_squashed_radix_ct = noise_squashing_key
        ...
        .unwrap();
    let _cpu_squashed_noise_ct = cpu_squashed_radix_ct.packed_blocks()[0].lwe_ciphertext();
    ...
    let _after_pbs_ct = pbs_result.as_ct_128_cpu(&streams);

    let squashed_noise_ct_cpu = cuda_squashed_radix_ct.to_squashed_noise_radix_ciphertext(&streams);
    let _squashed_noise_ct = squashed_noise_ct_cpu.packed_blocks()[0].lwe_ciphertext();
    // It is not clear if they should be exactly equal
