...
use crate::shortint::parameters::{
    AtomicPatternParameters, NoiseSquashingCompressionParameters, Variance,
};
    ...
    P: Into<AtomicPatternParameters> + Copy,
{
    ...
    let noise_simulation_modulus_switch_config = noise_squashing_key
        .key
        .noise_simulation_modulus_switch_config();
    let drift_key: Option<&CudaServerKey> = match noise_simulation_modulus_switch_config {
        NoiseSimulationModulusSwitchConfig::Standard => None,
        NoiseSimulationModulusSwitchConfig::DriftTechniqueNoiseReduction => {
            panic!("Drift technique not supported for GPU anymore");
        }
...
fn noise_check_encrypt_dp_ks_standard_pbs128_packing_ks_noise_gpu<P>(
    params: P,
    noise_squashing_params: NoiseSquashingParameters,
    noise_squashing_compression_params: NoiseSquashingCompressionParameters,
) where
    P: Into<AtomicPatternParameters>,
{
    ...
    let noise_simulation_drift_key =
        NoiseSimulationDriftTechniqueKey::new_from_atomic_pattern_parameters(atomic_params);
    ...
    let noise_simulation_modulus_switch_config = noise_squashing_key
        .key
        .noise_simulation_modulus_switch_config();
    let _drift_key: Option<&CudaServerKey> = match noise_simulation_modulus_switch_config {
        NoiseSimulationModulusSwitchConfig::Standard => None,
        NoiseSimulationModulusSwitchConfig::DriftTechniqueNoiseReduction => {
            panic!("Drift technique not supported for GPU anymore");
        }
