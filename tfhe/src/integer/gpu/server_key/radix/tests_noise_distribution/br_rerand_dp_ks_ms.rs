...
use crate::shortint::server_key::tests::noise_distribution::utils::noise_simulation::{
    ...
    NoiseSimulationGlwe, NoiseSimulationLweFourierBsk,
};
...
use crate::integer::gpu::{CudaGlweCiphertextList, CudaServerKey, CudaStreams};
...
use crate::integer::gpu::{
    cuda_backend_rerand_assign, CudaLweCompactCiphertextList, CudaRadixCiphertext,
};
...
fn encrypt_decomp_br_rerand_dp_ks_any_ms_inner_helper_gpu(
    ...
    let (cpk_private_key, cpk, cuda_ksk_rerand, comp_private_key, cuda_decomp_key, cks, cuda_sks) =
        if should_use_single_key_debug() {
            ...
            (thread_cuda_compression_key, thread_cuda_decompression_key) = radix_cks
    ...
    let decomp_rescale_lut = cuda_decomp_key.rescaling_lut(
    ...
    let input = d_input.as_ct_64_cpu(&cuda_side_resources);
    let after_br = d_after_br.as_ct_64_cpu(&cuda_side_resources);
    let input_zero_rerand = d_input_zero_rerand.as_ct_64_cpu(&cuda_side_resources);
    let after_ksed_zero_rerand = d_after_ksed_zero_rerand.as_ct_64_cpu(&cuda_side_resources);
    let after_rerand = d_after_rerand.as_ct_64_cpu(&cuda_side_resources);
    let after_dp = d_after_dp.as_ct_64_cpu(&cuda_side_resources);
    let after_ks = d_after_ks.as_ct_64_cpu(&cuda_side_resources);
    let after_ms = d_after_ms.as_ct_64_cpu(&cuda_side_resources);
    let d_before_ms: &CudaDynLwe = d_after_drift.as_ref().unwrap_or(&d_after_ks);
    let before_ms = d_before_ms.as_ct_64_cpu(&cuda_side_resources);
...
fn noise_check_encrypt_br_rerand_dp_ks_ms_noise_gpu(meta_params: MetaParameters) {
    ...
    //assert!(noise_simulation_ksk_rerand.matches_actual_shortint_keyswitching_key(&
    // cuda_rerand_ksk));
    ...
    let decomp_rescale_lut = cuda_decompression_key.rescaling_lut(
        cuda_sks.ciphertext_modulus,
        cuda_sks.message_modulus,
        CarryModulus(1),
        cuda_sks.message_modulus,
...
create_parameterized_test!(noise_check_encrypt_br_rerand_dp_ks_ms_noise_gpu {
    TEST_META_PARAM_CPU_2_2_KS_PBS_PKE_TO_SMALL_ZKV2_TUNIFORM_2M128,
});

fn noise_check_encrypt_br_rerand_dp_ks_ms_pfail_gpu(meta_params: MetaParameters) {
    let (params, cpk_params, rerand_ksk_params, compression_params) = {
        let compute_params = meta_params.compute_parameters;
        let dedicated_cpk_params = meta_params.dedicated_compact_public_key_parameters.unwrap();
        // To avoid the expand logic of shortint which would force a keyswitch + LUT eval after
...
create_parameterized_test!(noise_check_encrypt_br_rerand_dp_ks_ms_pfail_gpu {
    TEST_META_PARAM_CPU_2_2_KS_PBS_PKE_TO_SMALL_ZKV2_TUNIFORM_2M128,
});

fn sanity_check_encrypt_br_rerand_dp_ks_ms_pbs_gpu(meta_params: MetaParameters) {
    let (params, cpk_params, rerand_ksk_params, compression_params) = {
        let compute_params = meta_params.compute_parameters;
        let dedicated_cpk_params = meta_params.dedicated_compact_public_key_parameters.unwrap();
        // To avoid the expand logic of shortint which would force a keyswitch + LUT eval after
    ...
    for idx in 0..10 {
        ...
        let _shortint_rerand_seed = ReRandomizationSeed(rerand_xof_seed);
        ...
        let shortint_res = cuda_shortint_after_pbs
            .d_blocks
            .to_lwe_ciphertext_list(&cuda_side_resources.streams);
...
create_parameterized_test!(sanity_check_encrypt_br_rerand_dp_ks_ms_pbs_gpu {
    TEST_META_PARAM_CPU_2_2_KS_PBS_PKE_TO_SMALL_ZKV2_TUNIFORM_2M128,
});
