...
pub struct CudaPrinceKeys {
    // Key halves in circuit order: (k0, k1) to encrypt, (k1, k0) to decrypt.
    // The swap happens once here, so nothing downstream reorders them again.
    k_first: CudaUnsignedRadixCiphertext,        // NUM_U2_BLOCKS
    k_second: CudaUnsignedRadixCiphertext,       // NUM_U2_BLOCKS
...
impl CudaServerKey {
    ...
    fn prince_init(
        ...
        let (k_first, k_second) = if is_decrypt { (k1, k0) } else { (k0, k1) };

        let mut keys = CudaPrinceKeys {
            k_first: k_first.duplicate(streams),
            k_second: k_second.duplicate(streams),
