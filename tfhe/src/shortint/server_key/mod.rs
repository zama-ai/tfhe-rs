...
use super::atomic_pattern::{
    AtomicPatternMutOperations, AtomicPatternOperations, AtomicPatternParameters,
    ClassicalAtomicPatternServerKey, ServerKeyAtomicPattern,
};
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, NotVersioned)]
...
pub type ClassicalServerKey = GenericServerKey<ClassicalAtomicPatternServerKey>;
pub type ServerKeyView<'key> = GenericServerKey<&'key ServerKeyAtomicPattern>;
pub type ClassicalServerKeyView<'key> = GenericServerKey<&'key ClassicalAtomicPatternServerKey>;
...
impl Copy for ClassicalServerKeyView<'_> {}

impl From<ClassicalServerKey> for ServerKey {
    fn from(value: ClassicalServerKey) -> Self {
        let atomic_pattern = ServerKeyAtomicPattern::Classical(value.atomic_pattern);
...
impl TryFrom<ServerKey> for ClassicalServerKey {
...
impl<'key> TryFrom<ServerKeyView<'key>> for ClassicalServerKeyView<'key> {
    type Error = UnsupportedOperation;
...
impl<AP: AtomicPatternOperations> GenericServerKey<AP> {
    ...
    pub fn from_raw_parts(
        ...
        assert_eq!(
            atomic_pattern_key.ciphertext_modulus(),
            ciphertext_modulus,
            "Mismatch between the atomic pattern CiphertextModulus ({:?}) \
    ...
    pub fn conformance_params(&self) -> CiphertextConformanceParams {
        ...
        CiphertextConformanceParams {
            ...
            atomic_pattern: self.atomic_pattern.atomic_pattern(),
    ...
    pub(crate) fn unchecked_create_trivial_with_lwe_size(
        ...
        unchecked_create_trivial_with_lwe_size(
            ...
            self.message_modulus,
            self.carry_modulus,
            self.atomic_pattern.atomic_pattern(),
            self.ciphertext_modulus,
        )
    }
...
impl<AP: AtomicPatternMutOperations> GenericServerKey<AP> {
...
pub fn generate_lookup_table_no_encode<F>(
