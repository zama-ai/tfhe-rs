...
use crate::core_crypto::prelude::{
    blind_rotate_assign, switch_lwe_modulus, CastFrom, CastInto,
...
where
    Scalar: UnsignedInteger + CastInto<u64> + CastFrom<usize>,
{
    match bootstrapping_key {
        ShortintBootstrappingKey::Classic {
            ...
            let compressed = modulus_switch_noise_reduction_key
                .as_ref()
                .map_or_else(
                    || {
                        let ciphertext = LweCiphertext::from_container(
                            ciphertext.as_ref().to_vec(),
                            ciphertext.ciphertext_modulus(),
                        );

                        switch_lwe_modulus::<_, u64, _>(ciphertext, log_modulus)
                    },
                    |modulus_switch_noise_reduction_key| {
                        modulus_switch_noise_reduction_key
                            .switch_modulus::<_, u64>(&ciphertext, log_modulus)
                    },
                )
                .pack::<u64>();
