use super::dp_ks_ms::{dp_ks_any_ms, dp_ks_drift_ms};
        ...
        >,
{
    ...
    let (pbs_result, after_dp, ks_result, drift_technique_result, ms_result) = dp_ks_drift_ms(
        pbs_result,
        scalar,
        ksk,
    ...
    P: Into<AtomicPatternParameters>,
{
    ...
    for _ in 0..10 {
        ...
        let mut shortint_res = Ciphertext::new(
            input_pbs_result.into_lwe_64().unwrap(),
            id_lut.degree,
            NoiseLevel::NOMINAL,
            sks.message_modulus,
            sks.carry_modulus,
        ...
        assert_eq!(
            output_pbs_result.into_lwe_64().unwrap().as_view(),
...
fn encrypt_br_dp_ks_ms_inner_helper(
    ...
    DecryptionAndNoiseResult,
    DecryptionAndNoiseResult,
    Option<DecryptionAndNoiseResult>,
    DecryptionAndNoiseResult,
) {
    ...
    match &cks.atomic_pattern {
        AtomicPatternClientKey::Standard(standard_atomic_pattern_client_key) => (
            DecryptionAndNoiseResult::new_from_lwe(
                &input.into_lwe_64().unwrap(),
                &standard_atomic_pattern_client_key.small_lwe_secret_key(),
                msg,
                &output_encoding,
            ),
            DecryptionAndNoiseResult::new_from_lwe(
                &after_br.into_lwe_64().unwrap(),
                &standard_atomic_pattern_client_key.large_lwe_secret_key(),
                msg,
                &output_encoding,
            ),
            DecryptionAndNoiseResult::new_from_lwe(
                &after_dp.into_lwe_64().unwrap(),
                &standard_atomic_pattern_client_key.large_lwe_secret_key(),
                msg,
                &output_encoding,
            ),
            DecryptionAndNoiseResult::new_from_lwe(
                &after_ks.into_lwe_64().unwrap(),
                &standard_atomic_pattern_client_key.small_lwe_secret_key(),
                msg,
                &output_encoding,
            ),
            after_drift.map(|after_drift| {
                DecryptionAndNoiseResult::new_from_lwe(
                    &after_drift.into_lwe_64().unwrap(),
                    &standard_atomic_pattern_client_key.small_lwe_secret_key(),
                    msg,
                    &output_encoding,
                )
