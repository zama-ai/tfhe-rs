...
pub fn pfail_check(
    ...
    let (confidence_interval, confidence_interval_with_log2) = if measured_fails <= 0.0 {
        (
            ConfidenceInterval::NoBounds(NoBounds::new(
                "Unable to compute bounds, 0 fails measured",
            )),
            ConfidenceIntervalWithLog2::NoBounds(NoBounds::new(
                "Unable to compute bounds, 0 fails measured",
            )),
        )
    } else {
        (
            ...
            ConfidenceIntervalWithLog2::Bounded(
    ...
    let pfail_serialized = BoundedMeasurement::new(
        measured_pfail.to_string(),
        expected_pfail.to_string(),
        confidence_interval,
    );

    let pfail_original_serialized = BoundedLog2Measurement::new(
        ValueWithLog2::new(
    ...
    let write_json = |warning: Option<String>, pass: bool| {
        write_to_json_file(
            param_name,
            test_name,
            test_module_path,
            ...
            PfailTestResultJson::new(
                ...
                pfail_serialized.clone(),
                pfail_original_serialized.clone(),
            )
            .into_test_result(),
        )
        .unwrap();
    ...
    if measured_fails > 0.0 {
        ...
        write_json(Some(format_json_warning_message(warning_raw)), true);
        println!("{warning}");
    }
}
