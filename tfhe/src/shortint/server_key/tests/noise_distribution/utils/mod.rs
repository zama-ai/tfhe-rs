...
pub fn mean_and_variance_check<Scalar: UnsignedInteger>(
    ...
    let variance_cond = BoundedMeasurement::new(
    ...
    let mean_cond = BoundedMeasurement::new(
    ...
    pub fn generate_serializable_data(self) -> PfailMetadata {
        PfailMetadata::new(
            ValueWithLog2::new(
                ...
                Some(self.original_pfail_and_precision.pfail().log2().to_string()),
            ),
            ValueWithLog2::new(
                self.new_pfail_and_precision.pfail().to_string(),
                Some(self.new_pfail_and_precision.pfail().log2().to_string()),
            ),
...
pub fn pfail_check(
    ...
    let fails_serialized = Measurement {
        measured: measured_fails.to_string(),
        expected: expected_fails.to_string(),
    };
    ...
    let mut pfail_original_serialized = BoundedLog2Measurement::new_no_bounds(
        ValueWithLog2::new(
            ...
            Some(equivalent_measured_pfail.log2().to_string()),
        ),
        ValueWithLog2::new(
            original_pfail.to_string(),
            Some(original_pfail.log2().to_string()),
        ),
    );
    ...
    if measured_fails > 0.0 {
        ...
        pfail_original_serialized.set_bounds(
            ValueWithLog2::new(
                ...
                Some(equivalent_pfail_upper_bound.log2().to_string()),
            ),
            ValueWithLog2::new(
                equivalent_pfail_lower_bound.to_string(),
                Some(equivalent_pfail_lower_bound.log2().to_string()),
            ),
        );
