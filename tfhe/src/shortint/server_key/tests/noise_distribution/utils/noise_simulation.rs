...
use crate::core_crypto::commons::parameters::{
    ...
    GlweDimension, GlweSize, LweCiphertextCount, LweDimension, PolynomialSize,
};
...
impl NoiseSimulationModulus {
    pub fn from_ciphertext_modulus<Scalar: UnsignedInteger>(
        ...
        if modulus_scalar_bits > 128 {
            panic!("Unsupported bit width: {}", modulus_scalar_bits);
        }
        if modulus_scalar_bits == 128 {
    ...
    pub fn from_ciphertext_modulus_log(modulus_log: CiphertextModulusLog) -> Self {
        if modulus_log.0 > 128 {
            panic!("Unsupported bit width: {}", modulus_log.0);
        }
        if modulus_log.0 == 128 {
            Self::NativeU128
        } else {
    ...
    pub fn as_f64(&self) -> f64 {
        match self {
            NoiseSimulationModulus::NativeU128 => 2.0f64.powi(128),
            NoiseSimulationModulus::Other(val) => *val as f64,
        }
    }
}
...
impl AllocateClassicPBSModSwitchResult for NoiseSimulationLwe {
    type Output = Self;
    type SideResources = ();
...
impl ClassicPBSModSwitch<NoiseSimulationLwe> for NoiseSimulationLwe {
    type SideResources = ();
...
impl NoiseSimulationGlwe {
    pub fn new(
        glwe_dimension: GlweDimension,
        polynomial_size: PolynomialSize,
        variance_per_slot: Variance,
        modulus: NoiseSimulationModulus,
    ) -> Self {
        Self {
            glwe_dimension,
            polynomial_size,
            variance_per_occupied_slot: variance_per_slot,
            modulus,
        }
    }
    ...
    pub fn into_lwe(self) -> NoiseSimulationLwe {
        ...
        NoiseSimulationLwe {
            lwe_dimension,
            variance: self.variance_per_slot(),
            modulus: self.modulus(),
        }
    }
    ...
    pub fn variance_per_slot(&self) -> Variance {
...
#[derive(Clone, Copy)]
pub struct NoiseSimulationLweClassicBsk128 {
    input_lwe_dimension: LweDimension,
...
impl NoiseSimulationLweClassicBsk128 {
    // We can't really build a key from an already generated key as we need to know what the noise
    // distribution is.
...
impl ClassicFft128Bootstrap<NoiseSimulationLwe, NoiseSimulationLwe, NoiseSimulationGlwe>
    for NoiseSimulationLweClassicBsk128
{
    type SideResources = ();
