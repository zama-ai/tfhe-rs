...
use crate::core_crypto::commons::math::random::{
    ByteRandomGenerator, RandomGenerable, TUniform, UniformBinary, XofSeed,
};
use crate::core_crypto::commons::parameters::{
    CiphertextModulusLog, DynamicDistribution, LweCiphertextCount, PlaintextCount,
};
use crate::core_crypto::commons::traits::{
    Container, ContainerMut, ContiguousEntityContainer, SignedInteger, UnsignedInteger,
};
...
// Binary random draws bits from the bytes, we want all ones, so fill with u8::MAX
// It turns out that for TUniform, e.g. with b == 8 (max value == 2^8) if all input bits are
// 1 you obtain the max too:
// tuniform draws b + 2 bits to generate values with a bound b
// initial_value = 0b11_1111_1111 (truncated from the random bytes generated by the RNG)
// bp1 is the lowest bit == 1
// tmp = initial_value >> 1 == 0b01_1111_1111
// tmp = tmp + bp1 == 0b10_0000_0000
// res = tmp - (1 << b) == 0b01_0000_0000 == 2^b == 1 << b == max value
// So this struct just needs to return u8::MAX at all times for the test
struct WorstCaseCpkByteGenerator;

struct WorstCaseCpkByteGeneratorChildrenIterator;

impl Iterator for WorstCaseCpkByteGeneratorChildrenIterator {
    type Item = WorstCaseCpkByteGenerator;

    fn next(&mut self) -> Option<Self::Item> {
        Some(WorstCaseCpkByteGenerator)
    }
}

impl ByteRandomGenerator for WorstCaseCpkByteGenerator {
    type ChildrenIter = WorstCaseCpkByteGeneratorChildrenIterator;

    fn new(_params: impl Into<tfhe_csprng::generators::aes_ctr::AesCtrParams>) -> Self {
        Self
    }

    fn remaining_bytes(&self) -> tfhe_csprng::generators::ByteCount {
        tfhe_csprng::generators::ByteCount(u128::MAX)
    }

    fn next_table_index(&self) -> Option<tfhe_csprng::generators::aes_ctr::TableIndex> {
        unimplemented!("The dummy generator does not need next_table_index")
    }

    fn try_fork(
        &mut self,
        _n_children: tfhe_csprng::generators::ChildrenCount,
        _n_bytes: tfhe_csprng::generators::BytesPerChild,
    ) -> Result<Self::ChildrenIter, tfhe_csprng::generators::ForkError> {
        unimplemented!("The dummy generator does not need forking")
    }
}

impl Iterator for WorstCaseCpkByteGenerator {
    type Item = u8;
...
>(
    ...
    mask_and_body_noise_distribution: TUniform<Scalar>,
) where
    Scalar: UnsignedInteger
        + RandomGenerable<TUniform<Scalar>, CustomModulus = Scalar>
        + RandomGenerable<UniformBinary, CustomModulus = Scalar>,
    ...
    OutputCont: ContainerMut<Element = Scalar>,
{
    ...
    use crate::core_crypto::algorithms::slice_algorithms::slice_semi_reverse_negacyclic_convolution;

    let tuniform_max_value = mask_and_body_noise_distribution
        .max_value_inclusive()
        .into_unsigned();

    let mut dummy_seeder = new_seeder();
    let mut noise_generator =
        NoiseRandomGenerator::<WorstCaseCpkByteGenerator>::new(dummy_seeder.as_mut());
    ...
    let CompactPublicKeyRandomVectors {
        ...
        output,
        encoded,
        mask_and_body_noise_distribution,
        mask_and_body_noise_distribution,
        &mut noise_generator,
        slice_semi_reverse_negacyclic_convolution,
    );

    assert!(binary_random_vector.into_iter().all(|x| x == Scalar::ONE));
    assert!(mask_noise.into_iter().all(|x| x == tuniform_max_value));
    assert!(body_noise.into_iter().all(|x| x == tuniform_max_value));
}
...
fn sanity_check_encrypt_cpk_rerand_ks_any_ms_worst_case(
    ...
    let tuniform_distribution = match cpk_params.encryption_noise_distribution {
        DynamicDistribution::Gaussian(_) => {
            panic!("This test only supports TUniform noise distributions")
        }
        DynamicDistribution::TUniform(tuniform) => tuniform,
    };
