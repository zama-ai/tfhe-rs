...
fn sanity_check_encrypt_dp_ks_standard_pbs128_packing_ks(
    ...
    let after_packing_cond = after_packing.as_view() == extracted.as_view();
...
fn noise_check_encrypt_dp_ks_standard_pbs128_packing_ks_noise(
    ...
    let (after_packing_is_ok, variance_cond, mean_cond) = mean_and_variance_check(
    ...
    let noise_check = TestResult::DpKsPackingNoiseCheckResult(Box::new(
        super::utils::to_json::DpKsPackingNoiseCheckResult::new(variance_cond, mean_cond),
    ));
