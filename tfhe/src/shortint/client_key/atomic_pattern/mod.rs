...
pub trait EncryptionAtomicPattern {
    ...
    /// The secret key used to generate a KeySwitchingKey for these parameters
    fn keyswitch_encryption_key(
        &self,
        params: ShortintKeySwitchingParameters,
    ) -> LweSecretKeyView<'_, u64>;

    /// The noise distribution used to generate a KeySwitchingKey for these parameters
    fn keyswitch_encryption_noise(
        &self,
        params: ShortintKeySwitchingParameters,
    ) -> DynamicDistribution<u64>;

    fn keyswitch_encryption_key_and_noise(
        &self,
        params: ShortintKeySwitchingParameters,
    ) -> (LweSecretKeyView<'_, u64>, DynamicDistribution<u64>) {
        (
            self.keyswitch_encryption_key(params),
            self.keyswitch_encryption_noise(params),
        )
    }
}
...
impl EncryptionAtomicPattern for AtomicPatternClientKey {
    ...
    fn keyswitch_encryption_key(
        ...
        match self {
            Self::Standard(ap_cks) => ap_cks.keyswitch_encryption_key(params),
            Self::KeySwitch32(ap_cks) => ap_cks.keyswitch_encryption_key(params),
        }
    }
    ...
    fn keyswitch_encryption_noise(
        ...
        match self {
            Self::Standard(ap_cks) => ap_cks.keyswitch_encryption_noise(params),
            Self::KeySwitch32(ap_cks) => ap_cks.keyswitch_encryption_noise(params),
        }
    }
