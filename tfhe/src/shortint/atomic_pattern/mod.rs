...
pub enum AtomicPatternKind {
    ...
    Standard(PBSOrder),
    /// Similar to the standard AP, but the KeySwitch changes the ciphertext modulus to 2^32
    ///
...
impl AtomicPattern for AtomicPatternServerKey {
    ...
    fn prepare_for_noise_squashing(&self, ct: &Ciphertext) -> LweCiphertextOwned<u64> {
        match self {
            Self::Standard(ap) => ap.prepare_for_noise_squashing(ct),
...
#[cfg(test)]
mod test {
    use crate::shortint::parameters::v1_1::V1_1_PARAM_MESSAGE_2_CARRY_2_KS32_PBS_TUNIFORM_2M128;
    ...
    #[test]
    fn test_ks32_as_dyn_ap() {
        let (client_key, server_key) =
            gen_keys(V1_1_PARAM_MESSAGE_2_CARRY_2_KS32_PBS_TUNIFORM_2M128);
