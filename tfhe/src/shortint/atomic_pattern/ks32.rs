...
impl ParameterSetConformant for KS32AtomicPatternServerKey {
    ...
    fn is_conformant(&self, parameter_set: &Self::ParameterSet) -> bool {
        ...
        let pbs_params: PBSConformanceParams = parameter_set.into();

        let pbs_key_ok = bootstrapping_key.is_conformant(&pbs_params);

        let ksk_param: LweKeyswitchKeyConformanceParams<u32> = parameter_set.into();

        let ks_key_ok = key_switching_key.is_conformant(&ksk_param);

        *ciphertext_modulus == pbs_params.ciphertext_modulus && pbs_key_ok && ks_key_ok
    }
}
