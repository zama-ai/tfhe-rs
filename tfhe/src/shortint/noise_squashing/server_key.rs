...
use crate::core_crypto::prelude::switch_lwe_modulus;
...
impl NoiseSquashingKey {
    ...
    pub fn unchecked_squash_ciphertext_noise(
        ...
        let lwe_ciphertext_to_squash_noise = match &self.modulus_switch_noise_reduction_key {
            Some(key) => {
                key.switch_modulus(&lwe_before_noise_squashing.as_view(), br_input_modulus_log)
            }
            None => switch_lwe_modulus(lwe_before_noise_squashing, br_input_modulus_log),
        };
