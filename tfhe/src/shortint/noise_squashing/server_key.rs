...
impl From<(PBSParameters, NoiseSquashingParameters)>
...
impl From<(KeySwitch32PBSParameters, NoiseSquashingParameters)>
    for Shortint128BootstrappingKeyConformanceParams
{
    fn from(
    ...
    fn try_from(
        ...
        let noise_squashing_pbs_params = match ap_params {
            AtomicPatternParameters::Standard(pbs_params) => {
                (pbs_params, noise_squashing_params).into()
            }
            AtomicPatternParameters::KeySwitch32(ks32_params) => {
                (ks32_params, noise_squashing_params).into()
            }
        };
