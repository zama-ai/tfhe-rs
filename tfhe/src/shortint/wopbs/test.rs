...
use crate::shortint::{gen_keys, ParametersBig};
use paste::paste;
use rand::Rng;
...
fn generate_lut(params: (ParametersBig, ParametersBig)) {
...
fn generate_lut_modulus(params: (ParametersBig, ParametersBig)) {
    let keys = KEY_CACHE_WOPBS.get_from_param(params);
    let (cks, sks, wopbs_key) = (keys.client_key(), keys.server_key(), keys.wopbs_key());
    let mut rng = rand::thread_rng();
...
fn generate_lut_modulus_not_power_of_two(params: (ParametersBig, ParametersBig)) {
