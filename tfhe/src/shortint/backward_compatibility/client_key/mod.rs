...
impl<AP: Clone + 'static> Upgrade<GenericClientKey<AP>> for ClientKeyV0 {
    type Error = Error;
    ...
    fn upgrade(self) -> Result<GenericClientKey<AP>, Self::Error> {
        ...
        if TypeId::of::<AP>() == TypeId::of::<AtomicPatternClientKey>() {
            ...
            let ck = ClientKey { atomic_pattern };
            Ok((&ck as &dyn Any)
                .downcast_ref::<GenericClientKey<AP>>()
