    ...
    for Shortint128BootstrappingKeyV0
{
    ...
    fn upgrade(self) -> Result<Shortint128BootstrappingKey<Scalar>, Self::Error> {
        if TypeId::of::<Self>() == TypeId::of::<Shortint128BootstrappingKey<Scalar>>() {
            let bsk = Box::new(self) as Box<dyn Any>;
            Ok(*bsk.downcast().unwrap())
        } else {
            Err(Error::new(
    ...
    for CompressedShortint128BootstrappingKeyV0
{
    ...
    fn upgrade(self) -> Result<CompressedShortint128BootstrappingKey<Scalar>, Self::Error> {
        if TypeId::of::<Self>() == TypeId::of::<CompressedShortint128BootstrappingKey<Scalar>>() {
            let bsk = Box::new(self) as Box<dyn Any>;
            Ok(*bsk.downcast().unwrap())
        } else {
            Err(Error::new(
