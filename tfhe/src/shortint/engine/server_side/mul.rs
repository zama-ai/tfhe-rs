...
impl ShortintEngine {
    ...
    pub(crate) fn bc_unchecked_mul_lsb_assign(
        &mut self,
        server_key: &ServerKey,
        ct_left: &mut Ciphertext,
        ct_right: &Ciphertext,
    ...
    pub(crate) fn bc_unchecked_mul_lsb_small_carry_modulus(
        &mut self,
        server_key: &ServerKey,
        ct1: &mut Ciphertext,
        ct2: &mut Ciphertext,
    ...
    pub(crate) fn bc_unchecked_mul_lsb_small_carry_modulus_assign(
        &mut self,
        server_key: &ServerKey,
        ct1: &mut Ciphertext,
        ct2: &mut Ciphertext,
    ) -> EngineResult<()> {
        *ct1 = self.bc_unchecked_mul_lsb_small_carry_modulus(server_key, ct1, ct2)?;
        Ok(())
    }
    ...
    pub(crate) fn bc_smart_mul_lsb(
        ...
        let mut result = ct_left.clone();
        self.bc_smart_mul_lsb_assign(server_key, &mut result, ct_right)?;
        Ok(result)
    }

    pub(crate) fn bc_smart_mul_lsb_assign(
        &mut self,
        server_key: &ServerKey,
        ct_left: &mut Ciphertext,
        ct_right: &mut Ciphertext,
        ...
        if ct_left.message_modulus.0 > ct_left.carry_modulus.0 {
            ...
            self.bc_unchecked_mul_lsb_small_carry_modulus_assign(server_key, ct_left, ct_right)?;
        } else {
            //If the ciphertext cannot be added together without exceeding the capacity of a
            // ciphertext
            if !server_key.is_mul_possible(ct_left, ct_right) {
            ...
            self.bc_unchecked_mul_lsb_assign(server_key, ct_left, ct_right)?;
        }
        Ok(())
    }
