...
impl ShortintEngine {
    ...
    pub(crate) fn encrypt_many_with_compressed_public_key(
        &mut self,
        public_key: &CompressedPublicKey,
        messages: impl Iterator<Item = u64>,
    ) -> Vec<Ciphertext> {
        self.encrypt_many_with_message_modulus_and_compressed_public_key(
            public_key,
            messages,
            public_key.parameters.message_modulus(),
        )
    ...
    pub(crate) fn encrypt_many_with_message_modulus_and_compressed_public_key(
        &mut self,
        public_key: &CompressedPublicKey,
        messages: impl Iterator<Item = u64>,
        message_modulus: MessageModulus,
    ...
    pub(crate) fn encrypt_without_padding_with_compressed_public_key(
        ...
        self.encrypt_many_without_padding_with_compressed_public_key(
            public_key,
            std::iter::once(message),
        )
        .into_iter()
    ...
    pub(crate) fn encrypt_many_without_padding_with_compressed_public_key(
        &mut self,
        public_key: &CompressedPublicKey,
        messages: impl Iterator<Item = u64>,
    ) -> Vec<Ciphertext> {
