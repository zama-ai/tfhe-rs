...
impl CompactPublicKey {
    ...
    pub fn re_randomize_compact_ciphertext_lists<'a>(
        ...
        if !self.key.ciphertext_modulus().is_power_of_two() {
            return Err(crate::error!(
                "Compact lists re-randomization only supports power of 2 moduli",
            ));
        }
        ...
        for list in compact_lists {
            ...
            if self.key.ciphertext_modulus() != list.ct_list.ciphertext_modulus() {
                return Err(crate::error!(
                    "Mismatched CiphertextModulus between Compact Lists \
                    being re-randomized and provided CompactPublicKey",
                ));
            }
