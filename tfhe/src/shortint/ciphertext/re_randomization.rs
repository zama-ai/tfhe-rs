...
impl CompactPublicKey {
    ...
    ///
    /// The `range` is in unpacked block coordinates. If the list is packed, the range is
    /// automatically mapped to packed coordinates.
    pub fn re_randomize_compact_ciphertext_lists<'a>(
        ...
        for list in compact_lists {
            if key_lwe_size != list.ct_list.lwe_size() {
                let err = "Mismatched LweSize between Compact Lists \
                    being re-randomized and provided CompactPublicKey";
                return Err(crate::error!("{}", err));
            }
            ...
            slice_wrapping_add_assign(list.ct_list.as_mut(), encryption_of_zero.as_ref());
        }
...
mod test {
    ...
    fn test_rerand_compact_list_ci_run_filter() {
        ...
        re_rand_context.add_ciphertext_data_slice(enc[0].ct_list.as_ref());
        re_rand_context.add_bytes(&nonce);
        re_rand_context.add_bytes(b"expand");

        let mut seeder = re_rand_context.finalize();

        pubk.re_randomize_compact_ciphertext_lists(enc.iter_mut(), seeder.next_seed())
            .unwrap();

        let cast = enc[0].expand(casting_mode).unwrap();
