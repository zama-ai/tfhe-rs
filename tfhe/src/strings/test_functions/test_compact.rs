...
fn test_compact_list_string_last_block_sanitization_impl(fhe_params: ClassicPBSParameters) {
    ...
    // Digits have their bit 6 cleared. When the last block holds meaningful bits besides the
    // padding bit 7, setting all its bits changes bit 6 of the char, so a sanitization that
    // clears the whole block instead of just bit 7 would be detected. With 1 bit blocks the last
    // block only holds bit 7 and the string must come back unchanged.
    let string = "0123";
    ...
    // For blocks wider than 1 bit, the tampering must be visible after sanitization, otherwise
    // this test could not tell a correct LUT from one that clears the whole last block
    if bits_per_block > 1 {
        assert_ne!(expected, string);
    }
