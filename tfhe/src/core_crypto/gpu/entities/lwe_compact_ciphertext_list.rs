...
use crate::core_crypto::prelude::{lwe_compact_ciphertext_list_size, LweCompactCiphertextList, UnsignedInteger};
use crate::core_crypto::prelude::Container;

#[derive(Debug)]
pub struct CudaLweCompactCiphertextList<T: UnsignedInteger>(pub(crate) CudaLweList<T>);
...
impl<T: UnsignedInteger> CudaLweCompactCiphertextList<T> {
    ...
    pub unsafe fn from_lwe_compact_ciphertext_list_async<C: Container<Element = T>>(
        ...
        assert!(
            h_input.container_len() == expected_container_len,
    ...
    pub fn to_lwe_compact_ciphertext_list(&self, streams:
    &CudaStreams) ->
        ...
        unsafe {
            self.0.d_vec.copy_to_cpu_async(container.as_mut_slice(), streams, 0);
        }
        streams.synchronize();

        LweCompactCiphertextList::from_container(container, lwe_dimension.to_lwe_size(), lwe_ciphertext_count, ciphertext_modulus)
    }
}