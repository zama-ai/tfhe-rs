...
impl<T: UnsignedInteger> CudaLweCiphertextList<T> {
    ...
    pub fn from_lwe_ciphertext_list<C: Container<Element = T>>(
        ...
        unsafe {
            d_vec.copy_from_cpu_async(h_input.as_ref(), streams);
        }
        streams.synchronize();
        let cuda_lwe_list = CudaLweList {
            d_vec,
    ...
    pub fn to_lwe_ciphertext_list(&self, streams: &CudaStreams) -> LweCiphertextList<Vec<T>> {
        ...
        unsafe {
            self.0
                .d_vec
                .copy_to_cpu_async(container.as_mut_slice(), streams);
        }
        streams.synchronize();
    ...
    pub fn from_lwe_ciphertext<C: Container<Element = T>>(
        ...
        unsafe {
            d_vec.copy_from_cpu_async(h_ct.as_ref(), streams);
        }
        streams.synchronize();
    ...
    pub fn duplicate(&self, streams: &CudaStreams) -> Self {
        ...
        // Copy to the GPU
        let mut d_vec = CudaVec::new(self.0.d_vec.len(), streams);
        unsafe {
            d_vec.copy_from_gpu_async(&self.0.d_vec, streams);
        }
        streams.synchronize();
