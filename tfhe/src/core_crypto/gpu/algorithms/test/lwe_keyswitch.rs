...
fn test_round_to_closest_representable_gpu() {
    ...
    for _ in 0..runs_per_decomposer {
        ...
        let base_log_v = any_uint::<u64>() % (size_of::<u64>() as u64 / 2);
        let base_log = DecompositionBaseLog(base_log_v as usize);

        let level_count_v = any_uint::<u64>() % 6;
        let level_count = DecompositionLevelCount(level_count_v as usize);

        let decomposer = SignedDecomposer::new(base_log, level_count);

        let rounded = test_util_closest_representable_on_gpu(val, base_log, level_count);

        let epsilon = (1u64 << (64 - (decomposer.base_log * decomposer.level_count) - 1)) / 2u64;
