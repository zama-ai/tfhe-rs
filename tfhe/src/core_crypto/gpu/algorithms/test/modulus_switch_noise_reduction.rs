...
use tfhe_cuda_backend::bindings::{
    cuda_improve_noise_modulus_switch_inplace_64, cuda_modulus_switch_inplace_64,
};
...
//     assert!(
//         check_both_ratio_under(measured_individual_check_p_success,
// expected_individual_check_p_success, 1.1_f64),         "individual_check_p_success: measured
// (={measured_individual_check_p_success}) too far from expected
// (={expected_individual_check_p_success})",     );
...
//     let mut lwe = LweCiphertextOwned::new(0, sk.lwe_dimension().to_lwe_size(),
// ciphertext_modulus);
...
//         "average_number_checks: measured (={average_number_checks}) too far from expected
...
fn round_mask_gpu(
    ...
    let shift_to_map_to_native = u64::BITS as u32 - log_modulus.0 as u32;
...
fn check_noise_improve_modulus_switch_noise(
    ...
    let num_streams = 16;
    let vec_streams = (0..num_streams).map(|i| CudaStreams::new_single_gpu(GpuIndex::new(gpu_index))).collect::<Vec<_>>();
