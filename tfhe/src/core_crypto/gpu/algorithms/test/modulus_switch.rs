...
use tfhe_cuda_backend::bindings::{
    cuda_centered_modulus_switch_inplace_64, cuda_modulus_switch_inplace_64,
};
...
fn check_centered_modulus_switch_is_centered() {
    ...
    check_cuda_modulus_switch_is_centered(ModulusSwitchAlgorithm::REGULAR, number_loops, max_ratio);

    check_cuda_modulus_switch_is_centered(
        ModulusSwitchAlgorithm::CENTERED,
        number_loops,
        max_ratio,
    );
...
fn check_cuda_modulus_switch_is_centered(
    ...
    let (lut_application_left_error, lut_application_right_error): (Vec<bool>, Vec<bool>) = (0
        ...
        .map(|i| {
            ...
            let mut d_lwe = CudaLweCiphertextList::from_lwe_ciphertext(&lwe, &local_stream);
            unsafe {
                match ms {
                    ModulusSwitchAlgorithm::REGULAR => cuda_modulus_switch_inplace_64(
                        local_stream.ptr[0],
                        local_stream.gpu_indexes[0].get(),
                        d_lwe.0.d_vec.as_mut_c_ptr(0),
                        d_lwe.lwe_dimension().to_lwe_size().0 as u32,
                        log_modulus.0 as u32,
                    ),
                    ModulusSwitchAlgorithm::CENTERED => cuda_centered_modulus_switch_inplace_64(
                        local_stream.ptr[0],
                        local_stream.gpu_indexes[0].get(),
                        d_lwe.0.d_vec.as_mut_c_ptr(0),
                        d_lwe.lwe_dimension().0 as u32,
                        log_modulus.0 as u32,
                    ),
                }
            }
