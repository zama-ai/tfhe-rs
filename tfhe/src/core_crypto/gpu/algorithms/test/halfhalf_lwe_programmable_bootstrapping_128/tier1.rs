...
/// At `u128` the split-limb accumulation sits near the `fft128` mantissa limit, and the deviation
/// between accumulation orders is a function of the random key and input, so the tolerance is set
/// well above what is observed in practice (around 2^40) rather than at a bound a rare draw could
/// exceed.
...
fn assert_bootstrap_outputs_close(
    lhs: &LweCiphertextOwned<u128>,
    rhs: &LweCiphertextOwned<u128>,
    max_abs_diff: u128,
    context: &str,
) {
    assert_eq!(
        lhs.as_ref().len(),
        rhs.as_ref().len(),
        "{context}: LWE sizes differ"
    );
    for (index, (&lhs, &rhs)) in lhs.as_ref().iter().zip(rhs.as_ref()).enumerate() {
        let diff = lhs.wrapping_sub(rhs);
        let abs_diff = diff.min(diff.wrapping_neg());
        assert!(
