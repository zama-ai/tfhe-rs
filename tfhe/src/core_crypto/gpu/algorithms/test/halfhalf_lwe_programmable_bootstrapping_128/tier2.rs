...
//! is essentially complete, and the ratio of the two noise sums observed over repeated runs
//! matches the zero-correlation prediction. That is why this tier compares decrypted phases with a
...
//!
//! The test does not also bound `S_gpu` and `S_cpu` against each other individually (to catch a
//! structurally offset but individually quiet GPU output): that check is strictly dominated by the
//! difference check above, not merely redundant with it. In the zero-correlation regime that holds
//! here, with the GPU adding independent excess variance `v` on top of the shared floor `sigma^2`,
//! `S_gpu / S_cpu = 1 + v / sigma^2` and `S_d / S_cpu = 2 + v / sigma^2` are the same statistic
//! offset by exactly 2, for every value of `v`; at any shared ceiling the difference check always
//! fires first, so there is no defect the magnitude check could catch that the difference check
//! misses, and carrying it would double the flake budget for zero additional coverage. When the
//! difference check does fail, it prints `S_gpu` and `S_cpu` and their ratio so the failure message
//! still says whether the GPU is noisier, quieter, or merely different from the reference — a GPU
//! quieter than the reference while still decoding correctly is the interesting case, since it
//! suggests the GPU skipped part of the computation.
//!
...
//! magnitude past the ceiling above. The sample count matters for the reliability of this check: at
//! 8 samples instead of 16, a ceiling of 9 flakes around one
