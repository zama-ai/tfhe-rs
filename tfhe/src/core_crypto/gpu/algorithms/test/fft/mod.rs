...
/// Polynomial size the FFT128 forward-transform regression test is pinned to.
const FFT128_POLYNOMIAL_SIZE: usize = 2048;

/// Names of the four output planes of the forward FFT128, in the order
/// [`run_fft128_forward`] returns them and the order they appear in the golden
/// file.
const FFT128_PLANE_NAMES: [&str; 4] = [
    "EXPECTED_RE0",
    "EXPECTED_RE1",
    "EXPECTED_IM0",
    "EXPECTED_IM1",
...
/// Runs the forward FFT128 over a fixed deterministic `u128` polynomial and
/// returns the four output planes (re hi, re lo, im hi, im lo) as raw f64 bit
/// patterns, ready for a bit-exact comparison against golden data.
fn run_fft128_forward() -> [Vec<u64>; 4] {
    let size = FFT128_POLYNOMIAL_SIZE;
    let fourier_size = PolynomialSize(size).to_fourier_polynomial_size().0;
    ...
    unsafe {
        ...
        // The transform copies its result back on the stream without waiting for
        // it, so the planes are only readable once the device is idle.
        cuda_synchronize_device(gpu_index);
    }

    [fourier_re0, fourier_re1, fourier_im0, fourier_im1]
        .map(|plane| plane.iter().map(|x| x.to_bits()).collect())
}

fn test_regression_128() {
    assert_eq!(POLYNOMIAL_SIZE, FFT128_POLYNOMIAL_SIZE);

    let [fourier_re0, fourier_re1, fourier_im0, fourier_im1] = run_fft128_forward();
    ...
    for (i, (re0, e_re0, re1, e_re1, im0, e_im0, im1, e_im1)) in bundles.enumerate() {
        assert_eq!(*re0, *e_re0, "Bitwise mismatch in RE0 at index {i}");
        assert_eq!(*re1, *e_re1, "Bitwise mismatch in RE1 at index {i}");
        assert_eq!(*im0, *e_im0, "Bitwise mismatch in IM0 at index {i}");
        assert_eq!(*im1, *e_im1, "Bitwise mismatch in IM1 at index {i}");
    }
}

/// (Re)generates the FFT128 golden file. This is *not* a test — it is a manual
/// maintenance helper, the FFT128 counterpart of `generate_fft16x4x16_golden`.
/// To refresh the golden data, temporarily annotate it with `#[test]`
/// `#[ignore]`, run it with
/// `cargo test --release ... -- --ignored --exact`, then commit the regenerated
/// file and remove the annotations again.
#[allow(dead_code)]
fn generate_fft128_golden() {
    let planes = run_fft128_forward();

    let blocks: Vec<String> = FFT128_PLANE_NAMES
        .into_iter()
        .zip(planes)
        .map(|(name, plane)| {
            let mut body = String::new();
            for bits in plane {
                let _ = writeln!(body, "    {bits:#018x},");
            }
            format!("pub const {name}: [u64; POLYNOMIAL_SIZE / 2] = [\n{body}];\n")
        })
        .collect();

    let contents = format!(
        "// @generated by the `generate_fft128_golden` helper in\n\
         // core_crypto::gpu::algorithms::test::fft. Do not edit by hand.\n\
         pub const POLYNOMIAL_SIZE: usize = {FFT128_POLYNOMIAL_SIZE};\n{}",
        blocks.join("\n")
    );

    let path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/src/core_crypto/gpu/algorithms/test/fft/fft_data/golden_v1.rs"
    );
    std::fs::write(path, contents).expect("failed to write FFT128 golden file");
    println!("wrote FFT128 golden data to {path}");
}
...
//
// Mirrors `test_regression_128`: a fixed, deterministic input is run through
// the forward FFT16x4x16 core once to produce golden output, and every
