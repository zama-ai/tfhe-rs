...
// by the `#[ignore]`d `generate_fft16x4x16_golden` test below — run it on an
// H100 (sm_90) to refresh the file, then commit the result.
//
...
/// (Re)generates the FFT16x4x16 golden file. Ignored by default: run it
/// explicitly on an H100 (sm_90) to refresh the golden data, then commit.
///
/// ```text
/// cargo test --release --features=gpu \
///   core_crypto::gpu::algorithms::test::fft::generate_fft16x4x16_golden \
///   -- --ignored --exact
/// ```
#[test]
#[ignore = "regenerates committed golden data; run manually on sm_90 hardware"]
