...
fn test_cm_pbs_noise_analysis(params: &CmApParams) {
    ...
    let expected_variance = pbs_variance_132_bits_security_gaussian_impl(
...
fn test_cm_pbs_ap_sequence(params: &CmApParams) {
    ...
    let f_norm2: f64 = 2_f64.powf(params.log_nu);

    assert!((f_norm2 - f_norm2.round()) < 0.000_001);

    let norm2 = f_norm2.round() as u64;
    ...
    for _ in 0..NB_TESTS {
        ...
        for _ in 0..3 {
            cm_lwe_ciphertext_cleartext_mul_assign(&mut ct_big, Cleartext(norm2));
