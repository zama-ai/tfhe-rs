...
impl<G: ByteRandomGenerator> RandomGenerator<G> {
    ...
    /// Fill a slice with random uniform binary values.
    /// This will only draw as many bytes needed from the underlying csprng to fill the slice with random bits.
    /// If the slice len is n, it will draw ceil(n/8) bytes from the csprng.
    ///
    /// # Example
    ///
    /// ```rust
