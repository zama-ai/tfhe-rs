...
use tfhe_ntt::native64::Plan32;

#[derive(Clone, Debug)]
pub struct NttNative64 {
    plan: Arc<Plan32>,
}

impl NttNative64 {
    #[inline]
    pub fn as_view(&self) -> NttNative64View<'_> {
        NttNative64View { plan: &self.plan }
    }
}

#[derive(Clone, Copy, Debug)]
pub struct NttNative64View<'a> {
    plan: &'a Plan32,
}

impl NttNative64View<'_> {
...
mod avx512 {
    use super::*;
    use tfhe_ntt::native64::Plan52;

    #[derive(Clone, Debug)]
    pub struct NttNative64Avx512 {
        plan: Arc<Plan52>,
    }

    impl NttNative64Avx512 {
        #[inline]
        pub fn as_view(&self) -> NttNative64Avx512View<'_> {
            NttNative64Avx512View { plan: &self.plan }
        }
    }

    #[derive(Clone, Copy, Debug)]
    pub struct NttNative64Avx512View<'a> {
        plan: &'a Plan52,
    }

    impl NttNative64Avx512View<'_> {
        pub fn negacyclic_polymul(&self, prod: &mut [u64], lhs: &[u64], rhs: &[u64]) {
            self.plan.negacyclic_polymul(prod, lhs, rhs);
        }
    }
