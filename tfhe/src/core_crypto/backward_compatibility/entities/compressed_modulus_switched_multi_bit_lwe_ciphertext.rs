    ...
    Scalar: UnsignedInteger + CastInto<usize> + CastFrom<usize>,
{
    ...
    fn upgrade(
        ...
        Ok(CompressedModulusSwitchedMultiBitLweCiphertext {
            body: self.body.cast_into(),
            packed_mask: PackedIntegers::pack(&mask, self.packed_mask.log_modulus),
            packed_diffs: diffs_opt
                .map(|(diffs, log_modulus)| PackedIntegers::pack(&diffs, log_modulus)),
