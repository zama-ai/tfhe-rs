    ...
    ContMut: ContainerMut<Element = Scalar>,
{
    ...
    assert_eq!(chunks_lwe_dimension, output.input_lwe_dimension());

    let mut start: usize = 0;
    for chunk in chunks {
        assert_eq!(output.glwe_size(), chunk.glwe_size());
        assert_eq!(output.polynomial_size(), chunk.polynomial_size());
        assert_eq!(
