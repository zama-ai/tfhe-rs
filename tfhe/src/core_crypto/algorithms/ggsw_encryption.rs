...
/// ## Warning
///
/// Note that due to the current implementation of the [`SignedDecomposerNonNative`], using non
/// power of 2 moduli can lead to a slightly erroneous decryption where the absolute difference
/// between the encrypted value and the actually decrypted value is at most 1, i.e.
/// `abs(dec_ggsw(enc_ggsw(val)) - val) <= 1`.
///
/// # Example
///
/// ```
