...
#[test]
fn improve_modulus_switch_noise_test_individual_check_p_success_parameterized() {
...
fn improve_modulus_switch_noise_test_individual_check_p_success(
    ...
    let successes_count: Vec<_> = (0..number_loops)
        ...
        .map(|_| {
            ...
            for encryption_of_zero in encryptions_of_zero.iter() {
                ...
                let measure = measure_noise_estimation_for_binary_key(
                    r_sigma_factor,
                    log_modulus,
                    mask_sum,
                    body_sum,
    ...
    assert!(
        (measured_individual_check_p_success - expected_individual_check_p_success).abs()
            / (measured_individual_check_p_success + expected_individual_check_p_success)
            < 0.1_f64,
    ...
    dbg!(lower_bound_p_success, upper_bound_p_success);

    let expected_p_all_fail_log2 = (1. - expected_individual_check_p_success).log2()
        * (modulus_switch_zeros_count.0 as f64 + 1.);

    let upper_bound_p_all_fail_log2 =
        (1. - lower_bound_p_success).log2() * (modulus_switch_zeros_count.0 as f64 + 1.);

    dbg!(expected_p_all_fail_log2, upper_bound_p_all_fail_log2);
