...
fn encryption_ms_decryption<
    Scalar: UnsignedTorus + Sync + Send + CastInto<usize> + CastFrom<usize>,
>(
    ...
    usize: CastFrom<Scalar>,
{
    ...
    while msg != Scalar::ZERO {
        ...
        for _ in 0..NB_TESTS {
            ...
            let lwe = switch_lwe_modulus::<_, Scalar, _>(lwe.as_view(), log_modulus);

            // Can be stored using much less space than the standard lwe ciphertexts
            let compressed = lwe.pack::<usize>();
...
{
    let msed_ct = switch_lwe_modulus::<_, usize, _>(ct.as_view(), log_modulus);
