...
/// Perform a programmable bootstrap using a specialized implementation given an input [`LWE
/// ciphertext`](`LweCiphertext`) containing u128 coefficiennts, a look-up table passed as a [`GLWE
/// ciphertext`](`GlweCiphertext`) and an [`LWE bootstrap key`](`LweBootstrapKey`) in the fourier
/// domain using f128 see [`fourier LWE bootsrap
/// key`](`Fourier128LweBootstrapKey`). The result is written in the provided output [`LWE
/// ciphertext`](`LweCiphertext`).
///
/// If you want to manage the computation memory manually you can use
/// [`programmable_bootstrap_f128_lwe_ciphertext_u128_mem_optimized`].
///
/// Note that this specialized implementation for u128, though faster, may yields less correct bits
/// than the generic [`programmable_bootstrap_f128_lwe_ciphertext`] implementation.
///
/// # Example
///
/// ```
...
/// );
/// println!("Computing PBS...");
/// programmable_bootstrap_f128_lwe_ciphertext_u128(
///     &lwe_ciphertext_in,
///     &mut pbs_multiplication_ct,
///     &accumulator,
///     &fourier_bsk,
...
/// );
/// ```
pub fn programmable_bootstrap_f128_lwe_ciphertext_u128<InputCont, OutputCont, AccCont, KeyCont>(
    input: &LweCiphertext<InputCont>,
    output: &mut LweCiphertext<OutputCont>,
    accumulator: &GlweCiphertext<AccCont>,
    fourier_bsk: &Fourier128LweBootstrapKey<KeyCont>,
    ...
    KeyCont: Container<Element = f64>,
{
    ...
    buffers.resize(
        programmable_bootstrap_f128_lwe_ciphertext_u128_mem_optimized_requirement(
            fourier_bsk.glwe_size(),
            fourier_bsk.polynomial_size(),
            fft,
        )
...
/// Memory optimized version of [`programmable_bootstrap_f128_lwe_ciphertext_u128`], the caller must
/// provide a properly configured [`Fft128View`] object and a `PodStack` used as a memory buffer
/// having a capacity at least as large as the result of
/// [`programmable_bootstrap_f128_lwe_ciphertext_u128_mem_optimized_requirement`].
///
/// Note that this specialized implementation for u128, though faster, may yields less correct bits
/// than the generic [`programmable_bootstrap_f128_lwe_ciphertext_mem_optimized`] implementation.
pub fn programmable_bootstrap_f128_lwe_ciphertext_u128_mem_optimized<
    InputCont,
    OutputCont,
    AccCont,
    KeyCont,
>(
    ...
    KeyCont: Container<Element = f64>,
{
    fourier_bsk.bootstrap_u128(output, input, accumulator, fft, stack);
}

/// Return the required memory for
/// [`programmable_bootstrap_f128_lwe_ciphertext_u128_mem_optimized`].
pub fn programmable_bootstrap_f128_lwe_ciphertext_u128_mem_optimized_requirement(
    glwe_size: GlweSize,
    polynomial_size: PolynomialSize,
    fft: Fft128View<'_>,
) -> Result<StackReq, SizeOverflow> {
    bootstrap_scratch_f128::<u128>(glwe_size, polynomial_size, fft)
}
