...
>(
    ...
    mask_noise_distribution: MaskDistribution,
    body_noise_distribution: NoiseDistribution,
    encryption_generator: &mut NoiseRandomGenerator<EncryptionGen>,
) -> CompactPublicKeyRandomVectors<Scalar>
where
    Scalar: Encryptable<MaskDistribution, NoiseDistribution> + RandomGenerable<UniformBinary>,
    KeyCont: Container<Element = Scalar>,
    ...
    EncryptionGen: ByteRandomGenerator,
{
    ...
    encryption_generator.fill_slice_with_random_uniform_binary_bits(&mut binary_random_vector);
...
/// let seeder = seeder.as_mut();
/// let mut encryption_generator =
///     NoiseRandomGenerator::<DefaultRandomGenerator>::new(seeder.seed(), seeder);
///
/// // Create the LweSecretKey
/// let lwe_secret_key =
///     allocate_and_generate_new_binary_lwe_secret_key(lwe_dimension, &mut secret_generator);
