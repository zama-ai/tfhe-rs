...
use crate::core_crypto::fft_impl::fft64::crypto::cm_bootstrap::{
    cm_blind_rotate_assign_scratch, cm_bootstrap_scratch, FourierCmLweBootstrapKey,
};
...
/// Memory optimized version of [`cm_blind_rotate_assign`], the caller must provide
...
/// [`cm_blind_rotate_assign_mem_optimized_requirement`].
    ...
    KeyCont: Container<Element = c64>,
{
    ...
    fourier_bsk
        .as_view()
        .cm_blind_rotate_assign(lut.as_mut_view(), input.as_view(), fft, stack);
}

/// Return the required memory for [`cm_blind_rotate_assign_mem_optimized`].
...
/// Memory optimized version of [`cm_add_external_product_assign`], the caller must provide a
/// properly configured [`FftView`] object and a `PodStack` used as a memory buffer having a
/// capacity at least as large as the result of
/// [`cm_add_external_product_assign_mem_optimized_requirement`].
///
/// Compute the external product of `ggsw` and `glwe`, and add the result to `out`.
///
/// Strictly speaking this function computes:
...
/// Return the required memory for [`cm_add_external_product_assign_mem_optimized`].
...
/// Memory optimized version of [`cm_cmux_assign`], the caller must provide a properly configured
/// [`FftView`] object and a `PodStack` used as a memory buffer having a capacity at least as large
/// as the result of [`cm_cmux_assign_mem_optimized_requirement`].
///
/// # Example
///
/// ```rust
...
/// Return the required memory for [`cm_cmux_assign_mem_optimized`].
...
/// Perform a programmable bootstrap given an input [`CM LWE ciphertext`](`CmLweCiphertext`), a
/// look-up table passed as a [`CM GLWE ciphertext`](`CmGlweCiphertext`) and an [`LWE bootstrap
/// key`](`CmLweBootstrapKey`) in the fourier domain see [`fourier LWE bootstrap
/// key`](`FourierLweBootstrapKey`). The result is written in the provided output
/// [`CM LWE ciphertext`](`CmLweCiphertext`).
///
/// If you want to manage the computation memory manually you can use
...
/// Memory optimized version of [`programmable_bootstrap_cm_lwe_ciphertext`], the caller must
/// provide a properly configured [`FftView`] object and a `PodStack` used as a memory buffer having
/// a capacity at least as large as the result of
/// [`cm_programmable_bootstrap_lwe_ciphertext_mem_optimized_requirement`].
    ...
    KeyCont: Container<Element = c64>,
{
    ...
    fourier_bsk.as_view().cm_bootstrap(
        output.as_mut_view(),
        input.as_view(),
        accumulator.as_view(),
        fft,
...
/// Return the required memory for [`cm_programmable_bootstrap_lwe_ciphertext_mem_optimized`].
