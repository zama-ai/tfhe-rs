...
use crate::core_crypto::fft_impl::fft64::crypto::ggsw::{
    ...
    karatsuba_add_external_product_assign, update_with_fmadd_factor,
};
...
pub fn karatsuba_deterministic_blind_rotate_assign<Scalar, OutputCont, KeyCont>(
    switched_modulus_input: &impl MultiBitModulusSwitchedCt,
    accumulator: &mut GlweCiphertext<OutputCont>,
    multi_bit_bsk: &LweMultiBitBootstrapKey<KeyCont>,
    thread_count: ThreadCount,
    ...
    KeyCont: Container<Element = Scalar> + Sync,
{
    ...
    thread::scope(|s| {
        let produce_multi_bit_fourier_ggsw = |thread_id| {
            ...
            #[allow(clippy::type_complexity)]
            let (ready_for_consumer_lock, condvar, ggsw_buffer): &(
                Mutex<bool>,
                Condvar,
                Mutex<GgswCiphertext<Vec<Scalar>>>,
            ) = &multi_bit_ggsw_buffers[dest_idx];
            ...
            for work_index in (0..max_work_index).skip(thread_id).step_by(thread_count.0) {
                ...
                let mut ggsw_buffer = ggsw_buffer.lock().unwrap();
                ...
                ggsw_buffer
                    .as_mut()
                    .copy_from_slice(std_ggsw_buffer.as_ref());
...
pub fn std_multi_bit_f128_blind_rotate_assign<Scalar, InputCont, OutputCont, KeyCont>(
    input: &LweCiphertext<InputCont>,
    accumulator: &mut GlweCiphertext<OutputCont>,
    multi_bit_bsk: &LweMultiBitBootstrapKey<KeyCont>,
    thread_count: ThreadCount,
    ...
    KeyCont: Container<Element = Scalar> + Sync,
{
    ...
    // TODO, have a non deterministic execution, but does not seem particularly useful
    let _ = deterministic_execution;

    std_multi_bit_f128_deterministic_blind_rotate_assign(
        &multi_bitmodulus_switched_ct,
        accumulator,
        multi_bit_bsk,
        thread_count,
    ...
    thread::scope(|s| {
        let produce_multi_bit_fourier_ggsw = |thread_id| {
            ...
            for work_index in (0..max_work_index).skip(thread_id).step_by(thread_count.0) {
                ...
                fourier_ggsw_buffer
                    .as_mut_view()
                    .fill_with_forward_fourier(&std_ggsw_buffer, fft);
...
pub fn std_multi_bit_programmable_bootstrap_f128_lwe_ciphertext<
    Scalar,
    InputCont,
    OutputCont,
    AccCont,
    ...
    KeyCont: Container<Element = Scalar> + Sync,
{
    ...
    std_multi_bit_f128_blind_rotate_assign(
        input,
        &mut local_accumulator,
        multi_bit_bsk,
        thread_count,
...
pub fn multi_bit_f128_deterministic_blind_rotate_assign_return_noise<Scalar, OutputCont, KeyCont>(
    switched_modulus_input: &impl MultiBitModulusSwitchedCt,
    accumulator: &mut GlweCiphertext<OutputCont>,
    multi_bit_bsk: &Fourier128LweMultiBitBootstrapKey<KeyCont>,
    thread_count: ThreadCount,
...
where
    Scalar: UnsignedTorus + CastInto<usize> + Sync,
    OutputCont: ContainerMut<Element = Scalar>,
    KeyCont: Container<Element = f64> + Sync + Split,
{
    assert_eq!(
...
pub fn multi_bit_f128_blind_rotate_assign_return_noise<Scalar, InputCont, OutputCont, KeyCont>(
    input: &LweCiphertext<InputCont>,
    accumulator: &mut GlweCiphertext<OutputCont>,
    multi_bit_bsk: &Fourier128LweMultiBitBootstrapKey<KeyCont>,
    thread_count: ThreadCount,
    ...
    KeyCont: Container<Element = f64> + Sync + Split,
{
    ...
    multi_bit_f128_deterministic_blind_rotate_assign_return_noise(
        &multi_bitmodulus_switched_ct,
        accumulator,
        multi_bit_bsk,
        thread_count,
...
pub fn multi_bit_programmable_bootstrap_f128_lwe_ciphertext_return_noise<
    Scalar,
    InputCont,
    OutputCont,
    AccCont,
    ...
    KeyCont: Container<Element = f64> + Sync,
{
    ...
    let noise_vec = multi_bit_f128_blind_rotate_assign_return_noise(
        input,
        &mut local_accumulator,
        &multi_bit_bsk.as_view(),
        thread_count,
