//! `u128` (split lo/hi `u64`-limb) path of the combined "half-product + half-rotate" 128-bit FFT
//! bootstrap.
//!
//! See the [`fft128`
//! module][crate::core_crypto::fft_impl::fft128::crypto::bootstrap_half_product_half_rotate]
//! for the generic (non-`u128`) path and the type definition.

use super::super::math::fft::Fft128View;
use super::bootstrap::polynomial_wrapping_monic_monomial_div_assign_split;
use super::bootstrap_half_product::cmux_step_split_half_product;
    ...
    Cont: Container<Element = f64>,
{
    ...
    /// running the two sections one after the other.
...
mod tests {
    ...
    // Same bit-for-bit split/non-split comparison for the composed half-product + half-rotate key.
    #[test]
    fn test_half_product_half_rotate_split_pbs() {
        ...
        // Two sections, each a half-product one, with distinct decomposition parameters.
        ...
        let mut mem = PodBuffer::try_new(half_product_half_rotate_bootstrap_scratch::<u128>(
            glwe_dimension.to_glwe_size(),
            polynomial_size,
            fft,
        ))
