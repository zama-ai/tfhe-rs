...
impl<C: Container<Element = f64>> Fourier128HalfProductGgswCiphertext<C> {
    ...
    /// Split the containers into the mask block and the body block.
    fn split_blocks(self) -> (Self, Self)
    where
        C: Split,
    {
        let mask_len = half_product_ggsw_ciphertext_mask_row_count(
            self.glwe_size,
            self.decomposition_level_count_mask,
        ) * self.row_len();

        let (mask_re0, body_re0) = self.data_re0.split_at(mask_len);
        let (mask_re1, body_re1) = self.data_re1.split_at(mask_len);
        let (mask_im0, body_im0) = self.data_im0.split_at(mask_len);
        let (mask_im1, body_im1) = self.data_im1.split_at(mask_len);

        let rebuild = |data_re0, data_re1, data_im0, data_im1| Self {
            data_re0,
            data_re1,
            data_im0,
            data_im1,
        ...
        (
            rebuild(mask_re0, mask_re1, mask_im0, mask_im1),
            rebuild(body_re0, body_re1, body_im0, body_im1),
        )
    }
    ...
    where
        C: Split,
    {
        let (mask, _) = self.split_blocks();
        let level_count = mask.decomposition_level_count_mask.0;
        let row_count = mask.glwe_size.to_glwe_dimension().0;
        let polynomial_size = mask.polynomial_size;
        let glwe_size = mask.glwe_size;

        izip_eq!(
            mask.data_re0.split_into(level_count),
            mask.data_re1.split_into(level_count),
            mask.data_im0.split_into(level_count),
            mask.data_im1.split_into(level_count)
        )
        .enumerate()
    ...
    where
        C: Split,
    {
        let (_, body) = self.split_blocks();
        let level_count = body.decomposition_level_count_body.0;
        let polynomial_size = body.polynomial_size;
        let glwe_size = body.glwe_size;

        izip_eq!(
            body.data_re0.split_into(level_count),
            body.data_re1.split_into(level_count),
            body.data_im0.split_into(level_count),
            body.data_im1.split_into(level_count)
        )
        .enumerate()
        .map(move |(i, (data_re0, data_re1, data_im0, data_im1))| {
