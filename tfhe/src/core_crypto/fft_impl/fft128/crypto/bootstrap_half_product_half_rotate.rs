...
pub(crate) mod tests {
    ...
    use super::super::bootstrap_half_product::tests::{
        assert_bootstrap_outputs_close, gather_half_product_key_from_classic,
    };
    use super::*;
    ...
    // Two sections sharing one set of decomposition parameters, holding exactly the rows of a
    // classic key, must reproduce that key's bootstrap bit-for-bit. This pins both the mask split
    // offset and the half-product row ordering at once.
    // See `half_product_matches_classic_bsk_generic` for what `max_abs_diff` accounts for.
    pub(crate) fn half_product_half_rotate_matches_classic_bsk_generic<Scalar>(max_abs_diff: Scalar)
    where
        Scalar: UnsignedTorus
            + CastInto<usize>
            + CastFrom<usize>
            ...
            + Encryptable<Uniform, DynamicDistribution<Scalar>>,
    {
        ...
        let section = |data: &[Scalar]| {
            gather_half_product_key_from_classic(
        ...
        let half_std_bsk =
            LweHalfProductHalfRotateBootstrapKey::from_keys(section(start_data), section(end_data));
        ...
        let mut half_lwe_out = LweCiphertext::new(Scalar::ZERO, lwe_out_size, ciphertext_modulus);
        half_fourier_bsk.bootstrap(&mut half_lwe_out, &lwe_in, &accumulator, fft, stack);

        assert_bootstrap_outputs_close(
            &half_lwe_out,
            &classic_lwe_out,
            max_abs_diff,
            "half-product half-rotate bootstrap differs from the classic bootstrap on the same \
            key material",
        );
    }
}
