...
use crate::core_crypto::prelude::{CiphertextModulus, ContainerMut, GlweCiphertextCount};
...
pub fn batch_bootstrap_scratch<Scalar>(
    ...
    blind_rotate_assign_scratch::<Scalar>(glwe_size, polynomial_size, fft)?.try_and(
        StackReq::try_new_aligned::<Scalar>(
            glwe_ciphertext_size(glwe_size, polynomial_size) * glwe_ciphertext_count.0,
            CACHELINE_ALIGN,
        )?,
    )
}
