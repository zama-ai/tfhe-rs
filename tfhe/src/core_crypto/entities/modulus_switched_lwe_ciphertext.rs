...
impl<Scalar> StandardModulusSwitchedLweCiphertext<Scalar> {
    pub fn from_raw_parts(container: Vec<Scalar>, log_modulus: CiphertextModulusLog) -> Self {
        Self {
            container,
            log_modulus,
        }
...
impl<Scalar: Copy> ModulusSwitchedCt<Scalar> for StandardModulusSwitchedLweCiphertext<Scalar> {
...
pub struct LazyStandardModulusSwitchedCt<Scalar, SwitchedScalar, C>
where
    ...
    C: Container<Element = Scalar>,
{
    ...
    phatom: PhantomData<SwitchedScalar>,
}

impl<Scalar, SwitchedScalar, C> LazyStandardModulusSwitchedCt<Scalar, SwitchedScalar, C>
where
    Scalar: UnsignedInteger + CastInto<SwitchedScalar>,
    SwitchedScalar: UnsignedInteger,
    C: Container<Element = Scalar>,
...
impl<Scalar, SwitchedScalar, C> ModulusSwitchedCt<SwitchedScalar>
    for LazyStandardModulusSwitchedCt<Scalar, SwitchedScalar, C>
where
    Scalar: UnsignedInteger + CastInto<SwitchedScalar>,
    SwitchedScalar: UnsignedInteger,
    C: Container<Element = Scalar>,
{
    ...
    fn body(&self) -> SwitchedScalar {
        modulus_switch(
            (*self.lwe_in.get_body().data).wrapping_add(self.body_correction_to_add),
            self.log_modulus,
        )
        .cast_into()
    }
