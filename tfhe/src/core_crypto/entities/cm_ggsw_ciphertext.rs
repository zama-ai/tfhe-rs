...
use cm_glwe_ciphertext::cm_glwe_ciphertext_encryption_mask_sample_count;
use cm_glwe_ciphertext_list::{CmGlweCiphertextListMutView, CmGlweCiphertextListView};

/// A [`GGSW Ciphertext`](`CmGgswCiphertext`).
///
/// # Formal Definition
///
/// # GGSW Ciphertext
...
/// Return the number of elements in a [`CmGgswCiphertext`] given a [`GlweDimension`],
...
/// Return the number of elements in a [`GgswLevelMatrix`] given a [`GlweDimension`] and
/// [`PolynomialSize`].
pub fn cm_ggsw_level_matrix_size(
    ...
    (glwe_dimension.0 + cm_dimension.0) * (glwe_dimension.0 + cm_dimension.0) * polynomial_size.0
}

/// Return the number of elements in a [`FourierGgswCiphertext`] given a [`GlweDimension`],
...
/// Return the number of elements in a [`FourierGgswLevelMatrix`] given a [`GlweDimension`] and
...
/// Return the number of mask samples used during encryption of a [`CmGgswCiphertext`] given a
/// [`GlweDimension`], [`PolynomialSize`] and [`DecompositionLevelCount`].
...
/// Return the number of noise samples used during encryption of a [`CmGgswCiphertext`] given a
/// [`GlweDimension`], [`PolynomialSize`] and [`DecompositionLevelCount`].
...
impl<Scalar: UnsignedInteger, C: Container<Element = Scalar>> CmGgswCiphertext<C> {
    /// Create a [`CmGgswCiphertext`] from an existing container.
    ///
    /// # Note
    ///
    ...
    ///
    /// This docstring exhibits [`CmGgswCiphertext`] primitives usage.
    ///
    /// ```rust
    ...
    /// // computations
    /// // Define parameters for CmGgswCiphertext creation
    ...
    ///
    /// // Create a new CmGgswCiphertext
    /// let ggsw = CmGgswCiphertext::new(
    ///     0u64,
    ///     glwe_dimension,
    ///     cm_dimension,
    ///     polynomial_size,
    ...
    /// let ggsw = CmGgswCiphertext::from_container(
    ...
    /// Return the [`PolynomialSize`] of the [`CmGgswCiphertext`].
    ///
    /// See [`CmGgswCiphertext::from_container`] for usage.
    pub fn polynomial_size(&self) -> PolynomialSize {
        self.polynomial_size
    }

    /// Return the [`GlweDimension`] of the [`CmGgswCiphertext`].
    ///
    /// See [`CmGgswCiphertext::from_container`] for usage.
    ...
    /// Return the [`DecompositionLevelCount`] of the [`CmGgswCiphertext`].
    ///
    /// See [`CmGgswCiphertext::from_container`] for usage.
    ...
    /// Return the [`CiphertextModulus`] of the [`CmGgswCiphertext`].
    ///
    /// See [`CmGgswCiphertext::from_container`] for usage.
    ...
    /// Interpret the [`CmGgswCiphertext`] as a [`PolynomialList`].
    ...
    /// Interpret the [`CmGgswCiphertext`] as a [`GlweCiphertextList`].
    pub fn as_glwe_list(&self) -> CmGlweCiphertextListView<'_, Scalar> {
    ...
    /// Return a view of the [`CmGgswCiphertext`]. This is useful if an algorithm takes a view by
    /// value.
...
impl<Scalar: UnsignedInteger, C: ContainerMut<Element = Scalar>> CmGgswCiphertext<C> {
    /// Mutable variant of [`CmGgswCiphertext::as_polynomial_list`].
    ...
    /// Mutable variant of [`CmGgswCiphertext::as_glwe_list`].
    pub fn as_mut_glwe_list(&mut self) -> CmGlweCiphertextListMutView<'_, Scalar> {
        let polynomial_size = self.polynomial_size;
    ...
    /// Mutable variant of [`CmGgswCiphertext::as_view`].
...
/// A [`CmGgswCiphertext`] owning the memory for its own storage.
pub type CmGgswCiphertextOwned<Scalar> = CmGgswCiphertext<Vec<Scalar>>;
/// A [`CmGgswCiphertext`] immutably borrowing memory for its own storage.
...
impl<Scalar: UnsignedInteger> CmGgswCiphertextOwned<Scalar> {
    /// Allocate memory and create a new owned [`CmGgswCiphertext`].
    ///
    /// # Note
    ///
...
/// Metadata used in the [`CreateFrom`] implementation to create [`CmGgswCiphertext`] entities.
#[derive(Clone, Copy)]
...
/// A convenience structure to more easily write iterators on a [`CmGgswCiphertext`] levels.
...
impl<Scalar: UnsignedInteger, C: Container<Element = Scalar>> CmGgswLevelMatrix<C> {
    ...
    ///
    /// let container =
    ///     vec![0u64; cm_ggsw_level_matrix_size(glwe_dimension, cm_dimension, polynomial_size)];
    ///
    /// // Create a new GgswLevelMatrix
    /// let ggsw_level_matrix = CmGgswLevelMatrix::from_container(
    ///     container,
    ///     glwe_dimension,
    ///     cm_dimension,
    ///     polynomial_size,
    ...
    /// );
    ///
    /// assert_eq!(ggsw_level_matrix.glwe_dimension(), glwe_dimension);
    ...
    /// Return the [`GlweDimension`] of the [`GgswLevelMatrix`].
    ///
    ...
    pub fn as_glwe_list(&self) -> CmGlweCiphertextListView<'_, C::Element> {
        CmGlweCiphertextListView::from_container(
            self.data.as_ref(),
            self.glwe_dimension,
            self.cm_dimension,
    ...
    for CmGgswCiphertext<C>
{
    ...
    /// Unimplemented for [`CmGgswCiphertext`]. At the moment it does not make sense to
    /// return "sub" CmGgswCiphertext.
