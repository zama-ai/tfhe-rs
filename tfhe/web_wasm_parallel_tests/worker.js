...
async function compressedPublicKeyTest() {
  let params_name_small =
    ShortintParametersName.V1_3_PARAM_MESSAGE_2_CARRY_2_PBS_KS_GAUSSIAN_2M128;
...
async function compactPublicKeyBench32BitBig() {
  const params =
    ShortintParametersName.V1_3_PARAM_MESSAGE_2_CARRY_2_COMPACT_PK_KS_PBS_GAUSSIAN_2M128;
...
async function compactPublicKeyBench32BitSmall() {
  const params =
    ShortintParametersName.V1_3_PARAM_MESSAGE_2_CARRY_2_COMPACT_PK_PBS_KS_GAUSSIAN_2M128;
  const block_params = new ShortintParameters(params);
  let config = TfheConfigBuilder.default()
    .use_custom_parameters(block_params)
    .build();
...
async function compressedServerKeyBenchMessage1Carry1() {
  const params =
    ShortintParametersName.V1_3_PARAM_MESSAGE_1_CARRY_1_KS_PBS_GAUSSIAN_2M128;
  const block_params = new ShortintParameters(params);
  let config = TfheConfigBuilder.default()
    .use_custom_parameters(block_params)
    .build();
...
async function compressedServerKeyBenchMessage2Carry2() {
  const params =
    ShortintParametersName.V1_3_PARAM_MESSAGE_2_CARRY_2_KS_PBS_GAUSSIAN_2M128;
  const block_params = new ShortintParameters(params);
  let config = TfheConfigBuilder.default()
    .use_custom_parameters(block_params)
    .build();
...
async function compactPublicKeyZeroKnowledgeBench() {
  ...
  for (const params of params_to_bench) {
    ...
    // Proof configuration:
    // [
    //    [crs_size, [bits_to_encrypt_1, bits_to_encrypt_2, ...]],
    //    ...
    // ]
    let proof_configs = [
      [64, [64]],
    ...
    for (const proof_config of proof_configs) {
      console.log("Start CRS generation");
      console.time("CRS generation");
      let crs = CompactPkeCrs.from_config(config, proof_config[0]);
      console.timeEnd("CRS generation");
      ...
      for (const bits_to_encrypt of proof_config[1]) {
