...
# Step 5: copy fixtures (optional, used by x86CompatTest/Bench)
...
if [[ -f "$FIXTURES_SRC/public_key.bin" && -f "$FIXTURES_SRC/crs.bin" ]]; then
	...
	echo "==> Copied x86 fixtures into dist/fixtures/"
else
	echo "==> Fixtures missing in $FIXTURES_SRC/"
	echo "    Run \`make test_zk_wasm_x86_compat\` once to generate them."
	echo "    x86CompatTest/Bench buttons will fail at runtime."
fi
