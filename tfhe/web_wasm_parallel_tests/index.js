...
import { makeZkDemos } from "./full/tests/zk.js";

const IS_CLIENT = new URLSearchParams(window.location.search).has("client");

// Full-only demos that run on the main thread (not in the worker).
const MAIN_THREAD_DEMO_NAMES = IS_CLIENT
  ? []
...
async function setup() {
  ...
  // Demo names come from the worker itself, not a hardcoded list.
  const remote = Comlink.wrap(worker);
  const [demos, WORKER_DEMO_NAMES] = await Promise.all([
    remote.demos,
    remote.names,
  ]);
  ...
  // Detect whether fixture .bin files are served. The fixture-based bench
  // buttons require them and are kept disabled with an explanatory tooltip if
  // they're missing (run `make prepare_fixtures` to enable).
  const fixturesAvailable = await fetch("/fixtures/public_key.bin", {
    method: "HEAD",
  })
    .then((r) => r.ok)
    .catch(() => false);
  const FIXTURE_DEMOS = new Set([
    "fixtureEncryptProveTest",
    "fixtureEncryptProveBench",
  ]);

  for (const demo of WORKER_DEMO_NAMES) {
    if (FIXTURE_DEMOS.has(demo) && !fixturesAvailable) {
      const btn = document.getElementById(demo);
      if (btn) {
        btn.title =
          "Fixtures missing. Run `make prepare_fixtures` (in tfhe/web_wasm_parallel_tests/) to enable this bench.";
      }
      continue;
    }
  ...
  if (!crossOriginIsolated && MAIN_THREAD_DEMO_NAMES.length > 0) {
    ...
    const mainThreadDemos = makeZkDemos(pkg);
