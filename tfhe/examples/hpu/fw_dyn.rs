...
/// Custom zhc pipeline for MhMul generation
/// Generate a set of DOpStream for multi-hpu multiplication
fn gen_mh_mul_stream(
    mh_factor: usize,
    ...
    let config = MultiHpuConfig {
        ...
        hpu_config: hpu_config.clone(),
    };
    let builder = zhc::builder::mh_mul(ct_spec, schoolbook_depth);
    let ir = builder.optimize_ir().clone();
    // Extract IOpProto from builder
    let proto = {
        let Signature(sig_src, sig_dst) = builder.signature();
    ...
    for (hid, scheduled) in scheds.into_iter().enumerate() {
        ...
        streams.push(generate_translation_table(&allocated));
    }
    let stream = ZhcStream::new(None, streams);
