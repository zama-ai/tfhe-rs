...
impl ParamModulus {
    fn other_from_ciphertext_modulus<Scalar: UnsignedInteger>(
        ct_modulus: CiphertextModulus<Scalar>,
    ) -> Self {
        if ct_modulus.is_native_modulus() {
            ParamModulus::Other(
                1u128
                    .checked_shl(ct_modulus.associated_scalar_bits() as u32)
                    .unwrap(),
            )
        } else {
...
impl ParamDetails<u64> for AtomicPatternParameters {
    ...
    fn lwe_ciphertext_modulus(&self) -> ParamModulus {
        match self {
            Self::Standard(p) => {
                ParamModulus::other_from_ciphertext_modulus(p.ciphertext_modulus())
            }
            Self::KeySwitch32(p) => {
                ParamModulus::other_from_ciphertext_modulus(p.post_keyswitch_ciphertext_modulus)
            }
        }
    }

    fn glwe_ciphertext_modulus(&self) -> ParamModulus {
        ParamModulus::other_from_ciphertext_modulus(self.ciphertext_modulus())
    }
}
...
impl ParamDetails<u64> for CompressionParameters {
    ...
    fn glwe_ciphertext_modulus(&self) -> ParamModulus {
        ParamModulus::Other(1u128.checked_shl(u64::BITS).unwrap())
    }
}
