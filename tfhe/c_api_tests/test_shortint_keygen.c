...
void test_server_key_bc_trivial_encrypt(void) {
  ShortintClientKey *cks = NULL;
  ShortintServerKey *sks = NULL;
  ShortintParameters *params = NULL;
  ShortintCiphertext *ct = NULL;

  int get_params_ok = shortint_bc_get_parameters(2, 2, &params);
  assert(get_params_ok == 0);

  int gen_cks_ok = shortint_bc_gen_client_key(params, &cks);
  assert(gen_cks_ok == 0);

  int gen_sks_ok = shortint_bc_gen_server_key(cks, &sks);
  assert(gen_sks_ok == 0);

  int encrypt_ok = shortint_bc_server_key_create_trivial(sks, 3, &ct);
  assert(encrypt_ok == 0);
