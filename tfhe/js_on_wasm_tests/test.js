...
test('shortint_public_encrypt_decrypt', (t) => {
    let params = Shortint.get_shortint_parameters(2, 2);
    let cks = Shortint.new_client_key(params);
    let sks = Shortint.new_server_key(cks);
    ...
    let serialized_sks = Shortint.serialize_shortint_server_key(sks);
    let deserialized_sks = Shortint.deserialize_shortint_server_key(serialized_sks);

    let ct = Shortint.encrypt_with_compressed_public_key(deserialized_pk, deserialized_sks, BigInt(3));
