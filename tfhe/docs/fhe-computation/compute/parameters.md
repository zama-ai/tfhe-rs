...
## Default parameters
Currently, the default parameters use blocks that contain 2 bits of message and 2 bits of carry - a tweaked uniform (TUniform, defined [here](../../getting_started/security_and_cryptography.md###noise)) noise distribution, and have a $$2^{-128}$$ failure probability for the PBS. 
These are particularly suitable for applications that need to be secure in the IND-CPA^D model (see [here](../../getting\_started/security\_and_cryptography.md) for more details).
