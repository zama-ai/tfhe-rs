...
- [`tfhe::zk::CompactPkeCrs`](https://docs.rs/tfhe/latest/tfhe/zk/enum.CompactPkeCrs.html): the Common Reference String (CRS) shared between prover and verifier. Generated once during an offline setup phase. See the [CPU documentation](../../fhe-computation/advanced-features/zk-pok.md) for background on its role in ZK proof security.
- [`tfhe::zk::ZkComputeLoad`](https://docs.rs/tfhe/latest/tfhe/zk/enum.ZkComputeLoad.html): controls the trade-off between proof generation and verification cost. `ZkComputeLoad::Proof` produces a costlier proof but faster verification; `ZkComputeLoad::Verify` produces a faster proof but costlier verification.
