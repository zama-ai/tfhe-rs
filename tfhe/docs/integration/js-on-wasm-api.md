...
**TFHE-rs** supports 4 WASM `targets`:
...
* Web-parallel (cross-origin): For use in web browsers with parallelism support without requiring cross-origin isolation headers
...
async function main() {
    ...
    await init_panic_hook();
    // Ready — ZK proof operations will run in parallel
}
```
