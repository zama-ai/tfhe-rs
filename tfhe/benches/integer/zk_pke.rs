                ...
                for compute_load in [
                    ...
                    match get_bench_type() {
                        ...
                        BenchmarkType::Throughput => {
                            ...
                            let d_ksk_vec = gpu_sks_vec.par_iter().zip(local_streams.par_iter()).map(|(gpu_sks, local_stream)| {
                            ...
                            // bench_group.bench_function(&bench_id_verify_and_expand, |b| {
                            //     b.iter_batched(|| {CudaStreams::new_multi_gpu()}, |local_streams| {
                            //         gpu_cts.iter().for_each(|gpu_ct1| {
                            //             gpu_ct1
                            //                 .verify_and_expand(
                            //                     &crs, &pk, &metadata, &d_ksk, &local_streams,
                            //                 )
                            //                 .unwrap();
                            //         });
                            //     }, BatchSize::SmallInput);
