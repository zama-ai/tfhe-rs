...
mod cuda {
    ...
    use tfhe::zk::gpu::CudaProvenCompactCiphertextList;
    ...
    fn gpu_pke_zk_verify(c: &mut Criterion, results_file: &Path) {
        ...
        for (param_pke, param_ksk, param_fhe) in [
            ...
            for bits in [64usize, 640, 1280, 4096] {
                ...
                let compute_load = ZkComputeLoad::Verify;
                let zk_load = "compute_load_verify";
                ...
                match BENCH_TYPE.get().unwrap() {
                    ...
                    BenchmarkType::Throughput => {
                        ...
                        reset_pbs_count();
                        let _ = gpu_ct1.verify_and_expand(&crs, &pk, &metadata, &d_ksk, &streams);
                        let pbs_count = max(get_pbs_count(), 1); // Operation might not perform any PBS, so we take 1 as default
                        ...
                        let gpu_cts = cts.iter().map(|ct| {CudaProvenCompactCiphertextList::from_proven_compact_ciphertext_list(
                                &ct, &streams,
                            )}).collect_vec();
