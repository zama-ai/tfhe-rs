...
use tfhe::integer::{
    ClientKey, CompactPublicKey, IntegerCiphertext, IntegerKeyKind, RadixCiphertext,
    RadixClientKey, ServerKey, U256,
};
use tfhe::keycache::NamedParam;
use tfhe::shortint::engine::ShortintEngine;
use tfhe::shortint::parameters::bc::*;
use tfhe::shortint::parameters::*;
use tfhe::shortint::server_key::apply_programmable_bootstrap;
...
fn pke_ap(c: &mut Criterion) {
    ...
    for (param_name, param_pke, param_fhe, param_ksk) in PKE_PARAMS {
        ...
        let bench_id = format!("pke_ks_pbs::{param_name}");
        bench_group.bench_function(&bench_id, |b| {
            let encrypt_one_value = || {
                ...
                let ct1_extracted: RadixCiphertext = ct1.expand().into_iter().next().unwrap();
                ct1_extracted
            };
            ...
            b.iter_batched(
                ...
                |ct1_extracted| {
                    ...
                    let mut ct2: RadixCiphertext =
                        sks_fhe.create_trivial_radix(0u64, ct_cast.blocks().len());

                    // PBS to clean
                    let acc = sks_fhe.as_ref().generate_lookup_table(|x| x);
                    for (input, output) in ct_cast.blocks().iter().zip(ct2.blocks_mut()) {
                        ShortintEngine::with_thread_local_mut(|engine| {
                            let (_, buffers) = engine.get_buffers(sks_fhe.as_ref());
                            apply_programmable_bootstrap(
                                &sks_fhe.as_ref().bootstrapping_key,
                                &input.ct,
                                &mut output.ct,
                                &acc,
                                buffers,
...
criterion_group!(cast, pke_ap,);

fn pke_zk(c: &mut Criterion) {
    let mut bench_group = c.benchmark_group("pke_zk");
    bench_group
        .sample_size(15)
        .measurement_time(std::time::Duration::from_secs(60));
    ...
    for (param_name, param_pke) in [
        ...
        for fhe_uint_count in [5, 10] {
            ...
            for compute_load in [ZkComputeLoad::Proof, ZkComputeLoad::Verify] {
                ...
                let bench_id = format!("pke_zk::{param_name}_{fhe_uint_count}_{zk_load}");
                let input_msg = rng.gen::<u64>();
                let messages = vec![input_msg; fhe_uint_count];
                bench_group.bench_function(&bench_id, |b| {
                    b.iter(|| {
                        let ct1 = pk.encrypt_and_prove_radix_compact(
                            &messages,
                            num_block,
                            public_params,
                            compute_load,
