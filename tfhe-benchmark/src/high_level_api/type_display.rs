...
/// The type slot of a bench id, for a benchmark measuring `T`.
///
/// Written `type_tag::<FheUint64>()`, so the tag follows the type the benchmark
/// actually uses instead of being spelled out beside it.
pub fn type_tag<T: TypeDisplay>() -> TypeTag {
    T::fhe_type().into()
}
