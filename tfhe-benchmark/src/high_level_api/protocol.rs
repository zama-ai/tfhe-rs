...
//!
//! Same workflow as the gpu_protocol_long_run stress test, minus what only a test needs, split in
//! the two halves the protocol schedules as separate jobs.
...
// Stand-ins for the protocol's own separators and input hash.
...
// Stands in for the protocol's opcodes, only distinctness matters.
type NodeOpcode = u32;
...
/// Off is not a protocol mode, it just lets a benchmark price re-randomization by difference.
...
/// Seeds depend only on the opcode and the operands, which is what lets two coprocessors agree.
...
/// Passing the CPK covers both modes: a server key with a derived one ignores this.
...
/// The zero counts as an operand, GPU if_then_else won't take a clear input.
...
/// The ERC-7984 whitepaper transfer, the flavor the protocol runs. Cloning per node is deliberate:
/// a value feeding several nodes gets re-randomized in each, like a handle read several times.
pub fn transfer(
    from_amount: &FheUint64,
    to_amount: &FheUint64,
    amount: &FheUint64,
...
pub fn tfhew_decomp_transfer_comp(
...
pub fn full_transaction(
    ...
    let compressed_outputs = tfhew_decomp_transfer_comp(compressed_inputs, cpk, rerand);

    snsw_decomp_sns_comp(&compressed_outputs)
}
...
/// The metadata ties an input to its origin and gets absorbed into the re-randomization context.
...
/// Transfer results, so SNSw can be measured without running TFHEw first.
