//! Internal parameter aggregates used by the core_crypto benches.
//!
//! These structs collect the subset of crypto parameters each bench needs to set up keys, in a
...
//! `tfhe-benchmark-parser::write_to_json_unchecked` which only needs the bench id, alias, and
//! display name.
...
/// Parameters needed for PBS-style core_crypto benches (`ks_bench`, `ks_pbs_bench`, `pbs_bench`).
///
...
/// Parameters needed for the packing-keyswitch bench in `ks_bench.rs`.
#[derive(Clone, Copy)]
