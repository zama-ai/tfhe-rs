...
//!     --bench shortint-kreyvium-transciphering
//! ```
...
pub fn cpu_kreyvium_transciphering(c: &mut Criterion) {
    ...
    // ---- warmup ----
    let id = format!("{bench_name}::{}::warmup", &param_name);
    println!("{id}");
    bench_and_record(
        &mut group,
        &id,
    ...
    // ---- keystream_64bits ----
    let id = format!("{bench_name}::{}::keystream_64bits", &param_name);
    println!("{id}");
    bench_and_record(
        &mut group,
        &id,
        &param_name,
        "kreyvium_keystream_64bits",
    ...
    let id = format!("{bench_name}::{}::transcipher_64bits", &param_name);
    println!("{id}");
    bench_and_record(
        &mut group,
        &id,
        &param_name,
        "kreyvium_transcipher_64bits",
        KEYSTREAM_BITS as u32,
        vec![log2_msg; KEYSTREAM_BITS],
        |b| {
            b.iter_batched(
