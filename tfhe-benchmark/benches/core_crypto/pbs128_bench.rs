...
mod cuda {
    ...
    fn cuda_pbs_128_16_blocks(c: &mut Criterion) {
        let bench_name = "core_crypto::cuda::pbs128_16_blocks";
        let mut bench_group = c.benchmark_group(bench_name);
        bench_group
            .sample_size(10)
            .measurement_time(std::time::Duration::from_secs(30));
        ...
        let plaintext_vec = vec![input_message * delta; 16];
        let plaintext_list = PlaintextList::from_container(plaintext_vec);
        let lwe_ciphertext_count = LweCiphertextCount(num_blocks);
        ...
        match get_bench_type() {
            BenchmarkType::Latency => {
                ...
                let lwe_ciphertext_in_gpu =
                    CudaLweCiphertextList::from_lwe_ciphertext_list(&lwe_list, &streams);
                ...
                let out_lwe_list = LweCiphertextList::new(
                    0u128,
                    output_lwe_secret_key.lwe_dimension().to_lwe_size(),
                    lwe_ciphertext_count,
                    squash_params.ciphertext_modulus,
                ...
                let mut out_pbs_ct_gpu =
                    CudaLweCiphertextList::from_lwe_ciphertext_list(&out_lwe_list, &streams);
    ...
    fn cuda_multi_bit_pbs_128_16_blocks(c: &mut Criterion) {
        let bench_name = "core_crypto::cuda::multi_bit_pbs128_16_blocks";
        let mut bench_group = c.benchmark_group(bench_name);
        bench_group
            .sample_size(10)
            .measurement_time(std::time::Duration::from_secs(30));
        ...
        let plaintext_vec = vec![input_message * delta; 16];
        let plaintext_list = PlaintextList::from_container(plaintext_vec);
        let lwe_ciphertext_count = LweCiphertextCount(16);
        let lwe_dimension = input_params.lwe_dimension;
        ...
        match get_bench_type() {
            BenchmarkType::Latency => {
                ...
                let h_indexes = (0..lwe_ciphertext_count.0)
                    .map(CastFrom::cast_from)
                    .collect::<Vec<_>>();
                let cuda_indexes = CudaIndexes::new(&h_indexes, &streams, 0);

                let h_lut_indexes = [0u64; 16];
                let cuda_lut_indexes = CudaIndexes::new(&h_lut_indexes, &streams, 0);
                bench_id = format!("{bench_name}::{params_name}");
                {
                    bench_group.bench_function(&bench_id, |b| {
                        b.iter(|| {
    ...
    pub fn cuda_pbs128_16_blocks_group() {
        let mut criterion: Criterion<_> = Criterion::default().configure_from_args();
        cuda_pbs_128_16_blocks(&mut criterion);
    }
    ...
    pub fn cuda_multi_bit_pbs128_16_blocks_group() {
        let mut criterion: Criterion<_> = Criterion::default().configure_from_args();
        cuda_multi_bit_pbs_128_16_blocks(&mut criterion);
    }
}

#[cfg(feature = "gpu")]
use cuda::{
    cuda_multi_bit_pbs128_16_blocks_group, cuda_multi_bit_pbs128_group,
    cuda_pbs128_16_blocks_group, cuda_pbs128_group,
};
...
fn go_through_gpu_bench_groups() {
    cuda_pbs128_group();
    cuda_pbs128_16_blocks_group();
    cuda_multi_bit_pbs128_group();
    cuda_multi_bit_pbs128_16_blocks_group();
}
