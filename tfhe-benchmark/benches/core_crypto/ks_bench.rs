        ...
        for (name, params) in parameters.iter() {
            ...
            let lwe_noise_distribution_ksk = match params.lwe_noise_distribution.unwrap() {
                ...
                DynamicDistribution::TUniform(uniform_lwe_noise_distribution) => {
                    DynamicDistribution::<KSKSCalar>::new_t_uniform(match size_of::<KSKSCalar>() {
                        4 => uniform_lwe_noise_distribution.bound_log2() - 32,
                        _ => uniform_lwe_noise_distribution.bound_log2(),
                    })
                }
            };

            let lwe_sk: LweSecretKeyOwned<KSKSCalar> =
                allocate_and_generate_new_binary_lwe_secret_key(
                    lwe_dimension,
                    &mut secret_generator,
                );

            let glwe_sk: GlweSecretKeyOwned<KSKSCalar> =
            ...
            let ciphertext_modulus_out = CiphertextModulus::<KSKSCalar>::new_native();
