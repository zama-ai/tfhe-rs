...
use benchmark_spec::{
    get_bench_type, Backend, BenchCrate, BenchmarkMetric, BenchmarkSpec, BenchmarkType,
    CoreCryptoBench, OperandType, TfheLayer,
};
            ...
            BenchmarkType::Throughput => {
                ...
                let run = |inputs: &mut Res<Scalar>| {
                    inputs
                        ...
                        .zip(inputs.2.par_iter())
                        .zip(inputs.3.par_iter_mut())
                        .for_each(|(((input_ct, output_ct), accumulator), buffer)| {
                            programmable_bootstrap_lwe_ciphertext_mem_optimized(
                                input_ct,
                                output_ct,
                                &accumulator.as_view(),
    ...
    for (name, params) in parameters.iter() {
        ...
        match bench_type {
            ...
            BenchmarkType::Throughput => {
                ...
                type Res<Scalar> = (
                    Vec<LweCiphertextList<Vec<Scalar>>>,  // input_cts
                    Vec<LweCiphertextList<Vec<Scalar>>>,  // output_cts
                    Vec<GlweCiphertextList<Vec<Scalar>>>, // accumulators
                    Vec<ComputationBuffers>,              // buffers
                );
                let run = |inputs: &mut Res<Scalar>| {
                    inputs
                        ...
                        .zip(inputs.2.par_iter())
                        .zip(inputs.3.par_iter_mut())
                        .for_each(|(((input_ct_list, output_ct_list), accumulator), buffer)| {
                            batch_programmable_bootstrap_lwe_ciphertext_mem_optimized(
                                input_ct_list,
                                output_ct_list,
                                &accumulator.as_view(),
...
>(
    ...
    for (name, params, grouping_factor) in parameters.iter() {
        ...
        // For latency, encode "parallelized" as type_name; for throughput, use new_core_crypto.
        let benchmark_spec = match bench_type {
            BenchmarkType::Latency => BenchmarkSpec::new(
                BenchCrate::Tfhe(TfheLayer::CoreCrypto(cc_bench)),
                Backend::Cpu,
                name,
            ...
            BenchmarkType::Throughput => BenchmarkSpec::<str>::new_core_crypto(
                cc_bench,
                name,
                *bench_type,
                Backend::Cpu,
    ...
    for (name, params) in benchmark_parameters().iter_mut() {
        ...
        match bench_type {
            ...
            BenchmarkType::Throughput => {
                ...
                type Res = (
                    Vec<LweCiphertext<Vec<u64>>>,  // input_cts
                    Vec<LweCiphertext<Vec<u64>>>,  // output_cts
                    Vec<GlweCiphertext<Vec<u64>>>, // accumulators
                    Vec<ComputationBuffers>,       // buffers
                );
...
mod cuda {
    ...
    use benchmark::utilities::{
        ...
        CpuKeys, CpuKeysBuilder, CryptoParametersRecord, CudaIndexes,
        CudaLocalKeys, OperatorType, GPU_MAX_SUPPORTED_POLYNOMIAL_SIZE,
    };
    use benchmark_spec::{
        get_bench_type, Backend, BenchmarkSpec, BenchmarkType, CoreCryptoBench,
    };
    use criterion::{black_box, Criterion, Throughput};
    use rayon::prelude::*;
    use serde::Serialize;
