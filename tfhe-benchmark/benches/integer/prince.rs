...
pub mod cuda {
    ...
    use benchmark_spec::tfhe::transciphering::prince::PrinceFlavor;
    ...
    pub fn cuda_prince(c: &mut Criterion) {
        ...
        let bench_name = TranscipheringBench::Prince(PrinceFlavor::KeyPrep).to_string();
        ...
        for (atomic_param, param_name) in params {
...

            {
                let spec = BenchmarkSpec::new_transciphering(
                    TranscipheringBench::Prince(PrinceFlavor::KeyPrep),
                    &param_name,
                    Some(PrecisionTag::Bits(prince_op_bit_size).into()),
            ...
            for num_prince_inputs in [2usize, 384] {
                let spec = BenchmarkSpec::new_transciphering(
                    TranscipheringBench::Prince(PrinceFlavor::Encryption),
                    &param_name,
                    Some(PrecisionTag::Bits(prince_op_bit_size).into()),
                    BenchmarkMetric::Latency,
                    Some(num_prince_inputs as u64),
                );
                let bench_id = spec.to_string();
