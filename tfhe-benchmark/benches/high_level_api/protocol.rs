...
//!
//! Three spans either way: F1, F2, and the whole transaction. In throughput the gap between the
//! halves' sum and the whole prices what the fused pipeline gains in overlap and loses in memory,
//! so all three run at the same batch, otherwise nothing composes.
...
fn main() {
    ...
    let meta_params = match get_param_type() {
        ...
        _ => {
            benchmark::params_aliases::BENCH_META_PARAM_GPU_2_2_MULTI_BIT_GROUP_4_KS_PBS_PKE_TO_SMALL_ZKV2_TUNIFORM_2M128
        }
    };
    #[cfg(not(feature = "gpu"))]
    let meta_params =
