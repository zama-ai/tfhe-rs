use benchmark::utilities::{write_to_json, BenchmarkType, OperatorType};
...
use tfhe::prelude::*;
use tfhe::{
    ClientKey, CompressedServerKey, FheBool, FheIntegerType, FheUint10, FheUint12, FheUint128,
...
fn get_factor_for_op<Op>(op: Op, cks: &ClientKey) -> usize
where
    Op: FnOnce(),
{
    ...
    let a = FheBool::encrypt(true, cks);
    ...
    match BenchmarkType::from_env().unwrap() {
        BenchmarkType::Latency => {
            while kv_store.len() != num_elements {
                ...
                kv_store.insert(key, encrypted_value);
            }
